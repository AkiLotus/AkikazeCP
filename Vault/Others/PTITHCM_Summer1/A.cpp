...
/** -----GLOBAL VARIABLES----- **/
string words[] = {"khong", "mot", "hai", "ba", "bon", "nam", "sau", "bay", "tam", "chin", "muoi", "muoimot", "muoihai", "muoiba", "muoibon", "muoilam", "muoisau",
"muoibay", "muoitam", "muoichin", "haimuoi", "haimuoimot", "haimuoihai", "haimuoiba", "haimuoibon", "haimuoilam", "haimuoisau", "haimuoibay", "haimuoitam",
"haimuoichin", "bamuoi", "bamuoimot", "bamuoihai", "bamuoiba", "bamuoibon", "bamuoilam", "bamuoisau", "bamuoibay", "bamuoitam", "bamuoichin", "bonmuoi"};
i64 n, ans = 0;

/** -----EXTENSIVE FUNCTIONS----- **/
i64 diff(string a, string b) {
	i64 res = a.size() + b.size();
	for (i64 i=0; i<min(a.size(), b.size()); i++) res -= (a[i] == b[i]) * 2;
	return res;
}
...
void ProSolve() {
	for (i64 i=1; i<=n; i++) ans += diff(words[i-1], words[i]);
	cout << ans;
}
