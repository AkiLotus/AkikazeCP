...
void ProSolve() {
	while (T--) {
		cin >> N >> M; vector<set<i64>> adj(N); vi deg(N);
		i64 ans = N; set<i64> S;
		while (M--) {
			i64 u, v; cin >> u >> v; u--; v--;
		...
		for (i64 i=0; i<N; i++) {
			if (deg[i] != 2) continue;
			auto it = adj[i].begin(); i64 t1 = *it; it++; i64 t2 = *it;
			if (adj[t1].find(t2) == adj[t1].end()) continue;
			S.insert(i);
		}
		while (!S.empty()) {
			i64 z = *S.begin(); S.erase(z); ans--;
			auto it = adj[z].begin(); deg[z] = 0;
			i64 t1 = *it; it++; i64 t2 = *it;
			adj[t1].erase(z); adj[t2].erase(z); deg[t1]--; deg[t2]--;
			if (deg[t1] == 1) S.erase(t1); else if (deg[t1] == 2) {
				auto it2 = adj[t1].begin(); i64 z1 = *it2; it2++; i64 z2 = *it2;
				if (adj[z1].find(z2) != adj[z1].end()) S.insert(t1);
			}
			if (deg[t2] == 1) S.erase(t2); else if (deg[t2] == 2) {
				auto it2 = adj[t2].begin(); i64 z1 = *it2; it2++; i64 z2 = *it2;
				if (adj[z1].find(z2) != adj[z1].end()) S.insert(t2);
			}
		}
