...
void VarInput() {
	setbuf(stdout, NULL);
	for (i64 i=0; i<10; i++) palNum.pub(i);
	for (i64 i=1; i<10; i++) {
		for (i64 j=0; j<10; j++) {
			palNum.pub(i*101+j*10);
		}
	}
	for (i64 i=1; i<10; i++) palNum.pub(11*i);
	for (i64 i=10; i<100; i++) {
		i64 z = i*100 + (i % 10) * 10 + (i / 10);
		palNum.pub(z);
	}
	sort(palNum.begin(), palNum.end());
	cin >> T;
}
...
void ProSolve() {
	while (T--) {
		cin >> N;
		cout << "Case #" << ++cas << endl;
		if (find(palNum.begin(), palNum.end(), N) != palNum.end()) {cout << "1 " << N << endl; continue;}
		bool sumoftwo = false, sumofthree = false;
		for (i64 i=0; i<palNum.size(); i++) {
			if (palNum[i] * 2 > N) break;
			auto it = lower_bound(palNum.begin(), palNum.end(), N-palNum[i]);
			if (it == palNum.end()) continue;
			if (*it == N-palNum[i]) {
				cout << "2 " << N-palNum[i] << " " << palNum[i] << endl;
				sumoftwo = true; break;
			}
		}
		if (sumoftwo) continue;
		for (i64 i=0; i<palNum.size(); i++) {
			for (i64 j=i; j<palNum.size(); j++) {
				auto it = lower_bound(palNum.begin(), palNum.end(), N-palNum[i]-palNum[j]);
				if (it == palNum.end()) continue;
				if (*it == N-palNum[i]-palNum[j]) {
					vi ans; ans.pub(palNum[i]); ans.pub(palNum[j]); ans.pub(N-palNum[i]-palNum[j]);
					sort(ans.rbegin(), ans.rend()); cout << "3";
					for (auto t: ans) cout << " " << t; cout << endl;
					sumofthree = true; break;
				}
			}
			if (sumofthree) break;
		}
