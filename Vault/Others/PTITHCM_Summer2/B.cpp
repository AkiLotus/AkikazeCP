...
void ProSolve() {
	while (Q--) {
		cin >> no;
		if (no <= 1) {cin >> x; A.pub(x);}
		else if (no <= 2) {if (!A.empty()) A.pop_back();}
		else if (no <= 3) {cin >> p; if (A.size() >= p) A.erase(A.begin()+p-1);}
		else if (no <= 4) {cin >> p >> x; if (A.size() >= p) A.insert(A.begin()+p-1, x);}
		else if (no <= 5) {cout << A.size(); for (auto k: A) cout << " " << k; cout << endl;}
		else if (no <= 6) {A.clear();}
	}
}
