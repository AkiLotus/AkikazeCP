...
ld r; i64 n; vd theta;
ld maximumDist = -LINF; pair<ld,ld> pr;
...
void VarInput() {
	...
	cin >> r >> n; theta.resize(n);
	for (i64 i=0; i<n; i++) cin >> theta[i];
	for (i64 i=0; i<n; i++) {
		for (i64 j=i+1; j<n; j++) {
			ld tmp = abs(theta[i]-theta[j]);
			if (tmp - 180 > 0) tmp = 360 - tmp;
			if (tmp - maximumDist > 0) {
				maximumDist = tmp; pr = mp(i, j);
			}
		}
	}
}
...
void ProSolve() {
	...
	ld t1 = theta[pr.fi], t2 = theta[pr.se];
	if ((t1 - t2 < 0 && t2 - t1 <= 180.0) || (t1 - t2 > 180.0)) {
		ld tmp = t1; t1 = t2; t2 = tmp;
	}
	ld position;
	if (t1 > t2) {
		position = (t1 + t2) / 2;
