...
i64 N, Len, K, a; char c; string s; vector<vi> cnt;
/************************************************/
...
void Solve() {
	cnt[0][s[0]-'A'] = 1;
	for (i64 i=1; i<Len; i++) {
		for (i64 j=0; j<26; j++) cnt[i][j] = cnt[i-1][j];
		cnt[i][s[i]-'A']++;
	}
	while (K--) {
		cin >> a >> c;
		i64 st = 0, ans = 0;
		if (a % 2 == 0) st = (((a / 2) % Len) * ((a - 1) % Len)) % Len;
		else st = ((((a - 1) / 2) % Len) * (a % Len)) % Len;
		i64 en = (st + a - 1) % Len; while (en < 0) en += Len;
		if (st <= en) {
			a -= (en - st + 1); ans += cnt[en][c-'A'];
			if (st > 0) ans -= cnt[st-1][c-'A'];
			i64 zz = a / Len; zz *= cnt[Len-1][c-'A']; ans += zz;
		}
		else {
			ans += cnt[Len-1][c-'A']; if (st > 0) ans -= cnt[st-1][c-'A'];
			ans += cnt[en][c-'A']; a -= (Len - st); a -= (en + 1);
			i64 zz = a / Len; zz *= cnt[Len-1][c-'A']; ans += zz;
		}
		cout << ans << endl;
	}
