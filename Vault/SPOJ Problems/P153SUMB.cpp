...
vi a(12), b(3); i64 ans = LINF, sum = 0;
...
void ProSolve() {
	...
	for (i64 a1=0; a1<12; a1++) {
		b[0] += a[a1];
		for (i64 a2=a1+1; a2<12; a2++) {
			b[0] += a[a2];
			for (i64 a3=a2+1; a3<12; a3++) {
				b[0] += a[a3];
				for (i64 b1=0; b1<12; b1++) {
					if (b1 == a1 || b1 == a2 || b1 == a3) continue;
					b[1] += a[b1];
					for (i64 b2=b1+1; b2<12; b2++) {
						if (b2 == a1 || b2 == a2 || b2 == a3) continue;
						b[1] += a[b2];
						for (i64 b3=b2+1; b3<12; b3++) {
							if (b3 == a1 || b3 == a2 || b3 == a3) continue;
							b[1] += a[b3];
							for (i64 c1=0; c1<12; c1++) {
								if (c1 == a1 || c1 == a2 || c1 == a3 || c1 == b1 || c1 == b2 || c1 == b3) continue;
								b[2] += a[c1];
								for (i64 c2=c1+1; c2<12; c2++) {
									if (c2 == a1 || c2 == a2 || c2 == a3 || c2 == b1 || c2 == b2 || c2 == b3) continue;
									b[2] += a[c2];
									for (i64 c3=c2+1; c3<12; c3++) {
										if (c3 == a1 || c3 == a2 || c3 == a3 || c3 == b1 || c3 == b2 || c3 == b3) continue;
										b[2] += a[c3];
										i64 mn = min(*min_element(b.begin(), b.end()), sum-b[0]-b[1]-b[2]);
										i64 mx = max(*max_element(b.begin(), b.end()), sum-b[0]-b[1]-b[2]);
										ans = min(ans, mx-mn);
										b[2] -= a[c3];
									}
									b[2] -= a[c2];
