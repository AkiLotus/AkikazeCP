...
void ProSolve() {
	...
	i64 M, D; cin >> M >> D;
	i64 Quotient = M / D, Remainder = M % D;
	if (Remainder == 0) {
		cout << Quotient << ".0\n"; return;
	}
	vector<i64> digit(100000, 0LL);
	vector<i64> rem(100000, 0LL);
	vector<vector<i64>> visited(10, vector<i64>(D, -1LL));
	i64 workingOn = 0;
	i64 StartingPoint = -7, EndingPoint = -7;
	while (Remainder != 0) {
		digit[workingOn] = (Remainder * 10) / D;
		Remainder = (Remainder * 10) % D;
		rem[workingOn] = Remainder;
		if (visited[digit[workingOn]][rem[workingOn]] != -1) {
			StartingPoint = visited[digit[workingOn]][rem[workingOn]];
			EndingPoint = workingOn; break;
		}
		visited[digit[workingOn]][rem[workingOn]] = workingOn; workingOn++;
	}
	if (StartingPoint == -7) {
		cout << Quotient << ".";
		for (i64 i=0; i<workingOn; i++) cout << digit[i]; cout << endl;
	}
	else {
		cout << Quotient << ".";
		for (i64 i=0; i<StartingPoint; i++) cout << digit[i];
		cout << "(";
		for (i64 i=StartingPoint; i<EndingPoint; i++) cout << digit[i];
		cout << ")\n";
	}
}
