...
void ProSolve() {
		...
		cin >> ax >> ay >> bx >> by >> q;
		vector<vi> dp(13013, vi(13013, 0));
		for (i64 i=0; i<N; i++) {
			distA[i] = (a[i].fi - ax) * (a[i].fi - ax) + (a[i].se - ay) * (a[i].se - ay);
			distB[i] = (a[i].fi - bx) * (a[i].fi - bx) + (a[i].se - by) * (a[i].se - by);
			i64 notHigherA = lower_bound(Sqr.begin(), Sqr.begin()+13012, distA[i]) - Sqr.begin();
			i64 notHigherB = lower_bound(Sqr.begin(), Sqr.begin()+13012, distB[i]) - Sqr.begin();
			dp[notHigherA][notHigherB]++;
		}
		sort(distA.begin(), distA.end()); sort(distB.begin(), distB.end());
		for (i64 i=1; i<13013; i++) {
			for (i64 j=1; j<13013; j++) {
				dp[i][j] += dp[i][j-1];
				dp[i][j] += dp[i-1][j];
				dp[i][j] -= dp[i-1][j-1];
			}
