...
void VarInput() {
	...
	for (i64 i=1; i<19; i++) ftrl[i] = ftrl[i-1] * i;
	cin >> s >> K; n = s.size();
	cnt.clear(); cnt.rsz(26, 0);
}
	...
	while (s != "#" || K != 0) {
		...
		for (i64 i=0; i<n; i++) {
			...
			for (i64 j=0; j<26; j++) {
				...
				i64 tmp = Rem; cnt[j]--;
				for (i64 k=0; k<26; k++) {
					if (cnt[k]) tmp /= ftrl[cnt[k]];
				}
				cnt[j]++; cc += tmp;
				if (cc >= K) {ans[i] = char(65+j); cnt[j]--; cc -= tmp; break;}
			}
		}
		cout << ans << endl;
		cin >> s >> K; n = s.size(); cnt.clear(); cnt.rsz(26, 0);
	}
}
