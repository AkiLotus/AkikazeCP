...
i64 N, M, K, U, V, L; vector<vector<pii>> adj; vi dist, dp;
/************************************************/
...
void Input() {
	cin >> N >> M; adj.rsz(N);
	dist.rsz(N, LINF); dp.rsz(N, 0);
	while (M--) {
		cin >> K >> U >> V >> L; U--; V--;
		adj[U].pub(mp(V, L));
		if (K == 2) adj[V].pub(mp(U, L));
	}
}

void Solve() {
	priority_queue<pii> Q; Q.push(mp(0LL, 0LL));
	dist[0] = 0; dp[0] = 1; vb vis(N, false);
	while (!Q.empty()) {
		pii z = Q.top(); Q.pop(); z.fi = -z.fi;
		if (vis[z.se]) continue; vis[z.se] = true;
		for (auto t: adj[z.se]) {
			if (vis[t.fi]) continue;
			if (z.fi + t.se < dist[t.fi]) {
				dist[t.fi] = z.fi + t.se; dp[t.fi] = dp[z.se];
				Q.push(mp(-dist[t.fi], t.fi));
			}
			else {
				if (z.fi + t.se == dist[t.fi]) dp[t.fi] += dp[z.se];
			}
		}
	}
