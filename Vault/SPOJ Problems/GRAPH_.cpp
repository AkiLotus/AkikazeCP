...
/** -----GLOBAL VARIABLES----- **/
i64 n, m; vector<vi> adj; vb vis;
i64 ArtclPoints = 0, Bridges = 0;
vi paths, lastConnected, bridgeCnt;
...
void DFS(i64 z, i64 last) {
	...
	for (auto t: adj[z]) {
		if (t == last) continue;
		paths[t]++; lastConnected[t] = z;
		if (!vis[t]) DFS(t, z);
	}
	if (bridgeCnt[z] > 0) ArtclPoints++;
}

/** -----COMPULSORY FUNCTIONS----- **/
void VarInput() {
	//ios_base::sync_with_stdio(0); cin.tie(NULL);
	cin >> n >> m; adj.rsz(n, vi(0)); vis.rsz(n, false);
	paths.rsz(n, 0); lastConnected.rsz(n, -1); bridgeCnt.rsz(n, 0);
	while (m--) {
		i64 u, v; cin >> u >> v; u--; v--;
...
void ProSolve() {
	...
	for (i64 i=0; i<n; i++) {
		if (paths[i] != 1) continue; Bridges++;
		bridgeCnt[i]++; bridgeCnt[lastConnected[i]]++;
	}
	for (i64 i=0; i<n; i++) ArtclPoints += (bridgeCnt[i] > 0);
	cout << paths << endl << lastConnected << endl;
	cout << ArtclPoints << " " << Bridges;
}
