...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, src, sink, u, v, c; vector<vi> adj;
vector<vi> CapacityMatrix; vb vis; vi dist;
/************************************************/
...
i64 MaxFlow() {
	...
	while (true) {
		...
		i64 DinicCall = Dinic_DFS(src, LINF, dist);
		res += DinicCall;
	}
	return res;
}
...
void Input() {
	cin >> n >> m >> src >> sink; src--; sink--;
	adj.rsz(n); CapacityMatrix.rsz(n, vi(n, 0));
	while (m--) {
		cin >> u >> v >> c; u--; v--;
		adj[u].pub(v); adj[v].pub(u);
		CapacityMatrix[u][v] = c;
	}
}
