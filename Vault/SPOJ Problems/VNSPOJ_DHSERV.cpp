...
i64 n, m, k, u, v, c, q;
vector<vi> adj, mat; vb activated;
/************************************************/
...
void Input() {
	cin >> n >> m >> k; adj.rsz(n, vi(n, LINF));
	mat.rsz(n, vi(n, LINF)); activated.rsz(n, false);
	for (i64 i=0; i<n; i++) adj[i][i] = 0;
	while (m--) {
		cin >> u >> v >> c;
		u--; v--; adj[u][v] = c;
	}
...
void Solve() {
	while (k--) {
		...
		if (q == 1) {
			i64 z; cin >> z; z--;
			if (activated[z]) continue;
			for (i64 i=0; i<n; i++) {
				for (i64 j=0; j<n; j++) {
					mat[i][j] = min(mat[i][j], mat[i][z] + mat[z][j]);
				}
			}
		}
		else if (q == 2) {
			i64 u, v; cin >> u >> v; u--; v--;
			if (mat[u][v] == LINF) cout << "-1\n"; else cout << mat[u][v] << endl;
		}
	}
}
/*****************************************/
