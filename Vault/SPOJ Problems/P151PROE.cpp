...
/***********************************************
** File name: P151PROE.cpp                    **
** Time created: Tue Jan 8 2019 12:44pm       **
***********************************************/
...
/************** [LEGENDS/CONSTANTS] **************/
#define x1 dummy_linuxFriendly_x1
#define x2 dummy_linuxFriendly_x2
#define y1 dummy_linuxFriendly_y1
#define y2 dummy_linuxFriendly_y2
#define z1 dummy_linuxFriendly_z1
#define z2 dummy_linuxFriendly_z2
...
void Solve() {
	i64 MaxUp = min(x1-1, x2-1);
	x1 -= MaxUp; x2 -= MaxUp;
	i64 MaxLf = min(y1-1, y2-1);
	y1 -= MaxLf; y2 -= MaxLf;
	
	i64 MaxDw = min(m-x1, m-x2);
	i64 MaxRg = min(n-y1, n-y2);
	i64 x3 = x1 + MaxDw, y3 = y1 + MaxRg;
	i64 x4 = x2 + MaxDw, y4 = y2 + MaxRg;
	i64 IntsctX = 0, IntsctY = 0;
	if (x1 <= x2) IntsctX = max(IntsctX, x3 - x2 + 1);
	else IntsctX = max(IntsctX, x4 - x1 + 1);
	if (y1 <= y2) IntsctY = max(IntsctY, y3 - y2 + 1);
	else IntsctY = max(IntsctY, y4 - y1 + 1);
	
	cout << (m * n - (MaxDw + 1) * (MaxRg + 1) * 2 + IntsctX * IntsctY) << endl;
}
/*****************************************/
