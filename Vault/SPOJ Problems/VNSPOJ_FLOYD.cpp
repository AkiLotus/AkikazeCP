...
i64 N, M, K, u, v, c, q; vector<vector<pii>> adj;
vector<vi> Last, Dist; vb vis;
/************************************************/
...
void Input() {
	cin >> N >> M >> K; adj.rsz(N); vis.rsz(N, false);
	Last.rsz(N, vi(N, -1)); Dist.rsz(N, vi(N, LINF));
	while (M--) {
		cin >> u >> v >> c; u--; v--;
		adj[u].pub(mp(v, c)); adj[v].pub(mp(u, c));
	}
}

void Solve() {
	for (i64 i=0; i<N; i++) {
		Dist[i][i] = 0; priority_queue<pii> Q;
		...
		while (!Q.empty()) {
			...
			for (auto t: adj[z.se]) {
				...
				if (z.fi + t.se < Dist[i][t.fi]) {
					Dist[i][t.fi] = z.fi + t.se;
					Q.push(mp(-(z.fi+t.se), t.fi));
					Last[i][t.fi] = z.se;
				}
			}
		}
	...
	while (K--) {
		cin >> q >> u >> v; u--; v--;
		if (q == 0) {cout << Dist[u][v] << endl; continue;}
		i64 z = Last[u][v]; stack<i64> S; S.push(v);
		while (z != -1) {S.push(z); z = Last[u][z];}
		cout << S.size(); while (!S.empty()) {cout << " " << S.top()+1; S.pop();} cout << endl;
	}
}
/*****************************************/
