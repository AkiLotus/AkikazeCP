...
void ProSolve() {
	while (T--) {
		cin >> N; unordered_map<i64, bool> vis;
		queue<pii> Q; vis[N] = true;
		Q.push(mp(N, 0LL));
		while (!Q.empty()) {
			pii z = Q.front(); Q.pop();
			if (z.fi == 1) {cout << z.se << endl; break;}
			for (i64 i=sqrt(z.fi); i>1; i--) {
				if (z.fi % i != 0) continue;
				vis[max(z.fi/i, i)] = true;
				Q.push(mp(max(z.fi/i, i), z.se+1));
			}
			if (!vis[z.fi-1]) {Q.push(mp(z.fi-1, z.se+1)); vis[z.fi-1] = true;}
		}
	}
}
