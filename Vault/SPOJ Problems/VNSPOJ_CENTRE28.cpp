...
i64 N, M, U, V, L; vector<vector<pii>> adj;
vi dist1, dp1, distN, dpN;
/************************************************/
...
void Input() {
	cin >> N >> M; adj.rsz(N);
	dist1.rsz(N, LINF); dp1.rsz(N, 0);
	distN.rsz(N, LINF); dpN.rsz(N, 0);
	while (M--) {
		cin >> U >> V >> L; U--; V--;
		adj[U].pub(mp(V, L)); adj[V].pub(mp(U, L));
	}
}

void Solve() {
	priority_queue<pii> Q; Q.push(mp(0LL, 0LL));
	dist1[0] = 0; dp1[0] = 1; vb vis(N, false);
	while (!Q.empty()) {
		pii z = Q.top(); Q.pop(); z.fi = -z.fi;
		if (vis[z.se]) continue; vis[z.se] = true;
		for (auto t: adj[z.se]) {
			if (vis[t.fi]) continue;
			if (z.fi + t.se < dist1[t.fi]) {
				dist1[t.fi] = z.fi + t.se; dp1[t.fi] = dp1[z.se];
				Q.push(mp(-dist1[t.fi], t.fi));
			}
			else {
				if (z.fi + t.se == dist1[t.fi]) dp1[t.fi] += dp1[z.se];
			}
		}
	}
	Q.push(mp(0LL, N-1));
	distN[N-1] = 0; dpN[N-1] = 1; vis.clear(); vis.rsz(N, false);
	while (!Q.empty()) {
		pii z = Q.top(); Q.pop(); z.fi = -z.fi;
		if (vis[z.se]) continue; vis[z.se] = true;
		for (auto t: adj[z.se]) {
			if (vis[t.fi]) continue;
			if (z.fi + t.se < distN[t.fi]) {
				distN[t.fi] = z.fi + t.se; dpN[t.fi] = dpN[z.se];
				Q.push(mp(-distN[t.fi], t.fi));
			}
			else {
				if (z.fi + t.se == distN[t.fi]) dpN[t.fi] += dpN[z.se];
			}
		}
	}
	vi ans; for (i64 i=0; i<N; i++) if (dist1[i] + distN[i] != dist1[N-1] || (dp1[N-1] / dp1[i] != dpN[i] || dp1[N-1] % dp1[i] != 0)) ans.pub(i+1);
