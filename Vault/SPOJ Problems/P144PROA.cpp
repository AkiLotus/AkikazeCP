...
/***********************************************
** File name: P144PROA.cpp                    **
** Time created: Tue Jan 8 2019 11:18am       **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, q, u, v; vi A, L, d; vector<vi> adj;
vector<vi> Table, TableDist;
/************************************************/
...
void Preprocess() {
	d.rsz(n, 0); Table.rsz(n);
	TableDist.rsz(n); DFS(0, -1);
	for (i64 j=1; j<17; j++) {
		for (i64 i=0; i<n; i++) {
			if (Table[i].size() < j) continue;
			if (Table[Table[i][j-1]].size() < j) continue;
			Table[i].pub(Table[Table[i][j-1]][j-1]);
			TableDist[i].pub(TableDist[i][j-1] + TableDist[Table[i][j-1]][j-1]);
		}
	}
}
...
i64 Dist(i64 x, i64 y) {
	...
	if (d[x] == d[y]) {
		...
		return (TableDist[x][id] + TableDist[y][id] + Dist(Table[x][id], Table[y][id]));
	}
	if (d[x] < d[y]) {
		i64 mul = 1, id = 0;
		while (d[x] < d[y] - mul * 2) {
		...
		return (TableDist[y][id] + Dist(x, Table[y][id]));
	}
	if (d[x] > d[y]) {
		i64 mul = 1, id = 0;
		while (d[y] < d[x] - mul * 2) {
		...
		return (TableDist[x][id] + Dist(Table[x][id], y));
	}
}

void Input() {
	A.clear(); L.clear(); adj.clear();
	d.clear(); Table.clear(); TableDist.clear();
	cin >> n; A.rsz(n, -1); L.rsz(n, 0); adj.rsz(n);
	if (n == 0) Exit();
	for (i64 i=1; i<n; i++) {
		i64 j; cin >> j;
		A[i] = j; cin >> L[i];
