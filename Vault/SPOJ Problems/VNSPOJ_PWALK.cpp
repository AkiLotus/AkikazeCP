...
void Input() {
	cin >> N >> Q; adj.rsz(N); vis.rsz(N, false);
	for (i64 i=1; i<N; i++) {
		i64 a, b, l; cin >> a >> b >> l; a--; b--;
		adj[a].pub(mp(b,l)); adj[b].pub(mp(a,l));
	}
}
