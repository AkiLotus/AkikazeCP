...
void VarInput() {
	...
	// Initialize hashMul
	for (i64 x=0; x<8; x++) {
		for (i64 i=1; i<lenB; i++) hashMul[x] = (hashMul[x] * 256) % modArr[x];
	}
	
	// Calculate hashKey
	for (i64 x=0; x<8; x++) {
		for (i64 i=0; i<lenB; i++) hashKey[x] = (hashKey[x] * 256 + int(B[i])) % modArr[x];
	}
	...
	for (i64 x=0; x<8; x++) {
		for (i64 i=0; i<lenA; i++) {
			hashA[x][i] = int(A[i]);
			if (i < lenB) workingKey[x] = (workingKey[x] * 256 + hashA[x][i]) % modArr[x];
		}
	}
}
...
void ProSolve() {
	...
	for (i64 i=1; i<=lenA-lenB; i++) {
		for (i64 x=0; x<8; x++) {
			...
			workingKey[x] = (workingKey[x] * 256 + hashA[x][i+lenB-1]) % modArr[x];
		}
