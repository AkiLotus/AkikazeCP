...
vb Root; vector<vi> parentExp; vi Level, Log2(1001);
/************************************************/
...
void Input() {
	cin >> N; child.clear(); child.rsz(N+1);
	Root.clear(); Root.rsz(N+1, true);
	Level.clear(); Level.rsz(N+1, 0LL);
	parentExp.clear(); parentExp.rsz(N+1, vi(10));
	for (i64 i=1; i<=N; i++) {
		i64 z, x; cin >> z; child[i].rsz(z);
		for (i64 j=0; j<z; j++) {
			cin >> child[i][j];
	...
	for (i64 i=1; i<=N; i++) if (Root[i]) {R = i; break;}
}
...
void Solve() {
	...
	cin >> q;
	//cout << Level << endl;
	//for (i64 i=1; i<=N; i++) cout << parentExp[i] << endl;
	while (q--) {
		...
		while (v != w) {
			i64 k = 0;
			while (parentExp[v][k] != parentExp[w][k]) k++;
			//tracker3(v, w, k);
