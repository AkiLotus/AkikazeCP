...
i64 N, M, K, ans = 0; vi a; queue<pii> Q;
vvi boost; vvi dp(2000000, vi(20, -LINF));
...
void VarInput() {
	...
	boost.resize(N, vi(N, 0LL));
	dp.resize((1 << N), vi(N, -LINF));
	for (i64 i=0; i<N; i++) {
		cin >> a[i]; Q.push(mp((1 << i), i));
		dp[(1 << i)][i] = a[i];
	}
	while (K--) {
		i64 x, y, w; cin >> x >> y >> w;
...
void ProSolve() {
	...
	while (!Q.empty()) {
		i64 z = Q.front().fi, x = Q.front().se; Q.pop();
		if (cntbit(z) == M) {
			ans = max(ans, dp[z][x]); continue;
		}
		for (i64 i=0; i<N; i++) {
			if ((z & (1 << i)) != 0) continue;
			if (dp[(z | (1 << i))][i] == -LINF) Q.push(mp((z | (1 << i)), i));
			dp[(z | (1 << i))][i] = max(dp[(z | (1 << i))][i], dp[z][x] + a[i] + boost[x][i]);
		}
	}
	cout << ans;
}
