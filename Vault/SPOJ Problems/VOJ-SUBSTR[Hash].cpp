...
void VarInput() {
	...
	// Initialize hashMul
	for (i64 i=1; i<lenB; i++) hashMul = (hashMul * 256) % MOD;
	
	// Calculate hashKey
	for (i64 i=0; i<lenB; i++) hashKey = (hashKey * 256 + int(B[i])) % MOD;
	
	// Pre-processing hashA array and workingKey
	for (i64 i=0; i<lenA; i++) {
		hashA[i] = int(A[i]);
		if (i < lenB) workingKey = (workingKey * 256 + hashA[i]) % MOD;
	}
}
...
void ProSolve() {
	...
	for (i64 i=1; i<=lenA-lenB; i++) {
		...
		workingKey = (workingKey * 256 + hashA[i+lenB-1]) % MOD;
