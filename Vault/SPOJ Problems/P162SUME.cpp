...
i64 n, X, ans = 1, sum = 0, mini = -LINF; vi a;
...
i64 calculate(i64 x, i64 Exp) {
	map<i64, i64> M;
	i64 minimum = a[n-1];
	for (i64 i=0; i<n; i++) M[a[i]] += Exp;
	while (minimum > 0 && M[minimum] % x == 0) {
		M[minimum-1] += M[minimum] / x;
		M[minimum] = 0; minimum--;
	}
	return ((modPow(x, mini-minimum) * modPow(x, sum-mini)) % MOD);
}
...
void ProSolve() {
	...
	for (i64 i=2; i<=sqrt(X); i++) {
		...
		if (tmp > 0) ans = (ans * calculate(i, tmp)) % MOD;
	}
	if (X != 1) ans = (ans * calculate(X, 1)) % MOD;
	cout << ans;
}
