...
void VarInput() {
	...
	cin >> n >> m; R.resize(n, 0); B.resize(n, n-1);
}
...
void ProSolve() {
	while (m--) {
		i64 x, y; cin >> x >> y; x--; y--;
		if (x > y) swap(x, y);
		bool red = (Map[mp(x,y)] % 2 == 0);
		Map[mp(x,y)]++; S -= (R[x] * B[x] + R[y] * B[y]);
		if (red) {R[x]++; R[y]++; B[x]--; B[y]--;}
		else {R[x]--; R[y]--; B[x]++; B[y]++;}
		S += (R[x] * B[x] + R[y] * B[y]);
		cout << (n*(n-1)*(n-2)/6 - S/2) << endl;
	}
}
