...
void ProSolve() {
	for (i64 t=1; t<=T; t++) {
		cin >> deg >> prep >> hr;
		ld maxDifference = 1e9;
		i64 Hr, Se;
		if (prep == "after") {
			Hr = hr;
			for (i64 i=0; i<3600; i++) {
				if ((ld)abs((ld)i * 0.1L * 59.0L / 60.0L - deg) - maxDifference < -EPS) {
					Se = i; maxDifference = (ld)abs((ld)i * 0.1L * 59.0L / 60.0L - deg);
				}
			}
		}
		else if (prep == "til") {
			...
			for (i64 i=0; i<3600; i++) {
				if ((ld)abs((ld)i * 0.1L * 59.0L / 60.0L - deg) - maxDifference < -EPS) {
					Se = 3600-i; maxDifference = (ld)abs((ld)i * 0.1L * 59.0L / 60.0L - deg);
				}
			}
		}
		cout << "Case " << t << ": " << Hr << ":" << Se/60 << ":" << Se%60 << endl;
	}
}
