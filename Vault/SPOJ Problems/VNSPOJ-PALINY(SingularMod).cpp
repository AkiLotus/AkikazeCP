...
void GenerateHashVal() {
	...
	for (i64 i=0; i<N; i++) {
		if (i == 0) {
			hashS[i] = ((i64)int(S[i]) * powBase[i]) % MOD;
			hashR[i] = ((i64)int(R[i]) * powBase[i]) % MOD;
		}
		else {
			hashS[i] = (hashS[i-1] + ((i64)int(S[i]) * powBase[i]) % MOD) % MOD;
			hashR[i] = (hashR[i-1] + ((i64)int(R[i]) * powBase[i]) % MOD) % MOD;
		}
	}
}

bool match(i64 stS, i64 enS, i64 stR, i64 enR) {
	i64 valS = hashS[enS], valR = hashR[enR];
	if (stS > 0) {
		valS -= hashS[stS-1]; while (valS < 0) valS += MOD;
	}
	valS *= powBase[max(stS,stR)-stS]; valS %= MOD;
	if (stR > 0) {
		valR -= hashR[stR-1]; while (valR < 0) valR += MOD;
	}
	valR *= powBase[max(stS,stR)-stR]; valR %= MOD;
//	cout << "valS = " << valS << endl;
//	cout << "valR = " << valR << endl;
	return (valS == valR);
}
...
void VarInput() {
	...
	hashS.resize(N, 0); hashR.resize(N, 0); powBase.resize(N, 1);
}
