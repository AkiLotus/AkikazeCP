...
void VarInput() {
	...
	cin >> N >> K; C.resize(N, vi(N, 0));
	for (i64 i=0; i<N; i++) {
		for (i64 j=0; j<N; j++) cin >> C[i][j];
	}
}

void ProSolve() {
	//cout << "Case " << ++cas << ": " << ans << endl; // for multi-testcase problems
	queue<i64> Q; Q.push((1 << N) - 1); dp[(1 << N) - 1] = 0;
	while (!Q.empty()) {
		i64 z = Q.front(); Q.pop();
		//tracker1(z);
		if (cntset(z) == K) {
			//cout << "dp[" << z << "] = " << dp[z] << endl;
			ans = min(ans, dp[z]); continue;
		}
		//cout << "test\n";
		for (i64 i=0; i<N; i++) {
			//cout << "valid? " << (z & (1 << i)) << endl;
			if ((z & (1 << i)) == 0) continue;
			//tracker1(i);
			for (i64 j=0; j<N; j++) {
				if (i == j) continue;
				if ((z & (1 << j)) == 0) continue;
				if (dp[z ^ (1 << i)] == LINF) Q.push(z ^ (1 << i));
				dp[z ^ (1 << i)] = min(dp[z ^ (1 << i)], dp[z] + C[i][j]);
				//tracker2(i, j);
				// cout << "dp[" << (z ^ (1 << i)) << "] = " << dp[z ^ (1 << i)] << endl;
			}
		}
	}
	cout << ans;
