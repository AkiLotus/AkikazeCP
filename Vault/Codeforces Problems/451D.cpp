...
/************** [GLOBAL VARIABLES] **************/
i64 n, N = 0; string s, t = ""; vi E, O; vector<pii> A;
/************************************************/

/************** [FUNCTIONS] **************/
i64 calcEven(i64 len) {
	i64 maxEven = len - len % 2;
	i64 A = (maxEven / 2) * len;
	i64 B = (maxEven * maxEven / 4);
	return (A - B);
}

i64 calcOdd(i64 len) {
	i64 maxOdd = len - (1 - len % 2);
	i64 A = ((maxOdd + 1) / 2) * len;
	i64 B = ((maxOdd + 1) / 2) * ((maxOdd - 1) / 2);
	return (A - B);
}
...
void Solve() {
	for (i64 i=0; i<n; i++) {
		...
		A.pub(mp(i, j)); t += s[i]; N++; i = j;
	}
	for (i64 i=0; i<N; i++) {
		i64 len = A[i].se - A[i].fi + 1;
		i64 e = len / 2, o = len - e;
		E.pub(e); O.pub(o);
	}
	vi ans(2, 0); i64 even = 0, odd = 0, Len = 0;
	for (i64 i=0; i<N; i+=2) {
		ans[0] += calcEven(A[i].se - A[i].fi + 1);
		ans[1] += calcOdd(A[i].se - A[i].fi + 1);
		ans[Len % 2] += E[i] * even;
		ans[Len % 2] += O[i] * odd;
		ans[(Len + 1) % 2] += E[i] * odd;
		ans[(Len + 1) % 2] += O[i] * even;
		if (i + 2 < N) {
			if (Len % 2 != 0) {even += E[i]; odd += O[i];}
			else {even += O[i]; odd += E[i];}
			Len += A[i].se - A[i].fi + 1;
			Len += A[i+1].se - A[i+1].fi + 1;
		}
	}
	even = 0; odd = 0; Len = 0;
	for (i64 i=1; i<N; i+=2) {
