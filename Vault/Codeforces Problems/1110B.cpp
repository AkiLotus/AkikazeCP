...
/***********************************************
** File name: 1110B.cpp                       **
** Time created: Fri Feb 8 2019 10:01am       **
***********************************************/
...
void Solve() {
	i64 cnt = n, Len = n; multiset<i64> Gap;
	for (i64 i=1; i<n; i++) cnt -= (a[i-1] + 1 == a[i]);
	for (i64 i=1; i<n; i++) if (a[i-1] + 1 < a[i]) Gap.insert(a[i] - a[i-1] - 1);
	while (cnt > k) {
		Len += *Gap.begin(); cnt--;
		Gap.erase(Gap.begin());
	}
