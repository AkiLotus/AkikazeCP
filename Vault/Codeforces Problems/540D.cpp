...
void VarInput() {
	...
	cin >> r >> s >> p;
	dp.resize(r+1, vector<vd>(s+1, vd(p+1, 0.0)));
	dp[r][s][p] = 1.0;
}

void ProSolve() {
	//cout << "Case " << ++cas << ": " << ans << endl; // for multi-testcase problems
	queue<ppi> Q; Q.push({{r, s}, p});
	while (!Q.empty()) {
		ppi z = Q.front(); Q.pop();
		//cout << "dp[" << z.fi.fi << "][" << z.fi.se << "][" << z.se << "] = " << dp[z.fi.fi][z.fi.se][z.se] << endl;
		if (z.fi.se > 0 && z.se > 0) {
			if (dp[z.fi.fi][z.fi.se][z.se-1] == 0.0) Q.push({{z.fi.fi, z.fi.se}, z.se-1});
			dp[z.fi.fi][z.fi.se][z.se-1] += (ld)(z.fi.se * z.se) / (z.fi.fi * z.fi.se + z.fi.se * z.se + z.se * z.fi.fi) * dp[z.fi.fi][z.fi.se][z.se];
			//cout << "addition: " << "dp[" << z.fi.fi << "][" << z.fi.se << "][" << z.se-1 << "] = " << dp[z.fi.fi][z.fi.se][z.se-1] << endl;
		}
		if (z.fi.fi > 0 && z.se > 0) {
			if (dp[z.fi.fi-1][z.fi.se][z.se] == 0.0) Q.push({{z.fi.fi-1, z.fi.se}, z.se});
			dp[z.fi.fi-1][z.fi.se][z.se] += (ld)(z.fi.fi * z.se) / (z.fi.fi * z.fi.se + z.fi.se * z.se + z.se * z.fi.fi) * dp[z.fi.fi][z.fi.se][z.se];
			//cout << "addition: " << "dp[" << z.fi.fi-1 << "][" << z.fi.se << "][" << z.se << "] = " << dp[z.fi.fi-1][z.fi.se][z.se] << endl;
		}
		if (z.fi.se > 0 && z.fi.fi > 0) {
			if (dp[z.fi.fi][z.fi.se-1][z.se] == 0.0) Q.push({{z.fi.fi, z.fi.se-1}, z.se});
			dp[z.fi.fi][z.fi.se-1][z.se] += (ld)(z.fi.se * z.fi.fi) / (z.fi.fi * z.fi.se + z.fi.se * z.se + z.se * z.fi.fi) * dp[z.fi.fi][z.fi.se][z.se];
			//cout << "addition: " << "dp[" << z.fi.fi << "][" << z.fi.se-1 << "][" << z.se << "] = " << dp[z.fi.fi][z.fi.se-1][z.se] << endl;
		}
	}
	for (i64 i=1; i<=r; i++) ansR += dp[i][0][0];
	for (i64 i=1; i<=s; i++) ansS += dp[0][i][0];
	for (i64 i=1; i<=p; i++) ansP += dp[0][0][i];
	dout(15, ansR); dout(15, ansS); dout(15, ansP);
}
 ...
 * Main solution indicator at line 186.
