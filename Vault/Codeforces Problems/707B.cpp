...
i64 n, m, k; vector<vector<pii>> adj;
vector<i64> a; i64 ans = LINF;
vector<bool> storage;
...
void VarInput() {
	...
	cin >> n >> m >> k; a.resize(k);
	adj.resize(n+1, vector<pii>(0));
	storage.resize(n+1, false);
	while (m--) {
		i64 x, y, z; cin >> x >> y >> z;
		adj[x].pub(mp(y,z)); adj[y].pub(mp(x,z));
	}
	for (i64 i=0; i<k; i++) {
		cin >> a[i]; storage[a[i]] = true;
	}
}
...
void ProSolve() {
	...
	for (i64 i=0; i<k; i++) {
		for (i64 j=0; j<adj[a[i]].size(); j++) {
			if (storage[adj[a[i]][j].fi]) continue;
