...
void Solve() {
	...
	for (i64 i=0; i<2; i++) {
		for (i64 j=0; j<2; j++) {
			if (i) B = A - d1; else B = A + d1;
			if (j) C = B - d2; else C = B + d2;
			if ((k - A - B - C + min(A, min(B, C)) * 3) % 3 != 0 || k - A - B - C + min(A, min(B, C)) * 3 < 0) continue;
			if ((A + B + C + n - k) % 3 == 0 && (A + B + C + n - k) / 3 >= max(A, max(B, C))) {
				cout << "yes\n"; return;
			}
		}
	}
