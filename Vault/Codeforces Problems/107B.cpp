...
void ProSolve() {
	...
	if (total < n) {cout << "-1"; return;}
	if (total - s[h] < n-1) {cout << "1"; return;}
	/** nCr(total-s[h], n-1) / nCr(total-1, n-1) **/
	ld ans1 = 1, ans2 = 1;
	for (i64 i=1; i<=total-s[h]-n+1; i++) ans1 *= ((ld)(n-1+i) / i);
	for (i64 i=1; i<=total-1-n+1; i++) ans2 *= ((ld)(n-1+i) / i);
	dout(15, 1-ans1/ans2);
}
