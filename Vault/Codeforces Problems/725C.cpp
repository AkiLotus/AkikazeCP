...
void Solve() {
	...
	for (i64 i=0; i<26; i++) {
		if (L[i] == R[i]) continue;
		i64 Gap = R[i] - L[i] - 1;
		A[0][12-Gap/2] = char(65+i);
		i64 x = 0, y = 12 - Gap / 2;
		for (i64 r=L[i]+1; r<27; r++) {
			if (s[r] == char(65+i)) continue;
			if (y == 12 && x == 0) x = 1;
			else if (y == 0 && x == 1) x = 0;
			else if (x == 0) y++; else y--;
			A[x][y] = s[r];
		}
		x = 0; y = 12 - Gap / 2;
		for (i64 r=L[i]-1; r>=0; r--) {
			if (y == 0 && x == 0) x = 1;
			else if (y == 12 && x == 1) x = 0;
			else if (x == 0) y--; else y++;
			A[x][y] = s[r];
		}
	}
