...
/** -----GLOBAL VARIABLES----- **/
i64 n, mn = -2000000000LL, mx = 2000000000LL; string sgn, ans; i64 x;
...
void VarInput() {
	...
	cin >> n; while (n--) {
		cin >> sgn >> x >> ans;
		if ((sgn == ">" && ans == "Y") || (sgn == "<=" && ans == "N")) mn = max(mn, x+1);
		else if ((sgn == ">=" && ans == "Y") || (sgn == "<" && ans == "N")) mn = max(mn, x);
		else if ((sgn == "<" && ans == "Y") || (sgn == ">=" && ans == "N")) mx = min(mx, x-1);
		else if ((sgn == "<=" && ans == "Y") || (sgn == ">" && ans == "N")) mx = min(mx, x);
	}
	srand(time(0));
	if (mn > mx) cout << "Impossible"; else cout << ((mn + mx) / 2);
}
