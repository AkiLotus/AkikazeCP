...
void Solve() {
	i64 m = LCM(w, b);
	if (m == -1) {
		i64 p = min(min(w, b) - 1, t), q = t;
		i64 g = __gcd(p, q); p /= g; q /= g;
		cout << p << "/" << q; return;
	}
	i64 maximum = t - t % m, occ = t / m + 1;
	i64 subtract = 1;
	subtract += max((maximum + min(w, b) - 1) - t, 0LL);
	i64 p = occ * min(w, b) - subtract, q = t;
	i64 g = __gcd(p, q); p /= g; q /= g;
	cout << p << "/" << q;
}
/*****************************************/
