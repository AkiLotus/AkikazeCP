...
void Solve() {
	i64 cnt = 0; set<pii> segment; vi A;
	for (i64 i=0; i<n; i++) {
		if (s[i] == '1') continue;
		i64 j = i;
		...
		if ((j - i + 1) >= b) {
			cnt += (j - i + 1) / b;
			segment.insert(mp(i+1, j+1));
		}
		i = j;
	}
	while (cnt >= a) {
		pii z = *segment.begin(); segment.erase(z);
		i64 L = z.fi, R = z.se; cnt -= (R - L + 1) / b;
		i64 M = L + b - 1; A.pub(M);
		if ((M-1 - L + 1) >= b) {
			cnt += (M-1 - L + 1) / b;
			segment.insert(mp(L, M-1));
		}
		if ((R - M-1 + 1) >= b) {
			cnt += (R - M-1 + 1) / b;
			segment.insert(mp(M+1, R));
		}
	}
	cout << A.size() << endl;
	for (auto x: A) cout << x << " ";
