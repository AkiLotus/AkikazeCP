...
/***********************************************
** File name: 665A.cpp                        **
** Time created: Wed Jan 2 2019 11:49am       **
***********************************************/
...
void Solve() {
	vi A, B; i64 x = 300;
	i64 Min = ((Time[0] - '0') * 10 + Time[1] - '0') * 60 + (Time[3] - '0') * 10 + (Time[4] - '0');
	while (x < 1440) {B.pub(x); x += b;}
	i64 st = Min - tb + 1, en = Min + ta;
	i64 sid = lower_bound(B.begin(), B.end(), st) - B.begin();
	i64 eid = lower_bound(B.begin(), B.end(), en) - B.begin();
	cout << (eid - sid) << endl;
}
/*****************************************/
