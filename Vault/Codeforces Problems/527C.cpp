...
/** -----GLOBAL VARIABLES----- **/
i64 w, h, n; set<i64> H, V;
i64 maxH, maxV; char c; i64 p;
multiset<i64> lenH, lenV;
...
void VarInput() {
	...
	cin >> w >> h >> n; maxH = h; maxV = w;
	H.insert(0); H.insert(h); V.insert(0); V.insert(w);
...
void ProSolve() {
	while (n--) {
		cin >> c >> p;
		if (c == 'H') {
			auto it = H.lower_bound(p), it2 = it; it--;
			lenH.erase(lenH.find(*it2-*it)); lenH.insert(*it2-p); lenH.insert(p-*it);
			H.insert(p);
		}
		else {
			auto it = V.lower_bound(p), it2 = it; it--;
			lenV.erase(lenV.find(*it2-*it)); lenV.insert(*it2-p); lenV.insert(p-*it);
			V.insert(p);
		}
		auto itH = lenH.end(); itH--; maxH = *itH;
		auto itV = lenV.end(); itV--; maxV = *itV;
		//tracker2(maxH, maxV);
		cout << (maxH * maxV) << endl;
	}
}
