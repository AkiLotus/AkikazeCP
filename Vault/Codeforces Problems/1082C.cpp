...
/***********************************************
** File name: 1082C.cpp                       **
** Time created: Sat Dec 1 2018 07:18am       **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, m; vi s, r, sum, tmpsum; vector<pii> cnt;
vector<priority_queue<i64>> Heap;
/************************************************/
...
void Input() {
	cin >> n >> m; s.rsz(n); r.rsz(n); sum.rsz(m);
	cnt.rsz(m); Heap.rsz(m); tmpsum.rsz(m, 0);
	for (i64 i=0; i<m; i++) cnt[i].se = i;
	for (i64 i=0; i<n; i++) {
		cin >> s[i] >> r[i]; s[i]--;
		sum[s[i]] += r[i]; cnt[s[i]].fi++;
		Heap[s[i]].push(r[i]);
	}
}
...
void Solve() {
	...
	for (i64 i=1; i<=n; i++) {
		if (ptr == m) break;
		while (ptr < m && Heap[cnt[ptr].se].empty()) ptr++;
		i64 tmp = 0;
		for (i64 x=ptr; x<m; x++) {
			i64 id = cnt[x].se;
			tmpsum[id] += Heap[id].top();
			Heap[id].pop();
			tmp += max(0LL, tmpsum[id]);
		}
		ans = max(ans, tmp);
	}
	cout << ans;
