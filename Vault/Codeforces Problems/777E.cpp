...
/************** [GLOBAL VARIABLES] **************/
i64 n, N; vector<ppi> Data;
map<i64, i64> Map; vi Tree(888888, 0);
/************************************************/
...
void Discretization() {
	...
	N = Map.size(); vi A;
	for (auto it=Map.begin(); it!=Map.end(); it++) A.pub(it->fi);
	Map.clear(); for (i64 i=0; i<N; i++) Map[A[i]] = i;
	for (i64 i=0; i<n; i++) {
		Data[i].fi.fi = Map[Data[i].fi.fi];
		Data[i].fi.se = Map[Data[i].fi.se];
	}
}

void Update(i64 node, i64 st, i64 en, i64 i, i64 val) {
	if (st > en || en < i || i < st) return;
	if (st == en) {Tree[node] = max(Tree[node], val); return;}
...
void Input() {
	cin >> n; Data.rsz(n);
	for (i64 i=0; i<n; i++) cin >> Data[i].fi.fi >> Data[i].fi.se >> Data[i].se;
}

void Solve() {
	sort(Data.begin(), Data.end(), secondsort); Discretization();
	for (i64 i=0; i<n; i++) {
		i64 a = Data[i].fi.fi, b = Data[i].fi.se;
		i64 h = Data[i].se;
		i64 OptimalLast = Get(1, 0, N-1, 0, b-1);
		Update(1, 0, N-1, a, OptimalLast + h);
	}
