...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, t, p, x, k; vi a; set<pii> Set;
/************************************************/
...
void Solve() {
	...
	while (m--) {
		...
		if (t == 1) {
			cin >> p >> x; p--;
			while (x > 0 && Set.lower_bound(mp(p, -LINF)) != Set.end()) {
				pii z = *Set.lower_bound(mp(p, -LINF)); Set.erase(z);
				i64 filled = min(z.se, x); x -= filled;
				if (z.se > filled) Set.insert(mp(z.fi, z.se-filled)); p++;
			}
		}
		else if (t == 2) {
			cin >> k; k--;
			if (Set.lower_bound(mp(k, -LINF)) == Set.end() || Set.lower_bound(mp(k, -LINF))->fi != k) {
				cout << a[k] << endl; continue;
			}
			i64 accumulated = Set.lower_bound(mp(k, -LINF))->se; cout << a[k] - accumulated << endl;
		}
	}
}
/*****************************************/
