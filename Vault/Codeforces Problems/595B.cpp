...
/************** [FUNCTIONS] **************/
i64 calc(i64 limit, i64 divi) {
	if (limit < 0) return 0;
	return (limit / divi) + 1;
}

void Input() {
	cin >> n >> k; q = n / k; a.rsz(q); b.rsz(q);
	for (i64 i=0; i<q; i++) cin >> a[i];
	for (i64 i=0; i<q; i++) cin >> b[i];
}

void Solve() {
	i64 ans = 1, mul = 1;
	for (i64 i=1; i<k; i++) mul *= 10;
	for (i64 i=0; i<q; i++) {
		i64 sum = calc(mul*10-1, a[i]);
		sum -= calc(mul*(b[i]+1)-1, a[i]);
		sum += calc(mul*b[i]-1, a[i]);
		ans *= sum; ans %= Mod;
	}
	cout << ans;
}
