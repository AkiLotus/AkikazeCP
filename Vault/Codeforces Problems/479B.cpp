...
void VarInput() {
	...
	cin >> n >> k; a.rsz(n);
	for (i64 i=0; i<n; i++) {cin >> a[i]; S.insert(mp(a[i], i+1));}
}

void ProSolve() {
	if (n == 1) {cout << "0 0"; return;}
	vector<pii> steps; bool breakEarly = false;
	while (k--) {
		auto it=S.begin(); pii mn = *it;
		it = S.end(); it--; pii mx = *it;
		if (mx.fi - mn.fi < 2) {cout << mx.fi - mn.fi << " "; breakEarly = true; break;}
		S.erase(S.find(mn)); S.erase(S.find(mx));
		S.insert(mp(mn.fi+1,mn.se)); S.insert(mp(mx.fi-1,mx.se));
		steps.pub(mp(mx.se, mn.se));
	}
	if (!breakEarly) {
		auto it=S.begin(); pii mn = *it;
		it = S.end(); it--; pii mx = *it;
	...
	for (i64 i=0; i<steps.size(); i++) cout << steps[i].fi << " " << steps[i].se << endl;
}
