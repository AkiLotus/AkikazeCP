...
/** -----GLOBAL VARIABLES----- **/
i64 R, B; vector<pii> cR, cB;
vb assigned; bool flag = false;
vector<ppp> path;
...
bool realCut(ppp a, ppp b){
	return ccw(a.fi,a.se,b.fi)*ccw(a.fi,a.se,b.se)<0 &&
	ccw(b.fi,b.se,a.fi)*ccw(b.fi,b.se,a.se)<0;
}

void Try(i64 pos, i64 val) {
	assigned[val] = true; path[pos] = mp(cB[pos], cR[val]);
	if (pos == B-1) {
		bool flag2 = true;
		for (i64 i=0; i<B; i++) {
			for (i64 j=i+1; j<B; j++) {
				if (realCut(path[i], path[j])) {
					flag2 = false; break;
				}
			}
			if (!flag2) break;
...
void VarInput() {
	...
	cin >> R >> B; cR.rsz(R); cB.rsz(B);
	for (i64 i=0; i<R; i++) cin >> cR[i].fi >> cR[i].se;
	for (i64 i=0; i<B; i++) cin >> cB[i].fi >> cB[i].se;
}

void ProSolve() {
	if (R != B) {cout << "No"; return;}
	assigned.rsz(B, false); path.rsz(B);
	for (i64 i=0; i<R; i++) Try(0, i);
