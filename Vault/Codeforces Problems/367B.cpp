...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, p; vi a, b, List; map<i64, i64> Map;
/************************************************/
...
void Input() {
	cin >> n >> m >> p; a.rsz(n); b.rsz(m);
	for (auto &x: a) cin >> x;
	for (auto &x: b) {cin >> x; Map[x]++;}
}

void Solve() {
	i64 distinct = Map.size();
	for (i64 i=0; i<p; i++) {
		map<i64, i64> cc;
		i64 fulfilled = 0;
		for (i64 en=i; en<n; en+=p) {
			i64 st = en - p * (m - 1); cc[a[en]]++;
			if (Map.find(a[en]) != Map.end() && cc[a[en]] == Map[a[en]]) fulfilled++;
			if (Map.find(a[en]) != Map.end() && cc[a[en]] == Map[a[en]] + 1) fulfilled--;
			if (st >= 0) {
				if (fulfilled == distinct) List.pub(st+1); cc[a[st]]--;
				if (Map.find(a[st]) != Map.end() && cc[a[st]] == Map[a[st]]) fulfilled++;
				if (Map.find(a[st]) != Map.end() && cc[a[st]] == Map[a[st]] - 1) fulfilled--;
			}
		}
	}
