...
void Solve() {
	if (s % 2 != 0) {cout << "NO"; return;}
	i64 p = 0; multiset<i64> L, R;
	for (auto x: a) R.insert(x);
	for (i64 i=0; i<n-1; i++) {
		i64 x = a[i];
		p += x; L.insert(x); R.erase(R.find(x));
		if (p * 2 == s) {cout << "YES"; return;}
		if ((p - x) * 2 < s && R.find(s/2 - (p - x)) != R.end()) {cout << "YES"; return;}
		if ((p + a[i+1]) * 2 > s && L.find((p + a[i+1]) - s/2) != L.end()) {cout << "YES"; return;}
	}
	cout << "NO";
}
/*****************************************/
