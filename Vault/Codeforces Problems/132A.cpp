...
/************** [FUNCTIONS] **************/
int reverseBits(int n, int bit){
	int reverseN = 0;
	for (int i(0); i < bit; ++i){
		if ((1 << (7 - i))&n) reverseN += (1 << i);
	}
	return reverseN;
}
...
void Solve() {
	int charBefore = 0, secondStep, thirdStep;
	for (int i(0); i < s.length(); ++i)
	{
		thirdStep = reverseBits(s[i],8);
		secondStep = (charBefore - thirdStep) % 256;
		if (secondStep < 0) secondStep += 256;
		cout << secondStep << endl;
		charBefore = reverseBits(s[i],8);
	}
}
/*****************************************/
