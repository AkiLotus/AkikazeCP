...
i64 m[3], ans = LINF * 4;
i64 arr[4][3] = {{1, 1, 1}, {0, 1, 1}, {0, 0, 1}, {0, 0, 0}};
i64 dep[4][3] = {{1, 1, 1}, {1, 1, 0}, {1, 0, 0}, {0, 0, 0}};
...
void ProSolve() {
	...
	for (i64 i=0; i<4; i++) {
		for (i64 j=0; j<4; j++) {
			i64 tmp[3]; bool valid = true;
			for (i64 x=0; x<3; x++) {
				tmp[x] = m[x] - arr[i][x] - dep[j][x];
			}
			i64 zazz = *max_element(tmp, tmp+3);
			i64 tmpAns = zazz * 3;
			for (i64 i=0; i<3; i++) tmpAns -= tmp[i];
			ans = min(tmpAns, ans);
			//for (i64 i=0; i<3; i++) cout << tmp[i] << " "; cout << endl;
			//cout << "ans = " << ans << endl;
		}
	}
	cout << ans;
