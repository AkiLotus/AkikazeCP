...
/************** [GLOBAL VARIABLES] **************/
i64 n, q, k; vi a, b, l; vector<vi> List;
/************************************************/

/************** [FUNCTIONS] **************/
i64 countedTo(i64 pos) {
	i64 Rem = pos + 1;
	for (auto id: l) {
		i64 z = upper_bound(List[id].begin(), List[id].end(), pos) - List[id].begin();
		Rem -= z;
	}
	return Rem;
}

i64 binsearch_position(i64 count) {
	i64 top = 0, bot = n - 1, ans = n - 1;
	while (top <= bot) {
		i64 mid = (top + bot) / 2;
		if (countedTo(mid) >= count) {ans = mid; bot = mid - 1;}
		else top = mid + 1;
	}
	return ans;
}

i64 binsearch_trail(i64 count, i64 personID) {
	i64 N = List[personID].size();
	i64 top = 1, bot = N, ans = 1;
	while (top <= bot) {
		i64 mid = (top + bot) / 2;
		if (countedTo(List[personID][N-mid]) == count-mid+1) {
			ans = mid; top = mid + 1;
		}
		else bot = mid - 1;
	}
...
void Input() {
	cin >> n; a.rsz(n); b.rsz(n); List.rsz(n);
	for (i64 i=0; i<n; i++) {
		cin >> a[i] >> b[i]; a[i]--;
		List[a[i]].pub(i);
	}
...
void Solve() {
	...
	while (q--) {
		...
		if (Rem == 0) {cout << "0 0\n"; continue;}
		i64 idLast = binsearch_position(Rem);
		i64 trailingLen = binsearch_trail(Rem, a[idLast]);
		cout << a[idLast]+1 << " " << b[List[a[idLast]][List[a[idLast]].size()-trailingLen]] << endl;
	}
}
/*****************************************/
