...
/************** [GLOBAL VARIABLES] **************/
i64 w, m, k, ans = 0; vi Lowest(1, 0);
/************************************************/

/************** [FUNCTIONS] **************/
i64 calc(i64 Len) {
	i64 Min = m, Max = m + Len - 1, res = 0;
	for (i64 i=1; i<18; i++) {
		i64 L = Lowest[i], R = Lowest[i+1] - 1;
		i64 Intercept = max(min(R, Max) - max(L, Min) + 1, 0LL);
		if (Intercept > 0 && LINF / Intercept <= i * k) return LINF;
		res += Intercept * i * k;
	}
	return res;
}
...
void binsearch() {
	...
	while (top <= bot) {
		i64 mid = (top + bot) / 2;
		if (calc(mid) <= w) {ans = mid; top = mid + 1;}
		else bot = mid - 1;
	}
}
