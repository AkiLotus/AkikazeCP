...
void Solve() {
	vector<pii> Distribution; i64 L = LCM(x, y);
	for (i64 i=L/x; i<=L; i+=L/x) Distribution.pub(mp(i, 0LL));
	for (i64 i=L/y; i<=L; i+=L/y) Distribution.pub(mp(i, 1LL));
	sort(Distribution.begin(), Distribution.end());
	for (auto LastBreath: a) {
		LastBreath %= (x + y); LastBreath--;
		if (LastBreath == -1) {cout << "Both\n"; continue;}
		if (LastBreath > 0 && Distribution[LastBreath-1].fi == Distribution[LastBreath].fi) {cout << "Both\n"; continue;}
		if (LastBreath < x+y-1 && Distribution[LastBreath+1].fi == Distribution[LastBreath].fi) {cout << "Both\n"; continue;}
		if (Distribution[LastBreath].se == 0) cout << "Vanya\n"; else cout << "Vova\n";
	}
}
/*****************************************/
