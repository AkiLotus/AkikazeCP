...
/***********************************************
** File name: 1143B.cpp
** Time created: Sun Mar 31 2019 06:57
***********************************************/
...
void Solve() {
	...
	vector<vl> dp(m, vl(2, -1));
	dp[0][1] = n[0] - '0';
	if (n[0] > '1') dp[0][0] = n[0] - '1';
	i64 ans = 1; for (int i=1; i<m; i++) ans *= 9;
	for (int i=1; i<m; i++) {
		if (dp[i-1][0] != -1) dp[i][0] = max(dp[i][0], dp[i-1][0] * 9);
		dp[i][1] = dp[i-1][1] * (n[i] - '0');
		if (n[i] > '1') dp[i][0] = max(dp[i][0], dp[i-1][1] * (n[i] - '1'));
	}
	ans = max(ans, max(dp[m-1][0], dp[m-1][1]));
	cout << ans << endl;
}
/*****************************************/
