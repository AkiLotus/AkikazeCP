...
/************** [GLOBAL VARIABLES] **************/
i64 n; vs N, H, Main, alt, chosen; map<string, int> Map;
/************************************************/

/************** [FUNCTIONS] **************/
void DFS(string x, i64 src) {
	for (i64 i=0; i<n; i++) {
		if (chosen[i] != x) continue;
		if (i == src) continue;
		chosen[i] = alt[i];
		if (chosen[i] == x) {cout << "NO"; Exit();}
		DFS(chosen[i], i);
	}
}

void Input() {
	cin >> n; N.rsz(n); H.rsz(n);
	Main.rsz(n); alt.rsz(n); chosen.rsz(n);
	for (i64 i=0; i<n; i++) {
		cin >> N[i] >> H[i];
		Main[i] = N[i].substr(0, 3);
		alt[i] = N[i].substr(0, 2) + H[i].substr(0, 1);
		Map[Main[i]]++; chosen[i] = Main[i];
	}
}

void Solve() {
	for (i64 i=0; i<n; i++) {
		if (Map[Main[i]] == 1) continue;
		chosen[i] = alt[i]; DFS(alt[i], i);
	}
	cout << "YES";
	for (i64 i=0; i<n; i++) cout << endl << chosen[i];
}
/*****************************************/
