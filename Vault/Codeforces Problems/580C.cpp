...
i64 n, m, ans = 0; vector<vector<i64>> adj;
vector<bool> visited, cats;

/** -----EXTENSIVE FUNCTIONS----- **/
void DFS(i64 z, i64 cat) {
	visited[z] = true;
	if (cat > m) return;
	if (adj[z].size() == 1 && z != 1) ans++;
	for (i64 i=0; i<adj[z].size(); i++) {
		if (!visited[adj[z][i]]) {
			if (cats[adj[z][i]]) DFS(adj[z][i], cat+1);
			else DFS(adj[z][i], 0);
		}
	}
}
...
void VarInput() {
	...
	visited.resize(n+1, false); cats.resize(n+1, false);
	for (i64 i=1; i<=n; i++) {
		i64 z; cin >> z;
		cats[i] = (z == 1) ? true : false;
	}
	for (i64 i=1; i<n; i++) {
		i64 a, b; cin >> a >> b;
