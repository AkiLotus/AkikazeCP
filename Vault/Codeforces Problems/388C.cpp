...
/***********************************************
** File name: 388C.cpp                        **
** Time created: Sat Dec 15 2018 11:44am      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, Ciel = 0, Jiro = 0; vector<vi> Pile;
priority_queue<i64> OddHeap;
/************************************************/
...
void Input() {
	cin >> n; Pile.rsz(n);
	for (auto &Tmp: Pile) {
		i64 m; cin >> m; Tmp.rsz(m);
		for (i64 i=0; i<m; i++) {
			cin >> Tmp[i];
			if (m % 2 != 0 && i * 2 + 1 == m) {OddHeap.push(Tmp[i]); continue;}
			if (i * 2 < m) Ciel += Tmp[i]; else Jiro += Tmp[i];
		}
	}
}
...
void Solve() {
	i64 flip = 0;
	while (!OddHeap.empty()) {
		Ciel += (flip == 0) * OddHeap.top();
		Jiro += (flip == 1) * OddHeap.top();
		OddHeap.pop(); flip = 1 - flip;
	}
	cout << Ciel << " " << Jiro << endl;
}
/*****************************************/
