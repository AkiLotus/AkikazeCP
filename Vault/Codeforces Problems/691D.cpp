...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, a, b; vi p; vector<vi> adj; vb vis;
vi TmpPos, TmpVal;
/************************************************/

/************** [FUNCTIONS] **************/
void DFS(i64 z) {
	vis[z] = true; TmpPos.pub(z); TmpVal.pub(p[z]);
	for (auto t: adj[z]) if (!vis[t]) DFS(t);
}

void Input() {
	cin >> n >> m; p.rsz(n); adj.rsz(n); vis.rsz(n);
...
void Solve() {
	for (i64 i=0; i<n; i++) {
		if (vis[i]) continue;
		TmpPos.clear(); TmpVal.clear();
		DFS(i);
		sort(TmpPos.begin(), TmpPos.end());
		sort(TmpVal.rbegin(), TmpVal.rend());
		for (i64 i=0; i<TmpPos.size(); i++) p[TmpPos[i]] = TmpVal[i];
	}
