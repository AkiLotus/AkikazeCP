...
void Solve() {
	for (i64 i=1; i<=n; i++) {
		dp[i] = dp[i-1] + 20;
		i64 id2 = upper_bound(t.begin(), t.end(), t[i]-90) - t.begin();
		i64 id3 = upper_bound(t.begin(), t.end(), t[i]-1440) - t.begin();
		if (id2 > 0) id2--; if (id3 > 0) id3--;
		dp[i] = min(dp[i], dp[id2] + 50); dp[i] = min(dp[i], dp[id3] + 120);
