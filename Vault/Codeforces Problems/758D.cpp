...
void Solve() {
	u64 N = k.size(); vector<u64> dp(N, LINF);
	for (u64 i=0; i<N; i++) {
		for (u64 j=0; j<=i; j++) {
			u64 tmp = 0, flag = true;
			if (k[j] == '0' && j != i) continue;
			for (u64 x=j; x<=i; x++) {
				if (tmp * 10 + (k[x] - '0') >= n) {
					flag = false; break;
				}
			...
			if (!flag) continue;
			if (j == 0) dp[i] = min(dp[i], tmp);
			else if (LINF * 4 / n >= dp[j-1]) dp[i] = min(dp[i], dp[j-1] * n + tmp);
		}
	}
	cout << dp[N-1];
}
