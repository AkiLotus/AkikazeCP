...
/** -----GLOBAL VARIABLES----- **/
string s1, s2; i64 n1, n2, ans = 0;
vector<vi> pow29(100009, vi(4, 1));
...
i64 ILF_CF678D(i64 A, i64 B, i64 n, i64 x, i64 Mod) {
	i64 ans = (x * modPow(A, n, Mod)) % Mod;
	i64 p2Numer = modPow(A, n, Mod) - 1; while (p2Numer < 0) p2Numer += Mod;
	i64 p2Denom = A - 1; while (p2Denom < 0) p2Denom += Mod;
	if (A == 1) ans += ((B * (n % Mod)) % Mod);
	else ans += (B * ((p2Numer * modPow(p2Denom, Mod-2, Mod)) % Mod) % Mod);
	ans %= Mod; return ans;
}
...
void ProSolve() {
	vector<vi> hashVal1(n1, vi(4, 0));
	vector<vi> hashVal2(n2, vi(4, 0));
	for (i64 i=1; i<100009; i++) {
		for (i64 j=0; j<4; j++) {
			pow29[i][j] = (pow29[i-1][j] * 29) % HashMod[j];
		}
	}
	for (i64 i=0; i<n1; i++) {
		for (i64 j=0; j<4; j++) {
			if (i > 0) hashVal1[i][j] = hashVal1[i-1][j];
			hashVal1[i][j] += pow29[i][j] * (s1[i] - 'a' + 1);
			hashVal1[i][j] %= HashMod[j];
		}
	}
	for (i64 i=0; i<n2; i++) {
		for (i64 j=0; j<4; j++) {
			if (i > 0) hashVal2[i][j] = hashVal2[i-1][j];
			hashVal2[i][j] += pow29[i][j] * (s2[i] - 'a' + 1);
			hashVal2[i][j] %= HashMod[j];
		}
	}
	i64 GCD = __gcd(n1, n2);
	vi diviLen;
	for (i64 i=1; i<=sqrt(GCD); i++) {
		if (GCD % i == 0) {
			diviLen.pub(i);
			if (i != GCD / i) diviLen.pub(GCD / i);
		}
	}
	sort(diviLen.begin(), diviLen.end());
	vi expected1(4, 0), expected2(4, 0);
	for (i64 i=0; i<4; i++) {
		expected1[i] = hashVal1[n1-1][i];
		expected2[i] = hashVal2[n2-1][i];
	}
//	cout << expected1 << endl << expected2 << endl;
	for (auto x: diviLen) {
//		tracker1(x);
		vi created1(4, 0), created2(4, 0);
//		cout << hashVal1[x-1] << endl << hashVal2[x-1] << endl;
//		cout << "----------------------\n";
		if (hashVal1[x-1] != hashVal2[x-1]) break;
		for (i64 i=0; i<4; i++) {
			created1[i] = ILF_CF678D(pow29[x][i], hashVal1[x-1][i], n1/x-1, hashVal1[x-1][i], HashMod[i]);
			created2[i] = ILF_CF678D(pow29[x][i], hashVal2[x-1][i], n2/x-1, hashVal2[x-1][i], HashMod[i]);
		}
//		cout << created1 << endl << created2 << endl;
		if (created1 == expected1 && created2 == expected2) ans++;
	}
	cout << ans;
}
