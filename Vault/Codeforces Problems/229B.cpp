...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, a, b, c, k; vi Dist;
vector<vector<pii>> adj; vector<vi> disallow;
/************************************************/

/************** [FUNCTIONS] **************/
i64 earliestDeparture(i64 id, i64 Time) {
	i64 pos = lower_bound(disallow[id].begin(), disallow[id].end(), Time) - disallow[id].begin();
	if (pos == disallow[id].size() || disallow[id][pos] != Time) return Time;
	for (i64 i=pos+1; i<disallow[id].size(); i++) {
		if (disallow[id][i] - disallow[id][i-1] > 1) {
			return disallow[id][i-1] + 1;
		}
	}
	return disallow[id][disallow[id].size()-1] + 1;
}

void Input() {
	cin >> n >> m; Dist.rsz(n, LINF);
	adj.rsz(n); disallow.rsz(n);
	while (m--) {
		cin >> a >> b >> c; a--; b--;
		adj[a].pub(mp(b, c)); adj[b].pub(mp(a, c));
	}
	for (i64 i=0; i<n; i++) {
		cin >> k; disallow[i].rsz(k);
		for (i64 x=0; x<k; x++) cin >> disallow[i][x];
	}
}

void Solve() {
	priority_queue<pii> Q; vb vis(n, false);
	Dist[0] = 0; Q.push(mp(0LL, 0LL));
	while (!Q.empty()) {
		pii Z = Q.top(); Q.pop();
		i64 z = Z.se, d = -Z.fi;
		d = earliestDeparture(z, d);
	...
	if (Dist[n-1] == LINF) cout << "-1"; else cout << Dist[n-1];
}
/*****************************************/
