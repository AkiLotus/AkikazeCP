...
void Solve() {
	i64 r = 0, b = 0, y = 0, g = 0; string st = "BGRY";
	do {
		string t = st; r = 0; b = 0; y = 0; g = 0;
		for (i64 i=4; i<n; i++) t += t[i-4]; bool flag = true;
		for (i64 i=0; i<n; i++) {
			if (s[i] != t[i] && s[i] != '!') {
				flag = false; break;
			}
			if (s[i] != '!') continue;
			if (t[i] == 'R') r++; if (t[i] == 'B') b++;
			if (t[i] == 'G') g++; if (t[i] == 'Y') y++;
		}
		if (flag) cout << r << " " << b << " " << y << " " << g;
