...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi a, v1, v2; vector<pii> A;
/************************************************/

/************** [FUNCTIONS] **************/
i64 seek(i64 t) {
	i64 petr = -1, s1 = 0, s2 = 0;
	while (petr < n - 1) {
		i64 p1 = upper_bound(v1.begin(), v1.end(), petr) - v1.begin();
		i64 p2 = upper_bound(v2.begin(), v2.end(), petr) - v2.begin();
		if (p1 + t > v1.size() && p2 + t > v2.size()) return -1;
		if (p1 + t <= v1.size() && p2 + t <= v2.size()) {
			if (v1[p1+t-1] < v2[p2+t-1]) {s1++; petr = v1[p1+t-1];}
			else {s2++; petr = v2[p2+t-1];} continue;
		}
		if (p1 + t <= v1.size()) {s1++; petr = v1[p1+t-1];}
		else {s2++; petr =  v2[p2+t-1];}
	}
	if (a[n-1] == 1 && s1 <= s2) return -1;
	if (a[n-1] == 2 && s2 <= s1) return -1;
	if (a[n-1] == 1) return s1; if (a[n-1] == 2) return s2;
}
