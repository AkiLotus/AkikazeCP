...
/***********************************************
** File name: 961E-Fenwick.cpp                **
** Time created: Wed Dec 19 2018 09:58am      **
***********************************************/
...
void Input() {
	cin >> n; a.rsz(n+1); Tree.rsz(n+1); List.rsz(n+1);
	for (i64 i=1; i<=n; i++) {
		cin >> a[i];
...
void Solve() {
	i64 ans = 0;
	for (i64 i=1; i<=n; i++) {
		i64 st = i + 1, en = min(a[i], n);
		if (st <= en) ans += Range(st, en);
		for (auto id: List[i]) Update(id, -1);
	}
	cout << ans << endl;
}
/*****************************************/
