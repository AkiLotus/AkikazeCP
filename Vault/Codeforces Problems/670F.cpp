...
string finalVer, subStr;
vi finalLen(1000009, 2), cc(10, 0);
...
void VarInput() {
	...
	cin >> finalVer >> subStr;
	for (i64 i=0; i<finalVer.size(); i++) cc[finalVer[i]-'0']++;
...
void ProSolve() {
	...
	if (finalVer == "01" || finalVer == "10") {cout << "0"; return;}
	i64 nLen = finalLen[finalVer.size()];
	i64 tmp = nLen;
	while (tmp > 0) {
		cc[tmp % 10]--; tmp /= 10;
	}
	...
	if (cc[0] != 0) {
		for (i64 i=1; i<10; i++) {
			if (cc[i] != 0) {
				cc[i]--; beforeSubstring += char(48+i);
			}
		}
	}
	for (i64 i=0; i<10; i++) {
		for (i64 j=0; j<cc[i]; j++) beforeSubstring += char(48+i);
	}
	beforeSubstring += toStr(nLen);
}
