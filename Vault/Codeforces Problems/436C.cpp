...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, k, w; vector<vector<vc>> Map; vector<vi> A;
/************************************************/

/************** [FUNCTIONS] **************/
i64 diff(i64 i, i64 j) {
	if (i == 0) return (n * m);
	i64 res = 0;
	for (i64 x=0; x<n; x++) {
		for (i64 y=0; y<m; y++) {
			res += (Map[i][x][y] != Map[j][x][y]);
		}
	}
	return (res * w);
}
...
void Input() {
	cin >> n >> m >> k >> w; Map.rsz(k+1, vector<vc>(n, vc(m, '#')));
	for (i64 x=1; x<=k; x++) {
		for (i64 i=0; i<n; i++) {
			for (i64 j=0; j<m; j++) {
				cin >> Map[x][i][j];
	...
	A.rsz(k+1, vi(k+1, LINF));
}

void Solve() {
	i64 ans = 0; vector<pii> MSTList;
	priority_queue<ppi> Q; vb chosen(k+1, false);
	Q.push(mp(mp(0LL, 0LL), -1LL));
	for (i64 i=0; i<=k; i++) {
		for (i64 j=i+1; j<=k; j++) {
			A[i][j] = diff(i, j);
			A[j][i] = A[i][j];
	...
	while (!Q.empty()) {
		i64 z = Q.top().fi.se, tmp = -Q.top().fi.fi, Last = Q.top().se; Q.pop();
		if (chosen[z]) continue; chosen[z] = true; ans += tmp;
		if (Last != -1) MSTList.pub(mp(z, Last));
		for (i64 t=0; t<=k; t++) {
			if (chosen[t]) continue;
			Q.push(mp(mp(-A[z][t], t), z));
		}
	}
	cout << ans << endl;
	for (auto x: MSTList) cout << x.fi << " " << x.se << endl;
}
/*****************************************/
