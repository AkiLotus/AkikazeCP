...
/***********************************************
** File name: 1099F.cpp                       **
** Time created: Sun Jan 6 2019 11:11am       **
***********************************************/
...
void Solve() {
	...
	if (Len < k) {
		if (flake == -1) {cout << "Impossible\n"; return;}
		char toCopy = s[flake-1];
		s.insert(flake, k-Len, toCopy);
		for (i64 i=0; i<s.size(); i++) {
			if (i < s.size() && (s[i] == '?' || s[i] == '*')) {
				s.erase(i, 1); i--;
			}
