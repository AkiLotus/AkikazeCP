...
/***********************************************
** File name: 29C.cpp                         **
** Time created: Tue Dec 25 2018 12:52pm      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, A, B; map<i64, vi> List; map<i64, i64> cnt;
/************************************************/
...
void Input() {
	...
	while (n--) {
		cin >> A >> B; cnt[A]++; cnt[B]++;
		List[A].pub(B); List[B].pub(A);
	}
}

void Solve() {
	queue<i64> Q; map<i64, bool> vis;
	for (auto it=cnt.begin(); it!=cnt.end(); it++) {
		i64 z = it->fi, v = it->se;
		if (v == 1) {Q.push(z); vis[z] = true; break;}
	}
	while (!Q.empty()) {
		i64 z = Q.front(); Q.pop();
		cout << z << " ";
