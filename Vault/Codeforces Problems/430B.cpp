...
/************** [GLOBAL VARIABLES] **************/
i64 n, k, x, ans = 0; vi a;
/************************************************/

/************** [FUNCTIONS] **************/
i64 longestChain(i64 intLeft, i64 intRight) {
	if (intLeft < 0 || intRight >= n) return 0;
	if (a[intLeft] != a[intRight]) return 0;
	i64 newLeft = intLeft, newRight = intRight, cnt = 0;
	while (newLeft >= 0 && a[newLeft] == a[intLeft]) {newLeft--; cnt++;}
	while (newRight < n && a[newRight] == a[intRight]) {newRight++; cnt++;}
	if (cnt < 3) return 0;
	return cnt + longestChain(newLeft, newRight);
}
...
void Solve() {
	for (i64 i=0; i<n-1; i++) {
		...
		ans = max(ans, longestChain(i-1, i+2)+2);
	}
	cout << ans;
}
/*****************************************/
