...
/***********************************************
** File name: 768D.cpp                        **
** Time created: Sat Jan 5 2019 12:20pm       **
***********************************************/
...
const long double Pi = 3.141592653589793116, EPS = 1e-7, Gold = ((1+sqrt(5))/2);
...
void Input() {
	cin >> k >> q;
	dp.rsz(k+1); ans.rsz(1001);
}

void Solve() {
	dp[0] = 1.0L; i64 d = 1;
	for (i64 n=1; d<=1000; n++) {
		for (i64 x=k; x>0; x--) dp[x] = (x * dp[x] + (k - x + 1) * dp[x-1]) / k;
		while (d <= 1000 && 2000.0L * dp[k] >= (1.0L * d - EPS)) {ans[d] = n; d++;}
		dp[0] = 0;
	}
	while (q--) {
		i64 x; cin >> x;
