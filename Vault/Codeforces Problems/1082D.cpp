...
/***********************************************
** File name: 1082D.cpp                       **
** Time created: Sat Dec 1 2018 08:43am       **
***********************************************/
...
void Solve() {
	...
	i64 start = a[0].se, end = a[1].se;
	i64 id = n - 1, d = 1; vector<pii> Edge;
	priority_queue<pii> Heap;
	if (a[1].fi > 1) Heap.push(mp(a[1].fi-1, a[1].se));
	for (; id > 1; id--) {
		if (a[id].fi == 1) break;
		Edge.pub(mp(start, a[id].se));
		start = a[id].se; d++;
		if (a[id].fi > 2) Heap.push(mp(a[id].fi-2, a[id].se));
	}
	Edge.pub(mp(start, end));
	for (i64 i=2; i<=id; i++) {
		if (Heap.empty()) {cout << "NO\n"; return;}
		i64 z = Heap.top().se, deg = Heap.top().fi - 1;
		Edge.pub(mp(a[i].se, z));
		Heap.pop(); if (deg > 0) Heap.push(mp(deg, z));
	}
	cout << "YES " << d << endl;
	cout << Edge.size() << endl;
	for (auto E: Edge) cout << E.fi << " " << E.se << endl;
}
/*****************************************/
