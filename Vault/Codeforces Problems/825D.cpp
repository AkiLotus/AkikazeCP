...
void Solve() {
	...
	for (i64 i=0; i<26; i++) {
		if (cnt[i]) Heap.push(mp(mp(cc[i] / cnt[i], cc[i] % cnt[i]), i));
	}
	while (!Q.empty()) {
		i64 z = Q.front(); Q.pop();
		ppi Data = Heap.top(); Heap.pop();
		i64 a = Data.fi.fi, b = Data.fi.se, id = Data.se;
		s[z] = char(97+id); Heap.push(mp(mp(a+(b+1)/cnt[id], (b+1)%cnt[id]), id));
	}
	cout << s;
}
/*****************************************/
