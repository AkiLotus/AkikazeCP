...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, t, x, y, k; vi a, b, Tree, Lazy;
/************************************************/

/************** [FUNCTIONS] **************/
void BuildST(i64 node, i64 st, i64 en) {
	if (st > en) return;
	if (st == en) {Tree[node] = b[st]; return;}
...
void Propagate(i64 node, i64 st, i64 en) {
	...
	if (st != en) {
		...
		Lazy[node*2+1] = Lazy[node] + (mid - st);
	}
	else Tree[node] = a[Lazy[node]];
	Lazy[node] = -1;
}

void Update(i64 node, i64 st, i64 en, i64 L, i64 R, i64 init) {
	...
	Update(node*2, st, (st+en)/2, L, R, init);
	Update(node*2+1, (st+en)/2+1, en, L, R, init);
}

i64 Get(i64 node, i64 st, i64 en, i64 id) {
	Propagate(node, st, en);
	if (st > en || en < id || id < st) return 0;
	if (st == en && en == id) return Tree[node];
	i64 p1 = Get(node*2, st, (st+en)/2, id);
	i64 p2 = Get(node*2+1, (st+en)/2+1, en, id);
	return (p1 + p2);
}

void Input() {
	cin >> n >> m; a.rsz(n); b.rsz(n);
	Tree.rsz(n*4, 0); Lazy.rsz(n*4, -1);
	for (auto &z: a) cin >> z;
	for (auto &z: b) cin >> z;
}
