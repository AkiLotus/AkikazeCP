...
void Solve() {
	for (i64 i=1; i<n; i++) {
		for (i64 j=i+1; j<n; j++) {
			i64 len1 = i, len2 = j - i, len3 = n - len1 - len2;
			if (!valid(s.substr(0, len1))) continue;
			if (!valid(s.substr(i, len2))) continue;
			if (!valid(s.substr(j, len3))) continue;
			ans = max(ans, sum(s.substr(0, len1))+sum(s.substr(i, len2))+sum(s.substr(j, len3)));
		}
	}
	cout << ans;
}
