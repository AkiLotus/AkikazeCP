...
void Input() {
	cin >> n; for (i64 i=1; i<=n; i++) S.insert(i);
}

void Solve() {
	for (i64 i=1; i<=n; i++) {
		for (i64 j=1; j<=n; j++) {
			i64 x; cin >> x; if (x == -1) continue; if (x == 0) continue;
			if ((x == 1 || x == 3) && S.find(i) != S.end()) S.erase(i);
			if ((x == 2 || x == 3) && S.find(j) != S.end()) S.erase(j);
		}
	}
	cout << S.size() << endl; for (auto it=S.begin(); it!=S.end(); it++) cout << *it << " ";
}
/*****************************************/
