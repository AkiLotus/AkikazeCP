...
/************** [GLOBAL VARIABLES] **************/
i64 t; vector<vector<vi>> dp(31, vector<vi>(31, vi(51, LINF)));
/************************************************/
...
void Solve() {
	for (i64 i=1; i<31; i++) {
		for (i64 j=i; j<31; j++) {
			if (i * j == 1) continue;
			dp[i][j][0] = 0; dp[j][i][0] = 0;
			if (i * j < 51) dp[i][j][i*j] = 0;
			if (i * j < 51) dp[j][i][i*j] = 0;
			for (i64 x=1; x<i; x++) {
				for (i64 k=0; k<=min(50LL, x*j); k++) {
					dp[i][j][k] = min(dp[i][j][k], dp[x][j][k] + j * j);
					dp[j][i][k] = min(dp[j][i][k], dp[x][j][k] + j * j);
					if (k + (i-x)*j < 51) dp[i][j][k+(i-x)*j] = min(dp[i][j][k+(i-x)*j], dp[x][j][k] + j * j);
					if (k + (i-x)*j < 51) dp[j][i][k+(i-x)*j] = min(dp[j][i][k+(i-x)*j], dp[x][j][k] + j * j);
				}
			}
			for (i64 y=1; y<j; y++) {
				for (i64 k=0; k<=min(50LL, i*y); k++) {
					dp[i][j][k] = min(dp[i][j][k], dp[i][y][k] + i * i);
					dp[j][i][k] = min(dp[j][i][k], dp[i][y][k] + i * i);
					if (k + i*(j-y) < 51) dp[i][j][k+i*(j-y)] = min(dp[i][j][k+i*(j-y)], dp[i][y][k] + i * i);
					if (k + i*(j-y) < 51) dp[j][i][k+i*(j-y)] = min(dp[j][i][k+i*(j-y)], dp[i][y][k] + i * i);
				}
			}
		}
	}
