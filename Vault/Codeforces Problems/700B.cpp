...
/************** [GLOBAL VARIABLES] **************/
i64 n, k, u; vector<vi> adj; vi uniSub;
/************************************************/
...
void DFS(i64 z, i64 last) {
	for (auto t: adj[z]) {
		if (t == last) continue;
		DFS(t, z); uniSub[z] += uniSub[t];
	}
}

void Input() {
	cin >> n >> k; k *= 2; adj.rsz(n); uniSub.rsz(n, 0);
	for (i64 i=0; i<k; i++) {cin >> u; u--; uniSub[u]++;}
	for (i64 i=1; i<n; i++) {
		i64 x, y; cin >> x >> y; x--; y--;
...
void Solve() {
	DFS(0, -1); i64 sum = 0;
	for (i64 i=0; i<n; i++) sum += min(uniSub[i], k-uniSub[i]);
	cout << sum;
}
/*****************************************/
