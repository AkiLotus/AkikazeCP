...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi p, f; vector<vi> adj; vb vis; vi Num, ID;
/************************************************/

/************** [FUNCTIONS] **************/
void DFS(i64 z) {
	vis[z] = true;
	Num.pub(p[z]); ID.pub(z);
	for (auto t: adj[z]) {
		if (!vis[t]) DFS(t);
	}
}
...
void Input() {
	cin >> n; p.rsz(n); f.rsz(n);
	for (auto &x: p) {cin >> x; x--;}
	for (auto &x: f) cin >> x;
}

void Solve() {
	vis.rsz(n, false); adj.rsz(n);
	for (i64 i=0; i<n; i++) {
		if (i - f[i] >= 0) {
			adj[i].pub(i-f[i]);
			adj[i-f[i]].pub(i);
	...
	for (i64 i=0; i<n; i++) {
		if (vis[i]) continue;
		Num.clear(); ID.clear(); DFS(i);
		sort(Num.begin(), Num.end());
		sort(ID.begin(), ID.end());
		if (Num != ID) {cout << "NO"; return;}
	}
	cout << "YES";
}
/*****************************************/
