...
void Solve() {
	i64 equal = 0;
	for (i64 i=1; i<=4000000; i++) {
		i64 L = 3 * i * i + 3 * i, R = 6 * x + i * i * i - i;
		if (R % L != 0) continue;
		if (R / L < i) break;
		A.pub(mp(i, R/L)); if (i == R / L) equal++;
	}
	cout << A.size() * 2 - equal << endl;
	for (auto z: A) cout << z.fi << " " << z.se << endl;
	reverse(A.begin(), A.end());
	for (auto z: A) if (z.fi != z.se) cout << z.se << " " << z.fi << endl;
}
/*****************************************/
