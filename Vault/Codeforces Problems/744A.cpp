...
void VarInput() {
	...
	cin >> n >> m >> k; adj.resize(n+1, vector<i64>(0));
	visited.resize(n+1, false); gov.resize(k);
	for (i64 i=0; i<k; i++) cin >> gov[i];
	for (i64 i=0; i<m; i++) {
		i64 a, b; cin >> a >> b;
...
void ProSolve() {
	...
	for (i64 i=0; i<k; i++) {
		tmp = 0; DFS(gov[i]);
		Q.push(tmp); mx += tmp * (tmp - 1) / 2;
		n -= tmp;
	}
	i64 tp = Q.top(); mx += n * tp + n * (n - 1) / 2;
	cout << mx - m;
}
