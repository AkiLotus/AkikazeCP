...
/************** [GLOBAL VARIABLES] **************/
string n, ans = ""; vector<vi> id(3);
/************************************************/

/************** [FUNCTIONS] **************/
void removeOne(i64 s) {
	if (id[s].empty()) return;
	string Z = n; Z.erase(id[s].back(), 1);
	if (Z.size() == 0) return;
	i64 q = 0;
...
void removeTwo(i64 s) {
	if (id[3-s].size() < 2) return;
	string Z = n; Z.erase(id[3-s][id[3-s].size()-1], 1);
	Z.erase(id[3-s][id[3-s].size()-2], 1);
	if (Z.size() == 0) return;
	i64 q = 0;
	for (i64 i=0; i<Z.size(); i++) {
		if (i == Z.size() - 1 || Z[i] != '0') {
...
void Solve() {
	...
	if (s == 0) {cout << n; return;}
	removeOne(s); removeTwo(s);
	if (ans == "") cout << "-1";
	else cout << ans;
}
/*****************************************/
