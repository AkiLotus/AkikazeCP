...
i64 n, k, changed = 0, p = 0, ans = -LINF; string s;
char withinCheck = 'a';
...
void ProSolve() {
	...
	while (withinCheck < 'c') {
		p = 0; changed = (s[0] != withinCheck);
		for (i64 i=0; i<n; i++) {
			while (p < n && changed + (s[p+1] != withinCheck) <= k) {
				p++; changed += (s[p] != withinCheck);
			}
			if (p == n) p--;
			ans = max(ans, p-i+1);
			if (s[i] != withinCheck) changed--;
		}
		withinCheck++;
	}
	cout << ans;
