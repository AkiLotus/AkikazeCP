...
void Input() {
	cin >> n >> m >> c0 >> d0; a.rsz(m+1); b.rsz(m+1); c.rsz(m+1); d.rsz(m+1);
	for (i64 i=1; i<=m; i++) cin >> a[i] >> b[i] >> c[i] >> d[i];
}

void Solve() {
	vector<vi> dp(m+1, vi(n+1, 0)); i64 ans = 0;
	for (i64 i=0; i<m; i++) {
		for (i64 j=0; j<=n; j++) {
			for (i64 x=0; x<=a[i+1]/b[i+1]; x++) {
				if (j - c[i+1] * x < 0) continue;
				dp[i+1][j-c[i+1]*x] = max(dp[i+1][j-c[i+1]*x], dp[i][j] + d[i+1] * x);
			}
		}
	}
	for (i64 x=0; x<=n; x++) ans = max(ans, dp[m][x] + (i64)(x / c0) * d0);
	cout << ans;
}
/*****************************************/
