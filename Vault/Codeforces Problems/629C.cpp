...
void Solve() {
	...
	vector<vector<vi>> dp(n-m+1, vector<vi>(2011, vi(2, 0)));
	dp[0][1][0] = 1; if (Total >= 0 && Min >= 0 && Total < 2011) dp[0][Total][1]++;
	for (i64 i=0; i<n-m; i++) {
		for (i64 j=0; j<2011; j++) {
			if (j < 2010) {dp[i+1][j+1][0] += dp[i][j][0]; dp[i+1][j+1][0] %= Mod;}
			if (j > 0) {dp[i+1][j-1][0] += dp[i][j][0]; dp[i+1][j-1][0] %= Mod;}
			if (j < 2010) {dp[i+1][j+1][1] += dp[i][j][1]; dp[i+1][j+1][1] %= Mod;}
			if (j > 0) {dp[i+1][j-1][1] += dp[i][j][1]; dp[i+1][j-1][1] %= Mod;}
