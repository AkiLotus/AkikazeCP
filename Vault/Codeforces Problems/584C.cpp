...
void ProSolve() {
	i64 diff = 0;
	for (i64 i=0; i<n; i++) diff += (s1[i] != s2[i]);
	i64 exceeds = t - diff;
	srand(time(0));
	if (exceeds >= 0) {
		for (i64 i=0; i<n; i++) {
			if (s1[i] != s2[i]) {
				i64 token1 = s1[i] - 'a', token2 = s2[i] - 'a';
				i64 seed = -4;
				do seed = rand() % 26; while (seed == token1 || seed == token2);
				s3[i] = seed+97;
			}
			else if (exceeds > 0) {
				i64 token = s1[i] - 'a', seed = -4;
				do seed = rand() % 26; while (seed == token);
				s3[i] = seed+97; exceeds--;
			}
		}
	}
	else {
		i64 tmp = diff - t; t = tmp;
		if (tmp * 2 > diff) {cout << "-1"; return;}
		i64 i = 0;
		for (; i<n; i++) {
