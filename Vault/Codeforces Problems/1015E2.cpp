...
void Solve() {
	vector<vi> dpIncX, dpDecX, dpIncY, dpDecY;
	dpIncX.rsz(n, vi(m, 0)); dpDecX = dpIncX; dpIncY = dpIncX; dpDecY = dpIncX;
	for (i64 i=0; i<n-1; i++) {
		for (i64 j=0; j<m; j++) {
			if (a[i][j] != '*') continue;
			dpIncX[i+1][j] = dpIncX[i][j] + 1;
		}
	}
	for (i64 i=n-1; i>0; i--) {
		for (i64 j=0; j<m; j++) {
			if (a[i][j] != '*') continue;
			dpDecX[i-1][j] = dpDecX[i][j] + 1;
		}
	}
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<m-1; j++) {
			if (a[i][j] != '*') continue;
			dpIncY[i][j+1] = dpIncY[i][j] + 1;
		}
	}
	for (i64 i=0; i<n; i++) {
		for (i64 j=m-1; j>0; j--) {
			if (a[i][j] != '*') continue;
			dpDecY[i][j-1] = dpDecY[i][j] + 1;
		}
	}
	vector<ppi> steps; vector<vi> rowcnt(n, vi(m, 0)), colcnt(m, vi(n, 0));
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<m; j++) {
			if (a[i][j] != '*') continue;
			i64 step = min(min(dpIncX[i][j], dpDecX[i][j]), min(dpIncY[i][j], dpDecY[i][j]));
			if (step > 0) {
				steps.pub(mp(mp(i, j), step));
				rowcnt[i][j-step]++; if (j+step+1 < m) rowcnt[i][j+step+1]--;
				colcnt[j][i-step]++; if (i+step+1 < n) colcnt[j][i+step+1]--;
			}
		}
	}
	for (i64 i=0; i<n; i++) {
		for (i64 j=1; j<m; j++) {
			rowcnt[i][j] += rowcnt[i][j-1];
		}
	}
	for (i64 i=0; i<m; i++) {
		for (i64 j=1; j<n; j++) {
	...
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<m; j++) {
			if (a[i][j] == '*' && !rowcnt[i][j] && !colcnt[j][i]) {
				cout << "-1"; return;
			}
		}
	}
	cout << steps.size() << endl;
	for (auto x: steps) cout << x.fi.fi+1 << " " << x.fi.se+1 << " " << x.se << endl;
}
/*****************************************/
