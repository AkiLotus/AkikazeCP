...
void Solve() {
	ld opt = 1e18, dist = 1e18, id = 0;
	for (i64 i=1; i<n; i++) {
		ld d = (x[i] - xu) * (x[i] - xu) + yu * yu;
		d = sqrtl(d) / Vs; ld tmp = (ld)x[i] / Vb;
		if (tmp + d < opt + dist || (tmp + d == opt + dist && d < dist)) {
			opt = tmp; dist = d; id = i + 1;
		}
	}
	cout << id;
}
