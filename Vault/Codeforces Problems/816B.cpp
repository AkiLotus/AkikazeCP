...
i64 n, k, q; vi recipeCount(200001, 0);
vi admissibleCount(200001, 0);
...
void VarInput() {
	...
	while (n--) {
		i64 l, r; cin >> l >> r;
		recipeCount[l]++;
		if (r < 200000) recipeCount[r+1]--;
	}
	for (i64 i=1; i<200001; i++) recipeCount[i] += recipeCount[i-1];
	for (i64 i=1; i<200001; i++) admissibleCount[i] = admissibleCount[i-1] + (recipeCount[i] >= k);
}
...
void ProSolve() {
	...
	while (q--) {
		i64 a, b; cin >> a >> b;
		i64 ans = admissibleCount[b] - admissibleCount[a-1];
		cout << ans << endl;
	}
}
