...
void VarInput() {
	...
	cin >> n >> s; dp.rsz(n, vi(2, 0));
	for (i64 i=0; i<2; i++) dp[0][i] = (s[0] == char(48+i));
}

void ProSolve() {
	for (i64 i=1; i<n; i++) {
		dp[i][0] = dp[i-1][0]; dp[i][1] = dp[i-1][1];
		if (s[i] == '0') dp[i][0] = max(dp[i][0], dp[i-1][1]+1);
		else dp[i][1] = max(dp[i][1], dp[i-1][0]+1);
	}
	cout << min(n, max(dp[n-1][0], dp[n-1][1])+2);
}
