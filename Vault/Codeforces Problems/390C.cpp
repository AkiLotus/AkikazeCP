...
/***********************************************
** File name: 390C.cpp                        **
** Time created: Fri Jan 11 2019 09:08am      **
***********************************************/
...
void Solve() {
	vector<vector<i64>> CandyModulo(k);
	for (i64 i=0; i<n; i++) {
		if (s[i] == '0') continue;
		CandyModulo[i % k].pub(i);
	}
	while (w--) {
		cin >> l >> r; l--; r--;
		i64 res = 0;
		i64 demandedID = (l + k - 1) % k;
		for (i64 id=0; id<k; id++) {
			i64 x = lower_bound(CandyModulo[id].begin(), CandyModulo[id].end(), l) - CandyModulo[id].begin();
			i64 y = upper_bound(CandyModulo[id].begin(), CandyModulo[id].end(), r) - CandyModulo[id].begin();
			if (id == demandedID) {
				i64 TotalDemanded = (r - l + 1) / k;
				res += (TotalDemanded - (y - x));
			}
			else res += (y - x);
		}
		cout << res << endl;
