...
void Input() {
	cin >> n; vis.rsz(n, vb(n, false));
	gr.rsz(n, vb(n, false)); lw.rsz(n, vb(n, false));
	for (i64 i=1; i<n*(n-1)/2; i++) {
		cin >> x >> y; x--; y--;
		gr[x][y] = true; lw[y][x] = true;
...
void Solve() {
	for (i64 i=0; i<n; i++) {
		for (i64 j=i+1; j<n; j++) {
			...
			for (i64 x=0; x<n; x++) {
				if (x == i || x == j) continue;
				if (gr[i][x] && gr[x][j]) {cout << i+1 << " " << j+1; found = true; break;}
				else if(lw[i][x] && lw[x][j]) {cout << j+1 << " " << i+1; found = true; break;}
			}
