...
/************** [GLOBAL VARIABLES] **************/
string s; i64 x = 0, y = 0; map<pii, i64> Map;
/************************************************/
...
void Solve() {
	for (i64 i=0; i<s.size(); i++) {
		...
		if (z == 'U') y++;
		if (z == 'D') y--;
		if (Map.find(mp(x,y)) != Map.end()) {cout << "BUG"; return;}
		if (Map.find(mp(x-1,y)) != Map.end() && Map[mp(x-1,y)] < i) {cout << "BUG"; return;}
		if (Map.find(mp(x+1,y)) != Map.end() && Map[mp(x+1,y)] < i) {cout << "BUG"; return;}
		if (Map.find(mp(x,y-1)) != Map.end() && Map[mp(x,y-1)] < i) {cout << "BUG"; return;}
		if (Map.find(mp(x,y+1)) != Map.end() && Map[mp(x,y+1)] < i) {cout << "BUG"; return;}
		Map[mp(x,y)] = i + 1;
	}
	cout << "OK";
