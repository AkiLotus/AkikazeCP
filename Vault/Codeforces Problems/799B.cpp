...
/** -----GLOBAL VARIABLES----- **/
i64 n, m, clr; vi p, a, b;
vector<set<pii>> List(4);
...
void ProSolve() {
	for (i64 i=0; i<n; i++) {
		List[a[i]].insert(mp(p[i], i));
		List[b[i]].insert(mp(p[i], i));
	}
	cin >> m;
	while (m--) {
		cin >> clr;
		if (List[clr].empty()) {cout << "-1 "; continue;}
		pii z = *List[clr].begin(); i64 id = z.se;
		List[a[id]].erase(mp(p[id], id));
		List[b[id]].erase(mp(p[id], id));
		cout << z.fi << " ";
	}
}
