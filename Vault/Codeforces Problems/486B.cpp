...
void VarInput() {
	...
	cin >> m >> n; mat.resize(m, vi(n, 1)); ans = mat;
	for (i64 i=0; i<m; i++) {
		for (i64 j=0; j<n; j++) {
			cin >> mat[i][j];
			if (!mat[i][j]) {
				for (i64 k=0; k<m; k++) ans[k][j] = 0;
				for (i64 k=0; k<n; k++) ans[i][k] = 0;
			}
		}
	}
}
...
void ProSolve() {
	vector<vi> res(m, vi(n, 0));
	for (i64 i=0; i<m; i++) {
		for (i64 j=0; j<n; j++) {
			if (ans[i][j]) {
				for (i64 k=0; k<m; k++) res[k][j] = 1;
				for (i64 k=0; k<n; k++) res[i][k] = 1;
			}
		}
	}
	if (res != mat) cout << "NO";
