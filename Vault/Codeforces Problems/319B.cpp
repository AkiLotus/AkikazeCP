...
/************** [GLOBAL VARIABLES] **************/
i64 n, ans = 0; vi a, Time; stack<i64> S;
/************************************************/
...
void Input() {
	cin >> n; a.rsz(n); Time.rsz(n, 0);
	for (i64 i=0; i<n; i++) cin >> a[i];
}

void Solve() {
	for (i64 i=0; i<n; i++) {
		while (!S.empty() && a[S.top()] < a[i]) {if (Time[S.top()] > 0) Time[i] = max(Time[i], Time[S.top()] + 1); S.pop();}
		if (S.empty()) Time[i] = 0; else Time[i] = max(Time[i], 1LL);
		S.push(i);
	}
