...
i64 n; string s; bool paran = false;
/************************************************/
...
void Solve() {
	i64 ans1 = 0, ans2 = 0, cnt = 0;
	for (i64 i=0; i<n; i++) {
		if (s[i] == '(') {
			if (cnt > 0) ans1 = max(ans1, cnt); cnt = 0;
			paran = true; continue;
		}
		if (s[i] == ')') {
			if (cnt > 0) ans2++; cnt = 0;
			paran = false; continue;
		}
		if (s[i] == '_') {
			if (paran) {if (cnt > 0) ans2++;} else ans1 = max(ans1, cnt);
			cnt = 0; continue;
		}
		cnt++;
	}
