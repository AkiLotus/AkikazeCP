...
i64 n; vi w, h; vector<vb> adjMat;
vi maximum, Next; vb vis;
/************************************************/
...
void DFS(i64 z) {
	vis[z] = true;
	for (i64 t=0; t<=n; t++) {
		if (!adjMat[z][t]) continue;
		if (!vis[t]) DFS(t);
		if (maximum[t] + 1 > maximum[z]) {
			maximum[z] = maximum[t] + 1;
			Next[z] = t;
		}
	}
}
...
void Input() {
	cin >> n; w.rsz(n+1); h.rsz(n+1); adjMat.rsz(n+1, vb(n+1, false));
	for (i64 i=0; i<=n; i++) cin >> w[i] >> h[i];
}
...
void Solve() {
	...
	maximum.rsz(n+1, 0); Next.rsz(n+1, -1); vis.rsz(n+1, false); DFS(0);
	cout << maximum[0] << endl; i64 x = 0;
	while (Next[x] != -1) {
		cout << Next[x] << " ";
		x = Next[x];
	}
