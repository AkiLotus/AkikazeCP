...
i64 n, m, a, b, v, k, buck; vector<vi> adj;
vi EulerTour, Open, Close, Color, c, suffixSum, cnt;
/************************************************/

/************** [FUNCTIONS] **************/
bool sqrtsort(ppp x, ppp y) {
	if (x.fi.fi / buck != y.fi.fi / buck) return (x.fi.fi / buck < y.fi.fi / buck);
...
void Solve() {
	DFS(0, -1); vector<ppp> Query(m);
	vi Res(m, 0LL); cnt.rsz(100001, 0);
	suffixSum.rsz(100001, 0); suffixSum[0] = 100000;
	for (i64 i=0; i<m; i++) {
		cin >> v >> k; v--;
		i64 L = Open[v], R = Close[v];
		Query[i] = mp(mp(L, R), mp(k, i));
	}
	sort(Query.begin(), Query.end(), sqrtsort);
	for (i64 i=0; i<m; i++) {
		i64 j = i;
		while (j+1 < m && Query[i].fi.fi / buck == Query[j+1].fi.fi / buck) j++;
		i64 idBuck = Query[i].fi.fi / buck;
		i64 ptr1 = idBuck * buck, ptr2 = ptr1 - 1;
		for (i64 x=i; x<=j; x++) {
			i64 L = Query[x].fi.fi, R = Query[x].fi.se, key = Query[x].se.fi, id = Query[x].se.se;
			while (ptr2 < R) {
				ptr2++; cnt[Color[ptr2]]++;
				suffixSum[cnt[Color[ptr2]]]++;
			}
			while (ptr1 > L) {
				ptr1--; cnt[Color[ptr1]]++;
				suffixSum[cnt[Color[ptr1]]]++;
			}
			while (ptr1 < L) {
				suffixSum[cnt[Color[ptr1]]]--;
				cnt[Color[ptr1]]--; ptr1++;
			}
			Res[id] = suffixSum[key];
		}
		while (ptr1 <= ptr2) {
			suffixSum[cnt[Color[ptr1]]]--;
			cnt[Color[ptr1]]--; ptr1++;
