...
/************** [GLOBAL VARIABLES] **************/
i64 n, m; vector<vi> A, prefix, suffix, Line; vi Alen;
/************************************************/
...
void Input() {
	cin >> n >> m; Alen.rsz(n); A.rsz(n);
	prefix.rsz(n); suffix.rsz(n); Line.rsz(n);
	for (i64 i=0; i<n; i++) {
		cin >> Alen[i]; A[i].rsz(Alen[i]);
		prefix[i].rsz(Alen[i]+1, 0);
		suffix[i].rsz(Alen[i]+1, 0);
		Line[i].rsz(Alen[i]+1, 0);
...
void Solve() {
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<Alen[i]; j++) {
			prefix[i][j+1] = prefix[i][j] + A[i][j];
			suffix[i][j+1] = suffix[i][j] + A[i][Alen[i]-1-j];
		}
		for (i64 x=0; x<=Alen[i]; x++) {
			for (i64 y=0; x+y<=Alen[i]; y++) {
				Line[i][x+y] = max(Line[i][x+y], prefix[i][x] + suffix[i][y]);
			}
		}
	}
	vector<vi> dp(n, vi(m+1, 0));
	for (i64 i=0; i<=min(Alen[0], m); i++) dp[0][i] = max(dp[0][i], Line[0][i]);
	for (i64 i=1; i<n; i++) {
		for (i64 j=0; j<=m; j++) {
			for (i64 k=0; k<=Alen[i] && j+k <= m; k++) {
				dp[i][j+k] = max(dp[i][j+k], dp[i-1][j] + Line[i][k]);
			}
		}
	}
	cout << dp[n-1][m];
