...
void ProSolve() {
	...
	for (i64 l=0; l<n; l++) {
		...
		if (l == r) premade += s[l];
			...
			for (i64 i=0; i<2; i++) premade += s[l];
			l = r;
		}
	}
	i64 couple = 0;
	for (i64 i=0; i<premade.size(); i++) {
		if (i + 1 < premade.size() && premade[i] == premade[i+1]) {couple++; i++;}
		else {
			i64 st = i - couple * 2, cnt = 0;
			for (i64 j=st; j<i; j+=2) {
				ans += premade[j]; cnt++;
				if (cnt % 2) ans += premade[j];
			}
			ans += premade[i]; couple = 0;
		}
	}
	if (couple > 0) {
		i64 st = (i64)premade.size() - couple * 2, cnt = 0;
		for (i64 j=st; j<premade.size(); j+=2) {
			ans += premade[j]; cnt++;
			if (cnt % 2) ans += premade[j];
		}
	}
