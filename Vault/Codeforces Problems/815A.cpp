...
void ProSolve() {
	if (n <= m) {
		for (i64 i=0; i<n; i++) {
			...
			if (mn == 0) continue;
			string token = "row ";
			for (i64 x=0; x<mn; x++) ans.pub(token+ToString(i+1));
			for (i64 j=0; j<m; j++) g[i][j] -= mn;
		}
		for (i64 j=0; j<m; j++) {
			i64 mn = LINF;
			for (i64 i=0; i<n; i++) {
			...
			if (mn == 0) continue;
			string token = "col ";
			for (i64 x=0; x<mn; x++) ans.pub(token+ToString(j+1));
			for (i64 i=0; i<n; i++) g[i][j] -= mn;
		}
	}
	else {
		for (i64 j=0; j<m; j++) {
