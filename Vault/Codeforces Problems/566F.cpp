...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi a, dp; vi Map(1000001, -1);
/************************************************/
...
void Input() {
	cin >> n; a.rsz(n); dp.rsz(n, 1);
	for (i64 i=0; i<n; i++) {cin >> a[i]; Map[a[i]] = i;}
}

void Solve() {
	for (i64 i=0; i<n; i++) {
		for (i64 j=2; j<=1000000/a[i]; j++) {
			if (Map[a[i]*j] == -1) continue;
			dp[Map[a[i]*j]] = max(dp[Map[a[i]*j]], dp[i] + 1);
		}
	}
