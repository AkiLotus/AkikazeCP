...
void VarInput() {
	...
	for (i64 i=0; i<4; i++) {
		cin >> x[i].fi; x[i].se = char(65+i);
	}
}

void ProSolve() {
	sort(x.begin(), x.end());
	vector<vector<char>> a(50, vector<char>(50, x[0].se));
	i64 m = x[0].fi; for (i64 i=0; i<4; i++) x[i].fi -= m;
	for (i64 i=0; i<m; i++) {
		i64 z = (i64)(i / 25) * 2, t = (i64)(i % 25) * 2;
		if (i == m-1) {
			a[z][t] = x[3].se; a[z][t+1] = x[1].se;
			a[z+1][t] = x[2].se; a[z+1][t+1] = x[0].se;
		}
		else {
			a[z][t] = x[0].se; a[z][t+1] = x[1].se;
			a[z+1][t] = x[2].se; a[z+1][t+1] = x[3].se;
		}
	}
	i64 token = 0;
	for (i64 i=10; i<50; i+=2) {
		for (i64 j=0; j<50; j+=2) {
			while (token < 4 && x[token].fi == 0) token++;
			if (token == 4) break;
			a[i][j] = x[token].se; x[token].fi--;
		}
		if (token == 4) break;
	}
	cout << "50 50\n";
