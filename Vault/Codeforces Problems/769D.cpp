...
i64 possible[1 << 14];
i64 cc[1 << 14];
i64 n, k, pcnt = 0, ans = 0;

/** -----EXTENSIVE FUNCTIONS----- **/
void Try_(i64 pos, i64 num) {
    if (cntbit((i64)num) > k) return;
    if (cntbit((i64)num) + (14 - pos) < k) return;
    if (pos == 14) {if (cntbit((i64)num) == k) possible[pcnt++] = num; return;}
    Try_(pos+1, num*2); Try_(pos+1, num*2+1);
}
...
void ProSolve() {
	if (k == 0) {
	    for (i64 i=0; i<10001; i++) {
	        ans += (cc[i] * (cc[i] - 1)) / 2;
	    }
	    cout << ans; return;
	}
	Try_(1, 0); Try_(1, 1);
	//for (i64 i=0; i<possible.size(); i++) cout << possible[i] << " "; cout << endl;
	for (i64 i=0; i<10001; i++) {
        for (i64 j=0; j<pcnt; j++) {
            ans += cc[i] * cc[possible[j] ^ i];
        }
    }
    ans /= 2; cout << ans;
}
