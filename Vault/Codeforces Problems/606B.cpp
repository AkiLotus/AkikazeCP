...
void Solve() {
	vi cnt(n+1, 0); cnt[n] = x * y;
	vector<vb> vis(x+1, vb(y+1, false));
	vis[x0][y0] = true; cnt[0]++; cnt[n]--;
	for (i64 i=0; i<n-1; i++) {
		if (s[i] == 'D') x0 = min(x0+1, x);
		if (s[i] == 'U') x0 = max(x0-1, 1LL);
		if (s[i] == 'R') y0 = min(y0+1, y);
		if (s[i] == 'L') y0 = max(y0-1, 1LL);
		cnt[i+1] += (!vis[x0][y0]);
		cnt[n] -= (!vis[x0][y0]);
		vis[x0][y0] = true;
	}
