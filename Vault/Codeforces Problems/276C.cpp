...
void VarInput() {
	...
	cin >> n >> q; a.resize(n); cc.resize(n, 0);
	for (i64 i=0; i<n; i++) cin >> a[i];
	while (q--) {
		i64 l, r; cin >> l >> r; l--; r--;
		cc[l]++; if (r < n-1) cc[r+1]--;
	}
	for (i64 i=1; i<n; i++) cc[i] += cc[i-1];
	sort(cc.rbegin(), cc.rend()); sort(a.rbegin(), a.rend());
}

void ProSolve() {
	//cout << "Case " << ++cas << ": " << ans << endl; // for multi-testcase problems
	i64 ans = 0;
	for (i64 i=0; i<n; i++) ans += a[i] * cc[i];
	cout << ans;
}
