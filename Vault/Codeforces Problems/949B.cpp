...
/************** [FUNCTIONS] **************/
i64 calc(i64 L, i64 R, i64 pos, bool Left) {
	i64 cnt = R - L + 1;
	i64 odd = cnt / 2 + cnt % 2, even = cnt - odd;
	if (Left) {
		if (pos % 2 != 0) return (L - 1 + (pos + 1) / 2);
		return calc(L + odd, R, pos / 2, (cnt % 2 == 0));
	}
	else if (!Left) {
		if (pos % 2 == 0) return (L - 1 + pos / 2);
		return calc(L + even, R, (pos + 1) / 2, (cnt % 2 != 0));
	}
}
