...
/************** [GLOBAL VARIABLES] **************/
i64 n, k, a, b; vector<vi> adj; vi Default;
/************************************************/

/************** [FUNCTIONS] **************/
void DFS(i64 z, i64 last, vi &dp, i64 &ans, i64 &stacked, i64 &cnt) {
	for (auto t: adj[z]) {
		if (t == last) continue;
		vi dp2 = Default; i64 stk = 0, cnt2 = 1;
		DFS(t, z, dp2, ans, stk, cnt2);
		ans += stk * cnt + stacked * cnt2;
		stacked += stk;
		for (i64 i=0; i<k; i++) {
			for (i64 j=0; j<k; j++) {
				i64 sumdist = i + j + 1;
				ans += (sumdist / k + (sumdist % k > 0)) * dp[i] * dp2[j];
			}
		}
		for (i64 i=0; i<k; i++) {
			dp[(i+1)%k] += dp2[i]; cnt += dp2[i];
			if (i + 1 >= k) stacked += dp2[i];
		}
	}
}
...
void Solve() {
	vi dp = Default; i64 ans = 0, stk = 0, cnt = 1;
	DFS(0, -1, dp, ans, stk, cnt); cout << ans;
}
/*****************************************/
