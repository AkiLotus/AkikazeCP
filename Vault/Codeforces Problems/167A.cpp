...
void Solve() {
	...
	while (n--) {
		ld Time = 0.0L; cin >> t >> v;
		ld TimeTillFullSpeed = v / a;
		ld MovedTillFullSpeed = a * sqr(TimeTillFullSpeed) / 2;
		if (MovedTillFullSpeed >= d) Time = t + sqrtf(d * 2 / a);
		else Time = t + TimeTillFullSpeed + (d - MovedTillFullSpeed) / v;
		Time = max(Time, Last); Last = Time; cout << Time << endl;
	}
}
/*****************************************/
