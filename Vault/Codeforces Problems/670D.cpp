...
/** -----IDEAS/ALGORITHMS-----
	Noted: This submission is intended to solve both D1 and D2.
    -------------------------- **/
...
void ProSolve() {
	...
	i64 ans = *min_element(d.begin(), d.end());
	dEx.resize(n);
	for (i64 i=0; i<n; i++) dEx[i] = mp(mp((b[i] - ans * a[i]) / a[i], (b[i] - ans * a[i]) % a[i]), i);
	sort(dEx.begin(), dEx.end()); i64 ins = 0;
	vi sum(n, a[dEx[0].se]); for (i64 i=1; i<n; i++) sum[i] = sum[i-1] + a[dEx[i].se];
	while (k > 0) {
		i64 pos = lower_bound(dEx.begin(), dEx.end(), mp(mp(dEx[ins].fi.fi+1, 0LL), 0LL)) - dEx.begin();
		if (pos == n) {
			i64 rem = 0, total = sum[pos-1];
			for (i64 i=ins; i<pos; i++) rem += dEx[i].fi.se;
			if (k < (total-rem)) break;
			k -= (total-rem); ans++;
			ans += (k / total); break;
		}
		i64 rem = 0, total = sum[pos-1];
		...
		if (k < (total-rem)) break;
		k -= (total-rem); ans++;
		i64 q = min(k / total, dEx[pos].fi.fi-dEx[ins].fi.fi-1);
		ans += q; ins = pos; k -= q * total;
	}
	cout << ans;
}
