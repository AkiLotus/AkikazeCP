...
void VarInput() {
	...
	for (i64 i=0; i<n; i++) {
		i64 x, y; cin >> x >> y;
		A.insert(mp(mp(x,y), i));
		G.insert(mp(mp(y,x), i));
	}
}

void ProSolve() {
	string ans = ""; for (i64 i=0; i<n; i++) ans += "A";
	i64 diff = 0;
	while (!A.empty() || !G.empty()) {
		if (diff < -500) {
			ppi z = *A.begin(); A.erase(A.begin()); ans[z.se] = 'A';
			diff += z.fi.fi; ppi q = mp(mp(z.fi.se, z.fi.fi), z.se);
			G.erase(G.find(q));
		}
		else if (diff > +500) {
			ppi z = *G.begin(); G.erase(G.begin()); ans[z.se] = 'G';
			diff -= z.fi.fi; ppi q = mp(mp(z.fi.se, z.fi.fi), z.se);
			A.erase(A.find(q));
		}
		else {
			ppi z1 = *A.begin(), z2 = *G.begin();
			if (abs(diff + z1.fi.fi) < abs(diff - z2.fi.fi)) {
				ppi z = *A.begin(); A.erase(A.begin()); ans[z.se] = 'A';
				diff += z.fi.fi; ppi q = mp(mp(z.fi.se, z.fi.fi), z.se);
				G.erase(G.find(q));
			}
