...
void Input() {
	cin >> n >> m >> xc >> yc >> k; d.rsz(k);
	for (i64 i=0; i<k; i++) cin >> d[i].fi >> d[i].se;
}
...
void Solve() {
	...
	for (auto z: d) {
		i64 x = z.fi, y = z.se;
		i64 cntx = LINF, cnty = LINF;
		if (x < 0) cntx = (xc - 1) / (-x);
		else if (x > 0) cntx = (n - xc) / (+x);
		if (y < 0) cnty = (yc - 1) / (-y);
		else if (y > 0) cnty = (m - yc) / (+y);
		i64 steps = min(cntx, cnty);
		xc += steps * x; yc += steps * y; ans += steps;
	}
	cout << ans;
}
/*****************************************/
