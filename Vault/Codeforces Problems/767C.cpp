...
/************** [GLOBAL VARIABLES] **************/
i64 n, root = 0; vi p, Lumine; vector<vi> adj;
vi Desired; vb allowedDouble;
/************************************************/

/************** [FUNCTIONS] **************/
void DFS_Illuminate(i64 z) {
	for (auto t: adj[z]) {
		if (p[z] == t) continue;
		DFS_Illuminate(t);
		Lumine[z] += Lumine[t];
...
void DFS(i64 z) {
	...
	if (z == root) return;
	if (Lumine[z] == Lumine[root] / 3 && Desired.empty()) {
		Desired.pub(z+1); i64 x = z;
		while (p[x] != root) {
			x = p[x]; allowedDouble[x] = true;
		}
		return;
	}
	if (Desired.size() == 1) {
		if (allowedDouble[z] && Lumine[z] == Lumine[root] * 2 / 3) Desired.pub(z+1);
		if (!allowedDouble[z] && Lumine[z] == Lumine[root] / 3) Desired.pub(z+1);
	}
}

void Input() {
	cin >> n; Lumine.rsz(n);
	p.rsz(n); adj.rsz(n); allowedDouble.rsz(n, false);
	for (i64 i=0; i<n; i++) {
		cin >> p[i] >> Lumine[i];
		p[i]--; if (p[i] == -1) root = i;
		else {adj[i].pub(p[i]); adj[p[i]].pub(i);}
	}
}

void Solve() {
	DFS_Illuminate(root);
	if (Lumine[root] % 3 != 0) {cout << "-1"; return;}
	DFS(root);
	if (Desired.size() < 2) {cout << "-1"; return;}
	cout << Desired[0] << " " << Desired[1];
}
/*****************************************/
