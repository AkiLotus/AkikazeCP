...
/************** [GLOBAL VARIABLES] **************/
i64 n, m; vector<vc> A; vi Left, Right;
/************************************************/
...
void Input() {
	cin >> n >> m; A.rsz(n, vc(m, 'G'));
	Left.rsz(n, -1); Right.rsz(n, -1);
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<m; j++) {
			cin >> A[i][j];
			if (A[i][j] == 'G') continue;
...
void Solve() {
	vector<vector<vi>> dp(n, vector<vi>(m, vi(2, LINF)));
	if (Left[0] == -1) for (i64 i=0; i<m; i++) dp[0][i][0] = i;
	else for (i64 i=Right[i]; i<m; i++) dp[0][i][0] = i;
	for (i64 i=1; i<n; i++) {
		for (i64 j=0; j<m; j++) {
			for (i64 k=0; k<2; k++) {
				if (Left[i] == -1) {
					if (k == 1) for (i64 x=j; x<m; x++) dp[i][x][0] = min(dp[i][x][0], dp[i-1][j][1] + 1 + (x - j));
					if (k == 0) for (i64 x=j; x>=0; x--) dp[i][x][1] = min(dp[i][x][1], dp[i-1][j][0] + 1 + (j - x));
					continue;
				}
				if (k == 1 && Left[i] < j) continue;
				if (k == 0 && Right[i] > j) continue;
				if (k == 1) for (i64 x=Right[i]; x<m; x++) dp[i][x][0] = min(dp[i][x][0], dp[i-1][j][1] + 1 + (x - j));
				if (k == 0) for (i64 x=Left[i]; x>=0; x--) dp[i][x][1] = min(dp[i][x][1], dp[i-1][j][0] + 1 + (j - x));
			}
		}
	}
	for (i64 i=n-1; i>=0; i--) {
