...
i64 parent(i64 node, i64 dist) {
	if (dist == 0) return node;
	for (i64 i=16; i>=0; i--) {
		if (dist >= (1LL << i)) {
			return parent(Table[node][i], dist - (1LL << i));
		}
	}
}
...
void Solve() {
	...
	while (m--) {
		...
		i64 lca = LCA(a, b);
		if (d[a] == d[b]) {
			i64 ca = parent(a, d[a]-d[lca]-1), cb = parent(b, d[b]-d[lca]-1);
			cout << (n - subtree[ca] - subtree[cb]) << endl; continue;
		}
		if ((d[a] + d[b] - d[lca] * 2) % 2 != 0) {cout << "0\n"; continue;}
		i64 midpoint = parent(b, (d[a]+d[b]-d[lca]*2)/2), cm = parent(b, (d[a]+d[b]-d[lca]*2)/2-1);
		cout << (subtree[midpoint] - subtree[cm]) << endl;
	}
}
/*****************************************/
