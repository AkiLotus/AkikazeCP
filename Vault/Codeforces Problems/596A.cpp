...
void Solve() {
	...
	if (n == 2 && (a[0].fi == a[1].fi || a[0].se == a[1].se)) {cout << "-1"; return;}
	if (n == 2) {cout << abs(a[0].se - a[1].se) * abs(a[1].fi-a[0].fi); return;}
	sort(a.begin(), a.end());
	cout << max(abs(a[0].fi - a[2].fi) * abs(a[1].se - a[0].se), abs(a[0].fi - a[1].fi) * abs(a[2].se - a[0].se));
}
/*****************************************/
