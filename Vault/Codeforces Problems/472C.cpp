...
void ProSolve() {
	...
	vi deterministic(n, 0); vs ff = f, ss = s;
	for (i64 i=0; i<n; i++) {ff[i] = f[p[i]]; ss[i] = s[p[i]];}
	f = ff; s = ss;
	if ((f[0] < f[1] && s[0] < s[1]) || (f[0] < s[1] && s[0] < f[1])) deterministic[1] = 0;
	else if (f[0] < f[1] || s[0] < f[1]) deterministic[1] = 1;
	else if (f[0] < s[1] || s[0] < s[1]) deterministic[1] = 2;
	else {cout << "NO"; return;}
	for (i64 i=1; i<n; i++) {
		if (deterministic[i-1] == 1) {
			if (f[i-1] > s[i] && f[i-1] > f[i]) {
				cout << "NO"; return;
			}
			else if (f[i-1] < f[i] && f[i-1] < s[i]) deterministic[i] = 0;
			else if (f[i-1] < f[i]) deterministic[i] = 1; else deterministic[i] = 2;
		}
		else if (deterministic[i-1] == 2) {
			if (s[i-1] > s[i] && s[i-1] > f[i]) {
				cout << "NO"; return;
			}
			else if (s[i-1] < f[i] && s[i-1] < s[i]) deterministic[i] = 0;
			else if (s[i-1] < f[i]) deterministic[i] = 1; else deterministic[i] = 2;
		}
		else {
			if ((f[i-1] < f[i] && s[i-1] < s[i]) || (f[i-1] < s[i] && s[i-1] < f[i])) deterministic[i] = 0;
			else if (f[i-1] < f[i] || s[i-1] < f[i]) deterministic[i] = 1;
			else if (f[i-1] < s[i] || s[i-1] < s[i]) deterministic[i] = 2;
			else {cout << "NO"; return;}
		}
	}
	cout << "YES";
