...
/************** [GLOBAL VARIABLES] **************/
string rook, knight; vector<vb> vis(8, vb(8, false));
i64 xr, yr, xk, yk;
...
void Input() {
	cin >> rook >> knight;
	xr = (rook[0] - 'a'); yr = (rook[1] - '1');
	xk = (knight[0] - 'a'); yk = (knight[1] - '1');
	vis[xr][yr] = true; vis[xk][yk] = true;
}
...
void Solve() {
	for (i64 i=0; i<8; i++) {
		vis[xr][i] = true;
		vis[i][yr] = true;
		if (xk+dx[i] >= 0 && xk+dx[i] < 8 && yk+dy[i] >= 0 && yk+dy[i] < 8) {
			vis[xk+dx[i]][yk+dy[i]] = true;
		}
		if (xr+dx[i] >= 0 && xr+dx[i] < 8 && yr+dy[i] >= 0 && yr+dy[i] < 8) {
			vis[xr+dx[i]][yr+dy[i]] = true;
		}
	}
	i64 ans = 0;
	for (i64 i=0; i<64; i++) ans += (!vis[i/8][i%8]);
	cout << ans;
}
/*****************************************/
