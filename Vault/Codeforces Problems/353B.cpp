...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi a, cnt(100, 0), t;
map<i64, i64> Map; set<i64> Dist;
/************************************************/
...
void Input() {
	cin >> n; n *= 2; a.rsz(n); t.rsz(n, 1);
	for (auto &x: a) {cin >> x; cnt[x]++;}
}

void Solve() {
	i64 odd = 0, once = 1, converted = 0;
	for (i64 i=0; i<n; i++) {
		if (cnt[a[i]] == 1) {
			once = 3 - once; t[i] = once;
			converted += (once == 2);
		}
	}
	for (i64 i=0; i<100; i++) {
	...
	for (i64 i=0; i<n; i++) {
		...
		if (Map[a[i]] % 2 == 0) t[i] = 2;
		else if (Map[a[i]] == cnt[a[i]]) {
			if (odd > 0) {t[i] = 2; odd--;}
		}
	}
	for (i64 i=0; i<n; i++) {
