...
/************** [FUNCTIONS] **************/
void extendedEuclidean(i64 a, i64 b, i64 &x, i64 &y, i64 &gcd) {
	if (b == 0) {
		gcd = a;
		x = 1; y = 0;
		return;
	}
	extendedEuclidean(b, a % b, x, y, gcd);
...
void Solve() {
	if (A == 0) {if (C % B == 0) cout << "0 " << (-C / B); else cout << "-1"; return;}
	if (B == 0) {if (C % A == 0) cout << (-C / A) << " 0"; else cout << "-1"; return;}
	if (C % __gcd(A, B) != 0) {cout << "-1"; return;}
	i64 x, y, g; extendedEuclidean(A, B, x, y, g);
	cout << x * (-C / g) << " " << y * (-C / g);
}
/*****************************************/
