...
i64 n, l, r, mn = LINF, mx = -LINF;
pii optimal = mp(LINF, -LINF); map<pii, i64> Map;
/************************************************/
...
void Input() {
	cin >> n;
	for (i64 i=1; i<=n; i++) {
		cin >> l >> r; mn = min(mn, l); mx = max(mx, r);
		Map[mp(l, r)] = i;
		if (optimal.se < r || (optimal.se == r && optimal.fi > l)) {
			optimal = mp(l, r);
		}
	}
}
...
void Solve() {
	if (optimal == mp(mn, mx)) cout << Map[optimal]; else cout << "-1";
}
/*****************************************/
