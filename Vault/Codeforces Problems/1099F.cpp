...
/***********************************************
** File name: 1099F.cpp                       **
** Time created: Sun Jan 6 2019 11:52am       **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, T; vi x, t, p, l, dp; vector<vi> children;
vi FT_Time(1000001, 0), FT_Count(1000001, 0);
/************************************************/

/************** [FUNCTIONS] **************/
void update(i64 id, i64 val) {
	for (i64 i = id; i <= 1000000; i += i & -i) {
		FT_Time[i] += val * id;
		FT_Count[i] += val;
	}
}

i64 prefixTime(i64 id) {
	i64 res = 0;
	for (i64 i = id; i; i -= i & -i) {
		res += FT_Time[i];
	}
	return res;
}

i64 prefixCount(i64 id) {
	i64 res = 0;
	for (i64 i = id; i; i -= i & -i) {
		res += FT_Count[i];
	}
...
i64 maxCookies(i64 Rem) {
	i64 top = 0, bot = 1000000, ans = -1;
	while (top <= bot) {
		i64 mid = (top + bot) / 2;
		if (prefixTime(mid) <= Rem) {ans = mid; top = mid + 1;}
		else bot = mid - 1;
	}
	i64 cookies = prefixCount(ans);
	if (ans < 1000000) {
		i64 Stackable = min(prefixCount(ans+1) - prefixCount(ans), (Rem - prefixTime(ans)) / (ans + 1));
		cookies += Stackable;
	}
	return cookies;
}

void DFS_Precalc(i64 z, i64 TravellingTime) {
	if (TravellingTime >= T) return;
	update(t[z], +x[z]);
	dp[z] = maxCookies(T - TravellingTime);
	for (auto v: children[z]) {
		DFS_Precalc(v,  TravellingTime + 2 * l[v]);
	}
	update(t[z], -x[z]);
}
...
void DFS_DP(i64 z) {
	...
	if (z == 0) {
		dp[z] = max(dp[z], Heap.top());
		return;
	}
	if (Heap.size() < 2) return;
	Heap.pop(); dp[z] = max(dp[z], Heap.top());
}

void Input() {
	cin >> n >> T; x.rsz(n); t.rsz(n); dp.rsz(n);
	p.rsz(n, -1); l.rsz(n, 0); children.rsz(n);
	for (auto &z: x) cin >> z;
	for (auto &z: t) cin >> z;
	for (i64 i=1; i<n; i++) {
		cin >> p[i] >> l[i];
