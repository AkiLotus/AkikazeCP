...
/** -----GLOBAL VARIABLES----- **/
i64 a, b, c, m, equi = 0, price = 0;
priority_queue<i64, vi, greater<i64>> usb, ps2;
...
void VarInput() {
	...
	cin >> a >> b >> c >> m;
	while (m--) {
		i64 p; string t; cin >> p >> t;
		if (t == "USB") usb.push(p); else ps2.push(p);
	}
}

void ProSolve() {
	while (a > 0 && !usb.empty()) {
		a--; equi++; price += usb.top(); usb.pop();
	}
	while (b > 0 && !ps2.empty()) {
		b--; equi++; price += ps2.top(); ps2.pop();
	}
	priority_queue<i64, vi, greater<i64>> rem;
	while (!usb.empty()) {rem.push(usb.top()); usb.pop();}
	while (!ps2.empty()) {rem.push(ps2.top()); ps2.pop();}
	while (c > 0 && !rem.empty()) {
		c--; equi++; price += rem.top(); rem.pop();
	}
	cout << equi << " " << price;
}
