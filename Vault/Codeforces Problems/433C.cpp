...
void Solve() {
	i64 ans = 0, origin = 0;
	for (i64 i=1; i<m; i++) {
		origin += abs(a[i] - a[i-1]);
		ans += abs(a[i] - a[i-1]);
	}
	for (i64 i=0; i<n; i++) sort(adj[i].begin(), adj[i].end());
	for (i64 i=0; i<n; i++) {
		if (adj[i].empty()) continue;
		i64 curr = 0, sumLeft = 0, sumRight = 0, N = adj[i].size();
		for (auto j: adj[i]) {curr += abs(i-j); sumRight += j;}
		ans = min(ans, origin - curr + sumRight - adj[i][0] * N);
		ans = min(ans, origin - curr + adj[i][N-1] * N - sumLeft);
		for (i64 x=0; x<N-1; x++) {
			sumRight -= adj[i][x]; sumLeft += adj[i][x];
			ans = min(ans, origin - curr + adj[i][x+1] * (x + 1) - sumLeft + sumRight - adj[i][x+1] * (N - x - 1));
		}
	}
	cout << ans;
}
