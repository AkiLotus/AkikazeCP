...
void VarInput() {
	...
	cin >> n >> m; w.rsz(n); b.rsz(m); Stack.rsz(n);
	for (i64 i=0; i<n; i++) cin >> w[i];
	map<i64, i64> Map; i64 ptr1 = 0;
	for (i64 i=0; i<m; i++) {
		cin >> b[i]; if (!Map[b[i]]) Stack[ptr1++] = b[i];
		Map[b[i]]++;
	}
	for (i64 i=1; i<=n; i++) {
		if (!Map[i]) Stack[ptr1++] = i;
	}
}
...
void ProSolve() {
	i64 ans = 0;
	for (i64 i=0; i<m; i++) {
		i64 pos = find(Stack.begin(), Stack.end(), b[i]) - Stack.begin();
		while (pos > 0) {
			ans += w[Stack[pos-1]-1];
			swap(Stack[pos], Stack[pos-1]); pos--;
		}
	}
	cout << ans;
}
