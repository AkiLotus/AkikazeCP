...
void VarInput() {
	...
	for (i64 i=0; i<n; i++) {cin >> a[i]; a[i]--; ans[i] = i;}
}

void ProSolve() {
	//cout << a << endl;
	//REcheck;
	vis.resize(n, false); vis[0] = true; queue<i64> Q; Q.push(0);
	while (!Q.empty()) {
		i64 z = Q.front(); Q.pop(); //tracker1(z);
		if (z < n-1 && !vis[z+1]) {Q.push(z+1); ans[z+1] = min(ans[z+1], ans[z]+1); vis[z+1] = true;}
		if (z > 0 && !vis[z-1]) {Q.push(z-1); ans[z-1] = min(ans[z-1], ans[z]+1); vis[z-1] = true;}
		if (!vis[a[z]]) {Q.push(a[z]); ans[a[z]] = min(ans[z]+1, ans[a[z]]); vis[a[z]] = true;}
	}
