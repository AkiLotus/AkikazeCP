...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, u, v, w; vector<ppi> Edge;
long long MSTweight = 0;
vector<vector<pii>> GraphAdj, TreeAdj;
vi d, Parent; vector<vi> TableLCA, TableMax;
/************************************************/

/************** [FUNCTIONS] **************/
i64 DSUfind(i64 x, vi &DSUpar, vi &DSUsize) {
	while (DSUpar[x] != -1) x = DSUpar[x];
	return x;
}

void DSUmerge(i64 x, i64 y, vi &DSUpar, vi &DSUsize) {
	i64 a = DSUfind(x, DSUpar, DSUsize);
	i64 b = DSUfind(y, DSUpar, DSUsize);
	if (a == b) return;
	if (DSUsize[a] < DSUsize[b]) swap(a, b);
	DSUsize[a] += DSUsize[b]; DSUpar[b] = a;
}

void CreateMST() {
	vi DSUpar(n, -1), DSUsize(n, +1);
	priority_queue<pip, vector<pip>, greater<pip>> Heap;
	for (auto Z: Edge) Heap.push(mp(Z.se, Z.fi));
	while (!Heap.empty()) {
		pip z = Heap.top(); Heap.pop();
		w = z.fi; u = z.se.fi; v = z.se.se;
		if (DSUfind(u, DSUpar, DSUsize) != DSUfind(v, DSUpar, DSUsize)) {
			TreeAdj[u].pub(mp(v, w)); TreeAdj[v].pub(mp(u, w));
			DSUmerge(u, v, DSUpar, DSUsize); MSTweight += w;
		}
	}
}
...
void ProcessMST() {
	d.rsz(n, 0); TableLCA.rsz(n); TableMax.rsz(n); DFS(0, -1);
	for (i64 j=1; j<20; j++) {
		for (i64 i=0; i<n; i++) {
			if (TableLCA[i].size() < j) continue;
			if (TableLCA[TableLCA[i][j-1]].size() < j) continue;
			TableLCA[i].pub(TableLCA[TableLCA[i][j-1]][j-1]);
			TableMax[i].pub(max(TableMax[i][j-1], TableMax[TableLCA[i][j-1]][j-1]));
		}
	}
}

i64 GetMax(i64 x, i64 y) {
	if (x == y) return 0;
	if (d[x] == d[y]) {
		i64 id = 0, Init = min(TableLCA[x].size(), TableLCA[y].size()-1);
		for (i64 i=Init; i>=0; i--) {
			if (TableLCA[x][i] != TableLCA[y][i]) {id = i; break;}
		}
		return max(max(TableMax[x][id], TableMax[y][id]), GetMax(TableLCA[x][id], TableLCA[y][id]));
	}
	if (d[x] < d[y]) {
		i64 mul = 1, id = 0;
		while (d[x] < d[y] - mul * 2) {
		...
		return max(TableMax[y][id], GetMax(x, TableLCA[y][id]));
	}
	if (d[x] > d[y]) {
		i64 mul = 1, id = 0;
		while (d[y] < d[x] - mul * 2) {
		...
		return max(TableMax[x][id], GetMax(TableLCA[x][id], y));
	}
}

void Input() {
	cin >> n >> m; Edge.rsz(m); Parent.rsz(n, -1);
	GraphAdj.rsz(n); TreeAdj.rsz(n);
	for (auto &Z: Edge) {
		cin >> u >> v >> w; u--; v--;
		Z = mp(mp(u, v), w);
		GraphAdj[u].pub(mp(v, w));
		GraphAdj[v].pub(mp(u, w));
	}
}

void Solve() {
	CreateMST(); ProcessMST();
	for (auto Z: Edge) {
		u = Z.fi.fi; v = Z.fi.se; w = Z.se;
		i64 MaxEliminated = GetMax(u, v);
		cout << (MSTweight + w - MaxEliminated) << endl;
	}
}
/*****************************************/
