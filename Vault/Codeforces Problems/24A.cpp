...
/************** [GLOBAL VARIABLES] **************/
i64 n, a, b, c; vi Cycle; vb vis;
vector<vb> undirMatrix; vector<vi> costMatrix;
/************************************************/

/************** [FUNCTIONS] **************/
void DFS(i64 z) {
	vis[z] = true; Cycle.pub(z);
	for (i64 t=0; t<n; t++) {
		if (!undirMatrix[z][t]) continue;
		if (!vis[t]) DFS(t);
	}
}

void Input() {
	cin >> n; undirMatrix.rsz(n, vb(n, false));
	costMatrix.rsz(n, vi(n, LINF)); vis.rsz(n, false);
	for (i64 i=0; i<n; i++) {
		cin >> a >> b >> c; a--; b--;
		costMatrix[b][a] = c;
		undirMatrix[a][b] = true;
		undirMatrix[b][a] = true;
	}
}

void Solve() {
	DFS(0); i64 ans1 = 0, ans2 = 0;
	for (i64 i=0; i<n; i++) {
		i64 u = Cycle[i];
		i64 j = (i + 1) % n; i64 v = Cycle[j];
		if (costMatrix[u][v] < LINF) ans1 += costMatrix[u][v];
		j = (i + n - 1) % n; v = Cycle[j];
		if (costMatrix[u][v] < LINF) ans2 += costMatrix[u][v];
	}
	cout << min(ans1, ans2);
}
/*****************************************/
