...
i64 n, k1, k2; vi a, b; map<pvv, i64> Map;
/************************************************/
...
void Input() {
	cin >> n;
	cin >> k1; a.rsz(k1); for (i64 i=0; i<k1; i++) cin >> a[i];
	cin >> k2; b.rsz(k2); for (i64 i=0; i<k2; i++) cin >> b[i];
}

void Solve() {
	i64 fights = 0;
	while (!Map[mp(a,b)] && !a.empty() && !b.empty()) {
		Map[mp(a,b)]++; Map[mp(b,a)]++; fights++;
		if (a[0] > b[0]) {a.pub(b[0]); a.pub(a[0]);}
		else {b.pub(a[0]); b.pub(b[0]);}
		a.erase(a.begin()); b.erase(b.begin());
	}
	if (a.empty()) cout << fights << " 2";
	else if (b.empty()) cout << fights << " 1";
	else cout << "-1";
}
/*****************************************/
