...
void Input() {
	...
	for (i64 i=0; i<k; i++) wall[0] += "-";
	for (i64 i=0; i<k; i++) wall[1] += "-";
}

void Solve() {
	queue<ppi> Q; vector<vb> vis(2, vb(wall[0].size(), false));
	Q.push(mp(mp(0LL, 0LL), 0LL)); vis[0][0] = true;
	while (!Q.empty()) {
		ppi Data = Q.front(); Q.pop();
		i64 x = Data.fi.fi, y = Data.fi.se, t = Data.se;
		if (y >= n) {cout << "YES"; return;}
		if (y-1 >= 0 && y-1 > t && !vis[x][y-1] && wall[x][y-1] != 'X') {
			vis[x][y-1] = true; Q.push(mp(mp(x, y-1), t+1));
		}
		if (!vis[x][y+1] && wall[x][y+1] != 'X') {
			vis[x][y+1] = true; Q.push(mp(mp(x, y+1), t+1));
		}
		if (!vis[1-x][y+k] && wall[1-x][y+k] != 'X') {
			vis[1-x][y+k] = true; Q.push(mp(mp(1-x, y+k), t+1));
		}
	}
	cout << "NO";
}
