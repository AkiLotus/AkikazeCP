...
void Solve() {
	i64 ans = 0; Preprocess();
	for (i64 i=0; i<n; i++) {
		for (i64 j=i+1; j<n; j++) {
			i64 Dist1 = Dist(i, j);
			i64 A = LCA(i, j);
			vi vis(n, 0); i64 x = i, y = j;
			vis[x] = 1; vis[y] = 1; vis[A] = 1;
			while (!Table[x].empty() && Table[x][0] != A) {
				x = Table[x][0]; vis[x] = 1;
			}
			while (!Table[y].empty() && Table[y][0] != A) {
				y = Table[y][0]; vis[y] = 1;
			}
			for (i64 c=0; c<n; c++) {
				if (vis[c]) continue;
				queue<i64> Q; i64 root = -1;
				vis[c] = 2; Q.push(c);
				while (!Q.empty()) {
					i64 z = Q.front(); Q.pop();
					root = z;
				...
				queue<pii> QQ; vis[root] = 1;
				QQ.push(mp(root, 0LL));
				while (!QQ.empty()) {
					pii Data = QQ.front(); QQ.pop();
					i64 z = Data.fi, d = Data.se;
					ans = max(ans, Dist1 * d);
					for (auto t: adj[z]) {
						if (vis[t] == 1) continue;
						vis[t] = 1; QQ.push(mp(t, d+1));
					}
				}
			}
		}
