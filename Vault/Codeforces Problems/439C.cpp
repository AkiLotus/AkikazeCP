...
void ProSolve() {
	for (i64 i=0; i<p; i++) {
		if (!even.empty()) {
			vi tmp(1, even.front()); even.pop();
			partitions.pub(tmp);
		}
		else if (odd.size() > 1) {
			vi tmp;
			tmp.pub(odd.front()); odd.pop();
			tmp.pub(odd.front()); odd.pop();
			partitions.pub(tmp);
		}
		else {cout << "NO"; return;}
	...
	if (odd.size() % 2 == 0) {
		while (!even.empty()) {
			partitions[0].pub(even.front());
			even.pop();
		}
		while (!odd.empty()) {
			partitions[0].pub(odd.front());
			odd.pop();
		}
	}
	else {cout << "NO"; return;}
	cout << "YES\n";
	for (i64 i=0; i<partitions.size(); i++) {
		cout << partitions[i].size();
		for (i64 j=0; j<partitions[i].size(); j++) cout << " " << partitions[i][j];
		cout << endl;
	}
}
