...
void Input() {
	cin >> n >> Home; flights.rsz(n);
	for (i64 i=0; i<n; i++) cin >> flights[i];
}
...
void Solve() {
	...
	while (!flights.empty()) {
		string detected = "";
		for (i64 i=0; i<flights.size(); i++) {
			if (flights[i].substr(0, 3) != endPoint) continue;
			endPoint = flights[i].substr(5, 3); detected = "T";
			flights.erase(flights.begin()+i); break;
		}
		if (detected == "") break;
	}
	if (endPoint == Home) cout << "home"; else cout << "contest";
}
/*****************************************/
