...
/***********************************************
** File name: 392B.cpp                        **
** Time created: Mon Dec 31 2018 07:45am      **
***********************************************/
...
void Solve() {
	vector<vector<vi>> dp(n+1, vector<vi>(3, vi(3, LINF)));
	for (i64 i=0; i<3; i++) {
		for (i64 j=0; j<3; j++) {
			if (j == i) continue;
			i64 k = 0;
	...
	for (i64 x=2; x<=n; x++) {
		for (i64 i=0; i<3; i++) {
			for (i64 j=0; j<3; j++) {
				...
				dp[x][i][j] = min(dp[x][i][j], dp[x-1][i][k] + t[i][j] + min(dp[x-1][k][j], dp[x-1][k][i] + dp[x-1][i][j]));
				dp[x][i][j] = min(dp[x][i][j], dp[x-1][i][j] + t[i][k] + dp[x-1][j][i] + t[k][j] + dp[x-1][i][j]);
			}
		}
	}
	cout << dp[n][0][2] << endl;
