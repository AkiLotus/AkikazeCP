...
void Input() {
	cin >> n >> m; vis.rsz(n, false);
	for (i64 i=0; i<m; i++) {i64 x; cin >> x; vis[x-1] = true;}
	ftrl.rsz(n+1, 1);
	for (i64 i=1; i<=n; i++) ftrl[i] = (ftrl[i-1] * i) % Mod;
}
...
void Solve() {
	...
	for (i64 i=0; i<n; i++) {
		...
		ans *= modPow(ftrl[j-i+1], Mod-2); ans %= Mod;
		if (i > 0 && j < n-1) {ans *= modPow(2, j-i); ans %= Mod;} i = j;
	}
	cout << ans;
}
/*****************************************/
