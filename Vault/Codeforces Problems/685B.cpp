...
/************** [GLOBAL VARIABLES] **************/
i64 n, q, x; vector<vi> adj; vi p, centroid, sub;
/************************************************/
...
void DFS(i64 z) {
	i64 maximumSub = -1;
	for (auto t: adj[z]) {
		if (p[z] == t) continue; DFS(t);
		if (maximumSub == -1) maximumSub = t;
		else if (sub[maximumSub] < sub[t]) maximumSub = t;
		sub[z] += sub[t];
	}
	if (maximumSub > -1) {
		centroid[z] = centroid[maximumSub];
		while ((sub[z] - sub[centroid[z]]) * 2 > sub[z]) centroid[z] = p[centroid[z]];
	}
}

void Input() {
	cin >> n >> q; p.rsz(n, -1); centroid.rsz(n); adj.rsz(n); sub.rsz(n, 1);
	for (i64 i=0; i<n; i++) centroid[i] = i;
	for (i64 i=1; i<n; i++) {cin >> p[i]; p[i]--; adj[i].pub(p[i]); adj[p[i]].pub(i);}
}

void Solve() {
	DFS(0); while (q--) {cin >> x; cout << centroid[x-1]+1 << endl;}
}
/*****************************************/
