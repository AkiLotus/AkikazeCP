...
/** -----GLOBAL VARIABLES----- **/
vector<i64> a(0); i64 n;

/** -----EXTENSIVE FUNCTIONS----- **/
i64 cal(vi p) {
	i64 res = 0;
	for (i64 i=0; i<p.size(); i++) res = res * 10 + p[i];
	return res;
}

i64 gen(i64 len) {
	vi p(len/2, 4); for (i64 i=0; i<len/2; i++) p.pub(7);
	a.pub(cal(p));
	while (next_permutation(p.begin(), p.end())) a.pub(cal(p));
}
...
void VarInput() {
	...
	a.pub(4444477777LL); sort(a.begin(), a.end());
	cin >> n;
}
