...
const long double Pi = 3.141592653589793116, EPS = 1e-14, Gold = ((1+sqrt(5))/2);
...
ld calc(ld x) {
	// negative
	ld min_curr = 0, min_oall = 0;
	for (i64 i=0; i<n; i++) {
		min_curr = min(min_curr + a[i] - x, 0.0L);
		min_oall = min(min_oall, min_curr);
	}
	// positive
	ld max_curr = 0, max_oall = 0;
	for (i64 i=0; i<n; i++) {
		max_curr = max(max_curr + a[i] - x, 0.0L);
		max_oall = max(max_oall, max_curr);
	}
	return max(-min_oall, +max_oall);
}

ld ternary_search(ld top, ld bot) {
	if (bot - top <= EPS) return top;
	ld L = top + (bot - top) / 3.0L;
	ld R = top + (bot - top) / 3.0L * 2.0L;
	ld fL = calc(L), fR = calc(R);
	if (fL > fR) return ternary_search(L, bot);
	return ternary_search(top, R);
}
...
void Solve() {
	ld x = ternary_search(-10000.0L, +10000.0L);
	cout << fixed << setprecision(12) << calc(x);
}
/*****************************************/
