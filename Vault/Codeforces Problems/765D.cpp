...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi f, g, h, DSUpar, DSUsize; map<i64, i64> Map;
/************************************************/
...
void Solve() {
	...
	for (i64 i=0; i<n; i++) DSUmerge(i, f[i]);
	map<i64, i64> DisjointSet;
	for (i64 i=0; i<n; i++) {
		i64 root = DSUfind(i);
		if (DisjointSet.find(root) != DisjointSet.end()) {
			g[i] = DisjointSet[root]; continue;
		}
		i64 denoted = DisjointSet.size();
		DisjointSet[root] = denoted;
