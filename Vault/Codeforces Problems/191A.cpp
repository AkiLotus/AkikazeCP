...
/************** [GLOBAL VARIABLES] **************/
i64 n; vs a; vector<vi> dp(26, vi(26, -LINF));
/************************************************/
...
void Solve() {
	for (auto x: a) {
		i64 st = x[0] - 'a', en = x[x.size()-1] - 'a';
		for (i64 i=0; i<26; i++) dp[i][en] = max(dp[i][en], dp[i][st] + x.size());
		dp[st][en] = max(dp[st][en], (i64)x.size());
	}
	i64 ans = 0;
	for (i64 i=0; i<26; i++) ans = max(ans, dp[i][i]);
	cout << ans;
}
/*****************************************/
