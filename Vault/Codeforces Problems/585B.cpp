...
void Solve() {
	vector<vector<vi>> vis(n+1, vector<vi>(3, vi(n, 0LL)));
	for (i64 i=0; i<3; i++) {
		for (i64 j=0; j<n; j++) {
			...
			for (i64 x=0; x<n+1; x++) {
				i64 y = max(j-2*x, 0LL), z = k - 2 * x; if (z < 0) break;
				vis[x][i][y]++; if (z != n-1) vis[x][i][z+1]--;
			}
			j = k;
		}
	}
	...
	vector<vb> reachable(3, vb(n, false)); reachable[xs][0] = true;
	for (i64 i=1; i<n; i++) {
		if (reachable[0][i-1] && !vis[i-1][0][i] && !vis[i][0][i]) reachable[0][i] = true;
		if (reachable[1][i-1] && !vis[i-1][1][i] && !vis[i-1][0][i] && !vis[i][0][i]) reachable[0][i] = true;
		if (reachable[0][i-1] && !vis[i-1][0][i] && !vis[i-1][1][i] && !vis[i][1][i]) reachable[1][i] = true;
		if (reachable[1][i-1] && !vis[i-1][1][i] && !vis[i][1][i]) reachable[1][i] = true;
		if (reachable[2][i-1] && !vis[i-1][2][i] && !vis[i-1][1][i] && !vis[i][1][i]) reachable[1][i] = true;
		if (reachable[1][i-1] && !vis[i-1][1][i] && !vis[i-1][2][i] && !vis[i][2][i]) reachable[2][i] = true;
		if (reachable[2][i-1] && !vis[i-1][2][i] && !vis[i][2][i]) reachable[2][i] = true;
		if (!reachable[0][i] && !reachable[1][i] && !reachable[2][i]) {cout << "NO\n"; return;}
	}
	cout << "YES\n";
}
/*****************************************/
