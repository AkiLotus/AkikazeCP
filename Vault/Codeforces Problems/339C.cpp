...
/** -----GLOBAL VARIABLES----- **/
i64 m; string s; bool flag = false;
stack<i64> ans; vb vis;

/** -----EXTENSIVE FUNCTIONS----- **/
void DFS(i64 bal, i64 last, i64 pos) {
	if (pos == m-1) {
		if (!vis[pos]) ans.push(last); flag = true;
		vis[pos] = true; return;
	}
	for (i64 i=0; i<10; i++) {
		if (s[i] == '0') continue;
		if (i + 1 == last) continue;
		if (abs(bal) > abs(i)) continue;
		if (pos % 2 == 0) DFS(bal-i-1, i+1, pos+1);
		else DFS(bal+i+1, i+1, pos+1);
		if (flag) {if (!vis[pos]) ans.push(last); vis[pos] = true; return;}
	}
}
