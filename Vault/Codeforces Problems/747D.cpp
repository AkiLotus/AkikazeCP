...
void Solve() {
	...
	for (i64 i=0; i<n; i++) {
		if (t[i] >= 0) continue;
		i64 j = i; while (j+1 < n && t[j+1] < 0) j++;
		if (Last != -1) Q.push(i-Last-1);
		k -= (j - i + 1); i = j; Last = j; ans1 += 2;
	}
	i64 k2 = k - (n-1 - Last), ans2 = ans1 - 1;
	priority_queue<i64, vi, greater<i64>> Q2 = Q;
	if (ans1 == 0) {cout << "0"; return;}
	if (k < 0) {cout << "-1"; return;}
	while (!Q.empty() && Q.top() <= k) {
		k -= Q.top(); ans1 -= 2; Q.pop();
	}
	if (k2 < 0) ans2 = LINF;
	else while (!Q2.empty() && Q2.top() <= k2) {
		k2 -= Q2.top(); ans2 -= 2; Q2.pop();
	}
	cout << min(ans1, ans2);
}
/*****************************************/
