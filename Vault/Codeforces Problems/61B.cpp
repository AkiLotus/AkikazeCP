...
/************** [GLOBAL VARIABLES] **************/
string s1, s2, s3; i64 n; vs a; map<string, i64> Map;
/************************************************/
...
void Input() {
	cin >> s1 >> s2 >> s3 >> n; a.rsz(n);
	formalize(s1); formalize(s2); formalize(s3);
	for (i64 i=0; i<n; i++) {cin >> a[i]; formalize(a[i]);}
}

void Solve() {
	Map[s1+s2+s3]++; Map[s1+s3+s2]++; Map[s2+s1+s3]++;
	Map[s2+s3+s1]++; Map[s3+s1+s2]++; Map[s3+s2+s1]++;
	for (auto str: a) {
		if (!Map[str]) cout << "WA\n";
		else cout << "ACC\n";
	}
