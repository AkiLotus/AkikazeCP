...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, d, u, v; vi p; vector<vi> adj;
vi distDown, distUp;
/************************************************/
...
void DFS(i64 z, i64 last) {
	for (auto t: adj[z]) {
		if (t == last) continue;
		DFS(t, z);
		distDown[z] = max(distDown[z], distDown[t]+1);
	}
}

void DFS_Up1(i64 z, i64 last) {
	priority_queue<i64> Q;
	for (auto t: adj[z]) {
		if (t == last) continue;
		DFS_Up1(t, z); Q.push(distDown[t]+2);
	}
	if (Q.size() < 2) return;
	i64 ans1 = Q.top(); Q.pop(); i64 ans2 = Q.top();
	for (auto t: adj[z]) {
		if (t == last) continue;
		if (distDown[t] + 2 == ans1) distUp[t] = max(distUp[t], ans2);
		else distUp[t] = max(distUp[t], ans1);
	}
}

void DFS_Up2(i64 z, i64 last) {
	for (auto t: adj[z]) {
		if (t == last) continue;
		if (distUp[z] != -LINF) distUp[t] = max(distUp[t], distUp[z]+1);
		DFS_Up2(t, z);
	}
}

void Input() {
	cin >> n >> m >> d; p.rsz(m); adj.rsz(n);
	for (i64 i=0; i<m; i++) {cin >> p[i]; p[i]--;}
	for (i64 i=1; i<n; i++) {
		cin >> u >> v; u--; v--;
...
void Solve() {
	distDown.rsz(n, -LINF); distUp.rsz(n, -LINF);
	for (auto P: p) {distDown[P] = 0; distUp[P] = 0;}
	i64 ans = 0; DFS(0, -1); DFS_Up1(0, -1); DFS_Up2(0, -1);
	for (i64 i=0; i<n; i++) ans += (distDown[i] <= d && distUp[i] <= d);
	cout << ans;
}
/*****************************************/
