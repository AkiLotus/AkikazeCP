...
/***********************************************
** File name: 796D.cpp                        **
** Time created: Sat Dec 15 2018 11:30am      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, k, d, p, u, v; set<i64> P;
vector<vi> adj; map<pii, i64> id;
/************************************************/
...
void Input() {
	cin >> n >> k >> d; adj.rsz(n);
	while (k--) {cin >> p; p--; P.insert(p);}
	for (i64 i=1; i<n; i++) {
		cin >> u >> v; u--; v--;
		id[mp(u,v)] = i; id[mp(v,u)] = i;
		adj[u].pub(v); adj[v].pub(u);
	}
}

void Solve() {
	queue<i64> Q; vb vis(n, false), used(n, false);
	i64 deleted = n - 1;
	for (auto it=P.begin(); it!=P.end(); it++) {
		p = *it; vis[p] = true; Q.push(p);
	}
	while (!Q.empty()) {
		i64 z = Q.front(); Q.pop();
		for (auto t: adj[z]) {
