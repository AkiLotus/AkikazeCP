...
void Solve() {
	for (i64 i=0; i<s1.size(); i++) id[s1[i]-'a'].pub(i);
	i64 ans = 1, pos = -1;
	for (i64 i=0; i<s2.size(); i++) {
		if (id[s2[i]-'a'].empty()) {cout << "-1"; return;}
		if (upper_bound(id[s2[i]-'a'].begin(), id[s2[i]-'a'].end(), pos) == id[s2[i]-'a'].end()) {ans++; pos = id[s2[i]-'a'][0]; continue;}
		pos = *upper_bound(id[s2[i]-'a'].begin(), id[s2[i]-'a'].end(), pos);
	}
	cout << ans;
}
/*****************************************/
