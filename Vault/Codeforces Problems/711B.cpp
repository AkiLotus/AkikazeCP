...
void Solve() {
	...
	if (x == y) {
		A.pub(0LL);
		for (i64 i=0; i<n; i++) A[A.size()-1] += a[i][i];
	}
	if (x + y == n-1) {
		A.pub(0LL);
		for (i64 i=0; i<n; i++) A[A.size()-1] += a[i][n-1-i];
	}
	if (!allEqual(A)) {cout << "-1"; return;}
	for (i64 i=0; i<n; i++) {
		i64 cc1 = 0, cc2 = 0;
		for (i64 j=0; j<n; j++) {
			cc1 += a[i][j]; cc2 += a[j][i];
	...
	if (x != y) B.pub(cc1);
	if (x + y != n-1) B.pub(cc2);
	if (!allEqual(B)) {cout << "-1"; return;}
	if (B[0] -  A[0] <= 0) cout << "-1";
	else cout << (B[0] - A[0]);
}
/*****************************************/
