...
bool valid(vi a) {
	for (i64 id=0; id<m; id++) {
		if (t[id] == 1) {
			for (i64 x=l[id]; x<=r[id]; x++) {
				a[x] += p[id];
			}
			continue;
		}
		i64 Max = -INF;
		for (i64 x=l[id]; x<=r[id]; x++) Max = max(Max, a[x]);
		if (Max != p[id]) return false;
	}
	return true;
}

void Input() {
	cin >> n >> m; A.rsz(n, +INF); Diff.rsz(n, 0);
	t.rsz(m); l.rsz(m); r.rsz(m); p.rsz(m);
	for (i64 i=0; i<m; i++) cin >> t[i] >> l[i] >> r[i] >> p[i];
}
...
void Solve() {
	for (i64 id=0; id<m; id++) {
		l[id]--; r[id]--;
		for (i64 x=l[id]; x<=r[id]; x++) {
			if (t[id] == 1) Diff[x] += p[id];
			else if (t[id] == 2) A[x] = min(A[x], p[id] - Diff[x]);
		}
	}
	if (!valid(A)) {cout << "NO"; return;}
	cout << "YES\n"; for (auto x: A) cout << x << " ";
}
/*****************************************/
