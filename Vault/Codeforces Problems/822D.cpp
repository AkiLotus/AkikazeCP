...
/************** [GLOBAL VARIABLES] **************/
i64 t, l, r; vi f(5000001, 0), lastPrime(5000001, 0);
/************************************************/

/************** [FUNCTIONS] **************/
void Preprocess() {
	for (i64 i=0; i<5000001; i++) lastPrime[i] = i;
	for (i64 i=2; i<5000001; i++) {
		if (lastPrime[i] < i) continue;
		for (i64 j=2; j<=5000000/i; j++) {
			lastPrime[i*j] = min(lastPrime[i*j], i);
		}
	}
	for (i64 i=2; i<5000001; i++) {
		if (lastPrime[i] == i) f[i] = (i * (i - 1) / 2) % Mod;
		else f[i] = (f[i/lastPrime[i]] + (((lastPrime[i] * (lastPrime[i] - 1) / 2) % Mod) * (i / lastPrime[i])) % Mod) % Mod;
	}
}
