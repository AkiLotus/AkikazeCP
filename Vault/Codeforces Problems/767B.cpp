...
void Input() {
	cin >> ts >> tf >> t >> n; A.rsz(n);
	for (auto &x: A) cin >> x;
}

void Solve() {
	if (n == 0) {cout << ts; return;}
	i64 optimal = min(A[0] - 1, ts), wait = max(ts - optimal, 0LL);
	i64 Last = max(ts, A[0]) + t;
	for (i64 i=1; i<=n; i++) {
		if (Last > tf - t) break;
		if (i == n || Last < A[i]) {cout << Last; return;}
		if (i == n) break;
		if (A[i-1] < A[i]) {
			i64 Z = min(A[i]-1, Last);
			if (Last - Z < wait) {
				optimal = Z;
				wait = Last - Z;
			}
