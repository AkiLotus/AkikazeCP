...
void Solve() {
	vi segSum(n-k+1, 0), maxSeg(n-k+1, n-k);
	for (i64 i=0; i<k; i++) {
		segSum[0] += x[i];
	}
	...
	for (i64 i=n-k-1; i>=0; i--) {
		if (segSum[i] >= segSum[maxSeg[i+1]]) maxSeg[i] = i;
		else maxSeg[i] = maxSeg[i+1];
	}
	i64 mostAbsurd = -LINF, l = -1, r = -1;
	for (i64 i=0; i+k<n-k+1; i++) {
		if (segSum[i] + segSum[maxSeg[i+k]] > mostAbsurd) {
			mostAbsurd = segSum[i] + segSum[maxSeg[i+k]];
			l = i + 1; r = maxSeg[i+k] + 1;
		}
	}
	cout << l << " " << r;
}
