...
i64 n, k, d; vector<vector<i64>> dp;
...
void VarInput() {
	...
	cin >> n >> k >> d; dp.resize(n+1, vector<i64>(2, 0));
	dp[0][0] = 1;
}
...
void ProSolve() {
	...
	for (i64 i=1; i<=n; i++) {
		for (i64 j=0; j<=min(d-1, i); j++) {
			dp[i][0] = (dp[i][0] + dp[i-j][0]) % MOD;
			dp[i][1] = (dp[i][1] + dp[i-j][1]) % MOD;
		}
		for (i64 j=d; j<=min(i, k); j++) dp[i][1] = (dp[i][1] + dp[i-j][0] + dp[i-j][1]) % MOD;
	}
	cout << dp[n][1];
}
