...
/***********************************************
** File name: 999D.cpp                        **
** Time created: Sun Dec 2 2018 14:23pm       **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, lim; vi a, cnt; set<pii> Set;
/************************************************/

/************** [FUNCTIONS] **************/
i64 seekNext(i64 r) {
	if (Set.upper_bound(mp(r, LINF)) == Set.end()) return Set.begin()->fi;
	return Set.upper_bound(mp(r, LINF))->fi;
}

void Input() {
	cin >> n >> m; a.rsz(n);
	lim = n / m; cnt.rsz(m, 0);
	for (auto &x: a) {cin >> x; cnt[x % m]++;}
}
...
void Solve() {
	...
	for (i64 i=0; i<m; i++) {
		...
		Set.insert(mp(i, cnt[i]));
	}
	for (i64 i=0; i<n; i++) {
		if (cnt[a[i] % m] <= lim) continue;
		i64 Rem = seekNext(a[i] % m);
		i64 steps = (Rem - a[i] % m + m * 4) % m;
		cnt[a[i] % m]--; a[i] += steps; delta += steps;
		Set.erase(mp(a[i] % m, cnt[a[i]%m]));
		cnt[a[i] % m]++; if (cnt[a[i] % m] < lim) Set.insert(mp(a[i] % m, cnt[a[i]%m]));
	}
	cout << delta << endl;
