...
i64 n, m; vector<vector<i64>> adj(10007, vector<i64>(0));
vector<i64> dp(10007, 0);
vector<bool> visited(10007, false);
...
void VarInput() {
	...
	cin >> n >> m;
	for (i64 i=1; i<10007; i++) {
		if (i-1 > 0) adj[i].pub(i-1);
		if (i*2 < 10007) adj[i].pub(2*i);
	}
}

void ProSolve() {
	//cout << "Case " << ++cas << ": " << ans << endl; // for multi-testcase problems
	queue<i64> Q; Q.push(n); visited[n] = true;
	while (!Q.empty()) {
		i64 z = Q.front(); Q.pop();
		if (z == m) {
			cout << dp[m]; return;
		...
		for (i64 i=0; i<adj[z].size(); i++) {
			if (!visited[adj[z][i]]) {
				...
				Q.push(adj[z][i]); dp[adj[z][i]] = dp[z] + 1;
			}
		}
	}
}
