...
void Solve() {
	...
	vector<vb> dp(n, vb(5, false));
	vector<vector<pii>> Last(n, vector<pii>(5, Default));
	for (i64 i=0; i<5; i++) dp[0][i] = true;
	for (i64 i=0; i<n-1; i++) {
		for (i64 j=0; j<5; j++) {
			if (!dp[i][j]) continue;
			for (i64 k=0; k<5; k++) {
				...
				if (a[i] > a[i+1] && j <= k) continue;
				if (a[i] == a[i+1] && j == k) continue;
				dp[i+1][k] = true; Last[i+1][k] = mp(i, j);
			}
		}
	}
	i64 x = n-1, y = 0;
	while (y < 5 && !dp[x][y]) y++;
	if (y == 5) {cout << "-1"; return;}
	vi Finger(n, -1);
	while (true) {
		Finger[x] = y + 1;
		pii Data = Last[x][y];
