...
void Solve() {
	if (T < 2*n) {
		vi A(T*n), LNDS;
		for (i64 i=0; i<T*n; i++) A[i] = a[i%n];
		for (i64 i=0; i<T*n; i++) {
			if (upper_bound(LNDS.begin(), LNDS.end(), A[i]) == LNDS.end()) LNDS.pub(A[i]);
			else LNDS[upper_bound(LNDS.begin(), LNDS.end(), A[i]) - LNDS.begin()] = A[i];
		}
		cout << LNDS.size(); return;
	}
	vi dpEnds(n*n, 0), dpStarts(n*n, 0), A(n*n);
	for (i64 i=0; i<n*n; i++) A[i] = a[i%n];
	vi dp(n*n+1, 500); dp[0] = -LINF;
	for (i64 i=0; i<n*n; i++) {
		dpEnds[i] = upper_bound(dp.begin(), dp.end(), A[i]) - dp.begin();
		dp[dpEnds[i]] = A[i];
	}
	dp.clear(); dp.rsz(n*n+1, 500); dp[0] = -LINF; reverse(A.begin(), A.end());
	for (i64 i=0; i<n*n; i++) {
		dpStarts[i] = upper_bound(dp.begin(), dp.end(), -A[i]) - dp.begin();
		dp[dpStarts[i]] = -A[i];
	}
	reverse(dpStarts.begin(), dpStarts.end());
	i64 ans = 0;
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<n; j++) {
			for (i64 z=a[i]; z<=a[j]; z++) {
				ans = max(ans, dpEnds[(n-1)*n+i] + cnt[z] * (T - 2 * n) + dpStarts[j]);
			}
		}
	}
	cout << ans;
