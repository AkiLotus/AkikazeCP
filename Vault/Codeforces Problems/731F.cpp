...
/************** [GLOBAL VARIABLES] **************/
i64 n, ans = 0; vi a; vi prefixcnt(200007, 0); set<i64> S;
/************************************************/

/************** [FUNCTIONS] **************/
void PrefixProcess() {
	for (auto x: a) prefixcnt[x]++;
	for (i64 i=1; i<200007; i++) prefixcnt[i] += prefixcnt[i-1];
}

void Input() {
	cin >> n; a.rsz(n);
	for (i64 i=0; i<n; i++) {cin >> a[i]; S.insert(a[i]);}
}
...
void Solve() {
	...
	for (auto it=S.begin(); it!=S.end(); it++) {
		i64 compcnt = 0, x = *it;
		for (i64 i=x*2-1; i<600007; i+=x) {
			compcnt += (prefixcnt[min(i, 200006LL)] - prefixcnt[min(i-x, 200006LL)]) * (i / x);
		}
		ans = max(ans, compcnt * x);
	}
	cout << ans;
}
/*****************************************/
