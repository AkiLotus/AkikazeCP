...
/************** [GLOBAL VARIABLES] **************/
vector<vc> A(2, vc(2, 'X'));
vector<vc> B(2, vc(2, 'X'));
map<vector<vc>, i64> Map;
/************************************************/

/************** [FUNCTIONS] **************/
void Try(vector<vc> Z) {
	i64 bx = 0, by = 0;
	if (Z[0][1] == 'X') by = 1;
	else if (Z[1][0] == 'X') bx = 1;
	else if (Z[1][1] == 'X') {bx = 1; by = 1;}
	Map[Z]++;
	swap(Z[bx][by], Z[1-bx][by]);
	if (!Map[Z]) Try(Z);
	swap(Z[bx][by], Z[1-bx][by]);
	swap(Z[bx][by], Z[bx][1-by]);
	if (!Map[Z]) Try(Z);
}
...
void Solve() {
	Try(A);
	if (Map[B]) cout << "YES"; else cout << "NO";
}
/*****************************************/
