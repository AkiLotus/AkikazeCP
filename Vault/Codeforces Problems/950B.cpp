...
void VarInput() {
	...
	cin >> n >> m; x.rsz(n); y.rsz(m);
	for (i64 i=0; i<n; i++) cin >> x[i];
	for (i64 i=0; i<m; i++) cin >> y[i];
}

void ProSolve() {
	i64 ptr1 = 0, ptr2 = 0, ans = 1;
	i64 sum1 = x[0], sum2 = y[0];
	while (ptr1 < n-1 || ptr2 < m-1) {
		if (sum1 == sum2) ans++;
		if (sum1 > sum2) sum2 += y[++ptr2];
		else if (sum1 < sum2) sum1 += x[++ptr1];
		else {sum2 += y[++ptr2]; sum1 += x[++ptr1];}
	}
	cout << ans;
}
