...
/************** [GLOBAL VARIABLES] **************/
string s1, s2; pip A, B;
i64 monthcnt[] = {0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
/************************************************/

/************** [FUNCTIONS] **************/
pip toDate(string s) {
	i64 y = (s[0] - '0') * 1000 + (s[1] - '0') * 100 + (s[2] - '0') * 10 + (s[3] - '0');
	i64 m = (s[5] - '0') * 10 + (s[6] - '0'), d = (s[8] - '0') * 10 + (s[9] - '0');
	return mp(y, mp(m, d));
}
...
pip Inc(pip Q) {
	i64 y = Q.fi, m = Q.se.fi, d = Q.se.se; d++;
	if (m == 2 && !isLeap(y) && d == 29) {d = 1; m++;}
	else if (m == 2 && isLeap(y) && d == 30) {d = 1; m++;}
	else if (m != 2 && d == monthcnt[m] + 1) {d = 1; m++;}
	if (m > 12) {m -= 12; y++;}
	return mp(y, mp(m, d));
}
...
void Input() {
	cin >> s1 >> s2;
	A = toDate(s1); B = toDate(s2);
}

void Solve() {
	if (A > B) swap(A, B); i64 ans = 0;
	while (A < B) {ans++; A = Inc(A);}
	cout << ans;
}
/*****************************************/
