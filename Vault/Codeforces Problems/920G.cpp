...
/************** [GLOBAL VARIABLES] **************/
i64 x, p, k; vector<vi> primediv(1000001);
/************************************************/

/************** [FUNCTIONS] **************/
i64 elementCount(i64 z) {
	i64 n = primediv[p].size(), cnt = 0;
	...
	while (!Q.empty()) {
		...
		for (i64 j=last1+1; j<n; j++) Q.push(mp(mp(i+(1LL << j), divisor * primediv[p][j]), j));
	}
	return cnt;
}
...
void PreprocessSieve() {
	for (i64 i=2; i<1000001; i++) {
		...
		for (i64 j=1; j<=1000000/i; j++) primediv[i*j].pub(i);
	}
}
