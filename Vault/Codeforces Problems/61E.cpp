...
/************** [GLOBAL VARIABLES] **************/
i64 n; vector<ppi> a; vi TreeInv, TreeTrp, LazyInv, LazyTrp;
/************************************************/

/************** [FUNCTIONS] **************/
void IncInv(i64 node, i64 st, i64 en, i64 L, i64 R) {
	if (LazyInv[node] > 0) {
		TreeInv[node] += LazyInv[node] * (en - st + 1);
		if (st != en) {
			LazyInv[node*2+0] += LazyInv[node];
			LazyInv[node*2+1] += LazyInv[node];
		}
		LazyInv[node] = 0;
	}
	if (st > en || R < st || en < L) return;
	if (L <= st && en <= R) {
		TreeInv[node] += (en - st + 1);
		if (st != en) {
			LazyInv[node*2+0]++;
			LazyInv[node*2+1]++;
		}
	...
	IncInv(node*2, st, (st+en)/2, L, R);
	IncInv(node*2+1, (st+en)/2+1, en, L, R);
	TreeInv[node] = TreeInv[node*2] + TreeInv[node*2+1];
}

void UpdTrp(i64 node, i64 st, i64 en, i64 L, i64 R, i64 x) {
	if (LazyTrp[node] > 0) {
		TreeTrp[node] += LazyTrp[node] * (en - st + 1);
		if (st != en) {
			LazyTrp[node*2+0] += LazyTrp[node];
			LazyTrp[node*2+1] += LazyTrp[node];
		}
		LazyTrp[node] = 0;
	}
	if (st > en || R < st || en < L) return;
	if (L <= st && en <= R) {
		TreeTrp[node] += x * (en - st + 1);
		if (st != en) {
			LazyTrp[node*2+0] += x;
			LazyTrp[node*2+1] += x;
		}
	...
	UpdTrp(node*2, st, (st+en)/2, L, R, x);
	UpdTrp(node*2+1, (st+en)/2+1, en, L, R, x);
	TreeTrp[node] = TreeTrp[node*2] + TreeTrp[node*2+1];
}

i64 GetInv(i64 node, i64 st, i64 en, i64 i) {
	if (LazyInv[node] > 0) {
		TreeInv[node] += LazyInv[node] * (en - st + 1);
		if (st != en) {
			LazyInv[node*2+0] += LazyInv[node];
	...
	if (st > en || i < st || en < i) return 0;
	if (st == en && st == i) return TreeInv[node];
	i64 p1 = GetInv(node*2, st, (st+en)/2, i);
	i64 p2 = GetInv(node*2+1, (st+en)/2+1, en, i);
	return (p1 + p2);
}

i64 GetTrp(i64 node, i64 st, i64 en, i64 i) {
	if (LazyTrp[node] > 0) {
		TreeTrp[node] += LazyTrp[node] * (en - st + 1);
		if (st != en) {
			LazyTrp[node*2+0] += LazyTrp[node];
	...
	if (st > en || i < st || en < i) return 0;
	if (st == en && st == i) return TreeTrp[node];
	i64 p1 = GetTrp(node*2, st, (st+en)/2, i);
	i64 p2 = GetTrp(node*2+1, (st+en)/2+1, en, i);
	return (p1 + p2);
}

void Input() {
	cin >> n; a.rsz(n); LazyInv.rsz(4*n, 0); LazyTrp.rsz(4*n, 0);
	TreeInv.rsz(4*n, 0); TreeTrp.rsz(4*n, 0);
	for (i64 i=0; i<n; i++) {
		cin >> a[i].fi.fi; a[i].se = i;
		a[i].fi.se = 0;
	}
}

void Solve() {
	sort(a.begin(), a.end()); i64 ans = 0;
	for (i64 i=0; i<n; i++) {
		a[i].fi.se += GetInv(1, 0, n-1, a[i].se);
		ans += GetTrp(1, 0, n-1, a[i].se);
		if (a[i].se > 0) IncInv(1, 0, n-1, 0, a[i].se-1);
		if (a[i].se > 0) UpdTrp(1, 0, n-1, 0, a[i].se-1, a[i].fi.se);
	}
	cout << ans;
}
/*****************************************/
