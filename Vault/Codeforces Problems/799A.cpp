...
void Solve() {
	i64 time1 = 0, time2 = 0, cnt = 0;
	for (; time1 < 1000000; time1 += t) {
		if (time1 / t * k >= n) break;
	}
	for (; time2 < 1000000; time2++) {
		if (time2 % t > 0 && (time2 - d) % t > 0) continue;
		if (time2 / t * k + (time2 - d) / t * k >= n) break;
	}
	if (time1 <= time2) cout << "NO"; else cout << "YES";
}
/*****************************************/
