...
void VarInput() {
	...
	cin >> n; l.resize(n); c.resize(n);
	for (i64 i=0; i<n; i++) cin >> l[i];
	for (i64 i=0; i<n; i++) cin >> c[i];
}

void ProSolve() {
	i64 GCD = l[0];
	for (i64 i=1; i<n; i++) GCD = __gcd(GCD, l[i]);
	if (GCD > 1) {cout << "-1"; return;}
	i64 ans = LINF;
	for (i64 i=0; i<n; i++) {
		//tracker1(i);
		vi primeFactor; i64 tmp = l[i];
		...
		if (tmp != 1) primeFactor.pub(tmp);
		//cout << primeFactor << endl;
		i64 N = primeFactor.size();
		vi bitmask(1LL << N, LINF); bitmask[(1LL << N) - 1] = c[i];
		vb vis(1LL << N, false);
		priority_queue<pii> Q; Q.push(mp(-c[i], (1LL << N) - 1));
		while (!Q.empty()) {
			pii z = Q.top(); Q.pop();
			//tracker2(z.fi, z.se);
			...
			for (i64 j=0; j<n; j++) {
				...
				i64 newMask = 0;
				for (i64 x=0; x<N; x++) {
					if (l[j] % primeFactor[x] == 0) newMask |= (1LL << x);
				}
				newMask &= z.se;
				if (-z.fi + c[j] < bitmask[newMask]) {
					bitmask[newMask] = -z.fi + c[j];
					if (!vis[newMask]) {
						Q.push(mp(-bitmask[newMask], newMask));
						//cout << "added " << -bitmask[newMask] << ", " << newMask << endl;
					}
				}
			}
		}
		ans = min(ans, bitmask[0]);
	}
	cout << ans;
}
