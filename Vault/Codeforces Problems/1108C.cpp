...
/***********************************************
** File name: 1108C.cpp                       **
** Time created: Wed Jan 30 2019 05:37am      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n; string s; vector<string> pattern(6);
/************************************************/
...
void Input() {
	cin >> n >> s;
	pattern[0] = "BGR"; pattern[1] = "BRG";
	pattern[2] = "GBR"; pattern[3] = "GRB";
	pattern[4] = "RBG"; pattern[5] = "RGB";
}

void Solve() {
	pii optimal = {LINF, -1LL};
	for (i64 i=0; i<6; i++) {
		i64 steps = 0;
		for (i64 x=0; x<n; x++) steps += (pattern[i][x % 3] != s[x]);
		optimal = min(optimal, {steps, i});
	}
	cout << optimal.fi << endl;
	for (i64 i=0; i<n; i++) cout << pattern[optimal.se][i % 3];
	cout << endl;
}
/*****************************************/
