...
/************** [GLOBAL VARIABLES] **************/
i64 x, y, n; pii ans = mp(-1LL, -1LL);
/************************************************/

/************** [FUNCTIONS] **************/
i64 nearestwithDenom(i64 b) {
	i64 numer = x * b, denom = y * b;
	i64 Min = ((i64)numer / y) * y;
	i64 Max = Min + y;
	if (Max - numer < numer - Min) return (Max / y);
	return (Min / y);
}

bool nearer(pii A, pii B) {
	i64 numer = x * A.se * B.se;
	i64 Anum = A.fi * y * B.se;
	i64 Bnum = B.fi * y * A.se;
	if (abs(numer - Anum) != abs(Bnum - numer)) return(abs(numer - Anum) < abs(Bnum - numer));
	if (A.se != B.se) return (A.se < B.se); return (A.fi < B.fi);
}
...
void Solve() {
	for (i64 b=1; b<=n; b++) {
		i64 a = nearestwithDenom(b);
		if (ans == mp(-1LL, -1LL)) ans = mp(a, b);
		else if (nearer(mp(a, b), ans)) ans = mp(a, b);
	}
	cout << ans.fi << "/" << ans.se;
}
/*****************************************/
