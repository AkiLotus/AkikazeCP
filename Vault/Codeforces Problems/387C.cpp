...
/************** [FUNCTIONS] **************/
bool lower(i64 st1, i64 en1, i64 st2, i64 en2) {
	for (i64 x=0; x<(en1-st1+1); x++) {
		if (p[st1+x] > p[st2+x]) return false;
		if (p[st1+x] < p[st2+x]) return true;
	}
	return false;
}
...
void Solve() {
	...
	for (i64 j=n-1; j>0; j--) {
		...
		while (i > 0 && p[i] == '0') i--;
		if (i < j+1-i) break;
		if (i == j+1-i && lower(0, i-1, i, j)) break;
		ans++; j = i;
	}
	cout << ans;
}
