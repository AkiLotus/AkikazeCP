...
/***********************************************
** File name: 14C.cpp                         **
** Time created: Mon Feb 18 2019 11:14am      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
vector<vi> S(4, vi(4)); set<pii> PointSet;
vector<pii> Points; set<i64> xset, yset;
/************************************************/

/************** [FUNCTIONS] **************/
i64 seek(i64 x, i64 y) {
	return (find(Points.begin(), Points.end(), mp(x, y)) - Points.begin());
}

void Input() {
	for (i64 i=0; i<4; i++) {
		cin >> S[i][0] >> S[i][1] >> S[i][2] >> S[i][3];
		xset.insert(S[i][0]); xset.insert(S[i][2]);
		yset.insert(S[i][1]); yset.insert(S[i][3]);
		PointSet.insert({S[i][0], S[i][1]});
		PointSet.insert({S[i][2], S[i][3]});
		Points.pub({S[i][0], S[i][1]});
		Points.pub({S[i][2], S[i][3]});
	}
}

void Solve() {
	if (PointSet.size() != 4) {cout << "NO\n"; return;}
	if (xset.size() != 2) {cout << "NO\n"; return;}
	if (yset.size() != 2) {cout << "NO\n"; return;}
	sort(Points.begin(), Points.end());
	Points.erase(Points.begin()+7); Points.erase(Points.begin()+5);
	Points.erase(Points.begin()+3); Points.erase(Points.begin()+1);
	if (Points[0].fi != Points[1].fi) {cout << "NO\n"; return;}
	if (Points[2].fi != Points[3].fi) {cout << "NO\n"; return;}
	if (Points[0].se != Points[2].se) {cout << "NO\n"; return;}
	if (Points[1].se != Points[3].se) {cout << "NO\n"; return;}
	map<pii, i64> Map;
	for (i64 i=0; i<4; i++) {
		i64 id1 = seek(S[i][0], S[i][1]), id2 = seek(S[i][2], S[i][3]);
		if (id1 > id2) swap(id1, id2);
		if (id1 == id2) {cout << "NO\n"; return;}
		if (Map[{id1, id2}]) {cout << "NO\n"; return;}
		Map[{id1, id2}]++;
		if (id1 + id2 == 3) {cout << "NO\n"; return;}
	}
	cout << "YES\n";
}
/*****************************************/
