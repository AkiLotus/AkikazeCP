...
/** -----GLOBAL VARIABLES----- **/
i64 n, m; vi c; map<i64, i64> Map;
vector<set<i64>> ncd(100009);
...
void VarInput() {
	...
	cin >> n >> m; c.resize(n);
	for (i64 i=0; i<n; i++) {cin >> c[i]; Map[c[i]]++;}
	while (m--) {
		i64 u, v; cin >> u >> v; u--; v--;
		if (ncd[c[u]].find(c[v]) == ncd[c[u]].end() && c[u] != c[v]) {
			ncd[c[u]].insert(c[v]);
		}
		if (ncd[c[v]].find(c[u]) == ncd[c[v]].end() && c[u] != c[v]) {
			ncd[c[v]].insert(c[u]);
		}
	}
}
...
void ProSolve() {
	i64 ans = *min_element(c.begin(), c.end());
	for (i64 i=*min_element(c.begin(), c.end()); i<100009; i++) {
		if (ncd[i].size() > ncd[ans].size()) ans = i;
	}
	cout << ans;
}
