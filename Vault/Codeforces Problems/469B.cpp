...
bool match(i64 t) {
	for (auto x: X) {
		i64 c = x.fi + t, d = x.se + t;
		i64 posC = p, posD = p;
		bool withinC = false, withinD = false;
		for (i64 i=0; i<p; i++) {
			if (c < Z[i].fi) {posC = i; break;}
			if (Z[i].fi <= c && c <= Z[i].se) {posC = i; withinC = true; break;}
		}
		for (i64 i=0; i<p; i++) {
			if (d < Z[i].fi) {posD = i; break;}
			if (Z[i].fi <= d && d <= Z[i].se) {posD = i; withinD = true; break;}
		}
		if (posC < posD) return true;
		if (posC == posD && withinD) return true;
	}
	return false;
}

void Input() {
	cin >> p >> q >> l >> r; Z.rsz(p); X.rsz(q);
	for (i64 i=0; i<p; i++) cin >> Z[i].fi >> Z[i].se;
	for (i64 i=0; i<q; i++) cin >> X[i].fi >> X[i].se;
}

void Solve() {
	i64 cnt = 0;
	for (i64 i=l; i<=r; i++) cnt += match(i);
	cout << cnt;
}
/*****************************************/
