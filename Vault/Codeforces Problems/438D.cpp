...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, t, l, r, k, x; vi a;
vi TreeSum; vector<pii> TreeMax;
/************************************************/
...
void Update(i64 node, i64 st, i64 en, i64 id, i64 val) {
	...
	if (st == en && en == id) {
		TreeMax[node] = mp(val, id);
		TreeSum[node] = val; return;
	}
...
pii GetMax(i64 node, i64 st, i64 en, i64 L, i64 R) {
	if (st > en || en < L || R < st) return mp(-LINF, -LINF);
	if (L <= st && en <= R) return TreeMax[node];
	pii p1 = GetMax(node*2, st, (st+en)/2, L, R);
	pii p2 = GetMax(node*2+1, (st+en)/2+1, en, L, R);
	return max(p1, p2);
}

void Input() {
	cin >> n >> m; a.rsz(n);
	TreeSum.rsz(n*4); TreeMax.rsz(n*4);
	for (i64 i=0; i<n; i++) {
		cin >> a[i];
...
void Solve() {
	while (m--) {
		...
		if (t == 1) {
			...
			cin >> l >> r >> x; l--; r--;
			while (true) {
				pii Data = GetMax(1, 0, n-1, l, r);
				if (Data.fi < x) break;
				Update(1, 0, n-1, Data.se, Data.fi % x);
			}
		}
		else if (t == 3) {
			cin >> k >> x; k--;
