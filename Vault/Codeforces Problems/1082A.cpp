...
/***********************************************
** File name: 1082A.cpp                       **
** Time created: Sat Dec 1 2018 06:55am       **
***********************************************/
...
void Solve() {
	if (abs(x-y) % d == 0) {cout << (abs(x-y) / d) << endl; return;}
	i64 ans = LINF;
	if (abs(y-1) % d == 0) ans = min(ans, abs(x-1) / d + (abs(x-1) % d != 0) + abs(y-1) / d);
	if (abs(y-n) % d == 0) ans = min(ans, abs(x-n) / d + (abs(x-n) % d != 0) + abs(y-n) / d);
	if (ans == LINF) cout << "-1\n";
	else cout << ans << endl;
}
/*****************************************/
