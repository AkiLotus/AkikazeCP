...
void ProSolve() {
	...
	if (s[1] == t[1]) {
		...
		for (i64 i=0; i<abs(s[0] - t[0]); i++) {
			if (s[0] > t[0]) cout << "L\n"; else cout << "R\n";
		}
		return;
	}
	cout << max(abs(s[0]-t[0]), abs(s[1]-t[1])) << endl;
	for (i64 i=0; i<min(abs(s[0]-t[0]), abs(s[1]-t[1])); i++) {
		if (s[0] > t[0] && s[1] > t[1]) cout << "LD\n";
		else if (s[0] < t[0] && s[1] > t[1]) cout << "RD\n";
		else if (s[0] > t[0] && s[1] < t[1]) cout << "LU\n";
		else if (s[0] < t[0] && s[1] < t[1]) cout << "RU\n";
	}
	for (i64 i=0; i<max(abs(s[0]-t[0]), abs(s[1]-t[1]))-min(abs(s[0]-t[0]), abs(s[1]-t[1])); i++) {
		if (s[0] > t[0] && s[1] > t[1] && abs(s[0]-t[0]) > abs(s[1]-t[1])) cout << "L\n";
		else if (s[0] > t[0] && s[1] > t[1] && abs(s[0]-t[0]) < abs(s[1]-t[1])) cout << "D\n";
		else if (s[0] < t[0] && s[1] > t[1] && abs(s[0]-t[0]) > abs(s[1]-t[1])) cout << "R\n";
		else if (s[0] < t[0] && s[1] > t[1] && abs(s[0]-t[0]) < abs(s[1]-t[1])) cout << "D\n";
		else if (s[0] > t[0] && s[1] < t[1] && abs(s[0]-t[0]) > abs(s[1]-t[1])) cout << "L\n";
		else if (s[0] > t[0] && s[1] < t[1] && abs(s[0]-t[0]) < abs(s[1]-t[1])) cout << "U\n";
		else if (s[0] < t[0] && s[1] < t[1] && abs(s[0]-t[0]) > abs(s[1]-t[1])) cout << "R\n";
		else if (s[0] < t[0] && s[1] < t[1] && abs(s[0]-t[0]) < abs(s[1]-t[1])) cout << "U\n";
	}
}
