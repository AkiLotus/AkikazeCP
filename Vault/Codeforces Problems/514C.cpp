...
void ProSolve() {
	for (i64 i=1; i<1000009; i++) {
		for (i64 j=0; j<4; j++) {
			pow5[i][j] = (pow5[i-1][j] * 5) % HashMod[j];
		}
	}
	while (n--) {
		string s; cin >> s;
		...
		for (i64 i=0; i<s.size(); i++) {
			if (s[i] == 'a') {
				for (i64 j=0; j<4; j++) {
					hashVal[j] += pow5[i][j]; hashVal[j] %= HashMod[j];
					while (hashVal[j] < 0) hashVal[j] += HashMod[j];
				}
				Map[hashVal]++; //cout << "added " << hashVal << endl;
				for (i64 j=0; j<4; j++) {
					hashVal[j] += pow5[i][j]; hashVal[j] %= HashMod[j];
					while (hashVal[j] < 0) hashVal[j] += HashMod[j];
				}
				Map[hashVal]++; //cout << "added " << hashVal << endl;
				for (i64 j=0; j<4; j++) {
					hashVal[j] -= pow5[i][j] * 2; hashVal[j] %= HashMod[j];
					while (hashVal[j] < 0) hashVal[j] += HashMod[j];
				}
				//cout << "recovered: " << hashVal << endl;
			}
			else if (s[i] == 'b') {
				for (i64 j=0; j<4; j++) {
					hashVal[j] -= pow5[i][j]; hashVal[j] %= HashMod[j];
	...
	while (m--) {
		...
		//cout << "query hashVal = " << hashVal << endl;
		if (Map[hashVal]) cout << "YES\n";
		else cout << "NO\n";
	}
}
