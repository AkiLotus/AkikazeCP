...
struct ticket {
	...
	inline bool operator<(ticket target) {
		return (d < target.d);
	}
};

i64 n, m, k; vector<ticket> a;
/************************************************/
...
void Input() {
	cin >> n >> m >> k; a.rsz(m);
	for (i64 i=0; i<m; i++) cin >> a[i].d >> a[i].f >> a[i].t >> a[i].c;
}

void Solve() {
	sort(a.begin(), a.end());
	i64 petrTo = 0, petrFrom = -1, ans = LINF;
	i64 possiblecc = 0, sum = 0;
	vector<multiset<i64>> FlyTo(n+1), FlyFrom(n+1);
	for (i64 i=0; i<m; i++) {
		if (a[i].f == 0) {
			if (!FlyTo[a[i].t].empty()) sum -= *FlyTo[a[i].t].begin();
			FlyTo[a[i].t].insert(a[i].c); sum += *FlyTo[a[i].t].begin();
		}
	}
	for (i64 i=1; i<=1000003; i++) {
		i64 j = i + k + 1;
		while (petrFrom+1 < m && (a[petrFrom+1].f == 0 || a[petrFrom+1].d <= i)) {
			if (a[petrFrom+1].f != 0) {
				if (!FlyFrom[a[petrFrom+1].f].empty()) sum -= *FlyFrom[a[petrFrom+1].f].begin();
				FlyFrom[a[petrFrom+1].f].insert(a[petrFrom+1].c); sum += *FlyFrom[a[petrFrom+1].f].begin();
				if (FlyFrom[a[petrFrom+1].f].size() == 1 && !FlyTo[a[petrFrom+1].f].empty()) possiblecc++;
			}
			petrFrom++;
		}
		while (petrTo < m && (a[petrTo].f != 0 || a[petrTo].d < j)) {
			if (a[petrTo].f == 0) {
				sum -= *FlyTo[a[petrTo].t].begin();
				FlyTo[a[petrTo].t].erase(FlyTo[a[petrTo].t].find(a[petrTo].c));
				if (!FlyTo[a[petrTo].t].empty()) sum += *FlyTo[a[petrTo].t].begin();
				if (!FlyFrom[a[petrTo].t].empty() && FlyTo[a[petrTo].t].empty()) possiblecc--;
			}
			petrTo++;
		}
		if (possiblecc == n) ans = min(ans, sum);
	}
