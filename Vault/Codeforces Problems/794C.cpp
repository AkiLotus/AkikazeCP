...
void Solve() {
	...
	i64 ptr1L = 0, ptr2L = 0, turn = 0, ptr1R = (n+1)/2-1, ptr2R = n/2-1;
	string ansL = "", ansR = "";
	while (ansL.size() + ansR.size() < n) {
		if (turn == 0) {
			if (s[ptr1L] < t[ptr2L]) {ansL += s[ptr1L]; ptr1L++;}
			else {ansR += s[ptr1R]; ptr1R--;}
		}
		else {
			if (s[ptr1L] < t[ptr2L]) {ansL += t[ptr2L]; ptr2L++;}
			else {ansR += t[ptr2R]; ptr2R--;}
		}
		turn = 1 - turn;
	}
