...
void Solve() {
	if (n <= 100) {cout << n; return;}
	for (i64 x=3; x<=10; x++) {
		for (i64 i=0; i<(1LL<<x); i++) {
			for (i64 A=0; A<10; A++) {
				for (i64 B=A+1; B<10; B++) {
					if (i % 2 == 0 && A == 0) continue;
					i64 Num = 0;
					for (i64 z=0; z<x; z++) {
						Num *= 10;
						if ((i & (1LL << z)) == 0) Num += A;
			...
			for (i64 A=1; A<10; A++) {
				i64 Num = 0;
				for (i64 z=0; z<x; z++) Num = Num * 10 + A;
				if (Num <= n) Map[Num]++;
			}
		}
	}
	ans += Map.size(); cout << ans;
}
/*****************************************/
