...
/***********************************************
** File name: 173A.cpp                        **
** Time created: Wed Jan 2 2019 10:37am       **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n; string A, B; map<char, int> Map;
/************************************************/
...
void Input() {
	cin >> n >> A >> B;
	Map['R'] = 0; Map['S'] = 1; Map['P'] = 2;
}

void Solve() {
	i64 ma = A.size(), mb = B.size();
	i64 commoncycle = ma * mb / __gcd(ma, mb);
	i64 N = 0, P = 0, cyclecnt = n / commoncycle;
	for (i64 i=0; i<commoncycle; i++) {
		if (A[i % ma] == B[i % mb]) continue;
		if ((Map[A[i % ma]] + 1) % 3 == Map[B[i % mb]]) P += cyclecnt;
		else N += cyclecnt;
	}
	for (i64 i=0; i<n % commoncycle; i++) {
		if (A[i % ma] == B[i % mb]) continue;
		if ((Map[A[i % ma]] + 1) % 3 == Map[B[i % mb]]) P += 1;
		else N += 1;
	}
	cout << N << " " << P << endl;
}
