...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, u, v, w; vi a; vector<vector<pii>> adj;
/************************************************/
...
void Solve() {
	priority_queue<pii> Heap; vb vis(n, false);
	for (i64 i=0; i<n; i++) Heap.push(mp(-a[i], i));
	while (!Heap.empty()) {
		pii Data = Heap.top(); Heap.pop();
		i64 z = Data.se, d = -Data.fi;
		if (vis[z]) continue; vis[z] = true;
		for (auto T: adj[z]) {
			i64 t = T.fi, p = T.se;
			if (vis[t]) continue;
			if (d + p * 2 >= a[t]) continue;
			a[t] = d + p * 2; Heap.push(mp(-a[t], t));
		}
	}
	for (auto x: a) cout << x << " ";
}
