...
i64 n; vi dp(1000007, LINF);
vi previous(1000007, -1);
vi quasi;

/** -----EXTENSIVE FUNCTIONS----- **/
void recursive(i64 z) {
	quasi.pub(z); dp[z] = 1;
	if (z * 10 < 1e6+7) {
		recursive(z*10); recursive(z*10+1);
	}
}
...
void ProSolve() {
	...
	for (i64 i=2; i<=n; i++) {
		for (i64 j=0; j<quasi.size(); j++) {
			if (quasi[j] > i) continue;
			if (dp[i-quasi[j]] + 1 < dp[i]) {
				dp[i] = dp[i-quasi[j]] + 1;
				previous[i] = i - quasi[j];
			}
		}
	}
	cout << dp[n] << endl;
