...
/************** [GLOBAL VARIABLES] **************/
string s; i64 n, k; vector<vb> forbidden(26, vb(26, false));
/************************************************/
...
void Input() {
	...
	while (k--) {
		string a; cin >> a;
		forbidden[a[0]-'a'][a[1]-'a'] = true;
		forbidden[a[1]-'a'][a[0]-'a'] = true;
	}
}
...
void Solve() {
	...
	for (i64 i=0; i<26; i++) {
		for (i64 j=i+1; j<26; j++) {
			...
			for (i64 x=0; x<n; x++) {
				if (s[x]-'a' != i && s[x]-'a' != j) continue;
				i64 y = x; vi cnt(2, 0);
				while (y + 1 < n && (s[y+1]-'a' == i || s[y+1]-'a' == j)) y++;
				for (i64 z=x; z<=y; z++) {
					if (s[z]-'a' == i) cnt[0]++; else cnt[1]++;
				}
