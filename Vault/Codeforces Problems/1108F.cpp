...
/***********************************************
** File name: 1108F.cpp                       **
** Time created: Wed Jan 30 2019 06:07am      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, m; vi Last, siz;
vector<vi> List; vector<set<pii>> adj;
set<pip> EdgeList; i64 res = 0;
/************************************************/

/************** [FUNCTIONS] **************/
i64 DSUfind(i64 x) {
	while (Last[x] != -1) x = Last[x];
	return x;
}

void DSUmerge(i64 x, i64 y, i64 d) {
	i64 a = DSUfind(x), b = DSUfind(y);
	if (a == b) return;
	if (siz[a] < siz[b]) swap(a, b);
	siz[a] += siz[b]; Last[b] = a;
	for (auto u: List[b]) {
		List[a].pub(u);
		while (!adj[u].empty()) {
			pii Pair = *adj[u].begin(); adj[u].erase(Pair);
			i64 v = Pair.fi, w = Pair.se; adj[v].erase({u, w});
			EdgeList.erase({w, {min(u, v), max(u, v)}});
			i64 Mn = min(a, v), Mx = max(a, v);
			if (adj[a].lower_bound({v, -LINF}) != adj[a].end() && adj[a].lower_bound({v, -LINF})->fi == v) {
				i64 weight = adj[a].lower_bound({v, -LINF})->se;
				if (w > weight) continue;
				if (w == weight) {res++; continue;}
				adj[a].erase({v, weight}); adj[v].erase({a, weight}); EdgeList.erase({weight, {Mn, Mx}});
				adj[a].insert({v, w}); adj[v].insert({a, w}); EdgeList.insert({w, {Mn, Mx}});
			}
			else {adj[a].insert({v, w}); adj[v].insert({a, w}); EdgeList.insert({w, {Mn, Mx}});}
		}
	}
}

void Input() {
	cin >> n >> m; adj.rsz(n); List.rsz(n);
	Last.rsz(n, -1); siz.rsz(n, +1);
	for (i64 i=0; i<n; i++) List[i].pub(i);
	while (m--) {
		i64 u, v, w; cin >> u >> v >> w;
		u--; v--; if (u > v) swap(u, v);
		adj[u].insert({v, w}); adj[v].insert({u, w});
		EdgeList.insert({w, {u, v}});
	}
}

void Solve() {
	while (!EdgeList.empty()) {
		pip Edge = *EdgeList.begin(); EdgeList.erase(Edge);
		i64 w = Edge.fi, u = Edge.se.fi, v = Edge.se.se;
		adj[u].erase({v, w}); adj[v].erase({u, w});
		DSUmerge(u, v, w);
	}
	cout << res << endl;
}
