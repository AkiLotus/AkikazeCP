...
/************** [GLOBAL VARIABLES] **************/
struct analysis {
	i64 result = 0, maximal = 0;
};

i64 n, u, v; vi c; vector<vi> adj;
vi sub; vector<analysis> ans;
/************************************************/

/************** [FUNCTIONS] **************/
void DFS_Subtree(i64 z, i64 last) {
	for (auto t: adj[z]) {
		if (t == last) continue;
		DFS_Subtree(t, z); sub[z] += sub[t];
	}
}

void Increase(i64 id, i64 col, i64 cnt, map<i64, i64>&Map) {
	Map[col] += cnt;
	if (Map[col] > ans[id].maximal) ans[id].result = col;
	else if (Map[col] == ans[id].maximal) ans[id].result += col;
	ans[id].maximal = max(ans[id].maximal, Map[col]);
}

void ImportData(i64 dest, i64 src) {
	ans[dest] = ans[src];
}

void DFS(i64 z, i64 last, map<i64, i64>& Map) {
	i64 best = -1;
	for (auto t: adj[z]) {
		if (t == last) continue;
		if (best == -1 || sub[t] > sub[best]) best = t;
	}
	if (best != -1) {
		DFS(best, z, Map); ImportData(z, best);
		for (auto t: adj[z]) {
			if (t == last || t == best) continue;
			map<i64, i64> Map2; DFS(t, z, Map2);
	...
	Increase(z, c[z], 1, Map);
}

void Input() {
	cin >> n; c.rsz(n); adj.rsz(n);
	sub.rsz(n, 1); ans.rsz(n);
...
void Solve() {
	map<i64, i64> Map; DFS_Subtree(0, -1); DFS(0, -1, Map);
	for (i64 i=0; i<n; i++) cout << ans[i].result << " ";
}
/*****************************************/
