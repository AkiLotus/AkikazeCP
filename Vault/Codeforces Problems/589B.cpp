...
/************** [GLOBAL VARIABLES] **************/
i64 n; vector<ppi> A; set<ppi> S; set<i64> MaxSet;
/************************************************/
...
void Input() {
	...
	for (i64 i=0; i<n; i++) {
		...
		if (A[i].fi.fi > A[i].fi.se) swap(A[i].fi.fi, A[i].fi.se);
		A[i].se = i; S.insert(A[i]); MaxSet.insert(A[i].fi.se);
	}
}

void Solve() {
	sort(A.begin(), A.end(), secondsort);
	i64 petr = 0, ans = -LINF; pii opt = mp(-1LL, -1LL);
	for (auto it=MaxSet.begin(); it!=MaxSet.end(); it++) {
		i64 Max = *it;
		while (petr < n && A[petr].fi.se < Max) {
			S.erase(A[petr]); petr++;
		}
		i64 i = 0, N = S.size();
		for (auto it2=S.begin(); it2!=S.end(); it2++) {
			ppi Data = *it2; i64 Min = Data.fi.fi;
			if (ans < (N - i) * Min * Max) {
				ans = (N - i) * Min * Max;
				opt = mp(Min, Max);
			}
