...
void Input() {
	cin >> n >> m; disRow.rsz(n); disCol.rsz(n);
	while (m--) {
		cin >> x >> y; x--; y--;
		disRow[x] = true; disCol[y] = true;
	}
}
...
void Solve() {
	...
	for (i64 i=1; i<n/2; i++) {
		i64 tmp = 0;
		for (i64 mask=0; mask<256; mask++) {
			// 8-bit mask
			// NWRow-NWCol-NERow-NECol-SWRow-SWCol-SERow-SECol
			if ((mask & 3) == 3) continue; if ((mask & 129) == 129) continue;
			if ((mask & 3) == 3) continue; if ((mask & 66) == 66) continue;
			if ((mask & 12) == 12) continue; if ((mask & 36) == 36) continue;
			if ((mask & 12) == 12) continue; if ((mask & 24) == 24) continue;
			if ((mask & 48) == 48) continue; if ((mask & 24) == 24) continue;
			if ((mask & 48) == 48) continue; if ((mask & 36) == 36) continue;
			if ((mask & 192) == 192) continue; if ((mask & 66) == 66) continue;
			if ((mask & 192) == 192) continue; if ((mask & 129) == 129) continue;
			if ((mask & 5) == 5) continue; if ((mask & 80) == 80) continue;
			if ((mask & 34) == 34) continue; if ((mask & 136) == 136) continue;
			if ((mask & 1) == 1 && disRow[i]) continue;
			if ((mask & 2) == 2 && disCol[i]) continue;
			if ((mask & 4) == 4 && disRow[i]) continue;
			if ((mask & 8) == 8 && disCol[n-1-i]) continue;
			if ((mask & 16) == 16 && disRow[n-1-i]) continue;
			if ((mask & 32) == 32 && disCol[i]) continue;
			if ((mask & 64) == 64 && disRow[n-1-i]) continue;
			if ((mask & 128) == 128 && disCol[n-1-i]) continue;
			tmp = max(tmp, (i64)cntbit(mask));
		}
		ans += tmp;
	}
	if (n % 2 == 1) {
		if (!disRow[n/2] || !disCol[n/2]) ans++;
	}
	cout << ans;
}
/*****************************************/
