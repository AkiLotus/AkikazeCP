...
/***********************************************
** File name: 594B.cpp                        **
** Time created: Tue Jan 1 2019 08:32am       **
***********************************************/
...
/************** [FUNCTIONS] **************/
i64 binsearch_cycle(i64 Dist, i64 top, i64 bot) {
	i64 ans = top;
	while (top <= bot) {
		i64 mid = (top + bot) / 2;
		i64 TotalTravelled = ((ld)Pi * 2.0L * r * mid);
		if (TotalTravelled < Dist) {ans = mid; top = mid + 1;}
		else {bot = mid - 1;}
	}
	return ans;
}

ld binsearch_angle1(ld Len, ld top, ld bot) {
	ld ans = -1.0L;
	while (bot - top >= 0.000000000000001L) {
		ld mid = (top + bot) / 2.0L;
		ld func = r * sin(mid/2) * 2 + mid * r;
		if (func <= Len) {ans = mid; top = mid;}
		else {bot = mid;}
	}
	return ans;
}

ld binsearch_angle2(ld Len, ld top, ld bot) {
	ld ans = -1.0L;
	while (bot - top >= 0.000000000000001L) {
		ld mid = (top + bot) / 2.0L;
		ld func = mid * r;
		if (mid < Pi / 2) func = func - r + r * cos(mid);
		else if (mid > Pi / 2) func = func = func - r - r * sin(mid);
		if (func <= Len) {ans = mid; top = mid;}
		else {bot = mid;}
	}
	return ans;
...
void Input() {
	cin >> n >> r >> v; s.rsz(n); f.rsz(n);
	for (i64 i=0; i<n; i++) cin >> s[i] >> f[i];
}
...
void Solve() {
	...
	for (i64 i=0; i<n; i++) {
		i64 S = s[i], F = f[i];
		ld Cycle = binsearch_cycle(F-S, 0LL, INF);
		ld Len = (ld)F - (ld)S - (ld)Cycle * 2.0L * Pi;
		ld Angle = binsearch_angle1(Len, 0.0L, Pi);
		bool HalfPassed = false;
		if (Angle == -1.0L) {HalfPassed = true; Cycle += 0.5L; Len -= Pi * r; Angle = binsearch_angle2(Len, 0.0L, Pi);}
		tracker6(S, F, Cycle, Len, HalfPassed, Angle);
		cout << (Cycle * Pi * 2.0L * r + Angle * r) / v << endl;
	}
}
/*****************************************/
