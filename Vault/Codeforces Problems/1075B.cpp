...
/************** [GLOBAL VARIABLES] **************/
i64 n, m; vi A, t, cc; vector<pii> Taxi;
/************************************************/
...
void Input() {
	cin >> n >> m; A.rsz(n+m); t.rsz(n+m); cc.rsz(n+m);
	for (i64 i=0; i<n+m; i++) cin >> A[i];
	for (i64 i=0; i<n+m; i++) {cin >> t[i]; if (t[i]) Taxi.pub(mp(A[i], i));}
}

void Solve() {
	i64 LastTaxi = -LINF, LastPos = -1; Taxi.pub(mp(LINF, m+n));
	for (i64 i=0; i<n+m; i++) {
		if (t[i]) {LastTaxi = A[i]; LastPos = i; continue;}
		i64 id = lower_bound(Taxi.begin(), Taxi.end(), mp(A[i], LINF)) - Taxi.begin();
		if (A[i] - LastTaxi <= Taxi[id].fi - A[i]) cc[LastPos]++;
		else cc[Taxi[id].se]++;
