...
/***********************************************
** File name: 354C.cpp                        **
** Time created: Tue Dec 25 2018 13:53pm      **
***********************************************/
...
void Solve() {
	for (i64 i=1; i<1000001; i++) s[i] += s[i-1];
	i64 ans = min(k, *min_element(a.begin(), a.end()));
	for (i64 d=k+1; d<1000001; d++) {
		i64 cnt = 0;
		for (i64 i=d; i<1000001; i+=d) {
			i64 j = min(i + k, 1000000LL);
			cnt += Query(i, j);
		}
		if (cnt == n) ans = d;
	}
