...
void VarInput() {
	...
	cin >> n; path.rsz((1LL << n));
	for (i64 x=2; x<=(1LL << n); x*=2) {
		i64 segment = (1LL << n) / x;
		for (i64 i=0; i<x; i++) {
			i64 z; cin >> z;
			i64 L = segment * i, R = segment * (i + 1);
			path[L] += z; if (i < x-1) path[R] -= z;
		}
	}
}

void ProSolve() {
	for (i64 i=1; i<(1LL<<n); i++) path[i] += path[i-1];
	i64 token = *max_element(path.begin(), path.end());
	i64 ans = 0;
	for (i64 x=2; x<=(1LL << n); x*=2) {
		i64 segment = (1LL << n) / x;
		for (i64 i=0; i<x; i++) {
			i64 L = segment * i, R = segment * (i + 1);
			i64 val = *max_element(path.begin()+L, path.begin()+R);
			for (i64 q=L; q<R; q++) path[q] += (token - val); ans += (token - val);
		}
	}
	cout << ans;
}
