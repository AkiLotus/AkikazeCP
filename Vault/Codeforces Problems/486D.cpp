...
/************** [GLOBAL VARIABLES] **************/
i64 d, n, u, v; vector<pii> a;
vector<vi> adj; vb vis; map<i64, i64> Map;
/************************************************/

/************** [FUNCTIONS] **************/
i64 DFS(i64 z, i64 last, i64 src) {
	i64 cnt = 1;
	for (auto t: adj[z]) {
		if (t == last) continue;
		if (vis[t] || Map[t] > Map[src] + d) continue;
		cnt *= (DFS(t, z, src) + 1); cnt %= Mod;
	}
	return cnt;
}
