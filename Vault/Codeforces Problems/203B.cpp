...
/************** [GLOBAL VARIABLES] **************/
i64 n, m; bool Kuro[1008][1008];
/************************************************/

/************** [FUNCTIONS] **************/
bool squareFound(i64 x, i64 y) {
	for (i64 i=-2; i<=0; i++) {
		for (i64 j=-2; j<=0; j++) {
			if (x+i < 0 || y+j < 0) continue;
			bool flag = true;
			for (i64 a=x+i; a<x+i+3; a++) {
				for (i64 b=y+j; b<y+j+3; b++) {
					if (!Kuro[a][b]) {
						flag = false; break;
					}
				}
...
void Solve() {
	for (i64 i=1; i<=m; i++) {
		i64 x, y; cin >> x >> y; Kuro[x][y] = true;
		if (squareFound(x, y)) {cout << i; return;}
	}
	cout << "-1";
}
/*****************************************/
