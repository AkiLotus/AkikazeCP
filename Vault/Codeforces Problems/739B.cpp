...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi a, p, w; vector<vector<pii>> adj;
vi dp, dist; vector<vi> Parent;
/************************************************/
...
void GenerateParent() {
	Parent.rsz(n);
	for (i64 i=0; i<n; i++) {
		if (p[i] != -1) Parent[i].pub(p[i]);
	}
	for (i64 j=1; j<20; j++) {
		for (i64 i=0; i<n; i++) {
			if (Parent[i].size() < j) continue;
			if (Parent[Parent[i][j-1]].size() < j) continue;
			Parent[i].pub(Parent[Parent[i][j-1]][j-1]);
		}
	}
...
void process(i64 id) {
	if (p[id] == -1) return;
	//cout << "start processing: "; tracker1(id);
	//cout << "increase dp[" << p[id] << "] by 1\n";
	i64 u = id; dp[p[u]]++; i64 x = 0;
	//tracker2(u, x);
	do {
		//cout << "new loop\n";
		x = Parent[u].size(); bool unchanged = true;
		for (i64 i=0; i<Parent[u].size(); i++) {
			i64 Tracker = Parent[u][i];
			i64 Length = dist[id] - dist[Tracker];
			//tracker4(i, Tracker, Length, a[id]);	
			if (Length > a[id]) {x = i; unchanged = false; break;}
		}
		//cout << "Final x = " << x << endl;
		if (unchanged) {
			i64 v = Parent[u][Parent[u].size()-1];
			if (v == 0) {
				//cout << "early terminate\n";
				return;
			} else u = v;
		}
		else if (x > 0) u = Parent[u][x-1];
		//tracker2(u, x);
	} while (x > 0);
	dp[p[u]]--;
	//cout << "decrease dp[" << p[u] << "] by 1\n";
}

void DFS_Distance(i64 z) {
	for (auto T: adj[z]) {
		i64 t = T.fi, d = T.se;
		if (p[z] == t) continue;
		dist[t] = dist[z] + d;
...
void DFS_Accumulate(i64 z) {
	for (auto T: adj[z]) {
		i64 t = T.fi, d = T.se;
		if (p[z] == t) continue;
		DFS_Accumulate(t); dp[z] += dp[t];
	}
}

void Input() {
	cin >> n; a.rsz(n); adj.rsz(n);
	p.rsz(n, -1); w.rsz(n, LINF);
	dp.rsz(n, 0LL); dist.rsz(n, 0LL);
	for (i64 i=0; i<n; i++) cin >> a[i];
	for (i64 i=1; i<n; i++) {
		cin >> p[i] >> w[i]; p[i]--;
		adj[i].pub(mp(p[i], w[i]));
		adj[p[i]].pub(mp(i, w[i]));
	}
}
