...
void Input() {
	cin >> n >> m; a.rsz(n); deleted.rsz(n, false);
	for (i64 i=0; i<n; i++) cin >> a[i];
	while (m--) {i64 x; cin >> x; deleted[x-1] = true;}
}

void Solve() {
	i64 Size = LINF; string pattern = "";
	for (i64 i=0; i<n; i++) {
		if (!deleted[i]) continue;
		if (Size == LINF) Size = a[i].size();
		else if (a[i].size() != Size) {cout << "No"; return;}
	}
	for (i64 i=0; i<Size; i++) pattern += "?";
	for (i64 i=0; i<Size; i++) {
		...
		for (i64 j=0; j<n; j++) {
			...
			else if (pattern[i] != a[j][i]) pattern[i] = '?';
		}
	}
	for (i64 i=0; i<n; i++) {
		if (deleted[i]) continue;
		if (a[i].size() != Size) continue;
		if (match(pattern, a[i])) {cout << "No"; return;}
	}
	cout << "Yes\n" << pattern;
}
/*****************************************/
