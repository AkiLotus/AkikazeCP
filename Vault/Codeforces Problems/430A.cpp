...
/************** [GLOBAL VARIABLES] **************/
i64 n, m; vector<pii> pts, seg; vi Bal; vector<vi> A;
/************************************************/
...
void Input() {
	cin >> n >> m; pts.rsz(n); seg.rsz(m); A.rsz(n); Bal.rsz(m, 0);
	for (i64 i=0; i<n; i++) {cin >> pts[i].fi; pts[i].se = i;}
...
void Solve() {
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<m; j++) {
			if (seg[j].fi <= pts[i].fi && pts[i].fi <= seg[j].se) {
				A[i].pub(j);
			}
		}
	}
	sort(pts.begin(), pts.end()); vi Col(n, -1);
	for (auto z: pts) {
		i64 x = z.fi, id = z.se;
		for (auto t: A[id]) {
			if (Bal[t] == 0) continue;
			if (Bal[t] == -1) {
				if (Col[id] == 0) {cout << "-1"; return;}
				else Col[id] = 1;
			}
			else {
				if (Col[id] == 1) {cout << "-1"; return;}
				else Col[id] = 0;
			}
		}
