...
/************** [GLOBAL VARIABLES] **************/
i64 n, m; vi a; vector<ppi> Query; vi ans, Tree;
/************************************************/
...
void Update(i64 node, i64 st, i64 en, i64 i, i64 val) {
	if (st > en || en < i || i < st) return;
	if (st == en && en == i) {Tree[node] ^= val; return;}
	Update(node*2, st, (st+en)/2, i, val);
	Update(node*2+1, (st+en)/2+1, en, i, val);
...
void Input() {
	cin >> n; a.rsz(n);
	for (auto &x: a) cin >> x;
	cin >> m; Query.rsz(m); ans.rsz(m, 0);
	for (i64 i=0; i<m; i++) {
		cin >> Query[i].fi.fi >> Query[i].fi.se;
		Query[i].fi.fi--; Query[i].fi.se--;
		Query[i].se = i;
	}
}

void Solve() {
	sort(Query.begin(), Query.end(), querysort); Tree.rsz(n*4, 0); map<i64, i64> Map;
	vi prefixXor(n, a[0]); for (i64 i=1; i<n; i++) prefixXor[i] = prefixXor[i-1] ^ a[i];
	i64 Last = -1;
	for (i64 i=0; i<m; i++) {
		i64 j = i, Next = Query[i].fi.se;
		while (j+1 < m && Query[j+1].fi.se == Next) j++;
		for (i64 x=Last+1; x<=Next; x++) {
			if (Map.find(a[x]) != Map.end()) Update(1, 0, n-1, Map[a[x]], a[x]);
			Map[a[x]] = x; Update(1, 0, n-1, x, a[x]);
		}
		for (i64 x=i; x<=j; x++) {
			i64 id = Query[x].se;
			i64 L = Query[x].fi.fi, R = Query[x].fi.se;
			ans[id] = Get(1, 0, n-1, L, R) ^ prefixXor[R];
			if (L > 0) ans[id] ^= prefixXor[L-1];
		}
		Last = Next; i = j;
	}
