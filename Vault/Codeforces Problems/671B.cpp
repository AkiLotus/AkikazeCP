...
void Solve() {
	i64 RemInc = k, RemDec = k;
	while (RemInc > 0) {
		pii u = *Map.begin(); Map.erase(u.fi);
		if (Map.empty()) {
			i64 cycle = RemInc / u.se;
			i64 remainder = RemInc % u.se;
			Map[u.fi+cycle] += u.se-remainder; RemInc = 0;
			if (remainder > 0) Map[u.fi+cycle+1] += remainder;
		}
		else {
			pii v = *Map.begin();
			if ((v.fi - u.fi) * u.se <= RemInc) {
				RemInc -= (v.fi - u.fi) * u.se;
				Map[v.fi] += u.se;
			}
			else {
				i64 cycle = RemInc / u.se;
	...
	while (RemDec > 0) {
		pii u = *Map.rbegin(); Map.erase(u.fi);
		if (Map.empty()) {
			i64 cycle = RemDec / u.se;
			i64 remainder = RemDec % u.se;
			Map[u.fi-cycle] += u.se-remainder; RemDec = 0;
			if (remainder > 0) Map[u.fi-cycle-1] += remainder;
		}
		else {
			pii v = *Map.rbegin();
			if ((u.fi - v.fi) * u.se <= RemDec) {
				RemDec -= (u.fi - v.fi) * u.se;
				Map[v.fi] += u.se;
			}
			else {
				i64 cycle = RemDec / u.se;
	...
	cout << Map.rbegin()->fi - Map.begin()->fi;
}
/*****************************************/
