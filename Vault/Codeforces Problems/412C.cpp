...
void Solve() {
	i64 counter = 0; string Token = "kuroyukihime";
	for (i64 i=0; i<m; i++) {
		vi cnt(26, 0); i64 cc = 0, Last = -1;
		for (i64 x=0; x<n; x++) {
			if (a[x][i] == '?') continue;
			cnt[a[x][i]-'a']++;
			cc += (cnt[a[x][i]-'a'] == 1);
			if (cnt[a[x][i]-'a'] == 1) Last = a[x][i] - 'a';
		}
		if (cc == 0) {cout << Token[counter]; counter = (counter + 1) % 12;}
		else if (cc > 1) cout << "?";
		else cout << char(97+Last);
	}
}
/*****************************************/
