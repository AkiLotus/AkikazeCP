...
void ProSolve() {
	i64 minimum = min(n, m);
	i64 maximum = max(n, m);
	vector<pii> ans(1, mp(1,1));
	for (i64 i=2; i<=minimum; i++) {
		ans.pub(mp(i, i-1));
		ans.pub(mp(i, i));
	}
	for (i64 i=minimum+1; i<=maximum; i++) {
		if (minimum == n) ans.pub(mp(minimum, i));
		else ans.pub(mp(i, minimum));
	}
	cout << ans.size() << endl;
	for (i64 i=0; i<ans.size(); i++) cout << ans[i].fi << " " << ans[i].se << endl;
}
