...
/** -----GLOBAL VARIABLES----- **/
string s; i64 n, ans = LINF;

/** -----EXTENSIVE FUNCTIONS----- **/
i64 solve(char X, char Y) {
	i64 res = LINF;
	for (i64 i=0; i<n; i++) {
		if (s[i] != X) continue;
		for (i64 j=0; j<n; j++) {
			if (j == i) continue;
			if (s[j] != Y) continue;
			string S = s; i64 tmp = 0;
			i64 x = i, y = j;
			bool flag = false;
			if (x == 0 && y != 1 && S[1] == '0') flag = true;
			if (y == 0 && x != 1 && S[1] == '0') flag = true;
			if (x == 0 && y == 1 && S[2] == '0') flag = true;
			if (x == 1 && y == 0 && S[2] == '0') flag = true;
			if (flag) {
				i64 zz = 0;
				for (; zz < n; zz++) {
					if (zz == x || zz == y) continue;
					if (S[zz] == '0') continue;
					break;
				}
				while (zz > 0) {
...
void ProSolve() {
	...
	if (s == "57") {cout << "1\n"; return;}
	if (s.size() == 2) {cout << "-1\n"; return;}
	if (s[s.size()-2] == '0' && s[s.size()-1] == '0') {cout << "0\n"; return;}
	if (s[s.size()-2] == '2' && s[s.size()-1] == '5') {cout << "0\n"; return;}
	if (s[s.size()-2] == '5' && s[s.size()-1] == '0') {cout << "0\n"; return;}
	if (s[s.size()-2] == '7' && s[s.size()-1] == '5') {cout << "0\n"; return;}
