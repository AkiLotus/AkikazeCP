...
/************** [FUNCTIONS] **************/
pii reduced(i64 stretchX, i64 stretchY) {
	if (stretchX) {
		i64 unit = a / x;
		if (unit * y <= b) return mp(unit*x, unit*y);
		else return mp(-1LL, -1LL);
	}
	else if (stretchY) {
		i64 unit = b / y;
		if (unit * x <= a) return mp(unit*x, unit*y);
		else return mp(-1LL, -1LL);
	}
}
...
void Solve() {
	pii ans = max(reduced(1, 0), reduced(0, 1));
	if (ans == mp(-1LL, -1LL)) cout << "0 0"; else cout << ans.fi << " " << ans.se;
}
/*****************************************/
