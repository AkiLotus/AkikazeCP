...
/***********************************************
** File name: 165E.cpp                        **
** Time created: Mon Feb 25 2019 13:19pm      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
int n; vector<int> a; int vis[1 << 22];
int Max = (1 << 22) - 1;
/************************************************/
...
void Solve() {
	...
	for (auto z: a) {
		int token = z ^ Max;
		if (vis[token] != -1) continue;
		vis[token] = z; Q.push(token);
	}
	while (!Q.empty()) {
		int z = Q.front(); Q.pop();
		for (int i=0; i<22; i++) {
			...
			int t = z - (1 << i);
			if (vis[t] != -1) continue;
			vis[t] = vis[z]; Q.push(t);
		}
	}
	for (auto z: a) cout << vis[z] << " "; cout << endl;
}
/*****************************************/
