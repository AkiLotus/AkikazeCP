...
i64 n, l, r, x, ans = 0; vi bin, c;

/** -----EXTENSIVE FUNCTIONS----- **/
void recursive(i64 z) {
	if (z == n) {
		i64 sum = 0, cc = 0, mn = LINF, mx = -LINF;
		for (i64 i=0; i<n; i++) {
			sum += c[i] * bin[i];
			if (bin[i]) {
				mn = min(mn, c[i]);
		...
		if (cc > 1 && sum >= l && sum <= r && mx - mn >= x) ans++;
		return;
	}
	for (i64 i=0; i<2; i++) {
		bin[z] = i; recursive(z+1);
	}
}
...
void VarInput() {
	...
	cin >> n >> l >> r >> x; bin.resize(n, 0); c.resize(n, 0);
	for (i64 i=0; i<n; i++) cin >> c[i];
}

void ProSolve() {
	//cout << "Case " << ++cas << ": " << ans << endl; // for multi-testcase problems
	recursive(0); cout << ans;
}
