...
void Solve() {
	vb r(n), l(n), u(n), d(n); i64 ans = 0;
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<n; j++) {
			if (a[i].fi != a[j].fi && a[i].se != a[j].se) continue;
			if (a[i].fi < a[j].fi) {l[j] = true; r[i] = true;}
			if (a[i].fi > a[j].fi) {l[i] = true; r[j] = true;}
			if (a[i].se < a[j].se) {d[j] = true; u[i] = true;}
			if (a[i].se > a[j].se) {d[i] = true; u[j] = true;}
		}
	}
	for (i64 i=0; i<n; i++) ans += (l[i] && r[i] && d[i] && u[i]);
	cout << ans;
}
/*****************************************/
