...
void VarInput() {
	...
	cin >> n >> m; a.rsz(n); k.rsz(m);
	for (i64 i=0; i<n; i++) {cin >> a[i]; a[i]--;}
	for (i64 i=0; i<m; i++) {cin >> k[i]; sum += k[i];}
}

void ProSolve() {
	vi cc(m, 0); i64 ptr1 = 0, ptr2 = -1;
	i64 notFulfilled = m, ans = LINF;
	while (ptr2 < n-1) {
		ptr2++; cc[a[ptr2]]++;
		notFulfilled -= (cc[a[ptr2]] == k[a[ptr2]]);
		while (notFulfilled == 0 && ptr1 <= ptr2) {
			ans = min(ans, ptr2-ptr1+1-sum);
			cc[a[ptr1]]--;
			notFulfilled += (cc[a[ptr1]] < k[a[ptr1]]);
			ptr1++;
		}
	}
