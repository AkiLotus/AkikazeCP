...
void Input() {
	cin >> n >> t >> w; Data.rsz(n-1);
	for (i64 i=0; i<n-1; i++) cin >> Data[i].fi >> Data[i].se;
}

void Solve() {
	sort(Data.rbegin(), Data.rend());
	i64 petr = 0, floated = 0, optimal = LINF;
	multiset<i64> unfloated;
	do {
		if (!unfloated.empty() && t >= *unfloated.begin()) {
			i64 Min = *unfloated.begin();
			t -= Min; floated++;
			unfloated.erase(unfloated.find(Min));
		}
		while (petr < n-1 && Data[petr].fi > t) {
			unfloated.insert(Data[petr].se-Data[petr].fi+1);
			petr++;
		}
		optimal = min(optimal, petr-floated+1);
	} while (!unfloated.empty() && t >= *unfloated.begin());
	cout << optimal << endl;
}
/*****************************************/
