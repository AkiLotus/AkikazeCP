...
/************** [GLOBAL VARIABLES] **************/
i64 m, n, t, x, l, c, e, cnt = 0;
map<i64, i64> Map; map<pii, pii> Reduce;
/************************************************/
...
void Input() {
	...
	while (m--) {
		...
		if (t == 1) {
			cin >> x; Map[cnt] = x;
			Reduce[mp(cnt, cnt)] = mp(cnt, cnt); cnt++;
		}
		else if (t == 2) {
			cin >> l >> c;
			Reduce[mp(cnt, cnt+l*c-1)] = mp(0LL, l-1);
			cnt += l * c;
		}
	}
}
...
void Solve() {
	...
	while (n--) {
		cin >> e; e--;
		while (Reduce.find(mp(e, e)) == Reduce.end() || Reduce[mp(e, e)] != mp(e, e)) {
			auto zazz = Reduce.upper_bound(mp(e, LINF)); zazz--;
			pii seg = zazz->fi; i64 L = seg.fi, R = seg.se;
			i64 RL = Reduce[mp(L, R)].fi, RR = Reduce[mp(L, R)].se;
			i64 periodLen = (RR - RL + 1);
			e = (e - L) % periodLen + RL;
		}
		cout << Map[e] << " ";
	}
