...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi a, Tree; map<i64, set<i64>> Map;
priority_queue<i64, vi, greater<i64>> Heap;
/************************************************/

/************** [FUNCTIONS] **************/
i64 seekNearest(i64 val, i64 curr) {
	if (Map[val].lower_bound(curr) == Map[val].end()) return *Map[val].begin();
	return *Map[val].lower_bound(curr);
}

void Decrease(i64 node, i64 st, i64 en, i64 id) {
	if (st > en || en < id || id < st) return;
	if (st == en) {Tree[node]++; return;}
	Decrease(node*2, st, (st+en)/2, id);
	Decrease(node*2+1, (st+en)/2+1, en, id);
...
i64 gap(i64 st, i64 en) {
	if (st <= en) return Get(1, 0, n-1, st, en);
	return Get(1, 0, n-1, st, n-1) + Get(1, 0, n-1, 0, en);
}

void Input() {
	cin >> n; a.rsz(n); Tree.rsz(n*4, 0);
	for (i64 i=0; i<n; i++) {
		cin >> a[i];
		Map[a[i]].insert(i);
		Heap.push(a[i]);
...
void Solve() {
	i64 ans = n, ptr = 0;
	while (!Heap.empty()) {
		i64 Next = Heap.top(); Heap.pop();
		i64 id = seekNearest(Next, ptr);
		ans += (id - ptr - gap(ptr, id) + n * 4) % n; ptr = id;
		Decrease(1, 0, n-1, ptr); Map[Next].erase(ptr); ptr = (ptr + 1) % n;
	}
	cout << ans;
}
/*****************************************/
