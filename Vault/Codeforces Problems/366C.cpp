...
void Solve() {
	i64 offset = 100000; vector<vi> dp(n, vi(200001, -LINF));
	dp[0][offset] = 0; dp[0][offset+a[0]-b[0]*k] = a[0];
	for (i64 i=0; i<n-1; i++) {
		for (i64 j=0; j<200001; j++) {
			if (dp[i][j] == -LINF) continue;
			dp[i+1][j+a[i+1]-b[i+1]*k] = max(dp[i+1][j+a[i+1]-b[i+1]*k], dp[i][j] + a[i+1]);
			dp[i+1][j] = max(dp[i+1][j], dp[i][j]);
		}
	}
	if (dp[n-1][offset] <= 0) cout << "-1"; else cout << dp[n-1][offset];
}
/*****************************************/
