...
void VarInput() {
	...
	cin >> n >> m; v.resize(n); web.resize(n, 0); uv.resize(n);
	mat.resize(n, vector<i64>(n, 0));
	for (i64 i=0; i<n; i++) {
		i64 z; cin >> z; uv[i] = z;
		v[i] = mp(z, i);
	}
...
void ProSolve() {
	...
	for (i64 i=0; i<n; i++) {
		if (web[v[i].se] == 0) continue;
		for (i64 j=0; j<n; j++) {
			if (mat[v[i].se][j]) {
				mat[v[i].se][j] = 0; mat[j][v[i].se] = 0;
				web[v[i].se]--; web[j]--; ans += uv[j];
			}
		}
	}
	cout << ans;
