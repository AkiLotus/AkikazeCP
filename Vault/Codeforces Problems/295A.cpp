...
void VarInput() {
	...
	cin >> n >> m >> k; a.resize(n+3, 0); tmp.resize(n+3);
	for (i64 i=1; i<=n; i++) cin >> tmp[i];
	oper.resize(m+3); occ.resize(m+3, 0);
	for (i64 i=1; i<=m; i++) {
		i64 l, r, d; cin >> l >> r >> d;
		oper[i] = mp(mp(l, r), d);
	}
...
void ProSolve() {
	...
	for (i64 i=1; i<=m; i++) {
		occ[i] += occ[i-1];
		a[oper[i].fi.fi] += oper[i].se * occ[i];
		a[oper[i].fi.se+1] -= oper[i].se * occ[i];
	}
	for (i64 i=1; i<=n; i++) {
		a[i] += a[i-1];
		cout << a[i] + tmp[i] << " ";
