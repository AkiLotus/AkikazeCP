...
void Solve() {
	if (n > 40) {cout << n * 49 - 247; return;}
	vb vis(n*50+1, false); i64 ans = 0;
	for (i64 i=0; i<=n; i++) {
	    for (i64 j=0; i+j<=n; j++) {
	        for (i64 k=0; i+j+k<=n; k++) {
	            for (i64 l=0; i+j+k+l<=n; l++) {
	                if (i + j + k + l < n) continue;
	                if (vis[i*1+j*5+k*10+l*50]) continue;
	                vis[i*1+j*5+k*10+l*50] = true; ans++;
	            }
	        }
	    }
	}
