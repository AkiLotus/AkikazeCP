...
i64 n; string name, role; priority_queue<pis> Q;
/************************************************/
...
void Input() {
	...
	for (i64 i=0; i<n; i++) {
		cin >> name >> role;
		if (role == "rat") Q.push(mp(mp(3LL, -i), name));
		else if (role == "woman" || role == "child") Q.push(mp(mp(2LL, -i), name));
		else if (role == "man") Q.push(mp(mp(1LL, -i), name));
		else Q.push(mp(mp(0LL, -i), name));
	}
}
