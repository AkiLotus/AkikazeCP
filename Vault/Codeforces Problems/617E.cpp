...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, k, Base = 320, bucket; vi a, ps, ans;
vector<ppi> Query; unordered_map<i64, i64> Map; vector<vector<ppi>> Bucket;
/************************************************/
...
void Input() {
	cin >> n >> m >> k; a.rsz(n+1); Query.rsz(m); ps.rsz(n+1, 0);
...
void Solve() {
	...
	if (k == 0) {
		for (i64 x=0; x<bucket; x++) {
			for (auto it=Map.begin(); it!=Map.end(); it++) Map[it->fi] = 0;
			i64 st = max(x * Base - 1, 0LL), petr = st, res = 0;
			for (auto Q: Bucket[x]) {
	...
	for (i64 x=0; x<bucket; x++) {
		...
		for (auto Q: Bucket[x]) {
			i64 L = Q.fi.fi, R = Q.fi.se, id = Q.se;
			while (petr <= R) {
				if (Map.find(k ^ ps[petr]) != Map.end()) {
					res += Map[k ^ ps[petr]];
				}
				Map[ps[petr]]++; petr = petr + 1;
			}
			for (i64 i=st; i<L-1; i++) {
				if (Map.find(k ^ ps[i]) != Map.end()) {
					res -= Map[k ^ ps[i]];
				}
				Map[ps[i]]--;
			}
