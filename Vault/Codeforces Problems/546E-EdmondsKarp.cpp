...
void ProcessShortestPath() {
	...
	while (!Q.empty()) {
		i64 z = Q.front(); Q.pop();
		for (auto it=FlowNetAdj[z].begin(); it!=FlowNetAdj[z].end(); it++) {
...
i64 MaxFlow() {
	...
	do {
		...
		while (Last[pointer] != -1) {
			...
			PipesEncountered.pub(mp(Previous, pointer));
			pointer = Previous;
		}
		for (auto Pair: PipesEncountered) {
		...
		if (MinimumCapacity < LINF * 4) res += MinimumCapacity; ProcessShortestPath();
	} while (Last[N-1] != -1);
	return res;
}
