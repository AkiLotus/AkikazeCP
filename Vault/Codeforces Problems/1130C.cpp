...
/***********************************************
** File name: 1130C.cpp                       **
** Time created: Mon Feb 25 2019 08:39am      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, r1, c1, r2, c2; vector<vc> A;
i64 dx[] = {-1, +0, +0, +1};
i64 dy[] = {+0, -1, +1, +0};
/************************************************/

/************** [FUNCTIONS] **************/
void DFS(i64 x, i64 y, vector<vb> &vis, vector<pii> &List) {
	vis[x][y] = true; List.pub(mp(x, y));
	for (i64 i=0; i<4; i++) {
		if (x + dx[i] < 0 || x + dx[i] >= n) continue;
		if (y + dy[i] < 0 || y + dy[i] >= n) continue;
		if (vis[x+dx[i]][y+dy[i]]) continue;
		if (A[x+dx[i]][y+dy[i]] == '1') continue;
		DFS(x+dx[i], y+dy[i], vis, List);
	}
}

void Input() {
	cin >> n >> r1 >> c1;
	cin >> r2 >> c2; A.rsz(n, vc(n, '0'));
	r1--; c1--; r2--; c2--;
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<n; j++) {
			cin >> A[i][j];
...
void Solve() {
	vector<vb> vis1(n, vb(n, false));
	vector<vb> vis2(n, vb(n, false));
	vector<pii> List1, List2;
	DFS(r1, c1, vis1, List1);
	DFS(r2, c2, vis2, List2);
	i64 ans = LINF;
	for (auto C1: List1) {
		i64 i1 = C1.fi, j1 = C1.se;
		for (auto C2: List2) {
			i64 i2 = C2.fi, j2 = C2.se;
