...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi p, Kuro, TotalKuro; vector<vi> adj;
vector<vi> dp;
/************************************************/

/************** [FUNCTIONS] **************/
void DFS_InitKuro(i64 z) {
	for (auto t: adj[z]) {
		if (p[z] == t) continue;
		DFS_InitKuro(t);
		TotalKuro[z] += TotalKuro[t];
	}
	TotalKuro[z] += Kuro[z];
}

void DFS_Process(i64 z) {
	dp[z][0] = 1; dp[z][1] = 0;
	for (auto t: adj[z]) {
		if (p[z] == t) continue;
		DFS_Process(t);
		dp[z][1] *= dp[t][0];
		dp[z][1] += dp[z][0] * dp[t][1];
		dp[z][1] %= Mod;
		dp[z][0] *= dp[t][0];
		dp[z][0] %= Mod;
	}
	if (Kuro[z]) {dp[z][1] = dp[z][0]; dp[z][1] %= Mod;}
	else {dp[z][0] += dp[z][1]; dp[z][0] %= Mod;}
}

void Input() {
	cin >> n; p.rsz(n, -1);
	Kuro.rsz(n); adj.rsz(n);
	for (i64 i=1; i<n; i++) {
...
void Solve() {
	TotalKuro.rsz(n, 0); dp.rsz(n, vi(2, 0LL));
	DFS_InitKuro(0); DFS_Process(0);
	cout << dp[0][1];
}
/*****************************************/
