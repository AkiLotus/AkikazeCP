...
void ProSolve() {
	...
	while (n--) {
		i64 x; cin >> x;
		if (x == 50 && cent == 0) {cout << "NO"; return;}
		if (x == 100 && ((cent < 3 && half == 0) || (cent < 1 && half == 1) || cent == 0)) {cout << "NO"; return;}
		if (x == 25) cent++; else if (x == 50) {half++; cent--;} else if (x == 100) {
			quart++; if (half > 0) {half--; cent--;} else cent -= 3;
		}
	}
	cout << "YES";
}
