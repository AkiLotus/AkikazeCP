...
/***********************************************
** File name: 1089L.cpp                       **
** Time created: Wed Jan 2 2019 10:55am       **
***********************************************/
...
void Solve() {
	vi cnt(k, 0); i64 Rem = k;
	priority_queue<pii, vector<pii>, greater<pii>> Heap;
	for (auto x: a) {cnt[x]++; Rem -= (cnt[x] == 1);}
	for (i64 i=0; i<n; i++) {
		if (cnt[a[i]] > 1) Heap.push(mp(b[i], a[i]));
	}
	i64 ans = 0;
	while (!Heap.empty() && Rem > 0) {
		i64 val = Heap.top().fi, id = Heap.top().se; Heap.pop();
