...
void Input() {
	cin >> n >> m; l.rsz(m);
	for (i64 i=0; i<m; i++) {cin >> l[i]; l[i]--;}
}

void Solve() {
	vi perm(n, -1); map<i64, i64> Map; set<i64> S;
	for (i64 i=1; i<=n; i++) S.insert(i);
	for (i64 i=1; i<m; i++) {
		i64 step = (l[i] - l[i-1] + n * 4) % n;
		if (step == 0) step += n;
		if (perm[l[i-1]] == -1) {
			if (Map.find(step) == Map.end()) {
				perm[l[i-1]] = step;
				Map[step] = l[i-1]; S.erase(step);
			}
			else if (Map[step] != l[i-1]) {cout << "-1"; return;}
		}
		else if (perm[l[i-1]] != step) {cout << "-1"; return;}
	}
	for (i64 i=0; i<n; i++) {
		if (perm[i] == -1) {perm[i] = *S.begin(); S.erase(perm[i]);}
	}
