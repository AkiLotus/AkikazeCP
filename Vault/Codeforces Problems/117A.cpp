...
/***********************************************
** File name: 117A.cpp                        **
** Time created: Thu Dec 20 2018 10:42am      **
***********************************************/
...
void Input() {
	cin >> n >> m; s.rsz(n); f.rsz(n); t.rsz(n);
	for (i64 i=0; i<n; i++) {
		cin >> s[i] >> f[i] >> t[i];
		s[i]--; f[i]--;
	}
...
void Solve() {
	i64 cycle = m * 2 - 2;
	for (i64 i=0; i<n; i++) {
		i64 S = s[i], F = f[i], T = t[i];
		if (S == F) {cout << T << endl; continue;}
		i64 Modular = S;
		if (S > F) Modular = (m - 1) + (m - 1 - S);
		i64 cycleID = T / cycle;
		if (T  % cycle > Modular) cycleID++;
		i64 Endpoint = F;
		if (S > F) Endpoint = (m - 1) + (m - 1 - F);
		cout << cycle * cycleID + Endpoint << endl;
	}
}
/*****************************************/
