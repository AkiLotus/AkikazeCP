...
void Input() {
	cin >> n >> m >> k >> x >> s;
	a.rsz(m); b.rsz(m); c.rsz(k); d.rsz(k);
	for (i64 i=0; i<m; i++) cin >> a[i];
	for (i64 i=0; i<m; i++) cin >> b[i];
	for (i64 i=0; i<k; i++) cin >> c[i];
	for (i64 i=0; i<k; i++) cin >> d[i];
}

void Solve() {
	i64 ans = n*x;
	vi maximumEfficiency(k, c[0]);
	for (i64 i=1; i<k; i++) maximumEfficiency[i] = max(maximumEfficiency[i-1], c[i]);
	for (i64 i=0; i<k; i++) if (d[i] <= s) ans = min(ans, x * (n - c[i]));
	for (i64 i=0; i<m; i++) {
		i64 Rem = s - b[i];
		if (Rem < 0) continue;
		i64 id = upper_bound(d.begin(), d.end(), Rem) - d.begin() - 1;
		if (id < 0) ans = min(ans, a[i] * n);
		else ans = min(ans, a[i] * (n - maximumEfficiency[id]));
	}
	cout << ans;
}
/*****************************************/
