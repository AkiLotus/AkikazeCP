...
/************** [GLOBAL VARIABLES] **************/
string s; i64 n; vi cnt(26, 0);
vi DSUpar, DSUsize; vb isPrime;
/************************************************/
...
void SieveProcess() {
	isPrime.rsz(n+1, true);
	isPrime[0] = false; isPrime[1] = false;
	DSUpar.rsz(n, -1); DSUsize.rsz(n, +1);
	for (i64 i=2; i<=n; i++) {
...
void Solve() {
	SieveProcess(); vc Map(n, 'Z');
	priority_queue<pii> Heap1, Heap2;
	string ans = s; for (i64 i=0; i<n; i++) ans[i] = 'Z';
	for (i64 i=0; i<26; i++) Heap1.push(mp(cnt[i], i));
	for (i64 i=0; i<n; i++) {if (DSUfind(i) == i) Heap2.push(mp(DSUsize[i], i));}
	while (!Heap2.empty() && Heap2.top().fi <= Heap1.top().fi) {
		pii Data1 = Heap1.top(); Heap1.pop();
		pii Data2 = Heap2.top(); Heap2.pop();
		Data1.fi -= Data2.fi;
		Map[Data2.se] = char(97+Data1.se);
		Heap1.push(Data1);
	}
	for (i64 i=0; i<n; i++) ans[i] = Map[DSUfind(i)];
	if (*min_element(ans.begin(), ans.end()) == 'Z') cout << "NO";
	else cout << "YES\n" << ans;
}
/*****************************************/
