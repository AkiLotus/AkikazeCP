...
i64 n, m; string s; set<i64> chosen;
/************************************************/
...
void Solve() {
	set<pci> Set; string A = "";
	for (i64 i=0; i<n; i++) {
		Set.insert(mp(s[i], -i));
		if (i >= m-1) {
			if (chosen.lower_bound(i-m+1) == chosen.end()) {
				pci Z = *Set.begin(); Set.erase(Z);
				A += Z.fi; chosen.insert(-Z.se);
			}
			Set.erase(mp(s[i-m+1], -(i-m+1)));
		}
	}
	sort(A.begin(), A.end());
	char Last = A.back();
	for (i64 i=0; i<n; i++) {
		if (chosen.find(i) != chosen.end()) continue;
		if (s[i] < Last) A += s[i];
	}
	sort(A.begin(), A.end());
	cout << A;
