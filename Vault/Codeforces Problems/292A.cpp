...
void Input() {
	cin >> n; t.rsz(n); c.rsz(n);
	for (i64 i=0; i<n; i++) cin >> t[i] >> c[i];
}

void Solve() {
	i64 maxQueue = 0, curr = 0, Time = 0;
	for (i64 i=0; i<n; i++) {
		curr = max(0LL, curr - (t[i] - Time));
		curr += c[i]; maxQueue = max(maxQueue, curr); Time = t[i];
	}
	cout << (Time + curr) << " " << maxQueue;
}
/*****************************************/
