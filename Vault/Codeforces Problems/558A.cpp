...
void Solve() {
	sort(a.begin(), a.end());
	i64 s1 = 0; set<pii> Sp, Sn;
	bool left = false; i64 pos = 0;
	for (i64 i=0; i<n; i++) {
		pii z = a[i]; Sp.insert(z);
		z.fi = -z.fi; Sn.insert(z);
	}
	while (true) {
		if (left) {
			if (Sn.lower_bound(mp(-pos, -LINF)) == Sn.end()) break;
			pii z = *Sn.lower_bound(mp(-pos, -LINF));
			s1 += z.se; Sn.erase(z); z.fi = -z.fi;
			pos = z.fi; Sp.erase(z);
		}
		else {
			if (Sp.lower_bound(mp(pos, -LINF)) == Sp.end()) break;
			pii z = *Sp.lower_bound(mp(pos, -LINF));
			s1 += z.se; Sp.erase(z); pos = z.fi;
			z.fi = -z.fi; Sn.erase(z);
		}
		if (left) left = false; else left = true;
	}
	i64 s2 = 0; Sp.clear(); Sn.clear();
	left = true; pos = 0;
	for (i64 i=0; i<n; i++) {
		pii z = a[i]; Sp.insert(z);
		z.fi = -z.fi; Sn.insert(z);
	...
	while (true) {
		if (left) {
			if (Sn.lower_bound(mp(-pos, -LINF)) == Sn.end()) break;
			pii z = *Sn.lower_bound(mp(-pos, -LINF));
			s2 += z.se; Sn.erase(z); z.fi = -z.fi;
			pos = z.fi; Sp.erase(z);
		}
		else {
			if (Sp.lower_bound(mp(pos, -LINF)) == Sp.end()) break;
			pii z = *Sp.lower_bound(mp(pos, -LINF));
			s2 += z.se; Sp.erase(z); pos = z.fi;
			z.fi = -z.fi; Sn.erase(z);
		}
		if (left) left = false; else left = true;
	}
