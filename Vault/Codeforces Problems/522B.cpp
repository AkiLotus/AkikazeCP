...
i64 n, totalWidth = 0, trueMax = 0, subMax = 0;
vp frie;
priority_queue<i64> height;
...
void VarInput() {
	...
	cin >> n; frie.resize(n);
	for (i64 i=0; i<n; i++) {
		cin >> frie[i].fi >> frie[i].se;
		totalWidth += frie[i].fi;
		height.push(frie[i].se);
	}
	trueMax = height.top(); height.pop();
	subMax = height.top();
}
...
void ProSolve() {
	...
	for (i64 i=0; i<n; i++) {
		if (frie[i].se == trueMax) cout << ((totalWidth - frie[i].fi) * subMax) << " ";
		else cout << ((totalWidth - frie[i].fi) * trueMax) << " ";
	}
}
