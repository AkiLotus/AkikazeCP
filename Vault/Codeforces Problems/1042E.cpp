...
/***********************************************
** File name: 1042E.cpp                       **
** Time created: Tue Feb 12 2019 13:00pm      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, r, c; vector<vi> a; vi Inv(1000003);
/************************************************/

/************** [FUNCTIONS] **************/
i64 modPow(i64 a, i64 b, i64 res = 1) {
	while (b > 0) {
		if (b % 2 != 0) {b--; res *= a; res %= Mod;}
		else {b /= 2; a *= a; a %= Mod;}
	}
	return res;
}
...
void Input() {
	for (i64 i=1; i<1000003; i++) Inv[i] = modPow(i, Mod-2);
...
void Solve() {
	i64 res = 0; vector<pip> RemList;
	RemList.pub({a[r][c], {r, c}});
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<m; j++) {
			if (a[i][j] >= a[r][c]) continue;
			RemList.pub({a[i][j], {i, j}});
		}
	}
	i64 ForeignX1 = 0, ForeignX2 = 0;
	i64 ForeignY1 = 0, ForeignY2 = 0;
	i64 Probability = 0;
	sort(RemList.begin(), RemList.end());
	while (!RemList.empty()) {
		i64 j = RemList.size() - 1, i = j;
		while (i-1 >= 0 && RemList[i-1].fi == RemList[j].fi) i--;
		for (i64 x=i; x<=j; x++) {
			pii Pair = RemList[x].se;
			i64 e = Pair.fi, f = Pair.se;
			i64 indi_scorex = (ForeignX2 - 2LL * ForeignX1 * Probability * e + Probability * e * e) % Mod;
			i64 indi_scorey = (ForeignY2 - 2LL * ForeignY1 * Probability * f + Probability * f * f) % Mod;
			i64 indi_score = (indi_scorex + indi_scorey) % Mod;
			while (indi_score < 0) indi_score += Mod;
			res += indi_score; res %= Mod;
		}
		if (RemList.back().fi == a[r][c]) Probability = 1;
		for (i64 x=i; x<=j; x++) {
			pii Pair = RemList[x].se;
			i64 e = Pair.fi, f = Pair.se;
			ForeignX1 += (((Probability * Inv[i]) % Mod * e) % Mod + Mod * 4) % Mod; ForeignX1 %= Mod;
			ForeignX2 += (((Probability * Inv[i]) % Mod * e * e) % Mod + Mod * 4) % Mod; ForeignX2 %= Mod;
			ForeignY1 += (((Probability * Inv[i]) % Mod * f) % Mod + Mod * 4) % Mod; ForeignY1 %= Mod;
			ForeignY2 += (((Probability * Inv[i]) % Mod * f * f) % Mod + Mod * 4) % Mod; ForeignY2 %= Mod;
		}
		if (Probability == 1) Probability = Inv[i];
		else {Probability += Inv[i] * Probability; Probability %= Mod;}
		while (RemList.size() > i) RemList.pop_back();
	}
	cout << res << endl;
}
/*****************************************/
