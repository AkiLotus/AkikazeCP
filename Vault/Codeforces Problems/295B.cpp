...
void Input() {
	...
	for (i64 i=0; i<n; i++) {cin >> x[i]; x[i]--;}
}

void Solve() {
	vector<vi> shortestPath(n, vi(n, LINF));
	for (i64 k=n-1; k>=0; k--) {
		for (i64 i=n-1; i>=0; i--) {
			for (i64 j=n-1; j>=0; j--) {
				shortestPath[x[i]][x[j]] = min(min(shortestPath[x[i]][x[k]], a[x[i]][x[k]]) + min(shortestPath[x[k]][x[j]], a[x[k]][x[j]]), shortestPath[x[i]][x[j]]);
			}
		}
		//for (i64 i=0; i<n; i++) cout << shortestPath[i] << endl; cout << endl;
		for (i64 i=n-1; i>=k; i--) {
			for (i64 j=n-1; j>=k; j--) {
				if (shortestPath[x[i]][x[j]] != LINF) ans[k] += shortestPath[x[i]][x[j]];
			}
		}
	}
