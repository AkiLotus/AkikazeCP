...
/************** [GLOBAL VARIABLES] **************/
i64 na, ma, nb, mb; vector<vc> A, B;
i64 optimal = -LINF; pii Z = mp(-LINF, -LINF);
/************************************************/
...
void Input() {
	cin >> na >> ma; A.rsz(na, vc(ma, '0'));
	for (i64 i=0; i<na; i++) {
		for (i64 j=0; j<ma; j++) {
			cin >> A[i][j];
		}
	}
	cin >> nb >> mb; B.rsz(nb, vc(mb, '0'));
	for (i64 i=0; i<nb; i++) {
		for (i64 j=0; j<mb; j++) {
			cin >> B[i][j];
		}
...
void Solve() {
	for (i64 i=-50; i<=50; i++) {
		for (i64 j=-50; j<=50; j++) {
			i64 val = calc(i, j);
			if (val > optimal) {optimal = val; Z = mp(i, j);}
		}
	}
	cout << Z.fi << " " << Z.se;
}
