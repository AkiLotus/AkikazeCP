...
/************** [GLOBAL VARIABLES] **************/
i64 n, x; vi d, p, deepestLeaf; vector<vi> adj;
/************************************************/

/************** [FUNCTIONS] **************/
void DFS(i64 z, i64 last) {
	if (last != -1) d[z] = d[last] + 1;
	p[z] = last; deepestLeaf[z] = d[z];
	for (auto t: adj[z]) {
		if (t == last) continue;
		DFS(t, z);
		deepestLeaf[z] = max(deepestLeaf[z], deepestLeaf[t]);
	}
}

void Input() {
	cin >> n >> x; x--; adj.rsz(n);
	d.rsz(n, 0); p.rsz(n, 0); deepestLeaf.rsz(n, 0);
	for (i64 i=1; i<n; i++) {
		i64 a, b; cin >> a >> b; a--; b--;
...
void Solve() {
	DFS(0, -1); i64 Node = x, Max = 0;
	for (i64 i=0; i*2<d[x]; i++) {
		Max = max(Max, deepestLeaf[Node]);
		Node = p[Node];
	}
	cout << (Max * 2);
}
