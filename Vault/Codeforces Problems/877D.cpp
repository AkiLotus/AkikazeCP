...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, k, X1, Y1, X2, Y2; vector<vi> Dist;
vector<set<i64>> RowP, ColP, RowN, ColN, ObsR, ObsC; vector<vc> A;
/************************************************/
...
void Input() {
	cin >> n >> m >> k; A.rsz(n, vc(m));
	Dist.rsz(n, vi(m, LINF)); RowP.rsz(n); ColP.rsz(m);
	ObsR.rsz(n); ObsC.rsz(m); RowN.rsz(n); ColN.rsz(m);
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<m; j++) {
			cin >> A[i][j];
			if (A[i][j] == '#') {ObsR[i].insert(j); ObsC[j].insert(i);}
			else {RowP[i].insert(j); ColP[j].insert(i); RowN[i].insert(-j); ColN[j].insert(-i);}
		}
	}
	cin >> X1 >> Y1 >> X2 >> Y2;
	X1--; Y1--; X2--; Y2--;
...
void Solve() {
	queue<pii> Q; Q.push(mp(X1, Y1)); Dist[X1][Y1] = 0;
	RowP[X1].erase(Y1); ColP[Y1].erase(X1);
	RowN[X1].erase(-Y1); ColN[Y1].erase(-X1);
	while (!Q.empty()) {
		pii z = Q.front(); Q.pop();
		i64 x = z.fi, y = z.se;
		while (RowP[x].lower_bound(y) != RowP[x].end() && *RowP[x].lower_bound(y) <= y+k) {
			i64 y1 = *RowP[x].lower_bound(y);
			if (ObsR[x].lower_bound(y) != ObsR[x].end() && *ObsR[x].lower_bound(y) < y1) break;
			Q.push(mp(x, y1)); Dist[x][y1] = Dist[x][y] + 1;
			ColP[y1].erase(x); RowP[x].erase(y1); ColN[y1].erase(-x); RowN[x].erase(-y1);
		}
		while (RowN[x].lower_bound(-y) != RowN[x].end() && *RowN[x].lower_bound(-y) <= -(y-k)) {
			i64 y1 = *RowN[x].lower_bound(-y); y1 = -y1;
			if (ObsR[x].lower_bound(y1) != ObsR[x].end() && *ObsR[x].lower_bound(y1) < y) break;
			Q.push(mp(x, y1)); Dist[x][y1] = Dist[x][y] + 1;
			ColP[y1].erase(x); RowP[x].erase(y1); ColN[y1].erase(-x); RowN[x].erase(-y1);
		}
		while (ColP[y].lower_bound(x) != ColP[y].end() && *ColP[y].lower_bound(x) <= x+k) {
			i64 x1 = *ColP[y].lower_bound(x);
			if (ObsC[y].lower_bound(x) != ObsC[y].end() && *ObsC[y].lower_bound(x) < x1) break;
			Q.push(mp(x1, y)); Dist[x1][y] = Dist[x][y] + 1;
			ColP[y].erase(x1); RowP[x1].erase(y); ColN[y].erase(-x1); RowN[x1].erase(-y);
		}
		while (ColN[y].lower_bound(-x) != ColN[y].end() && *ColN[y].lower_bound(-x) <= -(x-k)) {
			i64 x1 = *ColN[y].lower_bound(-x); x1 = -x1;
			if (ObsC[y].lower_bound(x1) != ObsC[y].end() && *ObsC[y].lower_bound(x1) < x) break;
			Q.push(mp(x1, y)); Dist[x1][y] = Dist[x][y] + 1;
			ColP[y].erase(x1); RowP[x1].erase(y); ColN[y].erase(-x1); RowN[x1].erase(-y);
		}
	}
	if (Dist[X2][Y2] == LINF) cout << "-1"; else cout << Dist[X2][Y2];
}
/*****************************************/
