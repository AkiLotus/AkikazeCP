...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, base = 320, a, b; vector<vi> adj;
vi t, v, A; vector<vi> Table; vi Dist, d, C; vb vis;
/************************************************/
...
i64 distance(i64 x, i64 y) {
	...
	if (d[x] == d[y]) {
		...
		return ((1LL << id) * 2) + distance(Table[x][id], Table[y][id]);
	}
	if (d[x] < d[y]) {
		i64 mul = 1, id = 0;
		while (d[x] < d[y] - mul * 2) {
		...
		return mul + distance(x, Table[y][id]);
	}
	if (d[x] > d[y]) {
		i64 mul = 1, id = 0;
		while (d[y] < d[x] - mul * 2) {
		...
		return mul + distance(Table[x][id], y);
	}
}

void Input() {
	cin >> n >> m; adj.rsz(n); t.rsz(m); v.rsz(m); A.pub(0);
	for (i64 i=1; i<n; i++) {
		cin >> a >> b; a--; b--;
		adj[a].pub(b); adj[b].pub(a);
	}
	for (i64 i=0; i<m; i++) {cin >> t[i] >> v[i]; v[i]--;}
}

void Solve() {
	Preprocess();
	for (i64 i=0; i<m; i+=base) {
		for (i64 x=0; x<n; x++) {Dist[x] = 0; vis[x] = false;}
		queue<i64> Q; C.clear(); for (auto z: A) {Q.push(z); vis[z] = true;}
		while (!Q.empty()) {
			i64 z = Q.front(); Q.pop();
			for (auto t: adj[z]) {
				if (vis[t]) continue;
		...
		for (i64 x=i; x<min(i+base, m); x++) {
			if (t[x] == 1) {C.pub(v[x]); A.pub(v[x]);}
			else {
				i64 Min = Dist[v[x]];
				for (auto z: C) Min = min(Min, distance(z, v[x]));
				cout << Min << endl;
			}
		}
	}
