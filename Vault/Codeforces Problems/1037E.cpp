...
/***********************************************
** File name: 1037E.cpp                       **
** Time created: Wed Jan 2 2019 15:13pm       **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, k, x, y; vector<set<i64>> Adj;
vi Deg; set<pii> Set; vector<pii> Edges;
/************************************************/
...
void Input() {
	cin >> n >> m >> k; Adj.rsz(n); Deg.rsz(n); Edges.rsz(m);
	for (auto &Pair: Edges) {
		cin >> x >> y; x--; y--; Pair = {x, y};
		Adj[x].insert(y); Adj[y].insert(x);
		Deg[x]++; Deg[y]++;
	}
	for (i64 i=0; i<n; i++) Set.insert(mp(Deg[i], i));
}

void Solve() {
	reverse(Edges.begin(), Edges.end()); stack<i64> Result;
	while (!Set.empty() && Set.begin()->fi < k) {
		i64 u = Set.begin()->se; Set.erase(mp(Deg[u], u));
		for (auto v: Adj[u]) {
			if (Set.find(mp(Deg[v], v)) == Set.end()) continue;
			Set.erase(mp(Deg[v], v)); Deg[v]--;
			Set.insert(mp(Deg[v], v)); Adj[v].erase(u);
		}
		Adj[u].clear(); Deg[u] = 0;
	}
	for (auto Pair: Edges) {
		Result.push(Set.size());
		i64 x = Pair.fi, y = Pair.se;
		if (Adj[x].find(y) != Adj[x].end()) {
			Set.erase(mp(Deg[x], x)); Deg[x]--; Set.insert(mp(Deg[x], x));
			Set.erase(mp(Deg[y], y)); Deg[y]--; Set.insert(mp(Deg[y], y));
	...
	while (!Result.empty()) {cout << Result.top() << endl; Result.pop();}
}
/*****************************************/
