...
void Input() {
	cin >> n; p.rsz(n+1); change.rsz(n+1, 0LL);
...
void Solve() {
	...
	for (i64 i=1; i<=n; i++) {
		if (Q.empty() || Q.top().fi < p[i]) change[i]--;
		else {
			pii tmp = Q.top(); Q.pop();
			if (Q.empty() || Q.top().fi < p[i]) change[tmp.se]++;
			Q.push(tmp);
		}
		Q.push(mp(p[i], i));
	}
	i64 optimal = -LINF, val = 0;
	for (i64 i=1; i<=n; i++) {
		if (change[i] > optimal) {optimal = change[i]; val = p[i];}
		else if (change[i] == optimal && p[i] < val) {optimal = change[i]; val = p[i];}
	}
	cout << val;
}
/*****************************************/
