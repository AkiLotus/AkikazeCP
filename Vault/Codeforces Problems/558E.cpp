...
/***********************************************
** File name: 558E.cpp                        **
** Time created: Fri Dec 7 2018 10:15am       **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
struct SegTree {
	vi Tree, Lazy; vb Wipe;
	
	SegTree(i64 len) {
		Tree.clear(); Tree.rsz(len*4);
		Lazy.clear(); Lazy.rsz(len*4);
		Wipe.clear(); Wipe.rsz(len*4);
	}
	
	void Propagate(i64 node, i64 st, i64 en) {
		if (Wipe[node]) {
			if (st != en) {
				Tree[node*2+0] = 0; Lazy[node*2+0] = 0;
				Wipe[node*2+0] = true;
				Tree[node*2+1] = 0; Lazy[node*2+1] = 0;
				Wipe[node*2+1] = true;
			}
			Wipe[node] = false;
		}
	...
	void Wipeout(i64 node, i64 st, i64 en, i64 L, i64 R) {
		...
		Wipeout(node*2, st, (st+en)/2, L, R);
		Wipeout(node*2+1, (st+en)/2+1, en, L, R);
...
i64 n, q, i, j, k; string s; vector<SegTree> ST;
/************************************************/
...
void Solve() {
	...
	while (q--) {
		...
		for (i64 x=0; x<26; x++) {
			Token[x].se = x;
			Token[x].fi = ST[x].Get(1, 0, n-1, i, j);
			ST[x].Wipeout(1, 0, n-1, i, j);
		}
		if (k == 0) reverse(Token.begin(), Token.end());
		i64 ptr1 = i, ptr2 = i;
		for (auto z: Token) {
			i64 len = z.fi, id = z.se;
			ptr2 = ptr1 + len - 1;
			ST[id].Update(1, 0, n-1, ptr1, ptr2, 1);
			ptr1 = ptr2 + 1;
		}
	}
	for (i64 i=0; i<n; i++) {
		i64 id = 0;
		for (; id<26; id++) {
			if (ST[id].Get(1, 0, n-1, i, i) == 1) break;
		}
		cout << char(97 + id);
	}
	cout << endl;
