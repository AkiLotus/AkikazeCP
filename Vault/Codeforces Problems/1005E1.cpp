...
/************** [FUNCTIONS] **************/
void Increment(i64 node, i64 st, i64 en, i64 id) {
	if (st > en || en < id || id < st) return;
	if (st == en && en == id) {Tree[node]++; return;}
	Increment(node*2, st, (st+en)/2, id);
	Increment(node*2+1, (st+en)/2+1, en, id);
...
void Solve() {
	i64 Diff = 0, ans = 0;
	Tree.rsz(n*8+4, 0);
	Increment(1, 0, n*2, 0+n);
	for (i64 i=0; i<n; i++) {
		Diff += (a[i] >= m);
		Diff -= (a[i] < m);
		Increment(1, 0, n*2, Diff+n);
		if (Diff+n > 0) ans += Get(1, 0, n*2, 0, Diff+n-1);
	}
	Tree.clear(); Tree.rsz(n*8+4, 0);
	Increment(1, 0, n*2, 0+n); Diff = 0;
	for (i64 i=0; i<n; i++) {
		Diff += (a[i] >= m+1);
		Diff -= (a[i] < m+1);
		Increment(1, 0, n*2, Diff+n);
