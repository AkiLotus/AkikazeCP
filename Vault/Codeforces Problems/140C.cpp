...
void Solve() {
	...
	while (Q.size() >= 3) {
		...
		pii y = Q.top(); Q.pop();
		pii z = Q.top(); Q.pop();
		vi Tmp(3); Tmp[0] = x.se; Tmp[1] = y.se; Tmp[2] = z.se;
		sort(Tmp.rbegin(), Tmp.rend()); A.pub(Tmp);
		if (x.fi > 1) Q.push(mp(x.fi-1, x.se));
		if (y.fi > 1) Q.push(mp(y.fi-1, y.se));
		if (z.fi > 1) Q.push(mp(z.fi-1, z.se));
	}
	cout << A.size() << endl;
	for (auto Z: A) cout << Z[0] << " " << Z[1] << " " << Z[2] << endl;
}
/*****************************************/
