...
void VarInput() {
	...
	cin >> n >> p; l.resize(n); r = l;
	for (i64 i=0; i<n; i++) cin >> l[i] >> r[i];
}
...
void ProSolve() {
	...
	for (i64 i=0; i<n; i++) {
		i64 j = (i + 1) % n;
		i64 tmp1i = l[i] / p; if (l[i] % p != 0) tmp1i++;
		i64 tmp2i = r[i] / p;
		i64 tmp1j = l[j] / p; if (l[j] % p != 0) tmp1j++;
		i64 tmp2j = r[j] / p;
		i64 poss = (tmp2i - tmp1i + 1) * (r[j] - l[j] + 1) + (tmp2j - tmp1j + 1) * (r[i] - l[i] + 1) - (tmp2j - tmp1j + 1) * (tmp2i - tmp1i + 1);
		i64 total = (r[j] - l[j] + 1) * (r[i] - l[i] + 1);
		ans += (ld)poss / total * 2000;
	}
