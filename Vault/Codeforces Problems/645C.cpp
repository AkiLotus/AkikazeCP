...
void Solve() {
	for (i64 i=0; i<n; i++) if (s[i] == '0') Zero.pub(i);
	i64 ans = LINF;
	for (i64 petr1=0; petr1<=Zero.size()-k-1; petr1++) {
		i64 petr2 = petr1 + k;
		i64 RMid = upper_bound(Zero.begin(), Zero.end(), Zero[petr1] + (Zero[petr2] - Zero[petr1]) / 2) - Zero.begin();
		i64 LMid = RMid - 1;
		if (LMid >= 0) ans = min(ans, max(Zero[LMid] - Zero[petr1], Zero[petr2] - Zero[LMid]));
		if (RMid < Zero.size()) ans = min(ans, max(Zero[RMid] - Zero[petr1], Zero[petr2] - Zero[RMid]));
	}
	cout << ans;
}
/*****************************************/
