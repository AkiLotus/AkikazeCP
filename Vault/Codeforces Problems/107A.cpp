...
/************** [GLOBAL VARIABLES] **************/
i64 n, p, a, b, d; vb vis;
vector<vector<pii>> adjOut, adjIn; vector<ppi> A;
/************************************************/

/************** [FUNCTIONS] **************/
void DFS(i64 z, i64 src, i64 Min) {
	if (adjOut[z].empty() && !adjIn[z].empty()) {
		A.pub(mp(mp(src+1, z+1), Min)); return;
	}
	for (auto T: adjOut[z]) {
		i64 t = T.fi, diam = T.se;
		if (vis[t]) continue;
		DFS(t, src, min(Min, diam));
	}
}

void Input() {
	cin >> n >> p; adjIn.rsz(n);
	vis.rsz(n, false); adjOut.rsz(n);
	while (p--) {
		cin >> a >> b >> d; a--; b--;
		adjOut[a].pub(mp(b, d));
		adjIn[b].pub(mp(a, d));
...
void Solve() {
	for (i64 i=0; i<n; i++) {
		if (!vis[i] && !adjOut[i].empty() && adjIn[i].empty()) DFS(i, i, LINF);
	}
	cout << A.size() << endl;
	for (auto z: A) cout << z.fi.fi << " " << z.fi.se << " " << z.se << endl;
}
/*****************************************/
