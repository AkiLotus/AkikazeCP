...
void Solve() {
	...
	while(scanf("%lf %lf", &a, &d) == 2){
		i64 n;
		scanf("%lld", &n);
		double gese = 0.000000;
		//d -= (floor(d/(4*a)))*4*a;
		for(i64 i = 0; i < n; i++){
			//double gese = d*(i+1);
			//while((gese - 4*a) >= 0.000000001) gese -= 4*a;
			gese += d;
			gese -= (floor(gese/(4*a)))*4*a;
			//if(gese >= 4*a) gese -= 4*a;
			/*if((gese) >= 4*a){
				double t = (gese / (4*a));
				gese -= (t*4*a);
			}*/
			...
			double x, y;
			if(gese <= a){x = gese; y = 0;}
			else if(gese <= 2*a) {x = a; y = gese - a;}
			else if(gese <= 3*a) {x = 3*a - gese; y = a;}
			else {x = 0; y = 4*a - gese;}
			printf("%.10lf %.10lf\n", x, y);
		}
	}
