...
void VarInput() {
	...
	cin >> w >> b; dp.resize(w+1, vd(b+1, 0.0));
	dp[w][b] = 1.0;
}

void ProSolve() {
	//cout << "Case " << ++cas << ": " << ans << endl; // for multi-testcase problems
	queue<pii> Q; Q.push({w, b});
	while (!Q.empty()) {
		pii z = Q.front(); Q.pop();
		//cout << "dp[" << z.fi << "][" << z.se << "] = " << fixed << setprecision(15) << dp[z.fi][z.se] << endl;
		if (z.fi <= 0) continue;
		ans += dp[z.fi][z.se] * (ld)z.fi / (ld)(z.fi + z.se);
		//cout << "ans = " << ans << endl;
		if (z.se >= 3) {
			if (dp[z.fi][z.se-3] == 0) Q.push({z.fi, z.se - 3});
			dp[z.fi][z.se-3] += dp[z.fi][z.se] * ((ld)(z.se) / (z.fi + z.se)) * ((ld)(z.se - 1) / (z.fi + z.se - 1)) * ((ld)(z.se - 2) / (z.fi + z.se - 2));
		}
		if (z.se >= 2 && z.fi > 0) {
			if (dp[z.fi-1][z.se-2] == 0) Q.push({z.fi-1, z.se-2});
			dp[z.fi-1][z.se-2] += dp[z.fi][z.se] * ((ld)(z.se) / (z.fi + z.se)) * ((ld)(z.se - 1) / (z.fi + z.se - 1)) * ((ld)z.fi / (z.fi + z.se - 2));
		}
	}
	dout(15, ans);
}
