...
void Input() {
	cin >> n >> k; Parent.rsz(n, -1);
}

void Solve() {
	set<pii> Set; i64 Petr = k + 1;
	for (i64 i=1; i<=k; i++) {
		Parent[i] = 0;
		Set.insert(mp(1LL, i));
	}
	while (Petr < n) {
		pii z = *Set.begin(); Set.erase(z);
		i64 d = z.fi, id = z.se;
		Parent[Petr] = id;
		Set.insert(mp(d+1, Petr)); Petr++;
	}
	i64 maxDepth = Set.rbegin()->fi;
	Set.erase(*Set.rbegin()); maxDepth += Set.rbegin()->fi;
	cout << maxDepth << endl;
	for (i64 i=1; i<n; i++) cout << Parent[i]+1 << " " << i+1 << endl;
}
/*****************************************/
