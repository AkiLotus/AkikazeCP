...
void VarInput() {
	...
	cin >> n >> m; note.rsz(m);
	for (i64 i=0; i<m; i++) cin >> note[i].fi >> note[i].se;
	i64 ans = max(note[0].se+(note[0].fi-1), note[m-1].se+(n-note[m-1].fi));
	for (i64 i=1; i<m; i++) {
		if (abs(note[i].se - note[i-1].se) > abs(note[i].fi - note[i-1].fi)) {
			cout << "IMPOSSIBLE"; return;
		}
		ans = max(ans, max(note[i].se, note[i-1].se)+(abs(note[i].fi - note[i-1].fi) - abs(note[i].se - note[i-1].se))/2);
	}
	cout << ans;
}
