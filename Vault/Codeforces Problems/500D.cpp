...
/************** [GLOBAL VARIABLES] **************/
i64 n, q, r, w, sum = 0, tripletCount;
vector<vi> adj; vector<ppi> E; map<pii, i64> Map;
vi subtree, freq;
/************************************************/
...
void DFS(i64 z, i64 last) {
	for (auto t: adj[z]) {
		...
		i64 id = Map[mp(z, t)];
		freq[id] = subtree[t] * (n - subtree[t]);
		sum += freq[id] * E[id].se;
	}
}

void Input() {
	cin >> n; adj.rsz(n); E.rsz(n-1);
	subtree.rsz(n, 1); freq.rsz(n, 0);
	tripletCount = n * (n - 1) * (n - 2) / 6;
	for (i64 i=0; i<n-1; i++) {
		cin >> E[i].fi.fi >> E[i].fi.se >> E[i].se;
		E[i].fi.fi--; E[i].fi.se--;
		adj[E[i].fi.fi].pub(E[i].fi.se);
		adj[E[i].fi.se].pub(E[i].fi.fi);
		Map[mp(E[i].fi.fi, E[i].fi.se)] = i;
		Map[mp(E[i].fi.se, E[i].fi.fi)] = i;
	}
}

void Solve() {
	cout << fixed << setprecision(12); DFS(0, -1);
	cin >> q;
	while (q--) {
		cin >> r >> w; r--;
		sum -= freq[r] * E[r].se;
		E[r].se = w;
		sum += freq[r] * E[r].se;
		cout << (ld)sum / tripletCount * (n - 2) << endl;
	}
}
/*****************************************/
