...
/************** [GLOBAL VARIABLES] **************/
i64 n; vector<vi> a; vi p, row, col;
/************************************************/

/************** [FUNCTIONS] **************/
void Input() {
	cin >> n; p.rsz(n); a.rsz(n, vi(n));
	row.rsz(n, -LINF); col.rsz(n, -LINF);
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<n; j++) {
			cin >> a[i][j];
...
void Solve() {
	for (i64 required=1; required<=n; required++) {
		i64 i = 0;
		for (; i<n; i++) {
			if (a[i][i]) continue;
			if (row[i] <= required && col[i] <= required) break;
		}
		p[i] = required; a[i][i] = required;
	}
	for (auto x: p) cout << x << " ";
}
/*****************************************/
