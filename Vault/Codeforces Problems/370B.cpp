...
/************** [FUNCTIONS] **************/
bool contains(i64 i, i64 j) {
	i64 L = lower_bound(a[i].begin(), a[i].end(), a[j][0]) - a[i].begin();
	i64 R = lower_bound(a[i].begin(), a[i].end(), a[j][a[j].size()-1]) - a[i].begin();
	if (L == a[i].size() || R == a[i].size()) return false;
	i64 petr = 0;
	for (i64 x=L; x<=R; x++) {
		if (a[i][x] == a[j][petr]) petr++;
		if (petr == a[j].size()) return true;
	}
	return false;
}
...
void Solve() {
	for (i64 i=0; i<n; i++) {
		...
		for (i64 j=0; j<n; j++) {
			if (i == j) continue;
			if (contains(i, j)) {flag = false; break;}
		}
		if (flag) cout << "YES\n"; else cout << "NO\n";
	}
}
