...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi a, cnt(4004, 0); map<pii, i64> Map;
/************************************************/

/************** [FUNCTIONS] **************/
void Defactorization() {
	map<i64, i64> M; i64 siz = 0;
	for (i64 i=0; i<n; i++) {
		if (M.find(a[i]) == M.end()) {M[a[i]] = siz; siz++;}
		a[i] = M[a[i]];
	}
	for (auto x: a) cnt[x]++;
}
...
void Solve() {
	...
	Defactorization();
	i64 ans = *max_element(cnt.begin(), cnt.end());
	vector<vi> dp(4004, vi(4004, -LINF)); vi Set;
	for (i64 i=0; i<n; i++) {
		i64 x = a[i];
		for (auto y: Set) {
			if (x == y) continue;
	...
	for (i64 i=0; i<4004; i++) {
		for (i64 j=0; j<4004; j++) {
			ans = max(ans, dp[i][j]);
		}
	}
	cout << ans;
