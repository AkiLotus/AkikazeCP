...
void ProSolve() {
	...
	i64 lowerB = upper_bound(b.begin(), b.end(), a[0]-1) - b.begin();
	pii ans = mp(3*n, 3*m-lowerB);
	i64 z = 3*n, t = 0;
	for (i64 i=0; i<n-1; i++) {
		z--; if (i < n-1 && a[i] == a[i+1]) continue;
		lowerB = upper_bound(b.begin(), b.end(), a[i+1]-1) - b.begin();
		t = 3*m - lowerB;
		if (z-t > ans.fi-ans.se || (z-t == ans.fi-ans.se && z > ans.fi)) {
			ans = mp(z, t);
		}
	}
	if (2*(n-m) > ans.fi-ans.se || (2*(n-m) == ans.fi-ans.se && 2*n > ans.fi)) {
		ans = mp(2*n, 2*m);
	}
	cout << ans.fi << ":" << ans.se;
}
