...
void VarInput() {
	...
	cin >> k >> l >> m >> n >> d; vis.resize(d+1, false);
}

void ProSolve() {
	//cout << "Case " << ++cas << ": " << ans << endl; // for multi-testcase problems
	for (i64 i=1; i<=d/k; i++) {
		if (!vis[i*k]) ans++; vis[i*k] = true;
	}
	for (i64 i=1; i<=d/l; i++) {
		if (!vis[i*l]) ans++; vis[i*l] = true;
	}
	for (i64 i=1; i<=d/m; i++) {
		if (!vis[i*m]) ans++; vis[i*m] = true;
	}
	for (i64 i=1; i<=d/n; i++) {
		if (!vis[i*n]) ans++; vis[i*n] = true;
	}
	cout << ans;
}
