...
/***********************************************
** File name: 1143C.cpp
** Time created: Sun Mar 31 2019 07:09
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
int n, root; vi p, c; vector<vi> adj;
/************************************************/

/************** [FUNCTIONS] **************/
void DFS(int z, vi &Disrespected) {
	bool allDisrespected = true;
	for (auto t: adj[z]) {
		DFS(t, Disrespected);
		if (!c[t]) allDisrespected = false;
	}
	if (!c[z]) allDisrespected = false;
	if (allDisrespected) Disrespected.pub(z);
}

void Input() {
	cin >> n; p.rsz(n); c.rsz(n); adj.rsz(n);
	for (int i=0; i<n; i++) {
		cin >> p[i] >> c[i];
...
void Solve() {
	vi List; DFS(root, List);
	sort(List.begin(), List.end());
	if (List.empty()) {cout << "-1\n"; return;}
	for (auto z: List) cout << z+1 << " "; cout << endl;
}
/*****************************************/
