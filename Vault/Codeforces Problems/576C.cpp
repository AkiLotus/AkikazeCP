...
/***********************************************
** File name: 576C.cpp                        **
** Time created: Wed Dec 26 2018 11:59am      **
***********************************************/
...
/************** [FUNCTIONS] **************/
bool pointsort(ppi a, ppi b) {
	if (a.fi.fi / 1000 != b.fi.fi / 1000) return (a.fi.fi < b.fi.fi);
	if (a.fi.se != b.fi.se) {
		if ((a.fi.fi / 1000) % 2 == 0) return (a.fi.se < b.fi.se);
...
void Input() {
	cin >> n; A.rsz(n); i64 id = 1;
	for (auto &Point: A) {
		cin >> Point.fi.fi >> Point.fi.se;
		Point.se = id++;
	}
}

void Solve() {
	sort(A.begin(), A.end(), pointsort);
	for (auto Point: A) cout << Point.se << " "; cout << endl;
}
/*****************************************/
