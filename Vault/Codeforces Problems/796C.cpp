...
void Solve() {
	multiset<i64> Set; i64 ans = LINF;
	for (i64 i=0; i<n; i++) Set.insert(a[i]+2);
	for (i64 i=0; i<n; i++) {
		Set.erase(Set.find(a[i]+2)); Set.insert(a[i]);
		for (auto j: adj[i]) {Set.erase(Set.find(a[j]+2)); Set.insert(a[j]+1);}
		ans = min(ans, *Set.rbegin());
		Set.erase(Set.find(a[i])); Set.insert(a[i]+2);
		for (auto j: adj[i]) {Set.erase(Set.find(a[j]+1)); Set.insert(a[j]+2);}
	}
	cout << ans;
}
/*****************************************/
