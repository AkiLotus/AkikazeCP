...
bool splitted(i64 i, i64 j) {
	...
	if (i-1 >= 0 && A[i-1][j] == '#') {Q.push(mp(i-1, j)); vis[i-1][j] = true;}
	else if (j-1 >= 0 && A[i][j-1] == '#') {Q.push(mp(i, j-1)); vis[i][j-1] = true;}
	else if (i+1 < n && A[i+1][j] == '#') {Q.push(mp(i+1, j)); vis[i+1][j] = true;}
	else if (j+1 < m && A[i][j+1] == '#') {Q.push(mp(i, j+1)); vis[i][j+1] = true;}
	i64 cc = 1;
	while (!Q.empty()) {
		pii z = Q.front(); Q.pop();
		i64 x = z.fi, y = z.se;
		if (x-1 >= 0 && A[x-1][y] == '#' && !vis[x-1][y]) {cc++; Q.push(mp(x-1, y)); vis[x-1][y] = true;}
		if (y-1 >= 0 && A[x][y-1] == '#' && !vis[x][y-1]) {cc++; Q.push(mp(x, y-1)); vis[x][y-1] = true;}
		if (x+1 < n && A[x+1][y] == '#' && !vis[x+1][y]) {cc++; Q.push(mp(x+1, y)); vis[x+1][y] = true;}
		if (y+1 < m && A[x][y+1] == '#' && !vis[x][y+1]) {cc++; Q.push(mp(x, y+1)); vis[x][y+1] = true;}
	}
	return (cc < cnt - 1);
}
