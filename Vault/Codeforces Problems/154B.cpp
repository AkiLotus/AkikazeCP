...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, x; char c; vi primeActivation;
vb Activated; vector<vi> primeDiv;
/************************************************/

/************** [FUNCTIONS] **************/
void SieveProcess() {
	for (i64 i=2; i<=n; i++) {
		if (!primeDiv[i].empty()) continue;
		for (i64 j=1; j<=n/i; j++) {
			primeDiv[i*j].pub(i);
		}
	}
...
void Input() {
	cin >> n >> m; primeDiv.rsz(n+1);
	primeActivation.rsz(n+1, -1); Activated.rsz(n+1, false);
}
...
void Solve() {
	...
	while (m--) {
		cin >> c >> x;
		if (c == '-') {
			if (!Activated[x]) {cout << "Already off\n"; continue;}
			cout << "Success\n"; Activated[x] = false;
			for (auto p: primeDiv[x]) primeActivation[p] = -1;
		}
		else if (c == '+') {
			if (Activated[x]) {cout << "Already on\n"; continue;}
			i64 conflicted = -1;
			for (auto p: primeDiv[x]) {
				if (primeActivation[p] == -1) continue;
				conflicted = primeActivation[p];
			}
			if (conflicted != -1) {cout << "Conflict with " << conflicted << endl; continue;}
			for (auto p: primeDiv[x]) primeActivation[p] = x;
			cout << "Success\n"; Activated[x] = true;
		}
	}
}
