...
/***********************************************
** File name: 1046C.cpp                       **
** Time created: Sun Feb 24 2019 03:31am      **
***********************************************/
...
void Input() {
	cin >> N >> D; S.rsz(N); P.rsz(N);
	for (auto &z: S) cin >> z;
	for (auto &z: P) cin >> z;
}

void Solve() {
	multiset<i64> M; i64 ans = D; D--;
	for (i64 i=1; i<N; i++) M.insert(P[i]);
	i64 Threshold = S[D] + P[0];
	for (i64 i=D-1; i>=0; i--) {
		auto it = M.upper_bound(Threshold - S[i]);
		if (it == M.begin()) break;
		it--; M.erase(it); ans--;
	}
	cout << ans << endl;
