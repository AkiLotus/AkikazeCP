...
void Input() {
	cin >> n; type.rsz(n); a.rsz(n); adj.rsz(n);
	for (i64 i=0; i<n; i++) cin >> type[i];
	for (i64 i=0; i<n; i++) {cin >> a[i]; a[i]--; if (a[i] != -1) adj[a[i]].pub(i);}
}

void Solve() {
	i64 Max = 0; stack<i64> Answer;
	for (i64 i=0; i<n; i++) {
		if (!type[i] || adj[i].size()) continue;
		stack<i64> S; i64 x = i; S.push(x);
		while (a[x] != -1 && !type[a[x]] && adj[a[x]].size() == 1) {x = a[x]; S.push(x);}
		if (S.size() > Max) {Max = S.size(); Answer = S;}
	}
	cout << Max << endl;
	while (!Answer.empty()) {cout << Answer.top()+1 << " "; Answer.pop();}
}
/*****************************************/
