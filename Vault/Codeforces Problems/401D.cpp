...
/************** [GLOBAL VARIABLES] **************/
string n; i64 l, m; vi cnt(10, 0);
i64 dp[1LL << 18][100]; pair<int,int> Q[(1LL << 18) * 100];
i64 Front = 0, Back = -1;
/************************************************/
...
void Input() {
	cin >> n >> m; l = n.size();
	for (auto x: n) cnt[x-'0']++;
}

void Solve() {
	dp[0][0] = 1; Q[++Back] = mp(0, 0);
	while (Front <= Back) {
		pii Z = Q[Front]; Front++;
		i64 Mask = Z.fi, Rem = Z.se;
		vb vis(10, false);
		for (i64 x=0; x<l; x++) {
			if ((Mask & (1 << x)) != 0) continue;
			if (Mask == 0 && n[x] == '0') continue;
			if (vis[n[x]-'0']) continue; vis[n[x]-'0'] = true;
			i64 Mask1 = Mask + (1 << x);
			i64 Rem1 = (Rem * 10 + (n[x] - '0')) % m;
			if (dp[Mask1][Rem1] == 0) Q[++Back] = mp(Mask1, Rem1);
			dp[Mask1][Rem1] += dp[Mask][Rem];
		}
	}
	cout << dp[(1 << l) - 1][0];
}
