...
/** -----GLOBAL VARIABLES----- **/
i64 n, p; string s; bool flag = false;

/** -----EXTENSIVE FUNCTIONS----- **/
void inspect(i64 pos) {
	if (pos == n) {flag = true; cout << s; return;}
	i64 token = s[pos] - 'a';
	for (i64 i=token+1; i<p; i++) {
		if (pos > 0 && s[pos-1] == char(97+i)) continue;
		if (pos > 1 && s[pos-2] == char(97+i)) continue;
		s[pos] = i+97; inspect(pos+1);
		if (flag) return;
	}
	if (pos > 0) {s[pos] = char(96); inspect(pos-1);}
}
