...
void Solve() {
	for (i64 i=1,z=1; i<k && z<LINF; i++) {
		z *= 2; F.pub(z);
	}
	i64 sum = 0; vi A;
	while (F.size() >= k) {
		sum = 0;
		for (i64 i=F.size()-1; i>=F.size()-k; i--) {
			if (i == 0) sum++; else sum += F[i];
			if (sum >= LINF) break;
		}
		if (sum >= LINF) break; F.pub(sum);
	}
	for (i64 i=F.size()-1; i>=0; i--) {
		if (s >= F[i]) {A.pub(F[i]); s -= F[i];}
	}
	cout << A.size() << endl;
	for (auto x: A) cout << x << " ";
}
