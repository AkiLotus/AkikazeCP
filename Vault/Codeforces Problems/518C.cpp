...
i64 n, m, k, ans = 0; vi atPos, appPos;
...
void VarInput() {
	...
	cin >> n >> m >> k;
	atPos.resize(n+1); appPos.resize(n+1);
	for (i64 i=1; i<=n; i++) {
		cin >> atPos[i];
		appPos[atPos[i]] = i;
	}
...
void ProSolve() {
	...
	while (m--) {
		i64 app; cin >> app;
		i64 position = appPos[app];
		ans += (position - 1) / k + 1;
		if (position == 1) continue;
		i64 tmp = atPos[position-1];
		atPos[position-1] = atPos[position];
		atPos[position] = tmp;
		appPos[atPos[position-1]] = position-1;
		appPos[atPos[position]] = position;
	}
	cout << ans;
}
