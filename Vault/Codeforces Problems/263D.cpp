...
/***********************************************
** File name: 263D.cpp                        **
** Time created: Fri Dec 21 2018 13:29pm      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, k, a, b; vector<vi> adj;
vb vis; vi Dist, Last;
/************************************************/

/************** [FUNCTIONS] **************/
void DFS(i64 z, i64 &head, i64 &tail) {
	vis[z] = true;
	for (auto t: adj[z]) {
		if (Dist[z] - Dist[t] >= k) {
			head = t; tail = z; return;
		}
		if (vis[t]) continue;
		Dist[t] = Dist[z] + 1;
		Last[t] = z; DFS(t, head, tail);
		if (tail != -1) return;
	}
}
...
void Input() {
	cin >> n >> m >> k; adj.rsz(n);
	vis.rsz(n, false); Dist.rsz(n, LINF);
	Last.rsz(n, -1LL);
	while (m--) {
		cin >> a >> b; a--; b--;
...
void Solve() {
	queue<i64> Q; Q.push(0LL);
	vis[0] = true; Dist[0] = 0LL;
	i64 tail = -1LL, head = -1LL;
	DFS(0, head, tail);
	cout << Dist[tail] - Dist[head] + 1 << endl;
	while (tail != -1) {
		cout << (tail + 1) << " ";
		if (tail == head) break;
		tail = Last[tail];
