...
i64 n; vector<vi> a, a2, a5, d2, d5;
vector<vector<pii>> prev2, prev5;
pii zeroCoor = mp(-1, -1);
/************************************************/
...
void Input() {
	cin >> n; a.rsz(n, vi(n, 0)); a2 = a; a5 = a;
	d2.rsz(n, vi(n, INF)); d5 = d2;
	prev2.rsz(n, vector<pii>(n, mp(-1, -1))); prev5 = prev2;
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<n; j++) {
			cin >> a[i][j]; i64 tmp = a[i][j];
			if (a[i][j] == 0) {zeroCoor = mp(i, j); continue;}
			while (tmp % 2 == 0) {a2[i][j]++; tmp /= 2;}
			tmp = a[i][j];
			while (tmp % 5 == 0) {a5[i][j]++; tmp /= 5;}
		}
	}
	d2[0][0] = a2[0][0]; d5[0][0] = a5[0][0];
}
...
void Solve() {
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<n; j++) {
			if (a[i][j] == 0) continue;
			if (i > 0 && d2[i-1][j] + a2[i][j] < d2[i][j]) {
				d2[i][j] = d2[i-1][j] + a2[i][j];
				prev2[i][j] = mp(i-1, j);
			}
			if (j > 0 && d2[i][j-1] + a2[i][j] < d2[i][j]) {
				d2[i][j] = d2[i][j-1] + a2[i][j];
				prev2[i][j] = mp(i, j-1);
			}
			if (i > 0 && d5[i-1][j] + a5[i][j] < d5[i][j]) {
				d5[i][j] = d5[i-1][j] + a5[i][j];
				prev5[i][j] = mp(i-1, j);
			}
			if (j > 0 && d5[i][j-1] + a5[i][j] < d5[i][j]) {
				d5[i][j] = d5[i][j-1] + a5[i][j];
				prev5[i][j] = mp(i, j-1);
			}
		}
	}
//	for (i64 i=0; i<n; i++) cout << d2[i] << endl; cout << endl;
//	for (i64 i=0; i<n; i++) cout << d5[i] << endl;
//	cout << endl;
//	for (i64 i=0; i<n; i++) cout << prev2[i] << endl; cout << endl;
//	for (i64 i=0; i<n; i++) cout << prev5[i] << endl;
	i64 x = n-1, y = n-1; string path = "";
	if (d2[n-1][n-1] <= d5[n-1][n-1]) {
		if (zeroCoor == mp(-1, -1) || d2[n-1][n-1] <= 1) {
			cout << d2[n-1][n-1] << endl;
			while (prev2[x][y] != mp(-1, -1)) {
				i64 z = prev2[x][y].fi, t = prev2[x][y].se;
				if (z == x) path += "R"; else path += "D";
				x = z; y = t;
			}
		}
		else {
			x = zeroCoor.fi; y = zeroCoor.se;
			cout << "1\n";
			for (i64 i=0; i<x; i++) cout << "D";
			for (i64 i=0; i<y; i++) cout << "R";
			for (i64 i=0; i<n-1-x; i++) cout << "D";
			for (i64 i=0; i<n-1-y; i++) cout << "R";
			Exit();
		}
	}
	else {
		if (zeroCoor == mp(-1, -1) || d5[n-1][n-1] <= 1) {
			cout << d5[n-1][n-1] << endl;
			while (prev5[x][y] != mp(-1, -1)) {
				i64 z = prev5[x][y].fi, t = prev5[x][y].se;
				if (z == x) path += "R"; else path += "D";
				x = z; y = t;
			}
		}
