...
void Input() {
	cin >> n >> m; a.rsz(n); conn.rsz(n, vb(n, false));
	for (i64 i=0; i<n; i++) cin >> a[i];
	while (m--) {
		i64 u, v; cin >> u >> v; u--; v--;
		conn[u][v] = true; conn[v][u] = true;
	}
}
...
void Solve() {
	...
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<n; j++) {
			for (i64 k=0; k<n; k++) {
				if (!conn[i][j] || !conn[i][k] || !conn[j][k]) continue;
				if (ans == -1) ans = a[i] + a[j] + a[k];
				else ans = min(ans, a[i] + a[j] + a[k]);
			}
		}
	}
	cout << ans;
