...
void Input() {
	cin >> n; r1.rsz(n);
	for (i64 i=0; i<n; i++) cin >> r1[i];
	cin >> m; p1.rsz(m);
	for (i64 i=0; i<m; i++) cin >> p1[i];
	cin >> k; p2.rsz(k);
	for (i64 i=0; i<k; i++) cin >> p2[i];
	cin >> A >> B;
}
...
void Solve() {
	...
	for (auto Kuroyukihime: r1) {
		for (auto Lotus: p1) {
			ld val = (ld)(B * Lotus) / (B * Lotus + A * p2[0]);
			val = sqrt(val); val *= (ld)Kuroyukihime;
			ans = max(ans, val);
		}
	}
