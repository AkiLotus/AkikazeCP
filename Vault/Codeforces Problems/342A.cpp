...
/** -----GLOBAL VARIABLES----- **/
i64 n; vi a; map<i64, i64> Map; bool flag = true;
...
void ProSolve() {
	if (!flag) return;
	if (Map[1] != Map[4] + Map[6]) {cout << "-1"; return;}
	if (Map[4] > Map[2]) {cout << "-1"; return;}
	if (Map[3] > Map[6]) {cout << "-1"; return;}
	if (Map[2] - Map[4] + Map[3] != Map[6]) {cout << "-1"; return;}
	for (i64 i=0; i<Map[4]; i++) cout << "1 2 4\n";
	for (i64 i=0; i<Map[6]-Map[3]; i++) cout << "1 2 6\n";
	for (i64 i=0; i<Map[3]; i++) cout << "1 3 6\n";
}
