...
/***********************************************
** File name: 494B.cpp                        **
** Time created: Thu Dec 27 2018 08:18am      **
***********************************************/
...
void Solve() {
	...
	combine += "#"; combine += s;
	vi Z = Z_Algo(combine);
	vi dp(n, 0); i64 sum = 0, accumulated = 0;
	for (i64 j=m-1; j<n; j++) {
		i64 i = j - m + 1;
		if (i > 0) sum = (sum + dp[i-1] + 1) % Mod;
		else sum = (sum + 1) % Mod;
		if (Z[m+1+i] == m) accumulated = sum;
		if (j > 0) dp[j] = (dp[j-1] + accumulated) % Mod;
		else dp[j] = accumulated;
	}
	cout << dp[n-1] << endl;
}
/*****************************************/
