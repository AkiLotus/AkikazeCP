...
/************** [FUNCTIONS] **************/
void process(i64 N, i64 M, i64 id, i64 val) {
	if (N == 1) {P[id] = val; return;}
	if (N == 2) {
		P[id] = val; P[id+1] = val + 1;
		if (M == 2) swap(P[id], P[id+1]); return;
	}
	if (M > (1LL << (N - 2))) {
		P[id+N-1] = val;
		process(N-1, M-(1LL << (N-2)), id, val+1);
	}
	else {
		P[id] = val;
		process(N-1, M, id+1, val+1);
	}
}
