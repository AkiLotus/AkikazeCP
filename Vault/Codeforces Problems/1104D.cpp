...
/***********************************************
** File name: 1104D.cpp                       **
** Time created: Wed Jan 30 2019 11:23am      **
***********************************************/
...
void Input() {
	cin >> msg;
	if (msg == "mistake" || msg == "end") {
		if (msg == "mistake") cerr << "wrong answer\n";
		Exit();
	}
}
...
void Solve() {
	cout << "? 0 1\n";
	fflush(stdout); cin >> verd;
	if (verd == "e") {cerr << "limit exceeded\n"; Exit();}
	...
	while (true) {
		...
		fflush(stdout); cin >> verd;
		if (verd == "e") {cerr << "limit exceeded\n"; Exit();}
		if (verd == "x") break; L *= 2; R *= 2;
	}
	L = L + 1; i64 res = 0, Min = L - 1;
	while (L <= R) {
		i64 a = (L + R) / 2;
		cout << "? " << Min << " " << a << endl;
		fflush(stdout); cin >> verd;
		if (verd == "e") {cerr << "limit exceeded\n"; Exit();}
		if (verd == "x") {res = a; R = a - 1;}
		else L = a + 1;
	}
	cout << "! " << res << endl; fflush(stdout);
}
/*****************************************/
