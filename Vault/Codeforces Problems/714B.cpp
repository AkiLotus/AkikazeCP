...
void Solve() {
	if (Map.size() > 3) {cout << "NO"; return;}
	if (Map.size() < 3) {cout << "YES"; return;}
	vi A; for (auto it=Map.begin(); it!=Map.end(); it++) A.pub(it->first);
	if (A[2] - A[1] != A[1] - A[0]) cout << "NO"; else cout << "YES";
}
/*****************************************/
