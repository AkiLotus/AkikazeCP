...
/************** [GLOBAL VARIABLES] **************/
i64 n, k; vs A; vi p;
string DefaultMin = "", DefaultMax = "";
/************************************************/

/************** [FUNCTIONS] **************/
i64 Diff(string x, string y) {
	i64 res = 0;
...
void Input() {
	cin >> n >> k; A.rsz(n); p.rsz(k);
	for (i64 i=0; i<k; i++) DefaultMin += "0";
	for (i64 i=0; i<k; i++) DefaultMax += "9";
	for (i64 i=0; i<k; i++) p[i] = i;
...
void Solve() {
	i64 ans = LINF;
	do {
		string Min = DefaultMax, Max = DefaultMin;
		for (i64 i=0; i<n; i++) {
			string s = "";
			for (i64 j=0; j<k; j++) s += A[i][p[j]];
			Min = min(Min, s); Max = max(Max, s);
		}
		ans = min(ans, Diff(Max, Min));
