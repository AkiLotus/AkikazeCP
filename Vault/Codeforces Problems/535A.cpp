...
int s;
string od[10] = {"zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"};
string fttd[10] = {"ten", "eleven", "twelve", "thirteen", "fourteen", "fifteen", "sixteen", "seventeen", "eighteen", "nineteen"};
string rtd[10] = {"", "", "twenty", "thirty", "forty", "fifty", "sixty", "seventy", "eighty", "ninety"};
...
void ProSolve() {
	...
	if (s < 10) cout << od[s];
	else if (s < 20) cout << fttd[s-10];
	else if (s % 10 == 0) cout << rtd[s/10];
	else cout << rtd[s/10] << "-" << od[s%10];
}
