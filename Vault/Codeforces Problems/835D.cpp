...
/************** [GLOBAL VARIABLES] **************/
string s; i64 n; vector<vi> Hash; vi ans;
/************************************************/

/************** [FUNCTIONS] **************/
void Process(i64 L, i64 R) {
	i64 len = R - L + 1, petr = 0;
	while (true) {
		if (len < 2) break;
		if (Hash[L][L+len/2-1] != Hash[R-len/2+1][R]) break;
		R = L + len / 2 - 1; len = R - L + 1;
		ans[petr]++; petr++;
	}
	ans[petr]++;
}
...
void Solve() {
	for (i64 i=0; i<n; i++) {
		for (i64 j=i; j<n; j++) {
			if (i == j) Hash[i][j] = s[j];
			else Hash[i][j] = (Hash[i][j-1] * 257 + s[j]) % Mod;
		}
	}
	for (i64 i=0; i<n; i++) {
		i64 L = i, R = i;
		while (L >= 0 && R < n && s[L] == s[R]) {Process(L, R); L--; R++;}
	}
	for (i64 i=1; i<n; i++) {
