...
void Solve() {
	vi farthestRight(n, -LINF);
	for (i64 j=0; j<m; j++) {
		for (i64 i=0; i<n; i++) {
			i64 z = i;
			while (z+1 < n && a[z][j] <= a[z+1][j]) z++;
			//cout << "(" << i << ", " << z << ") ";
			for (i64 x=i; x<=z; x++) farthestRight[x] = max(farthestRight[x], z);
			i = z;
		}
		//cout << endl;
	}
