...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi a; vector<set<i64>> Suffix; set<i64> Total;
/************************************************/
...
void Input() {
	cin >> n; a.rsz(n); Suffix.rsz(n);
	for (i64 i=0; i<n; i++) cin >> a[i];
}

void Solve() {
	Suffix[0].insert(a[0]); Total.insert(a[0]);
	for (i64 i=1; i<n; i++) {
		Suffix[i].insert(a[i]);
		Total.insert(a[i]);
		for (auto it=Suffix[i-1].begin(); it!=Suffix[i-1].end(); it++) {
			Suffix[i].insert((*it) | a[i]);
			Total.insert((*it) | a[i]);
		}
	}
	cout << Total.size();
