...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, s = 0; vi DSUpar, DSUsize;
vector<pii> a; vector<vi> adj;
/************************************************/
...
void DSUmerge(i64 a, i64 b, i64 val) {
	i64 x = DSUfind(a), y = DSUfind(b);
	if (x == y) return;
	s += val * DSUsize[x] * DSUsize[y];
	if (DSUsize[x] < DSUsize[y]) swap(x, y);
	DSUsize[x] += DSUsize[y]; DSUpar[y] = x;
}

void Input() {
	cin >> n >> m; a.rsz(n); adj.rsz(n);
	DSUpar.rsz(n, -1); DSUsize.rsz(n, 1);
	for (i64 i=0; i<n; i++) {cin >> a[i].fi; a[i].se = i;}
	while (m--) {
		i64 x, y; cin >> x >> y; x--; y--;
		if (a[x] < a[y]) adj[x].pub(y);
		else adj[y].pub(x);
	}
}
...
void Solve() {
	...
	for (auto Z: a) {
		i64 Min = Z.fi, id = Z.se;
		for (auto q: adj[id]) DSUmerge(id, q, Min);
	}
