...
/***********************************************
** File name: 627B.cpp                        **
** Time created: Sat Feb 2 2019 15:15pm       **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, k, a, b, q; vi TreeA, TreeB;
/************************************************/

/************** [FUNCTIONS] **************/
void Update(vi &Tree, i64 node, i64 st, i64 en, i64 id, i64 x, i64 lim) {
	if (st > en || en < id || id < st) return;
	if (st == en) {Tree[node] = min(Tree[node]+x, lim); return;}
	Update(Tree, node*2+1, st, (st+en)/2+0, id, x, lim);
	Update(Tree, node*2+2, (st+en)/2+1, en, id, x, lim);
	Tree[node] = Tree[node*2+1] + Tree[node*2+2];
}

i64 Get(vi &Tree, i64 node, i64 st, i64 en, i64 L, i64 R) {
	if (st > en || en < L || R < st) return 0;
	if (L <= st && en <= R) return Tree[node];
	i64 p1 = Get(Tree, node*2+1, st, (st+en)/2+0, L, R);
	i64 p2 = Get(Tree, node*2+2, (st+en)/2+1, en, L, R);
	return (p1 + p2);
}

void Input() {
	cin >> n >> k >> a >> b >> q;
	TreeB.rsz(n*4, 0); TreeA.rsz(n*4, 0);
}
...
void Solve() {
	while (q--) {
		...
		if (t == 1) {
			cin >> d >> z; d--;
			Update(TreeA, 0, 0, n-1, d, z, a);
			Update(TreeB, 0, 0, n-1, d, z, b);
		}
		else if (t == 2) {
			cin >> p; p--; i64 ans = 0;
			i64 q = p + k - 1;
			if (p > 0) ans += Get(TreeB, 0, 0, n-1, 0, p-1);
			if (q < n-1) ans += Get(TreeA, 0, 0, n-1, q+1, n-1);
			cout << ans << endl;
		}
	}
}
