...
void Solve() {
	...
	while (t--) {
		cin >> i >> j; i--; j--;
		i64 id = i * m + j;
		i64 wasted = lower_bound(waste.begin(), waste.end(), id) - waste.begin();
		if (wasted < waste.size() && waste[wasted] == id) {cout << "Waste\n"; continue;}
		if ((id + 1 - wasted) % 3 == 1) cout << "Carrots\n";
		else if ((id + 1 - wasted) % 3 == 2) cout << "Kiwis\n"; else cout << "Grapes\n";
	}
}
/*****************************************/
