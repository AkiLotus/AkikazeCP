...
/** -----GLOBAL VARIABLES----- **/
i64 n; map<pii, i64> Map;
vector<vi> adj; vb vis; vi p, ans;
queue<i64> Q;

/** -----EXTENSIVE FUNCTIONS----- **/
void DFS_TS(i64 z, bool stat) {
    vis[z] = true;
    for (i64 i=0; i<adj[z].size(); i++) {
        if (vis[adj[z][i]]) continue;
        i64 x = adj[z][i];
        p[x] = z;
        if (Map[mp(z,x)] == 2) DFS_TS(x, true);
        else DFS_TS(x, false);
    }
    if (stat == true) Q.push(z);
}

void traceback(i64 z) {
	vis[z] = true;
	if (p[z] != -1 && !vis[p[z]]) {
		traceback(p[z]);
	}
...
void VarInput() {
	...
	cin >> n; adj.resize(n+1); vis.resize(n+1, false);
	p.resize(n+1, -1LL);
	for (i64 i=1; i<n; i++) {
	    i64 x, y, t; cin >> x >> y >> t;
	    Map[mp(x,y)] = t; Map[mp(y,x)] = t;
	    adj[x].pub(y); adj[y].pub(x);
	}
}

void ProSolve() {
    DFS_TS(1, false); for (i64 i=1; i<=n; i++) vis[i] = false;
    while (!Q.empty()) {
		i64 z = Q.front(); Q.pop();
		if (vis[z]) continue;
		ans.pub(z); traceback(z);
	}
    cout << ans.size() << endl;
