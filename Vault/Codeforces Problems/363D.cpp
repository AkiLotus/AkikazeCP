...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, a, r = 0, s = 0, id = -1; vi b, p;
/************************************************/

/************** [FUNCTIONS] **************/
bool possible_r(i64 x) {
	if (x > m) return false;
	i64 commonPaid = 0;
	for (i64 i=0; i<x; i++) {
		commonPaid += max(0LL, p[x-1-i] - b[n-1-i]);
	}
	return (commonPaid <= a);
}
...
void binsearch_r() {
	...
	while (top <= bot) {
		i64 mid = (top + bot) / 2;
		if (possible_r(mid)) {r = mid; top = mid + 1;}
		else bot = mid - 1;
	}
}

bool possible_id(i64 Min) {
	i64 commonPaid = 0;
	for (i64 i=0; i<r; i++) {
		commonPaid += max(0LL, p[i] - b[Min+i]);
	}
	return (commonPaid <= a);
}
...
void binsearch_id() {
	...
	while (top <= bot) {
		i64 mid = (top + bot) / 2;
		if (possible_id(mid)) {id = mid; bot = mid - 1;}
		else top = mid + 1;
	}
}

void Input() {
	cin >> n >> m >> a; b.rsz(n); p.rsz(m);
...
void Solve() {
	...
	sort(p.begin(), p.end());
	binsearch_r();
	if (r == 0) {cout << "0 0"; return;}
	vi ps(m, p[0]); for (i64 i=1; i<m; i++) ps[i] = ps[i-1] + p[i];
	binsearch_id(); s = max(ps[r-1] - a, 0LL);
	cout << r << " " << s;
}
/*****************************************/
