...
void VarInput() {
	...
	cin >> n >> m >> k; p.resize(n, 0); s = p;
	dp.resize(n, vi(k+1, 0));
	for (i64 i=0; i<n; i++) cin >> p[i]; s[0] = p[0];
	for (i64 i=1; i<n; i++) s[i] = s[i-1] + p[i];
}
	...
	for (i64 i=0; i<n; i++) {
		for (i64 j=1; j<=k; j++) {
			...
			if (i >= m-1) {
				i64 tmp = s[i];
				if (i >= m) tmp -= s[i-m];
				if (i >= m) dp[i][j] = max(dp[i][j], dp[i-m][j-1] + tmp);
				else dp[i][j] = tmp;
			}
		}
	}
//	for (i64 i=0; i<n; i++) {
//		for (i64 j=1; j<=k; j++) cout << dp[i][j] << " "; cout << endl;
//	}
	i64 ans = -LINF;
	for (i64 i=0; i<n; i++) ans = max(ans, dp[i][k]);
	cout << ans;
}
