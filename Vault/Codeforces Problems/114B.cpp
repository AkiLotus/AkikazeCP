...
/** -----GLOBAL VARIABLES----- **/
i64 n, m; vector<vb> getAlong; vs names;
map<string, i64> Map; i64 mx = 0, mask = LINF;

/** -----EXTENSIVE FUNCTIONS----- **/
void Try(i64 z, i64 m) {
	if (z == n-1) {
		vi list; bool flag = true;
		for (i64 i=0; i<n; i++) {
			if (m & (1LL << i)) list.pub(i);
		}
		for (i64 i=0; i<list.size(); i++) {
			for (i64 j=i+1; j<list.size(); j++) {
				if (!getAlong[list[i]][list[j]]) {
					flag = false; break;
				}
			}
			if (!flag) break;
	...
	Try(z+1, m); Try(z+1, m+(1LL << (z+1)));
}

/** -----COMPULSORY FUNCTIONS----- **/
void VarInput() {
	//ios_base::sync_with_stdio(0); cin.tie(NULL);
	cin >> n >> m; names.resize(n);
	getAlong.resize(n, vb(n, true));
	for (i64 i=0; i<n; i++) {cin >> names[i]; Map[names[i]] = i;}
	while (m--) {
		string a, b; cin >> a >> b;
		getAlong[Map[a]][Map[b]] = false;
		getAlong[Map[b]][Map[a]] = false;
	}
}
