...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, u, v, w, st; vector<vector<pii>> adj; map<pii, i64> Map;
/************************************************/
...
void Solve() {
	vector<pii> Dist(n, mp(LINF, LINF)); vi Last(n, -1LL);
	vb vis(n, false); Dist[st] = mp(0LL, 0LL);
	priority_queue<pii> Q; Q.push(mp(-Dist[st].fi, st));
	while (!Q.empty()) {
		pii Z = Q.top(); Q.pop();
		i64 z = Z.se, d = -Z.fi;
		...
		for (auto T: adj[z]) {
			...
			if (tmp < Dist[t]) {
				Dist[t] = tmp; Last[t] = z;
				Q.push(mp(-Dist[t].fi, t));
			}
		}
	}
	i64 Total = 0; vi A;
	for (i64 i=0; i<n; i++) Total += Dist[i].se;
	for (i64 i=0; i<n; i++) if (Last[i] != -1LL) A.pub(Map[mp(i, Last[i])]);
	cout << Total << endl;
	for (auto x: A) cout << x << " ";
}
/*****************************************/
