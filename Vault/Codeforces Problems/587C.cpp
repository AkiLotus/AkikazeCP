...
/***********************************************
** File name: 587C.cpp                        **
** Time created: Thu Jan 3 2019 15:29pm       **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, q, u, v, a; vector<vi> adj;
vector<vi> Table; vi c, d, subtree;
vector<vi> CitizenLists; vector<vector<vi>> CitizenTable;
/************************************************/
...
void DFS(i64 z, i64 last) {
	...
	for (auto t: adj[z]) {
		if (t == last) continue;
		DFS(t, z); subtree[z] += subtree[t];
		Table[t].pub(z); CitizenTable[t].pub(vi(0));
		vi Tmp;
		for (i64 i=0; i<10; i++) {
			if (i >= CitizenLists[t].size()) break;
			Tmp.pub(CitizenLists[t][i]);
		}
		sort(Tmp.begin(), Tmp.end());
		for (i64 i=0; i<10; i++) {
			if (i >= Tmp.size()) break;
			CitizenTable[t][0].pub(Tmp[i]);
		}
	}
}
...
void Preprocess() {
	subtree.rsz(n, 1);
	CitizenTable.rsz(n); Table.rsz(n);
	d.rsz(n, 0); DFS(0, -1);
	for (i64 j=1; j<17; j++) {
		for (i64 i=0; i<n; i++) {
			...
			CitizenTable[i].pub(vi(0)); vi Tmp;
			for (i64 r=0; r<10; r++) {
				if (r >= CitizenTable[i][j-1].size()) break;
				Tmp.pub(CitizenTable[i][j-1][r]);
			}
			for (i64 r=0; r<10; r++) {
				if (r >= CitizenTable[Table[i][j-1]][j-1].size()) break;
				Tmp.pub(CitizenTable[Table[i][j-1]][j-1][r]);
			}
			sort(Tmp.begin(), Tmp.end());
			for (i64 r=0; r<10; r++) {
				if (r >= Tmp.size()) break;
				CitizenTable[i][j].pub(Tmp[r]);
			}
		}
	}
}

i64 LCA(i64 x, i64 y, vi &Citizen, i64 Limit) {
	if (x == y) {
		for (i64 r=0; r<Limit; r++) {
			if (r >= CitizenLists[x].size()) break;
			Citizen.pub(CitizenLists[x][r]);
		}
		sort(Citizen.begin(), Citizen.end());
		while (Citizen.size() > Limit) Citizen.pop_back();
		return x;
	}
	if (d[x] == d[y]) {
		...
		for (i64 r=0; r<Limit; r++) {
			if (r >= CitizenTable[x][id].size()) break;
			Citizen.pub(CitizenTable[x][id][r]);
		}
		for (i64 r=0; r<Limit; r++) {
			if (r >= CitizenTable[y][id].size()) break;
			Citizen.pub(CitizenTable[y][id][r]);
		}
		sort(Citizen.begin(), Citizen.end());
		while (Citizen.size() > Limit) Citizen.pop_back();
		return LCA(Table[x][id], Table[y][id], Citizen, Limit);
	}
	if (d[x] < d[y]) {
		i64 mul = 1, id = 0;
		while (d[x] < d[y] - mul * 2) {
		...
		sort(Citizen.begin(), Citizen.end());
		while (Citizen.size() > Limit) Citizen.pop_back();
		return LCA(x, Table[y][id], Citizen, Limit);
	}
	if (d[x] > d[y]) {
		i64 mul = 1, id = 0;
		while (d[y] < d[x] - mul * 2) {
		...
		sort(Citizen.begin(), Citizen.end());
		while (Citizen.size() > Limit) Citizen.pop_back();
		return LCA(Table[x][id], y, Citizen, Limit);
	}
}

void Input() {
	cin >> n >> m >> q; adj.rsz(n);
	CitizenLists.rsz(n); c.rsz(m);
	for (i64 i=1; i<n; i++) {
		cin >> u >> v; u--; v--;
	...
	for (i64 i=0; i<m; i++) {
		cin >> c[i]; c[i]--;
		CitizenLists[c[i]].pub(i+1);
	}
	for (i64 i=0; i<n; i++) sort(CitizenLists[i].begin(), CitizenLists[i].end());
}
...
void Solve() {
	...
	while (q--) {
		cin >> v >> u >> a; v--; u--;
		vi Citizen; i64 Anc = LCA(v, u, Citizen, a);
		cout << Citizen.size();
		for (auto r: Citizen) cout << " " << r; cout << endl;
	}
}
/*****************************************/
