...
i64 n, m; vector<vector<char>> A; vector<vb> vis;
/************************************************/
...
void DFS(i64 x, i64 y, i64 lx, i64 ly) {
	vis[x][y] = true;
	if (A[x-1][y] == A[x][y] && (lx != x-1 || ly != y)) {
		if (vis[x-1][y]) {cout << "Yes"; Exit();} DFS(x-1, y, x, y);
	}
	if (A[x+1][y] == A[x][y] && (lx != x+1 || ly != y)) {
		if (vis[x+1][y]) {cout << "Yes"; Exit();} DFS(x+1, y, x, y);
	}
	if (A[x][y-1] == A[x][y] && (lx != x || ly != y-1)) {
		if (vis[x][y-1]) {cout << "Yes"; Exit();} DFS(x, y-1, x, y);
	}
	if (A[x][y+1] == A[x][y] && (lx != x || ly != y+1)) {
		if (vis[x][y+1]) {cout << "Yes"; Exit();} DFS(x, y+1, x, y);
	}
...
void Input() {
	cin >> n >> m; A.rsz(n+2, vector<char>(m+2, '\0'));
	vis.rsz(n+2, vb(m+2, false));
	for (i64 i=1; i<=n; i++) {
		for (i64 j=1; j<=m; j++) {
			cin >> A[i][j];
		}
