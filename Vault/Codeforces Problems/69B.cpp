...
/************** [GLOBAL VARIABLES] **************/
i64 n, m; pip Default = mp(-LINF, mp(-LINF, -LINF)); vector<pip> Bet;
/************************************************/
...
void Input() {
	cin >> n >> m; Bet.rsz(n, Default);
	for (i64 i=0; i<m; i++) {
		i64 l, r, t, c; cin >> l >> r >> t >> c; l--; r--;
		for (i64 x=l; x<=r; x++) Bet[x] = max(Bet[x], mp(-t, mp(-i, c)));
	}
}
