...
/************** [GLOBAL VARIABLES] **************/
typedef pair<pii, pair<char, i64>> ppci;
i64 n, m, xs, ys, xt, yt; vector<vc> a;
/************************************************/
...
void Input() {
	...
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<m; j++) {
			...
			if (a[i][j] == 'T') {xt = i; yt = j;}
		}
	}
}

void Solve() {
	vector<vector<vector<vb>>> vis(n, vector<vector<vb>>(m, vector<vb>(4, vb(3, false))));
	queue<ppci> Q;
	Q.push(mp(mp(xs, ys), mp('U', 0LL))); vis[xs][ys][0][0] = true;
	Q.push(mp(mp(xs, ys), mp('D', 0LL))); vis[xs][ys][1][0] = true;
	Q.push(mp(mp(xs, ys), mp('R', 0LL))); vis[xs][ys][2][0] = true;
	Q.push(mp(mp(xs, ys), mp('L', 0LL))); vis[xs][ys][3][0] = true;
	while (!Q.empty()) {
		ppci Z = Q.front(); Q.pop();
		i64 x = Z.fi.fi, y = Z.fi.se;
		char dir = Z.se.fi; i64 cnt = Z.se.se;
		if (mp(x, y) == mp(xt, yt)) {cout << "YES"; return;}
		if (x-1 >= 0 && a[x-1][y] != '*' && dir == 'U' && !vis[x-1][y][0][cnt]) {
			Q.push(mp(mp(x-1, y), mp('U', cnt))); vis[x-1][y][0][cnt] = true;
		}
		if (x-1 >= 0 && a[x-1][y] != '*' && dir != 'U' && cnt < 2 && !vis[x-1][y][0][cnt+1]) {
			Q.push(mp(mp(x-1, y), mp('U', cnt+1))); vis[x-1][y][0][cnt+1] = true;
		}
		if (x+1 < n && a[x+1][y] != '*' && dir == 'D' && !vis[x+1][y][1][cnt]) {
			Q.push(mp(mp(x+1, y), mp('D', cnt))); vis[x+1][y][1][cnt] = true;
		}
		if (x+1 < n && a[x+1][y] != '*' && dir != 'D' && cnt < 2 && !vis[x+1][y][1][cnt+1]) {
			Q.push(mp(mp(x+1, y), mp('D', cnt+1))); vis[x+1][y][1][cnt+1] = true;
		}
		if (y-1 >= 0 && a[x][y-1] != '*' && dir == 'L' && !vis[x][y-1][3][cnt]) {
			Q.push(mp(mp(x, y-1), mp('L', cnt))); vis[x][y-1][3][cnt] = true;
		}
		if (y-1 >= 0 && a[x][y-1] != '*' && dir != 'L' && cnt < 2 && !vis[x][y-1][3][cnt+1]) {
			Q.push(mp(mp(x, y-1), mp('L', cnt+1))); vis[x][y-1][3][cnt+1] = true;
		}
		if (y+1 < m && a[x][y+1] != '*' && dir == 'R' && !vis[x][y+1][2][cnt]) {
			Q.push(mp(mp(x, y+1), mp('R', cnt))); vis[x][y+1][2][cnt] = true;
		}
		if (y+1 < m && a[x][y+1] != '*' && dir != 'R' && cnt < 2 && !vis[x][y+1][2][cnt+1]) {
			Q.push(mp(mp(x, y+1), mp('R', cnt+1))); vis[x][y+1][2][cnt+1] = true;
		}
	}
	cout << "NO";
}
