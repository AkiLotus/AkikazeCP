...
void ProSolve() {
	while (T--) {
		cin >> N >> C >> D; if (C > D) swap(C, D);
		for (i64 i=0; i<N; i++) cin >> a[i];
		sort(a, a+N, greater<i64>()); i64 tmp1 = 0, tmp2 = 0;
		for (i64 i=0; i<C; i++) tmp1 += a[i]; for (i64 i=C; i<C+D; i++) tmp2 += a[i];
		ld ans = 0; if (C > 0) ans += (ld)tmp1 / C; if (D > 0) ans += (ld)tmp2 / D;
