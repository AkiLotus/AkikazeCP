...
void Solve() {
	i64 numer = 0, denom = n * 2;
	for (auto x: a) numer += 2 * x; sort(a.begin(), a.end());
	i64 leftcnt = 0, rightcnt = n - 1, left = 0, right = numer - a[0] * 2;
	for (i64 i=0; i<n; i++) {
		numer += (a[i] * 2 * leftcnt - left);
		numer += (right - a[i] * 2 * rightcnt);
		leftcnt++; rightcnt--; left += a[i] * 2;
		if (i < n - 1) right -= a[i+1] * 2;
	}
