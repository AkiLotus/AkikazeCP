...
void ProSolve() {
	while (n--) {
		i64 x; cin >> x;
		if (x > 0) s1.pub(x);
		else if (x == 0) s3.pub(x);
		else if (s2.empty()) s2.pub(x);
		else s3.pub(x);
	}
	...
	if (s1.empty()) {
		s1.pub(s3[s3.size()-1]);
		s1.pub(s3[s3.size()-2]);
		s3.erase(s3.end()-1); s3.erase(s3.end()-1);
	}
	cout << s2.size(); for (i64 i=0; i<s2.size(); i++) cout << " " << s2[i]; cout << endl;
	cout << s1.size(); for (i64 i=0; i<s1.size(); i++) cout << " " << s1[i]; cout << endl;
	cout << s3.size(); for (i64 i=0; i<s3.size(); i++) cout << " " << s3[i]; cout << endl;
}
