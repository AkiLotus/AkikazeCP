...
/***********************************************
** File name: 56B.cpp                         **
** Time created: Tue Dec 25 2018 12:47pm      **
***********************************************/
...
void Solve() {
	...
	for (i64 i=0; i<n; i++) {
		if (a[i] == i+1) continue;
		Pair = mp(min(i+1, a[i]), max(i+1, a[i])); break;
	}
	if (Pair == mp(-1LL, -1LL)) {cout << "0 0\n"; return;}
	vi A(n); for (i64 i=0; i<n; i++) A[i] = i + 1;
	reverse(A.begin()+Pair.fi-1, A.begin()+Pair.se);
	if (A == a) cout << Pair.fi << " " << Pair.se << endl;
	else cout << "0 0\n";
}
/*****************************************/
