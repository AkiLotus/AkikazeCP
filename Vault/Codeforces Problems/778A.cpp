...
void VarInput() {
	...
	cin >> t >> p; N = t.size(); M = p.size(); a.resize(N);
	for (i64 i=0; i<N; i++) cin >> a[i];
}

void ProSolve() {
	i64 ans = 0, top = 0, bot = N;
	while (top <= bot) {
		i64 x = top, y = bot;
		i64 mid = (top + bot) / 2;
		bool flag = true;
		string New = ""; vi vis(N, false);
		for (i64 i=0; i<mid; i++) vis[a[i]-1] = true;
		for (i64 i=0; i<N; i++) if (!vis[i]) New += t[i];
		vector<vi> pos(26, vi(0));
		for (i64 i=0; i<New.size(); i++) {
			pos[New[i]-'a'].pub(i);
		}
		i64 last = -1;
		for (i64 i=0; i<M; i++) {
			if (upper_bound(pos[p[i]-'a'].begin(), pos[p[i]-'a'].end(), last) == pos[p[i]-'a'].end()) {
				flag = false; break;
			}
			last = *upper_bound(pos[p[i]-'a'].begin(), pos[p[i]-'a'].end(), last);
		}
		if (flag) {ans = max(ans, mid); top = mid+1;}
		else bot = mid;
		if (x == y) break;
	}
	cout << ans;
