...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, s, f, t, l, r; map<i64, pii> Map;
/************************************************/
...
void Solve() {
	...
	while (s != f) {
		if (Map.find(turn) == Map.end()) {
			if (s < f) {cout << "R"; s++;}
			else {cout << "L"; s--;} turn++; continue;
		}
		if (Map[turn].fi <= s && s <= Map[turn].se || ((s > f && Map[turn].fi <= s-1 && s-1 <= Map[turn].se) || (s < f && Map[turn].fi <= s+1 && s+1 <= Map[turn].se))) cout << "X";
		else if (s < f) {cout << "R"; s++;} else {cout << "L"; s--;} turn++;
	}
}
/*****************************************/
