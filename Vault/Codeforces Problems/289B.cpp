...
void VarInput() {
	...
	cin >> n >> m >> d; a.resize(n*m, 0); s = a;
	for (i64 i=0; i<n*m; i++) {
		cin >> a[i]; M[a[i]]++;
	}
	sort(a.begin(), a.end()); s[0] = a[0];
	for (i64 i=1; i<n*m; i++) s[i] = s[i-1] + a[i];
	//for (i64 i=0; i<n*m; i++) cout << s[i] << " "; cout << endl;
}
...
void ProSolve() {
	...
	for (i64 i=0; i<n*m; i++) {
		...
		//tracker1(i);
		i64 l = i-1, r = upper_bound(a.begin(), a.end(), a[i]) - a.begin();
		//cout << "l,r = " << l << "," << r << endl;
		i64 tmp1 = 0; if (r < n*m) tmp1 = (s[n*m-1] - s[r-1]) - a[i] * (n*m-r);
		i64 tmp2 = 0; if (l >= 0) tmp2 = a[i] * (l+1) - s[l];
		//cout << "tmp1 = " << tmp1 << endl;
		//cout << "tmp2 = " << tmp2 << endl;
