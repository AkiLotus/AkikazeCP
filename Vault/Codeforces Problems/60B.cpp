...
/************** [GLOBAL VARIABLES] **************/
i64 k, n, m, x, y, z, ans = 0; vector<vector<vc>> A;
queue<pip> Q; vector<vector<vb>> vis;
/************************************************/
...
void Input() {
	cin >> k >> n >> m; A.rsz(k, vector<vc>(n, vc(m, '.')));
	vis.rsz(k, vector<vb>(n, vb(m, false)));
	for (i64 x=0; x<k; x++) {
		for (i64 y=0; y<n; y++) {
			for (i64 z=0; z<m; z++) {
				cin >> A[x][y][z];
	...
	cin >> x >> y; x--; y--;
	Q.push(mp(0LL, mp(x, y))); vis[0][x][y] = true;
}

void Solve() {
	while (!Q.empty()) {
		pip Z = Q.front(); Q.pop();
		x = Z.fi; y = Z.se.fi; z = Z.se.se;
		if (x-1 >= 0 && !vis[x-1][y][z] && A[x-1][y][z] != '#') {
			vis[x-1][y][z] = true; Q.push(mp(x-1, mp(y, z)));
		}
		if (x+1 < k && !vis[x+1][y][z] && A[x+1][y][z] != '#') {
			vis[x+1][y][z] = true; Q.push(mp(x+1, mp(y, z)));
		}
		if (y-1 >= 0 && !vis[x][y-1][z] && A[x][y-1][z] != '#') {
			vis[x][y-1][z] = true; Q.push(mp(x, mp(y-1, z)));
		}
		if (y+1 < n && !vis[x][y+1][z] && A[x][y+1][z] != '#') {
			vis[x][y+1][z] = true; Q.push(mp(x, mp(y+1, z)));
		}
		if (z-1 >= 0 && !vis[x][y][z-1] && A[x][y][z-1] != '#') {
			vis[x][y][z-1] = true; Q.push(mp(x, mp(y, z-1)));
		}
		if (z+1 < m && !vis[x][y][z+1] && A[x][y][z+1] != '#') {
			vis[x][y][z+1] = true; Q.push(mp(x, mp(y, z+1)));
		}
	}
	for (i64 x=0; x<k; x++) {
