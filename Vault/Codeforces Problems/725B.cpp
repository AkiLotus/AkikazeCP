...
void ProSolve() {
	i64 n = intParse(seats.substr(0, seats.size()-1)) - 1;
	char position = seats[seats.size()-1];
	i64 segment = n / 4; ans += segment * 16;
	if ((n % 4) % 2 != 0) ans += 7;
	if (position == 'f') ans++;
	else if (position == 'e') ans += 2;
	else if (position == 'd') ans += 3;
	else if (position == 'a') ans += 4;
	else if (position == 'b') ans += 5;
	else if (position == 'c') ans += 6;
	cout << ans;
}
