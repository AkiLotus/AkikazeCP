...
void Solve() {
	vector<vi> dp(n, vi(3, LINF));
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<m; j++) {
			if (a[i][j] >= '0' && a[i][j] <= '9') dp[i][0] = min(dp[i][0], min(j, m-j));
			else if (a[i][j] >= 'a' && a[i][j] <= 'z') dp[i][1] = min(dp[i][1], min(j, m-j));
			else dp[i][2] = min(dp[i][2], min(j, m-j));
		}
	}
	i64 ans = LINF;
	for (i64 i=0; i<n; i++) {
