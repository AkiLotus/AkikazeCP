...
void ProSolve() {
	...
	for (i64 i=0; i<n; i++) {
		string ope; i64 x; cin >> ope;
		if (ope == "insert") {
			cin >> x; m++;
			cmd.pub(mp("insert", x));
			heap.insert(x);
		}
		else if (ope == "removeMin") {
			if (heap.empty()) {
				cmd.pub(mp("insert", 0LL));
				heap.insert(0); m++;
			}
			cmd.pub(mp("removeMin", -1LL));
			heap.erase(heap.begin()); m++;
		}
		else if (ope == "getMin") {
			cin >> x;
			while (!heap.empty() && *heap.begin() < x) {
				cmd.pub(mp("removeMin", -1LL));
				heap.erase(heap.begin()); m++;
			}
			if (heap.empty() || *heap.begin() > x) {
				cmd.pub(mp("insert", x));
				heap.insert(x); m++;
			}
			cmd.pub(mp("getMin", x)); m++;
		}
	}
	cout << m << endl;
	for (i64 i=0; i<m; i++) {
		if (cmd[i].fi == "removeMin") cout << cmd[i].fi << endl;
		else cout << cmd[i].fi << " " << cmd[i].se << endl;
	}
}
