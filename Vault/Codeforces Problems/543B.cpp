...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, a, b, s1, t1, l1, s2, t2, l2; vector<vi> adj, d;
/************************************************/
...
void Input() {
	cin >> n >> m; adj.rsz(n); d.rsz(n, vi(n, LINF));
	for (i64 i=0; i<m; i++) {
		cin >> a >> b; a--; b--;
...
void Solve() {
	for (i64 i=0; i<n; i++) {
		d[i][i] = 0; queue<i64> Q;
		vb vis(n, false); vis[i] = true; Q.push(i);
		while (!Q.empty()) {
			i64 z = Q.front(); Q.pop();
			for (auto t: adj[z]) {
				if (vis[t]) continue;
				vis[t] = true; d[i][t] = d[i][z] + 1;
				Q.push(t);
			}
		}
	}
	if (d[s1][t1] > l1 || d[s2][t2] > l2) {cout << "-1"; return;}
	i64 ans = m - (d[s1][t1] + d[s2][t2]);
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<n; j++) {
			if (d[s1][i] + d[i][j] + d[j][t1] <= l1 && d[s2][i] + d[i][j] + d[j][t2] <= l2) ans = max(ans, m - (d[s1][i] + d[s2][i] + d[i][j] + d[j][t1] + d[j][t2]));
			if (d[s1][i] + d[i][j] + d[j][t1] <= l1 && d[s2][j] + d[j][i] + d[i][t2] <= l2) ans = max(ans, m - (d[s1][i] + d[s2][j] + d[i][j] + d[j][t1] + d[i][t2]));
		}
	}
	cout << ans;
}
