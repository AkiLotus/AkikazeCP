...
/************** [GLOBAL VARIABLES] **************/
typedef pair<double, int> dwarf;
int n, t1, t2, k, a, b; vector<dwarf> A;
/************************************************/
...
double calc(int x, int y) {
	i64 numer = 0, denom = 1;
	numer += denom * x * t1;
	numer *= (100 - k); denom *= 100;
	numer += denom * y * t2;
	return ((double)numer / denom);
}

bool dwarfsort(dwarf x, dwarf y) {
	if (x.fi != y.fi) return (x.fi > y.fi);
	return (x.se < y.se);
}

void Input() {
	cin >> n >> t1 >> t2 >> k; A.rsz(n);
	for (int i=0; i<n; i++) {
		cin >> a >> b;
		A[i] = mp(max(calc(a, b), calc(b, a)), i + 1);
	}
}

void Solve() {
	cout << fixed << setprecision(2);
	sort(A.begin(), A.end(), dwarfsort);
	for (auto z: A) cout << z.se << " " << z.fi << endl;
}
/*****************************************/
