...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi a; vector<vi> List(30), cnt(30, vi(30, 0));
/************************************************/

/************** [FUNCTIONS] **************/
bool validSubset(i64 id) {
	i64 N = List[id].size();
	for (i64 i=id-1; i>=0; i--) {
		if (cnt[id][i] == N) return false;
	}
	return true;
}
...
void Solve() {
	i64 Max = 0;
	for (i64 i=1; i<30; i++) {
		if (validSubset(i)) Max = i;
	}
	cout << List[Max].size() << endl;
	for (auto x: List[Max]) cout << x << " ";
}
/*****************************************/
