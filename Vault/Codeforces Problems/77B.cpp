...
/***********************************************
** File name: 77B.cpp                         **
** Time created: Sat Jan 5 2019 11:06am       **
***********************************************/
...
void Solve() {
	if (b == 0) {cout << fixed << setprecision(12) << 1.0L << endl; return;}
	if (a == 0) {cout << fixed << setprecision(12) << 0.5L << endl; return;}
	ld prob = 0.5L;
	i64 HalfRect = a * b;
	if (4 * b <= a) {
		// trapezoid probability
		ld TrapezoidArea = (ld)(b * (a + (a - b * 4))) / 2.0L;
		prob += 0.5L * TrapezoidArea / HalfRect;
	}
	else {
		// triangle probability
		ld TriangleArea = ((ld)a * a / 4.0L) / 2.0L;
		prob += 0.5L * TriangleArea / HalfRect;
	}
