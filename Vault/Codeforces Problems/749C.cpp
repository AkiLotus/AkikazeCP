...
void ProSolve() {
	i64 deniedD = 0, deniedR = 0;
	i64 remainingD = 0, remainingR = 0;
	for (i64 i=0; i<n; i++) {
		if (s[i] == 'D') remainingD++;
		else remainingR++;
	}
	i64 i = 0; string tmp = "";
	while (remainingD && remainingR) {
		if (s[i] == 'D') {
			if (deniedD) deniedD--;
			else {deniedR++; remainingR--; tmp += s[i];}
		}
		else {
			if (deniedR) deniedR--;
			else {deniedD++; remainingD--; tmp += s[i];}
		}
		i = (i + 1) % n;
		if (i == 0) {s = tmp; n = s.size(); tmp = "";}
	}
	if (remainingD < remainingR) cout << "R"; else cout << "D";
}
