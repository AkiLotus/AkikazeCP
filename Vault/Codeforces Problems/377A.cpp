...
i64 n, m, k, Rem = 0; vector<vector<char>> a; vector<vb> vis;
/************************************************/
...
void DFS(i64 x, i64 y) {
	vis[x][y] = true; a[x][y] = '.'; Rem--;
	if (Rem == k) return;
	if (x-1 > 0 && !vis[x-1][y] && a[x-1][y] == 'X') DFS(x-1, y);
	if (Rem == k) return;
	if (x+1 <= n && !vis[x+1][y] && a[x+1][y] == 'X') DFS(x+1, y);
	if (Rem == k) return;
	if (y-1 > 0 && !vis[x][y-1] && a[x][y-1] == 'X') DFS(x, y-1);
	if (Rem == k) return;
	if (y+1 <= m && !vis[x][y+1] && a[x][y+1] == 'X') DFS(x, y+1);
	if (Rem == k) return;
}

void Input() {
	cin >> n >> m >> k; vis.rsz(n+2, vb(m+2, false));
	a.rsz(n+2, vector<char>(m+2, '#'));
	for (i64 i=1; i<=n; i++) {
		for (i64 j=1; j<=m; j++) {
			cin >> a[i][j];
			if (a[i][j] == '.') {
