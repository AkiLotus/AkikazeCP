...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi a, A; vector<pii> List;
map<i64, i64> Map; vb isPrime;
/************************************************/
...
void ProcessSwap(i64 id1, i64 id2) {
	List.pub(mp(id1, id2));
	id1--; id2--; swap(A[id1], A[id2]);
	Map[A[id1]] = id1 + 1; Map[A[id2]] = id2 + 1;
}
...
void Solve() {
	sort(a.begin(), a.end()); SieveProcess();
	for (i64 i=0; i<n; i++) {
		i64 eid = i + 1, id = Map[a[i]];
		if (id == eid) continue;
		if ((id - eid + 1) % 2 == 0) {ProcessSwap(id-1, id); id--;}
		if (id == eid) continue;
		while (!isPrime[id-eid+1]) {
			ProcessSwap(id-2, id);
			id -= 2;
		}
		ProcessSwap(eid, id);
	...
	cout << List.size() << endl;
	for (auto x: List) cout << x.fi << " " << x.se << endl;
}
/*****************************************/
