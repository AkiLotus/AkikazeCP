...
/************** [GLOBAL VARIABLES] **************/
i64 h, w, n; vector<pii> A; vi ftrl(444444, 1);
/************************************************/

/************** [FUNCTIONS] **************/
bool diagonalsort(pii x, pii y) {
	if (x.fi + x.se < y.fi + y.se) return true;
...
void Input() {
	for (i64 i=1; i<444444; i++) ftrl[i] = (ftrl[i-1] * i) % Mod;
	cin >> h >> w >> n; A.rsz(n);
	for (i64 i=0; i<n; i++) {cin >> A[i].fi >> A[i].se;}
}

void Solve() {
	sort(A.begin(), A.end(), diagonalsort);
	i64 ans = calc(1LL, 1LL, h, w); vi Dec(n, 0LL);
	for (i64 i=0; i<n; i++) {
		i64 x = A[i].fi, y = A[i].se;
		i64 decreasecnt = calc(1LL, 1LL, x, y);
		for (i64 j=0; j<i; j++) {
			i64 a = A[j].fi, b = A[j].se;
			decreasecnt -= Dec[j] * calc(a, b, x, y);
			decreasecnt %= Mod;
			while (decreasecnt < 0) decreasecnt += Mod;
		}
		Dec[i] = decreasecnt;
		ans -= decreasecnt * calc(x, y, h, w);
