...
void VarInput() {
	...
	cin >> n; x.rsz(n);
	for (i64 i=0; i<n; i++) {cin >> x[i]; S.insert(x[i]);}
}

void ProSolve() {
	i64 ans = 0;
	while (!S.empty()) {
		ans++; i64 topping = 4, last = 1;
		auto it = S.begin(); topping = *it; S.erase(it);
		while (!S.empty() && last >= 0) {
			if (last <= topping) it = S.lower_bound(topping);
			else it = S.upper_bound(topping);
			if (it == S.end()) it = S.lower_bound(topping);
			if (it == S.end()) break;
			if (last > *it) break;
			topping = *it; S.erase(it); last++;
		}
	}
	cout << ans;
}
