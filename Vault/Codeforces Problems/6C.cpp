...
void ProSolve() {
	vi a = t; i64 ptr1 = 0, ptr2 = n-1, s1 = 0, s2 = 0;
	while (ptr1 <= ptr2) {
		if (ptr1 == ptr2) {s1++; ptr1++; continue;}
		if (a[ptr1] == 0) {s2++; break;}
		if (a[ptr2] == 0) {s1++; break;}
		if (a[ptr1] < a[ptr2]) {
			a[ptr2] -= a[ptr1]; s1++; a[ptr1] = 0;
			if (a[ptr2] == 0) {ptr1++; ptr2--; s2++;}
			else if (ptr1 + 1 != ptr2) ptr1++;
		}
		else {
			a[ptr1] -= a[ptr2]; s2++; a[ptr2] = 0;
			if (a[ptr1] == 0) {ptr1++; ptr2--; s1++;}
			else if (ptr2 - 1 != ptr1) ptr2--;
		}
	}
	cout << s1 << " " << s2;
}
