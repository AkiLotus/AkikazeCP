...
/************** [GLOBAL VARIABLES] **************/
i64 n, k, Min = 0; priority_queue<pip, vector<pip>, greater<pip>> Heap;
vector<vc> A; vector<vi> cc;
/************************************************/
...
void Solve() {
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<12; j++) {
			if (A[i][j] == 'S') {
				i64 cnt = 0;
				if (j > 0 && (A[i][j-1] == 'P' || A[i][j-1] == 'S')) cnt++;
				if (j < 11 && (A[i][j+1] == 'P' || A[i][j+1] == 'S')) cnt++;
				Min += cnt;
			}
			if (A[i][j] == '.') {
				i64 cnt = 0;
				if (j > 0 && (A[i][j-1] == 'S')) cnt++;
				if (j < 11 && (A[i][j+1] == 'S')) cnt++;
				Heap.push(mp(cnt, mp(i, j)));
			}
		}
	}
	while (k--) {
		pip Q = Heap.top(); Heap.pop();
		Min += Q.fi; i64 x = Q.se.fi, y = Q.se.se;
		A[x][y] = 'x';
	}
	cout << Min << endl;
	for (i64 i=0; i<n; i++) {
