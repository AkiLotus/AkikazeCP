...
void ProSolve() {
	i64 x1 = 0, y1 = 0, mx1 = Map1[0] + Map2[0] - mat[0][0];
	for (i64 sum=0; sum<=2*n-2; sum+=2) {
		i64 lowestEvenDiff = -(n - 1);
		if (lowestEvenDiff % 2 != 0) lowestEvenDiff++;
		for (i64 diff=lowestEvenDiff; diff<=-lowestEvenDiff; diff+=2) {
			i64 coorx = (sum + diff) / 2, coory = (sum - diff) / 2;
			if (coorx < 0 || coorx >= n || coory < 0 || coory >= n) continue;
			if (Map1[sum] + Map2[diff] - mat[coorx][coory] > mx1) {
				mx1 = Map1[sum] + Map2[diff] - mat[coorx][coory];
				x1 = coorx; y1 = coory;
			}
		}
	}
	i64 x2 = 0, y2 = 1, mx2 = Map1[1] + Map2[-1] - mat[0][1];
	for (i64 sum=1; sum<=2*n-2; sum+=2) {
		i64 lowestOddDiff = -(n - 1);
		if (lowestOddDiff % 2 == 0) lowestOddDiff++;
		for (i64 diff=lowestOddDiff; diff<=-lowestOddDiff; diff+=2) {
			i64 coorx = (sum + diff) / 2, coory = (sum - diff) / 2;
			if (coorx < 0 || coorx >= n || coory < 0 || coory >= n) continue;
			if (Map1[sum] + Map2[diff] - mat[coorx][coory] > mx2) {
				mx2 = Map1[sum] + Map2[diff] - mat[coorx][coory];
				x2 = coorx; y2 = coory;
			}
		}
	}
