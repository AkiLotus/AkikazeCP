...
/************** [GLOBAL VARIABLES] **************/
i64 n, m; vi a, cnt; string s; map<string, int> trie;
/************************************************/
...
void Input() {
	...
	for (i64 i=0; i<m; i++) {cin >> s; trie[s]++;}
}

void Solve() {
	for (auto it=trie.begin(); it!=trie.end(); it++) cnt.pub(it->se);
	sort(a.begin(), a.end()); sort(cnt.rbegin(), cnt.rend());
	i64 Min = 0, Max = 0;
	for (i64 i=0; i<cnt.size(); i++) {
		Min += cnt[i] * a[i];
		Max += cnt[i] * a[n-1-i];
	}
	cout << Min << " " << Max;
