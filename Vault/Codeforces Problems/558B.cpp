...
i64 n; vi a; map<i64, i64> Map, Min, Max;
/************************************************/
...
void Input() {
	...
	for (i64 i=0; i<n; i++) {
		cin >> a[i]; Map[a[i]]++;
		if (!Min[a[i]]) Min[a[i]] = i+1;
		Max[a[i]] = i + 1;
	}
}

void Solve() {
	i64 maxApp = 0, len = -LINF, L = -LINF, R = -1;
	for (auto it=Map.begin(); it!=Map.end(); it++) {
		if (maxApp > it->se) continue;
		if (maxApp < it->se) {
			maxApp = it->se; len = Max[it->fi] - Min[it->fi] + 1;
			L = Min[it->fi]; R = Max[it->fi]; continue;
		}
		if (Max[it->fi] - Min[it->fi] + 1 < len) {
			len = Max[it->fi] - Min[it->fi] + 1;
			L = Min[it->fi]; R = Max[it->fi];
		}
	}
	cout << L << " " << R;
}
