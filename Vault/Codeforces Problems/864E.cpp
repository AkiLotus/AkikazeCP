...
void Solve() {
	sort(A.begin(), A.end()); pii Default = mp(-1LL, -1LL);
	vector<vi> dp(n, vi(4000, 0)); vector<vector<pii>> Last(n, vector<pii>(4000, Default));
	for (i64 i=A[0].se.fi; i<A[0].fi.fi; i++) dp[0][i] = A[0].se.se;
	for (i64 i=1; i<n; i++) {
		for (i64 j=0; j<4000; j++) {
			dp[i][j] = dp[i-1][j]; Last[i][j] = mp(i-1, j);
			if (j < A[i].se.fi) continue;
			if (j >= A[i].fi.fi) continue;
			if (dp[i][j] < dp[i-1][j-A[i].se.fi] + A[i].se.se) {
				dp[i][j] = dp[i-1][j-A[i].se.fi] + A[i].se.se;
				Last[i][j] = mp(i-1, j-A[i].se.fi);
			}
		}
	}
	i64 Time = max_element(dp[n-1].begin(), dp[n-1].end()) - dp[n-1].begin();
	i64 id = n-1, sum = dp[id][Time]; vi List; cout << sum << endl;
	while (true) {
		pii Token = Last[id][Time];
		i64 newid = Token.fi, newTime = Token.se;
		if (Token == Default) {if (sum > 0) List.pub(A[id].fi.se); break;}
		i64 newsum = dp[newid][newTime];
		if (sum > newsum) List.pub(A[id].fi.se);
		sum = newsum; id = newid; Time = newTime;
	}
