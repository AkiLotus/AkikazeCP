...
/** -----GLOBAL VARIABLES----- **/
i64 n; vector<vi> d, mstd, dist, MSTAdj;
vi par, DSUsize; vb vis;

/** -----EXTENSIVE FUNCTIONS----- **/
i64 DSUfind(i64 z) {
	while (par[z] != -1) z = par[z];
	return z;
}

void DSUjoin(i64 x, i64 y) {
	i64 a = DSUfind(x), b = DSUfind(y);
	if (a == b) return;
	if (DSUsize[a] < DSUsize[b]) swap(a, b);
	par[b] = a; DSUsize[a] += DSUsize[b];
}

void DFS(i64 z, i64 src, i64 Dist) {
	vis[z] = true;
	if (dist[src][z] == 0) {dist[src][z] = Dist; dist[z][src] = Dist;}
	else {
		dist[src][z] = min(Dist, dist[src][z]);
		dist[z][src] = min(Dist, dist[z][src]);
	}
	for (auto i: MSTAdj[z]) {
		if (vis[i]) continue;
		DFS(i, src, Dist+mstd[z][i]);
	}
}
...
void VarInput() {
	...
	cin >> n; d.rsz(n, vi(n, 0)); mstd = d; dist = d;
	par.rsz(n, -1); DSUsize.rsz(n, 1); MSTAdj.rsz(n);
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<n; j++) {
			cin >> d[i][j];
		}
...
void ProSolve() {
	...
	for (i64 i=0; i<n; i++) {
		for (i64 j=i; j<n; j++) {
			if (i == j) {
				if (d[i][j] != 0) {cout << "NO"; return;}
				continue;
			}
			if (d[i][j] != d[j][i]) {cout << "NO"; return;}
			if (d[i][j] == 0) continue;
	...
	while (!Q.empty()) {
		pip z = Q.top(); Q.pop();
		i64 x = z.se.fi, y = z.se.se, t = -z.fi;
		if (DSUfind(x) == DSUfind(y)) continue;
		mstd[x][y] = t; mstd[y][x] = t; DSUjoin(x, y);
		edgecnt++; MSTAdj[x].pub(y); MSTAdj[y].pub(x);
	}
	if (edgecnt != n-1) {cout << "NO"; return;}
	for (i64 i=0; i<n; i++) {
		vis.clear();
		vis.rsz(n, false);
		DFS(i, i, 0LL);
	}
	if (dist == d) cout << "YES"; else cout << "NO";
}
