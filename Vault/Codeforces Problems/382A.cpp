...
void ProSolve() {
	i64 n1 = s1.size(), n2 = s2.size();
	i64 bound = s1.find('|');
	i64 L = bound, R = n1 - 1 - bound;
	if (abs(R-L) > n2) cout << "Impossible";
	else if ((L + R + n2) % 2 != 0) cout << "Impossible";
	else {
		i64 balance = abs(R - L);
		if (L < R) s1.insert(0, s2.substr(0, balance));
		else s1.insert(s1.size(), s2.substr(0, balance));
		s1.insert(0, s2.substr(balance, (n2-balance)/2));
		s1.insert(s1.size(), s2.substr(balance+(n2-balance)/2, (n2-balance)/2));
		cout << s1;
	}
}
