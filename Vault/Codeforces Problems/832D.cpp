...
/************** [GLOBAL VARIABLES] **************/
i64 n, q, a, b, c; vector<vi> adj;
vi d; vector<vi> Table;
/************************************************/
...
i64 calculate(i64 s, i64 f, i64 t) {
	i64 sf = LCA(s, f), tf = LCA(t, f), st = LCA(s, t);
	if (sf == tf) return ((d[st] - d[sf]) + (d[f] - d[sf]) + 1);
	return (d[f] - max(d[sf], d[tf]) + 1);
}
...
void Solve() {
	...
	while (q--) {
		cin >> a >> b >> c; a--; b--; c--;
		i64 p1 = calculate(a, b, c), p2 = calculate(a, c, b);
		i64 p3 = calculate(b, a, c), p4 = calculate(b, c, a);
		i64 p5 = calculate(c, a, b), p6 = calculate(c, b, a);
		cout << max(max(max(p1, p2), max(p3, p4)), max(p5, p6)) << endl;
	}
}
/*****************************************/
