...
void VarInput() {
	...
	cin >> d >> s; a.rsz(d); schedule.rsz(d);
	for (i64 i=0; i<d; i++) {
		cin >> a[i].fi >> a[i].se;
		curr += a[i].fienum name
		{
...
void ProSolve() {
	if (curr > s) {cout << "NO"; return;}
	for (i64 i=0; i<d; i++) {
		if (curr - a[i].fi + a[i].se >= s) {
			schedule[i] += (s - curr);
			cout << "YES\n";
			for (i64 x=0; x<d; x++) cout << schedule[x] << " ";
			return;
		}
		else {
			curr -= a[i].fi; curr += a[i].se;
			schedule[i] = a[i].se;
		}
	}
	cout << "NO"; return;
