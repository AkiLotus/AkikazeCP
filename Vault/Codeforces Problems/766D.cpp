...
void DSUmerge(i64 id1, i64 id2) {
	i64 z1 = DSUfind(id1), z2 = DSUfind(id2);
	if (z1 == z2) return;
	if (DSUsize[z1] < DSUsize[z2]) swap(z1, z2);
	DSUsize[z1] += DSUsize[z2]; DSUpar[z2] = z1;
...
void Input() {
	...
	while (m--) {
		...
		//tracker1(Antonym);
		cin >> t >> x >> y;
		i64 rootx = DSUfind(id[x]), rooty = DSUfind(id[y]);
		i64 antx = DSUfind(Antonym[rootx]), anty = DSUfind(Antonym[rooty]);
		if (t == 1) {
			if (Antonym[rootx] == rooty) {cout << "NO\n"; continue;}
			if (antx != -1 && anty != -1 && Antonym[antx] == anty) {cout << "NO\n"; continue;}
			DSUmerge(rootx, rooty);
			if (antx != -1 && anty != -1) DSUmerge(antx, anty);
			cout << "YES\n"; continue;
		}
		if (rootx == rooty || (antx != -1 && antx == anty)) {cout << "NO\n"; continue;}
		if (anty != -1) DSUmerge(rootx, anty); if (antx != -1) DSUmerge(rooty, antx);
		rootx = DSUfind(rootx); rooty = DSUfind(rooty);
