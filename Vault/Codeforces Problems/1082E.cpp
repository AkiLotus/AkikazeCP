...
/***********************************************
** File name: 1082E.cpp                       **
** Time created: Sun Dec 2 2018 09:55am       **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, c, curr = 0; vi a, ps;
vector<vi> List(500000);
/************************************************/
...
void Solve() {
	...
	for (i64 i=0; i<500000; i++) {
		...
		if (List[i].size() == 1) {maxDelta = max(maxDelta, (i64)(i != c)); continue;}
		i64 Min = 0;
		for (i64 x=0; x<List[i].size(); x++) {
			if (List[i][x] > 0) Min = min(Min, x - ps[List[i][x]-1]);
			i64 tmp = x + 1 - ps[List[i][x]];
			maxDelta = max(maxDelta, tmp - Min);
			Min = min(Min, tmp);
		}
	}
	cout << curr + maxDelta << endl;
}
/*****************************************/
