...
/***********************************************
** File name: 534D.cpp                        **
** Time created: Mon Feb 25 2019 11:08am      **
***********************************************/
...
void Solve() {
	i64 Next = 0; vi List;
	while (!S.empty()) {
		while (Next >= 0 && S.lower_bound(mp(Next, -LINF)) == S.end()) Next -= 3;
		if (Next < 0) {cout << "Impossible\n"; return;}
		if (S.lower_bound(mp(Next, -LINF))->fi != Next) {cout << "Impossible\n"; return;}
		pii Token = *S.lower_bound(mp(Next, -LINF));
		S.erase(Token); List.pub(Token.se);
		Next += 1;
	}
	cout << "Possible\n";
