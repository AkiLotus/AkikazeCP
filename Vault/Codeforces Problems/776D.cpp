...
/************** [GLOBAL VARIABLES] **************/
i64 n, m; vi Unlocked; vi vis;
vector<vi> Switches, Rooms;
/************************************************/

/************** [FUNCTIONS] **************/
void DFS(i64 z, i64 stat) {
	vis[z] = stat;
	for (auto w: Switches[z]) {
		for (auto t: Rooms[w]) {
			if (t == z) continue;
			if (vis[t] != -1) {
				if (vis[t] != (stat - (Unlocked[w] - 1)) % 2) {cout << "NO"; Exit();}
				continue;
			}
			DFS(t, (stat - (Unlocked[w] - 1)) % 2);
		}
	}
}

void Input() {
	cin >> n >> m; Unlocked.rsz(n);
	Switches.rsz(m); Rooms.rsz(n);
	for (i64 i=0; i<n; i++) cin >> Unlocked[i];
	for (i64 i=0; i<m; i++) {
		i64 x; cin >> x; Switches[i].rsz(x);
		for (i64 j=0; j<x; j++) {
			cin >> Switches[i][j];
			Switches[i][j]--;
			Rooms[Switches[i][j]].pub(i);
		}
	}
}
