...
/***********************************************
** File name: 1110D.cpp                       **
** Time created: Fri Feb 8 2019 10:17am       **
***********************************************/
...
void Input() {
	cin >> n >> m; a.rsz(n); cnt.rsz(m);
	for (auto &z: a) {cin >> z; z--; cnt[z]++;}
}

void Solve() {
	if (m == 1) {cout << cnt[0] / 3 << endl; return;}
	if (m == 2) {cout << cnt[0] / 3 + cnt[1] / 3 << endl; return;}
	vector<vector<vi>> dp(m, vector<vi>(3, vi(3, -LINF)));
	for (i64 x=0; x<3; x++) {
		if (x > cnt[0] || x > cnt[1] || x > cnt[2]) break;
		dp[0][0][x] = x + (cnt[0] - x) / 3;
	}
	for (i64 i=0; i<m-1; i++) {
		for (i64 p1=0; p1<3; p1++) {
			for (i64 p2=0; p2<3; p2++) {
				if (dp[i][p1][p2] == -LINF) continue;
				for (i64 p3=0; p3<3; p3++) {
					if (p3 > 0 && i+3 >= m) continue;
					if (p3 > 0 && cnt[i+1] < p1 + p2 + p3) continue;
					if (p3 > 0 && cnt[i+2] < p2 + p3) continue;
					if (p3 > 0 && cnt[i+3] < p3) continue;
					dp[i+1][p2][p3] = max(dp[i+1][p2][p3], dp[i][p1][p2] + p3 + (cnt[i+1] - p1 - p2 - p3) / 3);
				}
			}
		}
	}
