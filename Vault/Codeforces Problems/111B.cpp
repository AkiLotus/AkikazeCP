...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi x, y, res; vector<vi> divList, idList;
/************************************************/
...
void Input() {
	cin >> n; x.rsz(n); y.rsz(n); res.rsz(n);
	divList.rsz(n); idList.rsz(100001);
	for (i64 i=0; i<n; i++) {
		cin >> x[i] >> y[i];
		for (i64 d=1; d<=sqrt(x[i]); d++) {
			if (x[i] % d != 0) continue;
			idList[d].pub(i); divList[i].pub(d);
			if (x[i] / d != d) {idList[x[i]/d].pub(i); divList[i].pub(x[i]/d);}
		}
	}
}
...
void Solve() {
	for (i64 i=0; i<n; i++) {
		if (y[i] == 0) {res[i] = divList[i].size(); continue;}
		for (auto d: divList[i]) {
			i64 L = lower_bound(idList[d].begin(), idList[d].end(), i-y[i]) - idList[d].begin();
			i64 R = upper_bound(idList[d].begin(), idList[d].end(), i-1) - idList[d].begin();
			res[i] += (L == R);
		}
	}
	for (i64 i=0; i<n; i++) cout << res[i] << endl;
}
/*****************************************/
