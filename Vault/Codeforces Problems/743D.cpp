...
bool linear() {
	...
	return (adj[0].size() == 1 && cnt[1] == 2 && cnt[2] == n - 2);
}

void DFS_InitDP(i64 z, i64 last) {
	sum[z] += a[z];
	for (auto t: adj[z]) {
		if (t == last) continue;
		DFS_InitDP(t, z);
...
void DFS_Process(i64 z, i64 last) {
	priority_queue<i64> Q;
	for (auto t: adj[z]) {
		if (t == last) continue;
		Q.push(dp[t]); DFS_Process(t, z);
	}
	if (Q.size() < 2) return;
	i64 res = Q.top(); Q.pop();
	res += Q.top(); Q.pop();
...
void Solve() {
	if (linear()) {cout << "Impossible"; return;}
	DFS_InitDP(0, -1); DFS_Process(0, -1);
	cout << ans;
}
/*****************************************/
