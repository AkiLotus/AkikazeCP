...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi a, L(5001, -1), R(5001, -1);
vector<vb> ok; vi dp; vector<vi> seg;
/************************************************/
...
void Solve() {
	ok.rsz(n, vb(n, false));
	dp.rsz(n, 0); seg.rsz(n, vi(n, 0));
	for (i64 i=0; i<n; i++) {
		vi cnt(5001, 0); i64 tmp = 0, underway = 0;
		for (i64 j=i; j<n; j++) {
			cnt[a[j]]++;
			if (cnt[a[j]] == 1) {
				tmp ^= a[j]; underway++;
			...
			if (R[a[j]] == j) {underway--;}
			seg[i][j] = tmp; ok[i][j] = (underway == 0);
		}
	}
	if (ok[0][0]) dp[0] = a[0];
	for (i64 j=1; j<n; j++) {
		for (i64 i=0; i<=j; i++) {
			if (i > 0) dp[j] = max(dp[j], dp[i-1]);
			if (!ok[i][j]) continue;
			if (i > 0) dp[j] = max(dp[j], dp[i-1] + seg[i][j]);
			else dp[j] = max(dp[j], seg[i][j]);
		}
	}
	cout << dp[n-1];
}
