...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, ps; string p; vi y;
/************************************************/

/************** [FUNCTIONS] **************/
bool isCycle(string s, i64 period) {
	for (i64 i=period; i<s.size(); i++) {
		if (s[i] != s[i-period]) return false;
	}
	return true;
}

void Input() {
	cin >> n >> m >> p; ps = p.size(); y.rsz(m);
	for (i64 i=0; i<m; i++) {cin >> y[i]; y[i]--;}
}

void Solve() {
	i64 cycle = 0, ans = 1; vi sum(n);
	for (i64 i=1; i<m; i++) {
		if (y[i] - y[i-1] >= ps) continue;
		cycle = __gcd(cycle, y[i] - y[i-1]);
	}
	if (!isCycle(p, cycle)) {cout << "0"; return;}
	for (auto L: y) {
		i64 R = L + ps;
		sum[L]++; if (R < n) sum[R]--;
	}
	for (i64 i=1; i<n; i++) sum[i] += sum[i-1];
	for (i64 i=0; i<n; i++) {
		if (sum[i]) continue;
		ans = (ans * 26) % Mod;
	}
