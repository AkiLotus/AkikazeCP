...
void Solve() {
	vi dp(n, 0), update(n, -LINF);
	while (!Q.empty()) {
		w = Q.top().fi; stack<i64> S;
		while (!Q.empty() && Q.top().fi == w) {
			u = Q.top().se.fi; v = Q.top().se.se; Q.pop();
			S.push(v); update[v] = max(update[v], dp[u] + 1);
		}
		while (!S.empty()) {
			i64 z = S.top(); S.pop();
			dp[z] = max(dp[z], update[z]); update[z] = -LINF;
		}
	}
