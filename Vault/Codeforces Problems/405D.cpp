...
/************** [GLOBAL VARIABLES] **************/
i64 n, x; vb vis(1000003, false); set<i64> S;
/************************************************/
...
void Input() {
	cin >> n; while (n--) {cin >> x; vis[x] = true;}
}

void Solve() {
	i64 symmetry = 0;
	for (i64 i=1; i<=500000; i++) {
		if (vis[i] && vis[1000001-i]) {symmetry++; continue;}
		if (!vis[i] && !vis[1000001-i]) continue;
		if (!vis[i]) S.insert(i); else S.insert(1000001-i);
		vis[i] = true; vis[1000001-i] = true;
	}
	for (i64 i=1; i<=500000; i++) {
		if (symmetry == 0) break;
		if (vis[i] || vis[1000001-i]) continue;
		S.insert(i); S.insert(1000001-i); symmetry--;
	}
	cout << S.size() << endl;
