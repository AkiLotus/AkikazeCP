...
/************** [GLOBAL VARIABLES] **************/
vector<vc> A(8, vc(8, '.'));
vector<vector<vb>> vis(8, vector<vb>(8, vb(100, false)));
vector<vector<vb>> statuette(8, vector<vb>(8, vb(100, false)));
/************************************************/
...
void Input() {
	for (i64 i=0; i<8; i++) {
		for (i64 j=0; j<8; j++) {
			...
			if (A[i][j] == 'S') {
				for (i64 x=0; i+x<8; x++) {
					statuette[i+x][j][x] = true;
					statuette[i+x][j][x+1] = true;
				}
			}
		}
	}
...
void Solve() {
	queue<ppi> Q; vis[7][0][0] = true;
	Q.push(mp(mp(7LL, 0LL), 0LL));
	while (!Q.empty()) {
		ppi Z = Q.front(); Q.pop();
		i64 x = Z.fi.fi, y = Z.fi.se, t = Z.se;
		if (x == 0 && y == 7) {cout << "WIN"; return;}
		for (i64 i=-1; i<=1; i++) {
			for (i64 j=-1; j<=1; j++) {
				if (x+i < 0 || x+i > 7) continue;
				if (y+j < 0 || y+j > 7) continue;
				if (vis[x+i][y+j][t+1]) continue;
				if (statuette[x+i][y+j][t+1]) continue;
				vis[x+i][y+j][t+1] = true; Q.push(mp(mp(x+i, y+j), t+1));
			}
		}
	}
	cout << "LOSE";
