...
void VarInput() {
	...
	cin >> n; x.resize(n); h.resize(n);
	dp.resize(n, vi(3, 0));
	for (i64 i=0; i<n; i++) cin >> x[i] >> h[i];
	dp[0][1] = 1;
	if (n > 1 && x[0] + h[0] < x[1]) dp[0][2] = 1;
}
...
void ProSolve() {
	...
	for (i64 i=1; i<n; i++) {
		...
		if (x[i-1] < x[i] - h[i]) {
			...
			if (dp[i-1][2] + 1 > dp[i][1] && x[i-1] + h[i-1] < x[i] - h[i]) dp[i][1] = dp[i-1][2] + 1;
		}
		if (i == n-1 || (i < n-1 && x[i] + h[i] < x[i+1])) dp[i][2] = dp[i][0] + 1;
	}
