...
/***********************************************
** File name: 645D.cpp                        **
** Time created: Tue Dec 25 2018 14:18pm      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, ans = -1; vector<pii> Edges;
/************************************************/

/************** [FUNCTIONS] **************/
void Toposort(i64 z, vector<vi> &adj, vi &vis, stack<i64> &Stack) {
	vis[z] = 2;
	for (auto t: adj[z]) {
		if (vis[t] != 0) continue;
		Toposort(t, adj, vis, Stack);
	}
	vis[z] = 1; Stack.push(z);
}

bool valid(i64 cnt) {
	vector<vi> adj(n); stack<i64> Stack;
	vi vis(n, 0);
	for (i64 i=0; i<cnt; i++) {
		adj[Edges[i].fi].pub(Edges[i].se);
	}
	for (i64 i=0; i<n; i++) if (!vis[i]) Toposort(i, adj, vis, Stack);
	while (!Stack.empty()) {
		i64 z = Stack.top(); Stack.pop();
		if (Stack.empty()) break;
		i64 t = Stack.top();
		bool flag = true;
		if (find(adj[z].begin(), adj[z].end(), t) == adj[z].end()) flag = false;
...
void binsearch(i64 top, i64 bot) {
	while (top <= bot) {
		i64 mid = (top + bot) / 2;
		if (valid(mid)) {ans = mid; bot = mid - 1;}
		else top = mid + 1;
	}
}
