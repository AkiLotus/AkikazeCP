...
void VarInput() {
	...
	cin >> N >> M >> K; c.rsz(N); p.rsz(N, vi(M+1));
...
void ProSolve() {
	vector<vector<vi>> dp(N, vector<vi>(K+2, vi(M+1, LINF)));
	if (c[0] != 0) dp[0][1][c[0]] = 0;
	else for (i64 i=1; i<=M; i++) dp[0][1][i] = p[0][i];
	for (i64 i=1; i<N; i++) {
		for (i64 j=0; j<=K; j++) {
			for (i64 k=1; k<=M; k++) {
				if (dp[i-1][j][k] == LINF) continue;
				if (c[i] != 0) {
					if (c[i] == k) dp[i][j][c[i]] = min(dp[i][j][c[i]], dp[i-1][j][k]);
					else dp[i][j+1][c[i]] = min(dp[i][j+1][c[i]], dp[i-1][j][k]);
				}
				else for (i64 z=1; z<=M; z++) {
					if (z == k) dp[i][j][z] = min(dp[i][j][z], dp[i-1][j][k] + p[i][z]);
					else dp[i][j+1][z] = min(dp[i][j+1][z], dp[i-1][j][k] + p[i][z]);
				}
			}
		}
	}
	i64 ans = LINF;
	for (i64 i=1; i<=M; i++) ans = min(ans, dp[N-1][K][i]);
