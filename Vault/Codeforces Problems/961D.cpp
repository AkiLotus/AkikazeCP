...
/************** [FUNCTIONS] **************/
bool inLine(i64 p1, i64 p2, i64 p3) {
	pii vec1 = mp(x[p2]-x[p1], y[p2]-y[p1]);
	pii vec2 = mp(x[p3]-x[p1], y[p3]-y[p1]);
	if (vec1.fi < 0) {vec1.fi = -vec1.fi; vec1.se = -vec1.se;}
	if (vec2.fi < 0) {vec2.fi = -vec2.fi; vec2.se = -vec2.se;}
	if (vec1.fi == 0) vec1.se = 1;
	else if (vec1.se == 0) vec1.fi = 1;
	else {i64 g = __gcd(vec1.fi, vec1.se); vec1.fi /= g; vec1.se /= g;}
	if (vec2.fi == 0) vec2.se = 1;
	else if (vec2.se == 0) vec2.fi = 1;
	else {i64 g = __gcd(vec2.fi, vec2.se); vec2.fi /= g; vec2.se /= g;}
	return (vec1 == vec2);
}
...
void Solve() {
	if (n <= 4) {cout << "YES"; return;}
	i64 p1 = 0, p2 = 1, p3 = -1;
	for (i64 i=2; i<n; i++) {
		if (!inLine(p1, p2, i)) p3 = i;
	}
	if (p3 == -1) {cout << "YES"; return;}
	bool flag12 = true, flag13 = true, flag23 = true; i64 p4;
	p4 = 0; while (p4 < n && (p4 == p1 || p4 == p2 || p4 == p3 || inLine(p1, p2, p4))) p4++;
	for (i64 i=p4+1; i<n; i++) {
		if (i == p1 || i == p2 || i == p3 || i == p4) continue;
		if (!inLine(p3, p4, i) && !inLine(p1, p2, i)) {flag12 = false; break;}
	}
	p4 = 0; while (p4 < n && (p4 == p1 || p4 == p2 || p4 == p3 || inLine(p1, p3, p4))) p4++;
	for (i64 i=p4+1; i<n; i++) {
		if (i == p1 || i == p2 || i == p3 || i == p4) continue;
		if (!inLine(p2, p4, i) && !inLine(p1, p3, i)) {flag13 = false; break;}
	}
	p4 = 0; while (p4 < n && (p4 == p1 || p4 == p2 || p4 == p3 || inLine(p2, p3, p4))) p4++;
	for (i64 i=p4+1; i<n; i++) {
		if (i == p1 || i == p2 || i == p3 || i == p4) continue;
		if (!inLine(p1, p4, i) && !inLine(p2, p3, i)) {flag23 = false; break;}
	}
	if (flag12 || flag13 || flag23) cout << "YES"; else cout << "NO";
}
/*****************************************/
