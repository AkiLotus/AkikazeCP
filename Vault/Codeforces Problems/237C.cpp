...
/** -----GLOBAL VARIABLES----- **/
i64 a, b, k; vi isPrime(1000009, true);

/** -----EXTENSIVE FUNCTIONS----- **/
void EratosthenesSieve() {
	isPrime[0] = false; isPrime[1] = false;
	for (i64 i=2; i<1000009; i++) {
		if (!isPrime[i]) continue;
		for (i64 j=2; j<=1000008/i; j++) {
			isPrime[i*j] = false;
		}
	}
}
...
void ProSolve() {
	...
	while (top <= bot) {
		i64 x = top, y = bot;
		i64 mid = (top + bot) / 2;
		//tracker3(top, bot, mid);
		bool flag = true;
		i64 ptr1 = a, ptr2 = a + mid - 1, cnt = 0;
		for (i64 i=ptr1; i<=ptr2; i++) cnt += isPrime[i];
		if (cnt < k) flag = false;
		while (ptr2 < b) {
			cnt -= isPrime[ptr1]; cnt += isPrime[ptr2+1];
			ptr1++; ptr2++; if (cnt < k) {flag = false; break;}
		}
		if (flag) {
			bot = mid; if (ans == -1) ans = mid;
