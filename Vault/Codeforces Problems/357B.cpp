...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, x, y, z; vi a; vector<set<i64>> adj;
/************************************************/
...
void Input() {
	...
	for (i64 i=1; i<m; i++) {
		cin >> x >> y >> z; x--; y--; z--;
		if (a[x] == 0) {
			if (a[y] != 1 && a[z] != 1) a[x] = 1;
			else if (a[y] != 2 && a[z] != 2) a[x] = 2;
			else if (a[y] != 3 && a[z] != 3) a[x] = 3;
		}
		if (a[y] == 0) {
			if (a[x] != 1 && a[z] != 1) a[y] = 1;
			else if (a[x] != 2 && a[z] != 2) a[y] = 2;
			else if (a[x] != 3 && a[z] != 3) a[y] = 3;
		}
		if (a[z] == 0) {
