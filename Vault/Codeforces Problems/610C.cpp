...
/************** [GLOBAL VARIABLES] **************/
i64 k; vector<vc> A; map<char, char> anti;
/************************************************/
...
void Try(i64 n) {
	if (n == 0) return;
	Try(n-1);
	for (i64 i=0; i<(1LL<<(n-1)); i++) {
		for (i64 j=0; j<(1LL<<(n-1)); j++) {
			A[i+(1LL<<(n-1))][j] = A[i][j];
			A[i][j+(1LL<<(n-1))] = A[i][j];
			A[i+(1LL<<(n-1))][j+(1LL<<(n-1))] = anti[A[i][j]];
		}
	}
}
...
void Input() {
	anti['+'] = '*'; anti['*'] = '+';
	cin >> k; A.rsz((1LL << k), vc(1LL << k, '+'));
}

void Solve() {
	Try(k);
	for (i64 i=0; i<(1LL<<k); i++) {
		for (i64 j=0; j<(1LL<<k); j++) {
			cout << A[i][j];
		}
		cout << endl;
	}
