...
void Solve() {
	...
	vector<vector<vi>> dp(n, vector<vi>(m, vi(k+1, 0)));
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<m; j++) {
			for (i64 z=0; z<=k; z++) {
				if (i > 0) dp[i][j][z] = max(dp[i][j][z], dp[i-1][j][z]);
				if (j > 0) dp[i][j][z] = max(dp[i][j][z], dp[i][j-1][z]);
				if (z == 0 || !Match[i][j]) continue;
				if (i >= Match[i][j] && j >= Match[i][j]) {
					dp[i][j][z] = max(dp[i][j][z], dp[i-Match[i][j]][j-Match[i][j]][z-1] + Match[i][j]);
				}
				else dp[i][j][z] = max(dp[i][j][z], Match[i][j]);
			}
		}
	}
	i64 ans = 0;
	for (i64 i=0; i<=k; i++) if (dp[n-1][m-1][i] >= k) ans = max(ans, dp[n-1][m-1][i]);
	cout << ans;
}
/*****************************************/
