...
bool right() {
	...
	if (sqr(root[0]) == d[0] && sqr(root[1]) == d[1] && sqr(root[2]) == d[2]) {
		sort(root.begin(), root.end()); if (root[0] + root[1] == root[2]) return false;
	}
	sort(d.begin(), d.end());
	return (d[0] + d[1] == d[2]);
}
