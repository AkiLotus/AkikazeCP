...
/************** [GLOBAL VARIABLES] **************/
i64 n, a, b; vector<vi> adj; vi Rank, d, p;
i64 farthest = -1, dist = -1;
/************************************************/

/************** [FUNCTIONS] **************/
void DFS(i64 z, i64 last) {
	if (last != -1) d[z] = d[last] + 1;
	if (d[z] > dist) {dist = d[z]; farthest = z;}
	for (auto t: adj[z]) {
...
void SeekCentroid(i64 st, i64 lvl) {
	farthest = -1; dist = -1;
	d[st] = 0; DFS(st, -1);
	st = farthest; farthest = -1; dist = -1;
	d[st] = 0; DFS(st, -1); i64 en = farthest;
	i64 centroid = en;
	while (d[centroid] != d[en] / 2) centroid = p[centroid];
	Rank[centroid] = lvl;
	for (auto z: adj[centroid]) {
		if (Rank[z] == 0) SeekCentroid(z, lvl+1);
	}
}

void Input() {
	cin >> n; adj.rsz(n); Rank.rsz(n);
	d.rsz(n); p.rsz(n, -1);
	for (i64 i=1; i<n; i++) {
		cin >> a >> b; a--; b--;
...
void Solve() {
	SeekCentroid(0, 1);
	if (*max_element(Rank.begin(), Rank.end()) > 26) {cout << "Impossible!\n"; return;}
	for (auto x: Rank) cout << char(64+x) << " ";
}
/*****************************************/
