...
/************** [GLOBAL VARIABLES] **************/
i64 n, x; vi a; map<i64, i64> Map, AndMap;
/************************************************/
...
void Input() {
	cin >> n >> x; a.rsz(n);
	for (i64 i=0; i<n; i++) {
		cin >> a[i]; Map[a[i]]++; AndMap[a[i] & x]++;
		if (Map[a[i]] > 1) {cout << "0"; Exit();}
	}
}
...
void Solve() {
	for (i64 i=0; i<n; i++) {
		Map[a[i]]--;
		if (Map[a[i] & x]) {cout << "1"; return;}
		Map[a[i]]++;
	}
	for (auto it=AndMap.begin(); it!=AndMap.end(); it++) {
		if (it->se > 1) {cout << "2"; return;}
	}
	cout << "-1";
}
/*****************************************/
