...
void VarInput() {
	...
	cin >> n; b.rsz(n); ans.rsz(n);
	for (i64 i=0; i<n; i++) {cin >> b[i].fi; b[i].se = i;}
}

void ProSolve() {
	sort(b.begin(), b.end()); ans[b[0].se] = b[0].fi;
	for (i64 i=1; i<n; i++) {
		ans[b[i].se] = max(ans[b[i-1].se]+1, b[i].fi);
	}
