...
/************** [GLOBAL VARIABLES] **************/
i64 n, v, t, p; vector<pii> kayak, catamaran;
/************************************************/
...
void Input() {
	...
	for (i64 i=1; i<=n; i++) {
		cin >> t >> p;
		if (t == 1) kayak.pub(mp(p, i));
		else catamaran.pub(mp(p, i));
	}
}

void Solve() {
	i64 optimal = -1, catcnt = -1;
	i64 z = kayak.size(), kay = 0, cat = 0;
	sort(kayak.rbegin(), kayak.rend());
	sort(catamaran.rbegin(), catamaran.rend());
	for (auto rr: kayak) kay += rr.fi;
	for (i64 i=0; i*2<=v; i++) {
		while (i * 2 + z > v) {z--; kay -= kayak[z].fi;}
		if (kay + cat > optimal) {catcnt = i; optimal = kay + cat;}
		if (i >= catamaran.size()) break; cat += catamaran[i].fi;
	}
	if (optimal == -1) {cout << "0"; return;}
	cout << optimal << endl;
	for (i64 i=0; i<catcnt; i++) {
		cout << catamaran[i].se << " ";
	}
	for (i64 i=0; i<min(v-catcnt*2, (i64)kayak.size()); i++) {
		cout << kayak[i].se << " ";
	}
}
/*****************************************/
