...
/************** [GLOBAL VARIABLES] **************/
i64 n, a, b; vi A, F; vector<set<i64>> adj; map<i64, i64> id;
/************************************************/
...
void Input() {
	cin >> n >> a >> b; A.rsz(n); adj.rsz(n); F.rsz(n, -1);
	for (i64 i=0; i<n; i++) {cin >> A[i]; id[A[i]] = i;}
}

void Solve() {
	set<pii> Set;
	for (i64 i=0; i<n; i++) {
		if (id.find(a - A[i]) != id.end()) {adj[i].insert(id[a-A[i]]); adj[id[a-A[i]]].insert(i);}
		if (id.find(b - A[i]) != id.end()) {adj[i].insert(id[b-A[i]]); adj[id[b-A[i]]].insert(i);}
	}
	//for (i64 i=0; i<n; i++) tracker2(i, adj[i]);
	for (i64 i=0; i<n; i++) Set.insert(mp(adj[i].size(), i));
	while (!Set.empty()) {
		if (Set.begin()->fi == 0) {cout << "NO"; return;}
		i64 z = Set.begin()->se, t = *adj[z].begin();
		//tracker3(z, t, A[z]+A[t]);
		Set.erase(mp(adj[t].size(), t)); Set.erase(mp(adj[z].size(), z));
		if (A[z] + A[t] == a) {F[z] = 0; F[t] = 0;} else {F[z] = 1; F[t] = 1;}
		for (auto it=adj[t].begin(); it!=adj[t].end(); it++) {
			i64 q = *it; if (q == t || q == z) continue;
			Set.erase(mp(adj[q].size(), q)); adj[q].erase(t);
			Set.insert(mp(adj[q].size(), q));
		}
		adj[t].clear(); adj[z].clear();
	}
	cout << "YES\n"; for (auto x: F) cout << x << " ";
}
/*****************************************/
