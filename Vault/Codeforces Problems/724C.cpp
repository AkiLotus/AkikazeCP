...
/***********************************************
** File name: 724C.cpp                        **
** Time created: Wed Jan 2 2019 11:03am       **
***********************************************/
...
void Input() {
	...
	PointsinLine.rsz(2, vector<vi>((n+m)*2+2));
	offset = n + m;
	for (i64 i=0; i<k; i++) {
		i64 x, y; cin >> x >> y;
		Points[i] = {x, y};
		PointsinLine[0][x+y+offset].pub(i);
		PointsinLine[1][x-y+offset].pub(i);
	}
}
...
void Solve() {
	...
	while (!Q.empty()) {
		...
		for (auto id: PointsinLine[LineType][LineID]) {
