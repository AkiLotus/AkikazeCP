...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi a, b, dp, slope, coeff;
vector<ppi> Stack; i64 Size = 0;
/************************************************/

/************** [FUNCTIONS] **************/
i64 binsearch(i64 x) {
	i64 top = 0, bot = Size-1, ans = 0;
	while (top <= bot) {
		i64 mid = (top + bot) / 2;
...
void UpdateStack(i64 id) {
	while (Size > 0) {
		...
		if (Topxl * slope[TopID] + coeff[TopID] >= Topxl * slope[id] + coeff[id]) {Size--; continue;}
		i64 top = Topxl, bot = Topxr, res = Topxl;
		while (top <= bot) {
			i64 mid = (top + bot) / 2;
			if (mid * slope[TopID] + coeff[TopID] < mid * slope[id] + coeff[id]) {
				res = mid; top = mid + 1;
			}
			else bot = mid - 1;
		}
		Stack[Size-1].fi.se = res; break;
	}
	if (Size == 0) {Stack[Size++] = mp(mp(-INF, +INF), id); return;}
	Stack[Size] = mp(mp(Stack[Size-1].fi.se+1, +INF), id); Size++;
}
...
void Solve() {
	dp.rsz(n, LINF); slope.rsz(n); coeff.rsz(n);
	dp[0] = b[0]; slope[0] = b[0]; coeff[0] = b[0];
	Stack.rsz(n); Size = 1; Stack[0] = mp(mp(-INF, +INF), 0);
	for (i64 i=1; i<n; i++) {
		i64 id = binsearch(a[i]-1);
		dp[i] = slope[id] * (a[i] - 1) + coeff[id] + b[i];
		slope[i] = b[i]; coeff[i] = dp[i];
		UpdateStack(i);
	}
	cout << dp[n-1];
}
