...
void ProSolve() {
	sort(a.begin(), a.end());
	vi prefixSum(n, a[0].se);
	for (i64 i=1; i<n; i++) prefixSum[i] = prefixSum[i-1] + a[i].se;
	i64 pos = lower_bound(prefixSum.begin(), prefixSum.end(), 1000000LL-s) - prefixSum.begin();
	if (pos == n) {cout << "-1"; return;}
	cout << fixed << setprecision(12) << (ld)sqrt((ld)a[pos].fi);
}
