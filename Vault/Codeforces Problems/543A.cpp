...
void VarInput() {
	...
	cin >> n >> m >> b >> mod; a.rsz(n+1);
	for (i64 i=1; i<=n; i++) cin >> a[i];
}

void ProSolve() {
	vector<vi> def(m+1, vi(b+1, 0)), dp = def;
	vector<vi> last(m+1, vi(b+1, 0)); last[0][0] = 1;
	for (i64 i=1; i<=n; i++) {
		for (i64 j=0; j<=m; j++) {
			if (i == 0 && j != 0) break;
			for (i64 k=0; k<=b; k++) {
				dp[j][k] += last[j][k]; dp[j][k] %= mod;
				if (j-1 >= 0 && k-a[i] >= 0) dp[j][k] += dp[j-1][k-a[i]];
				dp[j][k] %= mod;
			}
		}
		last = dp; dp = def;
	...
	i64 ans = 0;
	for (i64 i=0; i<=b; i++) {ans += last[m][i]; ans %= mod;}
	cout << ans;
}
