...
void DFSSpread(i64 z) {
	good[z] = true; vis[z] = true;
	for (auto t: adj[z]) {
		if (good[t]) continue;
		if (vis[t]) continue;
		DFSSpread(t);
...
void Solve() {
	i64 ans = 0; vis.clear(); vis.rsz(n, false); DFSSpread(s);
	while (!allGood()) {
		cnt.clear(); cnt.rsz(n, 0);
		for (i64 i=0; i<n; i++) {
			if (good[i]) continue; vis.clear();
			vis.rsz(n, false); DFS(i);
		}
		i64 id = max_element(cnt.begin(), cnt.end()) - cnt.begin();
		vis.clear(); vis.rsz(n, false); DFSSpread(id); ans++;
	}
	cout << ans;
}
/*****************************************/
