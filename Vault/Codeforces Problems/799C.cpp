...
i64 n, c, d; vector<ppc> a;
multiset<pii> Coin, Diamond;
/************************************************/
...
void Solve() {
	i64 ans = 0; vector<pii> CoinVec, DiamondVec;
	multiset<pii> CoinHeap, DiamondHeap;
	for (i64 i=0; i<n; i++) {
		if (a[i].se == 'C' && a[i].fi.se <= c) {
			Coin.insert(mp(a[i].fi.fi, a[i].fi.se));
			CoinVec.pub(mp(a[i].fi.se, a[i].fi.fi));
			CoinHeap.insert(mp(a[i].fi.se, a[i].fi.fi));
		}
		if (a[i].se == 'D' && a[i].fi.se <= d) {
			Diamond.insert(mp(a[i].fi.fi, a[i].fi.se));
			DiamondVec.pub(mp(a[i].fi.se, a[i].fi.fi));
			DiamondHeap.insert(mp(a[i].fi.se, a[i].fi.fi));
		}
	}
	sort(CoinVec.begin(), CoinVec.end()); sort(DiamondVec.begin(), DiamondVec.end());
	if (!Coin.empty() && !Diamond.empty()) ans = Coin.rbegin()->fi + Diamond.rbegin()->fi;
	i64 petrCoin = 0, petrDiamond = 0;
	for (i64 i=0; i<CoinVec.size(); i++) {
		if (CoinHeap.find(CoinVec[i]) == CoinHeap.end()) break;
		CoinHeap.erase(CoinHeap.find(CoinVec[i]));
		Coin.erase(Coin.find(mp(CoinVec[i].se, CoinVec[i].fi)));
		while (!CoinHeap.empty() && CoinHeap.rbegin()->fi > c - CoinVec[i].fi) {
			pii z = *CoinHeap.rbegin(); CoinHeap.erase(CoinHeap.find(z));
			Coin.erase(Coin.find(mp(z.se, z.fi)));
		}
		if (Coin.empty()) break;
		ans = max(ans, CoinVec[i].se + Coin.rbegin()->fi);
	}
	for (i64 i=0; i<DiamondVec.size(); i++) {
		if (DiamondHeap.find(DiamondVec[i]) == DiamondHeap.end()) break;
		DiamondHeap.erase(DiamondHeap.find(DiamondVec[i]));
		Diamond.erase(Diamond.find(mp(DiamondVec[i].se, DiamondVec[i].fi)));
		while (!DiamondHeap.empty() && DiamondHeap.rbegin()->fi > d - DiamondVec[i].fi) {
			pii z = *DiamondHeap.rbegin(); DiamondHeap.erase(DiamondHeap.find(z));
			Diamond.erase(Diamond.find(mp(z.se, z.fi)));
		}
		if (Diamond.empty()) break;
		ans = max(ans, DiamondVec[i].se + Diamond.rbegin()->fi);
	}
	cout << ans;
}
/*****************************************/
