...
i64 ProceedAnswer(i64 L, i64 R, i64 x) {
	if (L < x && x < R) return LCA(LCASegment(L, x-1), LCASegment(x+1, R));
	if (L == x) return LCASegment(x+1, R); return LCASegment(L, x-1);
}
...
void Solve() {
	...
	while (q--) {
		cin >> l >> r; l--; r--; i64 TotalLCA = LCASegment(l, r);
		i64 l1 = binsearch_prefix(l, r, TotalLCA), r1 = binsearch_suffix(l, r, TotalLCA);
		i64 LCA_l1 = ProceedAnswer(l, r, l1), LCA_r1 = ProceedAnswer(l, r, r1);
