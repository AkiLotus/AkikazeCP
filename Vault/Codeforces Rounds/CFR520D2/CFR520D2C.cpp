...
/************** [GLOBAL VARIABLES] **************/
i64 n, q, l, r; string s; vi Ichi, Zero;
/************************************************/
...
void Input() {
	cin >> n >> q >> s; Ichi.rsz(n); Zero.rsz(n);
	Ichi[0] = (s[0] == '1'); Zero[0] = (s[0] == '0');
}

void Solve() {
	for (i64 i=1; i<n; i++) {
		Ichi[i] = Ichi[i-1] + (s[i] == '1');
	...
	while (q--) {
		...
		cout << ((modPow(2, ichicnt) + (Mod - 1)) * modPow(2, zerocnt)) % Mod << endl;
	}
}
/*****************************************/
