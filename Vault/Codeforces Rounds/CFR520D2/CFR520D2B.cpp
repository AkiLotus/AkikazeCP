...
void Solve() {
	...
	for (i64 i=2; i<=sqrt(n); i++) {
		while (n % i == 0) {Map[i]++; n /= i;}
	}
	if (n > 1) Map[n]++;
	i64 Min = 1, MaxExp = -LINF, MinExp = LINF;
	i64 RequiredExp = 1, sqrtcnt = 0, multiply = 1;
	for (auto it=Map.begin(); it!=Map.end(); it++) {
		Min *= (it->fi); MaxExp = max(MaxExp, it->se);
		MinExp = min(MinExp, it->se);
	}
	while (RequiredExp < MaxExp) {RequiredExp *= 2; sqrtcnt++;}
	if (MinExp == MaxExp && MinExp == RequiredExp) multiply = 0;
	cout << Min << " " << (multiply + sqrtcnt);
}
/*****************************************/
