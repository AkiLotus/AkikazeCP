...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, q, r, c; map<i64, i64> Map;
vi DSUsize, DSUparent;
/************************************************/

/************** [FUNCTIONS] **************/
i64 DSUfind(i64 z) {
    while (DSUparent[z] != -1) z = DSUparent[z];
    return z;
}
...
void DSUjoin(i64 x, i64 y) {
    ...
    DSUparent[b] = a; DSUsize[a] += DSUsize[b];
}

void Input() {
	cin >> n >> m >> q;
	DSUsize.rsz(n+m, 1); DSUparent.rsz(n+m, -1);
	while (q--) {
	    cin >> r >> c; r--; c--;
	    DSUjoin(r, n+c);
	}
...
void Solve() {
	for (i64 i=0; i<n+m; i++) Map[DSUfind(i)]++;
	cout << Map.size() - 1;
}
/*****************************************/
