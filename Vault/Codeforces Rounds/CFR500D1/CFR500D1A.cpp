...
void Solve() {
	sort(a.begin(), a.end());
	vi PreMin(2*n, a[0]), SufMin(2*n, a[2*n-1]); 
	vi PreMax = PreMin, SufMax = SufMin; i64 ans = LINF;
	for (i64 i=1; i<2*n; i++) PreMin[i] = min(PreMin[i-1], a[i]);
	for (i64 i=2*n-2; i>=0; i--) SufMin[i] = min(SufMin[i+1], a[i]);
	for (i64 i=1; i<2*n; i++) PreMax[i] = max(PreMax[i-1], a[i]);
	for (i64 i=2*n-2; i>=0; i--) SufMax[i] = max(SufMax[i+1], a[i]);
	multiset<i64> S; for (i64 i=0; i<n; i++) S.insert(a[i]);
	auto it1 = S.begin(), it2 = S.end(); it2--; ans = (*it2 - *it1) * (SufMax[n] - SufMin[n]);
	for (i64 i=n; i<2*n; i++) {
		S.erase(S.find(a[i-n])); S.insert(a[i]); it1 = S.begin(); it2 = S.end(); it2--;
		i64 Mn = PreMin[i-n]; if (i < 2*n-1) Mn = min(Mn, SufMin[i+1]);
		i64 Mx = PreMax[i-n]; if (i < 2*n-1) Mx = max(Mx, SufMax[i+1]);
		ans = min(ans, (*it2 - *it1) * (Mx - Mn));
	}
	cout << ans;
}
/*****************************************/
