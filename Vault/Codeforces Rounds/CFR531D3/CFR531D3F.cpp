...
/***********************************************
** File name: CFR531D3.cpp                    **
** Time created: Wed Jan 9 2019 14:58pm       **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, m; vector<vi> a;
vector<vi> colTransit, rowTransit;
/************************************************/
...
void Input() {
	...
	colTransit.rsz(n, vi(n, LINF));
	rowTransit.rsz(n, vi(n, LINF));
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<m; j++) {
			cin >> a[i][j];
		}
...
void Solve() {
	if (n == 1) {
		i64 ans = LINF;
		for (i64 i=1; i<m; i++) ans = min(ans, abs(a[0][i-1] - a[0][i]));
		cout << ans << endl; return;
	}
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<n; j++) {
			if (i == j) continue;
			for (i64 z=0; z<m; z++) rowTransit[i][j] = min(rowTransit[i][j], (i64)abs(a[i][z] - a[j][z]));
			for (i64 z=1; z<m; z++) colTransit[i][j] = min(colTransit[i][j], (i64)abs(a[i][z-1] - a[j][z-0]));
		}
	}
	//for (auto A: rowTransit) cout << A << endl; cout << endl;
	//for (auto A: colTransit) cout << A << endl; cout << endl;
	vector<vector<vi>> dp(1LL << n, vector<vi>(n, vi(n, -LINF)));
	queue<pip> Q;
	for (i64 i=0; i<n; i++) Q.push(mp(1LL << i, mp(i, i)));
	while (!Q.empty()) {
		pip Z = Q.front(); Q.pop();
		i64 mask = Z.fi, x = Z.se.fi, y = Z.se.se;
		//cout << "old dp: " << dp[mask][x][y] << " - "; tracker3(mask, x, y);
		if (mask == ((1LL << n) - 1)) continue;
		for (i64 i=0; i<n; i++) {
			if ((mask | (1LL << i)) == mask) continue;
			i64 newval = min(dp[mask][x][y], rowTransit[x][i]);
			if (dp[mask][x][y] == -LINF) newval = rowTransit[x][i];
			if ((mask | (1LL << i)) == ((1LL << n) - 1)) newval = min(newval, colTransit[i][y]);
			if (dp[(mask | (1LL << i))][i][y] == -LINF) Q.push(mp((mask | (1LL << i)), mp(i, y)));
			dp[(mask | (1LL << i))][i][y] = max(dp[(mask | (1LL << i))][i][y], newval);
			//tracker4((mask | (1LL << i)), i, y, dp[(mask | (1LL << i))][i][y]);
		}
	}
	i64 ans = -LINF;
	for (i64 i=0; i<n; i++) {
