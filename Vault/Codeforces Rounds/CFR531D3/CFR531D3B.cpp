...
/***********************************************
** File name: CFR531D3B.cpp                   **
** Time created: Wed Jan 9 2019 16:56pm       **
***********************************************/
...
void Solve() {
	i64 Max = 0;
	for (i64 i=0; i<n; i++) {
		if (Max + (n - i) == k) col[i] = Max + 1;
		else {cnt[a[i]]++; if (cnt[a[i]] > k) {cout << "NO\n"; return;} col[i] = cnt[a[i]];}
		Max = max(Max, col[i]);
	}
	cout << "YES\n";
	for (auto z: col) cout << z << " "; cout << endl;
}
/*****************************************/
