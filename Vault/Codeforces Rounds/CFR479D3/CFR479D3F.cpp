...
/** -----GLOBAL VARIABLES----- **/
i64 n; vi a; map<i64, set<i64>> Map;
map<i64, bool> touched;
...
void VarInput() {
	...
	cin >> n; a.rsz(n);
	for (i64 i=0; i<n; i++) {
		cin >> a[i]; Map[a[i]].insert(i);
	}
}

void ProSolve() {
	vi ans(0); i64 Mx = *max_element(a.begin(), a.end());
	for (auto it=Map.begin(); it!=Map.end(); it++) {
		if (touched[it->first]) continue;
		if (it->first > Mx) break;
		if (Map[it->first].empty()) continue;
		vi tmp(0);
		i64 val = it->first, index = *Map[val].begin();
		//tracker2(val, index)
		while (true) {
			if (index == *Map[val].begin()) touched[val] = true; tmp.pub(index+1); val++;
			if (Map[val].lower_bound(index) == Map[val].end()) break;
			index = *Map[val].lower_bound(index);
		}
		if (tmp.size() > ans.size()) ans = tmp;
	}
	cout << ans.size() << endl; for (i64 i=0; i<ans.size(); i++) cout << ans[i] << " ";
}
