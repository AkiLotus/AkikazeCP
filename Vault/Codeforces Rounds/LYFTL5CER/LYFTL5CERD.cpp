...
void Solve() {
	i64 ans = 1; vector<vi> GCD(n, vi(n, 0));
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<n; j++) {
	...
	for (i64 i=0; i<n; i++) {
		i64 x = a[i];
		if (root4(x) != -1) {Map[root4(x)] += 4; continue;}
		if (root3(x) != -1) {Map[root3(x)] += 3; continue;}
		if (root2(x) != -1) {Map[root2(x)] += 2; continue;}
		i64 p1 = -LINF, p2 = -LINF, id = -1;
		for (i64 j=0; j<n; j++) {
			if (GCD[i][j] > 1 && GCD[i][j] < a[i]) {
				if (p1 == -LINF) p1 = GCD[i][j];
				else if (p1 != GCD[i][j]) p2 = GCD[i][j];
			}
			else if (GCD[i][j] == a[i] && j < i && id == -1) id = j;
		}
		if (p1 != -LINF && p2 == -LINF) p2 = a[i] / p1;
		if (p1 != -LINF && p2 != -LINF) {Map[p1]++; Map[p2]++;}
		else if (id != -1) {Map[-(id+4)]++; Map[-(id+4+LINF)]++;}
		else {Map[-(i+4)]++; Map[-(i+4+LINF)]++;}
	}
	for (auto it=Map.begin(); it!=Map.end(); it++) ans = (ans * (it->se + 1)) % Mod;
	cout << ans;
}
/*****************************************/
