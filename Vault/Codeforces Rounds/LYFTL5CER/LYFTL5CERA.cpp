...
/************** [GLOBAL VARIABLES] **************/
i64 n, ax, ay, bx, by, cx, cy; vector<vb> vis;
/************************************************/
...
void Input() {
	...
	ax--; ay--; bx--; by--; cx--; cy--; vis.rsz(n, vb(n, false));
}

void Solve() {
	queue<pii> Q; vis[bx][by] = true; Q.push(mp(bx, by));
	while (!Q.empty()) {
		pii z = Q.front(); Q.pop();
		i64 x = z.fi, y = z.se;
		for (i64 i=-1; i<=1; i++) {
	...
	if (!vis[cx][cy]) cout << "NO"; else cout << "YES";
}
/*****************************************/
