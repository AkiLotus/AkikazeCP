...
void Solve() {
	...
	ld cargo = m;
	for (i64 i=0; i<2*n; i++) {
		i64 token = i / 2, capacity = 0;
		if (i % 2 == 0) {
			if (token == 0) capacity = b[0];
			else capacity = b[n-token];
		}
		else capacity = a[n-1-token];
		ld demands = cargo / (capacity - 1);
		cargo += demands;
	}
	cargo -= m;
	cout << fixed << setprecision(9) << cargo;
}
/*****************************************/
