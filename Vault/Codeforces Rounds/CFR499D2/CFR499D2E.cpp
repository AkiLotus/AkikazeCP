...
void Solve() {
	map<i64, i64> Map; vi A; vi ans;
	for (auto x: a) Map[x % k]++; vb vis(k, false); vis[0] = true;
	for (auto it=Map.begin(); it!=Map.end(); it++) {
		if (it->first == 0) continue;
		A.pub(it->first);
	}
	vi GCD = A;
	for (i64 i=0; i<GCD.size(); i++) GCD[i] = __gcd(A[i], k);
	Map.clear(); for (auto x: GCD) Map[x]++;
	GCD.clear(); for (auto it=Map.begin(); it!=Map.end(); it++) GCD.pub(it->first);
	map<pii, i64> MaP; queue<i64> Q; Q.push(0);
	while (!Q.empty()) {
		i64 i = Q.front(); Q.pop(); ans.pub(i);
		for (auto x: GCD) {
		    if (MaP[mp(x, i % x)]) continue;
		    MaP[mp(x, i % x)]++;
			for (i64 j=(i+x)%k; j!=i; j=(j+x)%k) {
				if (vis[j]) continue;
				Q.push(j); vis[j] = true;
			}
		}
