...
/***********************************************
** File name: HELLO19C.cpp                    **
** Time created: Fri Jan 4 2019 14:23pm       **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n; vs a; vi Open(500001, 0); map<i64, i64> Close;
/************************************************/
...
void Solve() {
	for (auto str: a) {
		i64 diff = 0; bool flagopen = true;
		bool flagclose = true; i64 kadane = 0;
		for (auto c: str) {
			if (c == '(') diff++;
			else diff--;
			if (c == '(') kadane = max(0LL, kadane+1);
			else kadane = max(0LL, kadane-1);
			if (diff < 0) flagopen = false;
		}
		if (kadane > 0) flagclose = false;
		if (diff == 0) {
			if (flagopen && flagclose) Open[0]++;
			continue;
		}
		if (flagopen && diff > 0) Open[diff]++;
		if (flagclose && diff < 0) Close[diff]++;
	}
	i64 ans = Open[0] / 2;
	for (i64 i=1; i<500001; i++) {
		i64 chosen = min(Open[i], Close[-i]);
		ans += chosen;
	}
	cout << ans << endl;
}
