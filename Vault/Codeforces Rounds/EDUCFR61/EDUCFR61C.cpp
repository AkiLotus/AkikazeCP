...
/***********************************************
** File name: EDUCFR61C.cpp                   **
** Time created: Tue Mar 5 2019 15:00pm       **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
int n, q; vector<pair<int, int>> Painters; vector<int> Colored;
/************************************************/
...
void Input() {
	cin >> n >> q; Painters.rsz(q); Colored.rsz(n);
	for (auto &P: Painters) {
		int x, y; cin >> x >> y; x--; y--;
		P = make_pair(x, y);
		Colored[x]++; if (y < n-1) Colored[y+1]--;
	}
}

void Solve() {
	for (int i=1; i<n; i++) Colored[i] += Colored[i-1];
	vector<vector<int>> PaintedOnce(n, vector<int>(n, 0));
	vector<vector<int>> PaintedTwice(n, vector<int>(n, 0));
	for (int i=0; i<n; i++) {
		for (int j=i; j<n; j++) {
			PaintedOnce[i][j] += (Colored[j] == 1);
			PaintedTwice[i][j] += (Colored[j] == 2);
		}
	}
	int ans = 0, AlreadyPainted = 0;
	for (auto z: Colored) AlreadyPainted += (z > 0);
	for (int i=0; i<q; i++) {
		for (int j=i+1; j<q; j++) {
			pair<int, int> P1 = Painters[i], P2 = Painters[j];
			if (P1 > P2) swap(P1, P2);
			int x1 = P1.fi, y1 = P1.se, x2 = P2.fi, y2 = P2.se;
			if (y1 < x2) ans = max(ans, AlreadyPainted - PaintedOnce[x1][y1] - PaintedOnce[x2][y2]);
			else ans = max(ans, AlreadyPainted - PaintedOnce[x1][x2] - PaintedTwice[x2][y1] - PaintedOnce[y1][y2]);
		}
	}
	cout << ans << endl;
}
