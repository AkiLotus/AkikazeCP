...
/***********************************************
** File name: EDUCFR61D.cpp                   **
** Time created: Tue Mar 5 2019 14:59pm       **
***********************************************/
...
/************** [FUNCTIONS] **************/
bool finishable(i64 x) {
	vector<vector<int>> depletedAt(k); vector<int> charged(n, 0);
	for (int i=0; i<n; i++) {
		i64 holdingTime = (a[i] + x * charged[i]) / b[i] + 1;
		if (holdingTime < k) depletedAt[holdingTime].pub(i);
	}
	int exceededCharge = 0;
	for (int i=0; i<k; i++) {
		for (auto id: depletedAt[i]) {
			while (exceededCharge > 0 && a[id] + x * charged[id] - 1LL * i * b[id] < 0) {exceededCharge--; charged[id]++;}
			if (a[id] + x * charged[id] - 1LL * i * b[id] < 0) return false;
			i64 holdingTime = (a[id] + x * charged[id]) / b[id] + 1;
			if (holdingTime < k) depletedAt[holdingTime].pub(id);
		}
		exceededCharge++;
		depletedAt[i].clear();
	}
...
i64 binsearch(i64 L, i64 R) {
	...
	while (L <= R) {
		i64 mid = (L + R) / 2;
		if (finishable(mid)) {res = mid; R = mid - 1;}
		else L = mid + 1;
	}
	return res;
}
...
void Input() {
	cin >> n >> k; a.rsz(n); b.rsz(n);
	for (auto &z: a) cin >> z;
	for (auto &z: b) cin >> z;
}

void Solve() {
	cout << binsearch(0LL, 10000000000000LL) << endl;
}
/*****************************************/
