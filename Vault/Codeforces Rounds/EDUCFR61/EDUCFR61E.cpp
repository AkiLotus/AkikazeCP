...
void Input() {
	cin >> W;
	for (auto &z: cnt) cin >> z;
}

void Solve() {
	i64 L = 840;
	vector<vi> dp(L, vi(L*8+1, -1));
	for (i64 i=0; i<L; i++) {
		if (i > cnt[0]) break;
		dp[0][i] = (cnt[0] - i) / L;
	}
	for (i64 i=0; i<7; i++) {
		for (i64 j=0; j<=L*8; j++) {
			...
			for (i64 k=0; k<L/(i+2); k++) {
				if (k > cnt[i+1]) break;
				if (j+k*(i+2) > L*8) continue;
				dp[i+1][j+k*(i+2)] = max(dp[i+1][j+k*(i+2)], dp[i][j] + (cnt[i+1] - k) / (L / (i + 2)));
			}
		}
	}
	i64 ans = 0;
	for (i64 i=0; i<=L*8; i++) {
		if (i > W) break;
		i64 MaximumAllowed = (W - i) / L;
		ans = max(ans, min(MaximumAllowed, dp[7][i]) * L + i);
	}
	cout << ans << endl;
}
/*****************************************/
