...
/************** [GLOBAL VARIABLES] **************/
i64 n, m; vi x, y; map<pii, i64> ID;
vector<pip> xy; vi psx, psy;
vector<vector<pip>> xyExcluded; vector<vi> psxEx, psyEx;
/************************************************/
...
void Input() {
	cin >> n >> m; x.rsz(n); y.rsz(n);
	xyExcluded.rsz(n); xy.rsz(n);
	psxEx.rsz(n); psx.rsz(n);
	psyEx.rsz(n); psy.rsz(n);
	for (i64 i=0; i<n; i++) {
		cin >> x[i] >> y[i]; ID[mp(x[i], y[i])] = i;
		xy[i] = mp(x[i] - y[i], mp(x[i], y[i]));
	}
	while (m--) {
		i64 u, v; cin >> u >> v; u--; v--;
		xyExcluded[u].pub(mp(x[v]-y[v], mp(x[v], y[v])));
		xyExcluded[v].pub(mp(x[u]-y[u], mp(x[u], y[u])));
	}
}
...
void Solve() {
	...
	for (i64 i=0; i<n; i++) {
		if (i == 0) {psx[i] = xy[i].se.fi; psy[i] = xy[i].se.se;}
		else {
			psx[i] = psx[i-1] + xy[i].se.fi;
			psy[i] = psy[i-1] + xy[i].se.se;
		}
	}
	for (i64 Z=0; Z<n; Z++) {
		sort(xyExcluded[Z].begin(), xyExcluded[Z].end());
		for (i64 i=0; i<xyExcluded[Z].size(); i++) {
			if (i == 0) {
				psxEx[Z].pub(xyExcluded[Z][i].se.fi);
				psyEx[Z].pub(xyExcluded[Z][i].se.se);
			}
			else {
				psxEx[Z].pub(psxEx[Z][i-1] + xyExcluded[Z][i].se.fi);
				psyEx[Z].pub(psyEx[Z][i-1] + xyExcluded[Z][i].se.se);
			}
		}
	}
	vi Total(n, 0);
	for (i64 i=0; i<n; i++) {
		i64 xi = xy[i].se.fi, yi = xy[i].se.se, id = ID[mp(xi, yi)];
		//tracker3(id, xi, yi);
		if (i > 0) Total[id] += yi * i + psx[i-1];
		//cout << "xiyj phase: "; tracker2(i, Total);
		Total[id] += xi * (n - i - 1) + psy[n-1] - psy[i];
		//cout << "xjyi phase: "; tracker2(i, Total);
		if (xyExcluded[id].empty()) {continue;}
		//cout << "exclusion starts: "; tracker2(i, Total);
		i64 pos = lower_bound(xyExcluded[id].begin(), xyExcluded[id].end(), mp(xi-yi, mp(-LINF, -LINF))) - xyExcluded[id].begin();
		if (pos > 0) Total[id] -= yi * pos + psxEx[id][pos-1];
		//cout << "xiyj phase: "; tracker2(i, Total);
		Total[id] -= xi * (xyExcluded[id].size() - pos) + psyEx[id][psyEx[id].size()-1];
		if (pos > 0) Total[id] += psyEx[id][pos-1];
		//cout << "xjyi phase: "; tracker2(i, Total);
	}
