...
void DFS(i64 z) {
	vis[z] = true; cnt++;
	if (Next[z] != -1) DFS(Next[z]);
}

void Input() {
	cin >> n >> m; a.rsz(m, vi(n));
	Prev.rsz(n, -1); Next = Prev; vis.rsz(n, false);
	for (i64 i=0; i<m; i++) {
		for (i64 j=0; j<n; j++) {
			cin >> a[i][j];
			a[i][j]--;
...
void Solve() {
	for (i64 i=1; i<m; i++) {
		for (i64 j=0; j<n; j++) {
			if (Prev[a[i][j]] == -1 || j == 0 || a[i][j-1] != Prev[a[i][j]]) Prev[a[i][j]] = -1;
			if (Next[a[i][j]] == -1 || j == n-1 || a[i][j+1] != Next[a[i][j]]) Next[a[i][j]] = -1;
		}
	}
	long long ans = 0;
	for (i64 i=0; i<n; i++) {
