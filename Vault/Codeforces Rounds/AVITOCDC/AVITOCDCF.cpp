...
void ProSolve() {
	...
	cout << a << endl << b << endl;
	vi maximumInconvenience(n, -LINF);
	for (i64 i=0; i<n; i++) {
		i64 pos2 = lower_bound(b.begin(), b.end(), (a[i] + L/2) % L) - b.begin();
		maximumInconvenience[(pos2-i+6*n)%n] = max(maximumInconvenience[(pos2-i+6*n)%n], min(L - abs(a[i] - b[pos2 % n]), abs(a[i] - b[pos2 % n])));
		maximumInconvenience[(pos2-1-i+6*n)%n] = max(maximumInconvenience[(pos2-1-i+6*n)%n], min(L - abs(a[i] - b[(pos2 - 1 + 6*n) % n]), abs(a[i] - b[(pos2 - 1 + 6*n) % n])));
		cout << maximumInconvenience << endl;
	}
	i64 ans = LINF; cout << maximumInconvenience << endl;
	for (auto x: maximumInconvenience) if (x != -LINF) ans = min(ans, x);
	cout << ans;
}
