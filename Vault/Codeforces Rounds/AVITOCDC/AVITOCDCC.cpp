...
/** -----GLOBAL VARIABLES----- **/
i64 n; vector<vi> Adj; bool flag = true;
...
void ProSolve() {
	if (n == 2) {cout << "Yes\n1\n1 2"; return;}
	i64 root = 0, leaf = 0; vi leaflist;
	if (Adj[0].size() == 1) leaflist.pub(0);
	for (i64 i=1; i<n; i++) {
		if (Adj[i].size() > Adj[root].size()) root = i;
		if (Adj[i].size() == 1) {leaf++; leaflist.pub(i);}
	}
	DFS(root, -1);
	if (!flag) {cout << "No"; return;}
	cout << "Yes\n" << leaflist.size() << endl;
	for (auto x: leaflist) cout << root+1 << " " << x+1 << endl;
}
