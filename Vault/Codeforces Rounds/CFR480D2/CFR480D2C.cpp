...
void ProSolve() {
	vi vis(256, -1), Map(256, 0);
	for (i64 i=0; i<n; i++) {
		//tracker1(i);
		if (vis[a[i]] != -1) {a[i] = vis[a[i]]; continue;}
		i64 decoded = max(a[i] - k + 1, 0LL);
		for (; decoded<a[i]; decoded++) {
			//cout << "decoded = " << decoded << endl;
			//cout << "vis[decoded] = " << vis[decoded] << endl;
			if (vis[decoded] == -1) break;
			if (a[i] - vis[decoded] >= k) continue;
			else break;
		}
		//cout << "decoded = " << decoded << endl;
		for (i64 x=decoded; x<=a[i]; x++) {
			i64 code = decoded; if (vis[decoded] != -1) code = vis[decoded];
			vis[x] = code; Map[code]++;
		}
		a[i] = vis[a[i]];
		//for (i64 x=0; x<16; x++) cout << vis[x] << " "; cout << endl;
	}
