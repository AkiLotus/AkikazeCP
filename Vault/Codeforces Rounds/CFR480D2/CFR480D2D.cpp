...
void ProSolve() {
	...
	for (i64 x=1; x<=n; x++) {
		...
		i64 perSqr = 0, negps = 0, siz = 0, zero = 0;
		for (i64 j=0; j<x; j++) {
			if (a[j] == 0) {zero++; continue;}
			else if (perfectSquare[j]) {perSqr++; continue;}
			else if (negPS[j]) {negps++; continue;}
			Map2[Map[j]]++; if (Map2[Map[j]] == 1) siz++;
		}
		ans[siz+(perSqr != 0)+(negps != 0)+(zero != 0 && perSqr == 0 && negps == 0 && siz == 0)]++;
		tracker2(0, x-1);
		cout << "added " << siz+(perSqr != 0)+(negps != 0)+(zero != 0 && perSqr == 0 && negps == 0 && siz == 0) << endl;
		for (i64 j=x; j<n; j++) {
			if (a[j-x] == 0) zero--;
			else if (perfectSquare[j-x]) perSqr--;
			else if (negPS[j-x]) negps--;
			else {Map2[Map[j-x]]--; if (Map2[Map[j-x]] == 0) siz--;}
			if (a[j] == 0) zero++;
			else if (perfectSquare[j]) perSqr++; 
