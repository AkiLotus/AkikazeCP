...
i64 n, m, k; vector<pii> edges; map<pii, i64> Map;
vector<vi> adj; vb vis; i64 cnt = 0;
/************************************************/
...
void Input() {
	cin >> n >> m >> k; edges.rsz(m);
	adj.rsz(n); vis.rsz(n, false); vis[0] = true;
	for (i64 i=0; i<m; i++) {
		cin >> edges[i].fi >> edges[i].se; edges[i].fi--; edges[i].se--;
		if (edges[i].fi > edges[i].se) swap(edges[i].fi, edges[i].se);
		adj[edges[i].fi].pub(edges[i].se); adj[edges[i].se].pub(edges[i].fi);
		Map[mp(edges[i].fi, edges[i].se)] = i;
	}
}

void Solve() {
	bitset<m> chosen; vector<bitset<m>> ans;
	vi levels(n, 0); queue<i64> Q; Q.push(0);
	while (!Q.empty()) {
		i64 z = Q.front(); Q.pop();
		for (auto t: adj[z]) {
			if (vis[t]) continue;
			vis[t] = true; Q.push(t);
			chosen[Map[mp(min(z,t), max(z,t))]] = 1;
		}
	}
}
/*****************************************/
