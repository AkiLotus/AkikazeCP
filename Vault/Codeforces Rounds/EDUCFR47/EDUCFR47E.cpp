...
void Input() {
	cin >> n; a.rsz(n+1); InvPow.rsz(n+1, 1);
	for (i64 i=1; i<=n; i++) cin >> a[i]; InvPow[1] = modPow(2, Mod-2);
	for (i64 i=2; i<=n; i++) InvPow[i] = (InvPow[i-1] * InvPow[1]) % Mod;
}

void Solve() {
	p.rsz(n+1, a[1]); i64 ans = a[1];
	for (i64 i=2; i<=n; i++) {
		p[i] = p[i-1];
		p[i] -= a[i-1] * InvPow[i-1]; p[i] %= Mod;
		p[i] += a[i] * InvPow[i-1]; p[i] %= Mod;
		while (p[i] < 0) p[i] += Mod;
		ans += p[i]; ans %= Mod;
	}
