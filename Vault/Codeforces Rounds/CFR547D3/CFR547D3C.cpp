...
/***********************************************
** File name: CFR547D3C.cpp                   **
** Time created: Wed Mar 27 2019 09:42am      **
***********************************************/
...
/************** [FUNCTIONS] **************/
bool isPerm(vi P, int N) {
	if (P.size() != N) return false;
	vi cnt(N+1, 0);
	for (int i=0; i<N; i++) {
		if (P[i] < 1 || P[i] > N) return false;
		cnt[P[i]]++; if (cnt[P[i]] > 1) return false;
	}
	return true;
}
...
void Solve() {
	...
	for (auto &z: p) {z -= Min; z++;}
	if (isPerm(p, n)) {for (auto z: p) cout << z << " "; cout << endl;}
	else cout << "-1\n";
}
/*****************************************/
