...
/***********************************************
** File name: CFR547D3F2.cpp                  **
** Time created: Wed Mar 27 2019 10:55am      **
***********************************************/
...
void Solve() {
	...
	for (int i=0; i<n; i++) {
		...
		for (int j=i; j<n; j++) {
			sum += a[j];
			Map[sum].push_back({i+1, j+1});
		}
	}
	vector<pair<int, int>> PairList;
	for (auto it=Map.begin(); it!=Map.end(); it++) {
		int key = it->first; vector<pair<int, int>> List = it->second;
		sort(List.begin(), List.end(), customsort);
		int LastEnd = -1;
		vector<pair<int, int>> NewList;
		for (auto P: List) {
			int L = P.first, R = P.second;
			if (LastEnd >= L) continue;
			LastEnd = R; NewList.push_back({L, R});
		}
		if (NewList.size() > PairList.size()) PairList = NewList;
	}
