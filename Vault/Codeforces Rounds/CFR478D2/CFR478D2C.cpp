...
void VarInput() {
	...
	cin >> n >> q; a.rsz(n); k.rsz(q);
	for (i64 i=0; i<n; i++) cin >> a[i];
	for (i64 i=0; i<q; i++) cin >> k[i];
}

void ProSolve() {
	vi s(n, a[0]); i64 curr = 0;
	...
	for (i64 i=0; i<q; i++) {
		...
		i64 killed = lower_bound(s.begin(), s.end(), curr) - s.begin();
		if (curr == *lower_bound(s.begin(), s.end(), curr)) killed++;
		cout << (n - killed) << endl;
	}
}
