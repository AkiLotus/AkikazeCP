...
void VarInput() {
	...
	cin >> n >> k; a.resize(n); t = a;
	absSum.resize(n, 0); realSum.resize(n, 0);
	for (i64 i=0; i<n; i++) cin >> a[i];
	for (i64 i=0; i<n; i++) cin >> t[i];
}

void ProSolve() {
	absSum[0] = a[0]; realSum[0] = a[0] * t[0];
	i64 ans = 0;
	for (i64 i=1; i<n; i++) {
		absSum[i] = absSum[i-1] + a[i];
		realSum[i] = realSum[i-1] + a[i] * t[i];
	}
	for (i64 i=0; i+k-1<n; i++) {
		i64 tmp = realSum[n-1] - realSum[i+k-1];
		if (i > 0) tmp += realSum[i-1];
		tmp += absSum[i+k-1];
		if (i > 0) tmp -= absSum[i-1];
		ans = max(ans, tmp);
	}
	cout << ans;
}
