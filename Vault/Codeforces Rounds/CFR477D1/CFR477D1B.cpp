...
/** -----GLOBAL VARIABLES----- **/
i64 n, x1, x2; vector<pii> c;
vi minimum1, minimum2;
vi maximum1, maximum2;
...
void VarInput() {
	...
	cin >> n >> x1 >> x2; c.rsz(n);
	for (i64 i=0; i<n; i++) {cin >> c[i].fi; c[i].se = i+1;}
}

void ProSolve() {
	sort(c.begin(), c.end());
	maximum1.resize(n+1); maximum2.rsz(n+1);
	minimum1.resize(n+1); minimum2.rsz(n+1);
	for (i64 i=1; i<=n; i++) {
		minimum1[i] = x1 / (n - i) + (x1 % (n - i) != 0);
		minimum2[i] = x2 / (n - i) + (x2 % (n - i) != 0);
		maximum1[i] = n - (lower_bound(c.begin(), c.end(), mp(minimum1[i], 0LL)) - c.begin());
		maximum2[i] = n - (lower_bound(c.begin(), c.end(), mp(minimum2[i], 0LL)) - c.begin());
		minimum1[i] = -minimum1[i]; minimum2[i] = -minimum2[i];
	}
	for (i64 i=1; i<=n; i++) {
		i64 j = lower_bound(minimum2.begin()+1, minimum2.end(), minimum1[i]) - minimum2.begin();
		if (minimum1[i] == minimum2[j]) {
			i64 z = maximum1[i];
			if (i + j <= z) {
				cout << "Yes\n" << i << " " << j << endl;
				for (i64 x=0; x<i; x++) cout << c[n-1-x].se << " "; cout << endl;
				for (i64 x=i; x<i+j; x++) cout << c[n-1-x].se << " "; cout << endl;
				return;
			}
		}
		i64 top = j, bot = n;
		while (top <= bot) {
			i64 x = top, y = bot;
			i64 jj = (top + bot) / 2;
			i64 z1 = maximum1[i], z2 = maximum2[jj];
			if (i <= z1 && i + jj <= z2) {
				cout << "Yes\n" << i << " " << jj << endl;
				for (i64 x=0; x<i; x++) cout << c[n-1-x].se << " "; cout << endl;
				for (i64 x=i; x<i+jj; x++) cout << c[n-1-x].se << " "; cout << endl;
				return;
			}
			else bot = jj;
			if (x == y) break;
		...
		while (top <= bot) {
			...
			if (jj <= z2 && i + jj <= z2) {
				cout << "Yes\n" << i << " " << jj << endl;
				for (i64 x=0; x<jj; x++) cout << c[n-1-x].se << " "; cout << endl;
				for (i64 x=jj; x<i+jj; x++) cout << c[n-1-x].se << " "; cout << endl;
				return;
			}
			else bot = jj;
		}
