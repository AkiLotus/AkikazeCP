...
void ProSolve() {
	i64 q; cin >> q;
	while (q--) {
		i64 x1, x2, y1, y2; cin >> x1 >> y1 >> x2 >> y2;
		i64 ans = LINF; //swap(x1, y1); swap(x2, y2);
		//cout << "(y2 - y1) % v != 0: " << ((y2 - y1) % v != 0) << endl;
		if (y1 <= y2) {
			if (cl > 0) {
				if (lower_bound(l.begin(), l.end(), y1) != l.end() && *lower_bound(l.begin(), l.end(), y1) < y2) {
					ans = min(ans, abs(x2-x1)+(y2-y1));
					//cout << "ans = " << ans << endl;
				}
				else {
					ans = min(ans, abs(x2-x1)+(y2-y1)+2*(*lower_bound(l.begin(), l.end(), y1) - y2));
					//cout << "ans = " << ans << endl;
					i64 z = lower_bound(l.begin(), l.end(), y1) - l.begin() - 1;
					if (z >= 0) ans = min(ans, abs(x2-x1)+(y2-y1)+2*(y1-l[z]));
					//cout << "ans = " << ans << endl;
				}
			}
			if (ce > 0) {
				if (lower_bound(e.begin(), e.end(), y1) != e.end() && *lower_bound(e.begin(), e.end(), y1) < y2) {
					ans = min(ans, abs(x2-x1)/v+(abs(x2-x1) % v != 0)+(y2-y1));
					//cout << "ans = " << ans << endl;
				}
				else {
//					cout << (abs(x2-x1)/v) << endl;
//					cout << ((abs(x2-x1) % v != 0)) << endl;
//					cout << (*lower_bound(e.begin(), e.end(), y1) - y2) << endl;
					ans = min(ans, abs(x2-x1)/v+(abs(x2-x1) % v != 0)+2*(*lower_bound(e.begin(), e.end(), y1) - y2)+(y2-y1));
					//cout << "ans = " << ans << endl;
					i64 z = lower_bound(e.begin(), e.end(), y1) - e.begin() - 1;
					if (z >= 0) ans = min(ans, abs(x2-x1)/v+(abs(x2-x1) % v != 0)+2*(y1-e[z])+(y2-y1));
					//cout << "ans = " << ans << endl;
				}
			}
		}
			...
			if (cl > 0) {
				if (lower_bound(l.begin(), l.end(), y2) != l.end() && *lower_bound(l.begin(), l.end(), y2) < y1) {
					ans = min(ans, abs(x2-x1)+(y1-y2));
					//cout << "ans = " << ans << endl;
				}
				else {
					ans = min(ans, abs(x2-x1)+(y1-y2)+2*(*lower_bound(l.begin(), l.end(), y2) - y1));
					//cout << "ans = " << ans << endl;
					i64 z = lower_bound(l.begin(), l.end(), y2) - l.begin() - 1;
					if (z >= 0) ans = min(ans, abs(x2-x1)+(y1-y2)+2*(y2-l[z]));
					//cout << "ans = " << ans << endl;
				}
			}
			if (ce > 0) {
				if (lower_bound(e.begin(), e.end(), y2) != e.end() && *lower_bound(e.begin(), e.end(), y2) < y1) {
					ans = min(ans, abs(x2-x1)/v+(abs(x2-x1) % v != 0)+(y1-y2));
					//cout << "ans = " << ans << endl;
				}
				else {
					ans = min(ans, abs(x2-x1)/v+(abs(x2-x1) % v != 0)+2*(*lower_bound(e.begin(), e.end(), y2) - y1)+(y1-y2));
					//cout << "ans = " << ans << endl;
					i64 z = lower_bound(e.begin(), e.end(), y2) - e.begin() - 1;
					if (z >= 0) ans = min(ans, abs(x2-x1)/v+(abs(x2-x1) % v != 0)+2*(y2-e[z])+(y1-y2));
					//cout << "ans = " << ans << endl;
				}
			}
		}
