...
void ProSolve() {
	...
	i64 coreX = r - (n - r + 1) + 1, coreY = r - (m - r + 1) + 1, ccX = (n - r + 1) * 2 - 1, ccY = (m - r + 1) * 2 - 1;
	i64 zX = coreX, zY = coreY;
	while (zX * zY < k) {
		zX += 2; zY += 2; ccX--; ccY--;
	}
	priority_queue<ppp> Heap;
	for (i64 i=coreX; i>=zX; i--) {
		for (i64 j=coreY; j>=zY; z--) {
			Heap.push(mp(i*j, mp(i, j)));
		}
	}
}
