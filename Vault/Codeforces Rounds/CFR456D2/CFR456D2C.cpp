...
i64 n, m, bounty, increase, damage;
vi maxHP, startHP, regen;
...
void VarInput() {
	...
	maxHP.resize(n); startHP = maxHP; regen = maxHP;
	for (i64 i=0; i<n; i++) cin >> maxHP[i] >> startHP[i] >> regenHP[i];
}

void ProSolve() {
	//cout << "Case " << ++cas << ": " << ans << endl; // for multi-testcase problems
	if (*max_element(maxHP.begin(), maxHP.end()) <= damage) {
		cout << "-1"; return;
	}
	i64 ans = -LINF;
}
