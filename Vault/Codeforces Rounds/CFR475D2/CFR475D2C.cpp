...
/** -----GLOBAL VARIABLES----- **/
i64 n, a, b, k; string s;
i64 periodValue = 0, unit = 0;
...
void ProSolve() {
	for (i64 i=0; i<k; i++) {
		if (s[i] == '+') {
			periodValue += (modPow(a, n-i) * modPow(b, i)) % MOD;
			periodValue %= MOD;
		}
		else if (s[i] == '-') {
			periodValue -= (modPow(a, n-i) * modPow(b, i)) % MOD;
			periodValue %= MOD; while (periodValue < 0) periodValue += MOD;
		}
	}
	unit = (modPow(modPow(a, k), MOD-2) * modPow(b, k)) % MOD;
	//cout << "periodValue = " << periodValue << endl;
	//cout << "unit = " << unit << endl;
	i64 ans = (periodValue * (modPow(unit, (n+1)/k) - 1)) % MOD; while (ans < 0) ans += MOD;
	ans = (ans * modPow((unit - 1 + MOD * MOD) % MOD, MOD-2)) % MOD;
	if (unit == 1) ans = (periodValue * ((n + 1) / k)) % MOD;
	cout << ans;
}
