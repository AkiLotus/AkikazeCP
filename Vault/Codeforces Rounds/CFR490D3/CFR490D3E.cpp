...
/** -----GLOBAL VARIABLES----- **/
i64 n, m, s, ans = 0; vector<vi> adj; vb vis, vis1;
...
void VarInput() {
	...
	cin >> n >> m >> s; adj.rsz(n);
	vis.rsz(n, false); vis1.rsz(n, false); s--;
	while (m--) {i64 u, v; cin >> u >> v; u--; v--; adj[u].pub(v);}
}
...
void ProSolve() {
	...
	for (i64 i=0; i<n; i++) {
		i64 tmp = (!vis[i]); if (i == s) continue;
		for (i64 i=0; i<n; i++) vis1[i] = false; DFS1(i);
		for (i64 i=0; i<n; i++) {
			if (i == s) continue;
			tmp += (!vis1[i] && !vis[i]);
		}
