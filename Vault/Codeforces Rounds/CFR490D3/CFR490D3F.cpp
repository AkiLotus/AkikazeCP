...
void VarInput() {
	...
	cin >> n >> k; c.rsz(n*k); f.rsz(n); h.rsz(k+1);
	for (i64 i=0; i<n*k; i++) {cin >> c[i]; cnt[c[i]]++;}
	for (i64 i=0; i<n; i++) {cin >> f[i]; Map[f[i]]++;}
	for (i64 i=1; i<=k; i++) cin >> h[i];
}
...
void ProSolve() {
	...
	sort(f.begin(), f.end());
	for (i64 q=0; q<n; q++) {
		if (q > 0 && f[q-1] == f[q]) continue;
		i64 N = Map[f[q]], M = cnt[f[q]];
		vector<vi> dp(N+1, vi(M+1, 0));
		for (i64 i=1; i<=N; i++) {
			for (i64 j=M; j>=0; j--) {
				for (i64 x=0; x<=min(k,j); x++) {
					dp[i][j-x] = max(dp[i][j-x], dp[i-1][j]+h[x]);
				}
			}
		}
		//for (i64 i=0; i<=N; i++) cout << dp[i] << endl;
		i64 tmp = -LINF;
		for (i64 i=0; i<=M; i++) tmp = max(tmp, dp[N][i]);
		ans += tmp;
	}
	cout << ans;
}
