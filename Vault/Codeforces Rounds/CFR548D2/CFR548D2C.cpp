...
/***********************************************
** File name: CFR548D2C.cpp                   **
** Time created: Wed Mar 27 2019 14:49pm      **
***********************************************/
...
void DFS(int z, vb &vis, int &cnt) {
	vis[z] = true; cnt++;
	for (auto t: adj[z]) {
		if (!vis[t]) DFS(t, vis, cnt);
	}
}
...
void Solve() {
	...
	for (int i=0; i<n; i++) {
		if (vis[i]) continue;
		int cnt = 0; DFS(i, vis, cnt);
		ans -= modPow(cnt, k); ans %= Mod;
		while (ans < 0) ans += Mod;
	}
	cout << ans << endl;
}
