...
/** -----GLOBAL VARIABLES----- **/
i64 n, a, b, baseAns = 0, ans = 0; vi h, d, diff;
...
void VarInput() {
	...
	cin >> n >> a >> b; h.rsz(n); d.rsz(n);
	for (i64 i=0; i<n; i++) {
		cin >> h[i] >> d[i]; baseAns += d[i];
		if (h[i] > d[i]) diff.pub(h[i] - d[i]);
	}
}
...
//	cout << diff << endl;
	i64 addable = 0, N = diff.size();
	for (i64 i=max(N-b, 0LL); i<N; i++) addable += diff[i];
	for (i64 i=0; i<n; i++) {
		i64 newHP = h[i] * pow2(a);
//		tracker2(i, newHP);
		if (newHP <= d[i]) continue;
		if (h[i] > d[i]) {
//			cout << "addable = " << addable << endl;
//			cout << "newHP - d_i = " << newHP - d[i] << endl;
//			cout << "h_i - d_i = " << h[i] - d[i] << endl;
			ans = max(ans, baseAns+addable+(newHP - d[i])-(h[i] - d[i]));
			continue;
		}
		if (b < N) ans = max(ans, baseAns+addable+(newHP - d[i]));
		if (newHP <= diff[0]) continue;
		ans = max(ans, baseAns+addable+(newHP - d[i])-diff[0]);
	}
	cout << ans;
}
