...
void ProSolve() {
	N = d[n-1] + 1; vi adj(N, 0); i64 e = 0;
	for (i64 i=0; i<n; i++) {adj[i] = d[i]; e += d[i];}
	for (i64 i=n; i<N; i++) {adj[i] = d[0]; e += d[0];}
	if (e % 2 != 0) {
		adj[N-1] = 0; e -= d[0];
		for (i64 i=0; i<n; i++) {
			if ((e + d[i]) % 2 == 0) {
	...
	for (i64 i=0; i<N; i++) {
		if (adj[i] == 0) continue;
		for (i64 j=i+1; j<N; j++) {
			if (adj[j] == 0 || adj[i] == 0) continue;
			ans.pub(mp(i,j)); adj[i]--; adj[j]--;
		}
	}
	cout << ans.size() << endl;
	for (i64 i=0; i<ans.size(); i++) cout << ans[i].fi+1 << " " << ans[i].se+1 << endl;
}
