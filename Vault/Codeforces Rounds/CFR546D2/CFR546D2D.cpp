...
/***********************************************
** File name: CFR546D2D.cpp                   **
** Time created: Mon Mar 11 2019 16:30pm      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
int n, m; vi p; vector<set<int>> adj;
/************************************************/
...
void Solve() {
	...
	for (int i=n-1; i>=0; i--) {
		int ptr = i;
		while (ptr+1 < n && adj[p[ptr]].find(p[ptr+1]) != adj[p[ptr]].end()) {
			swap(p[ptr], p[ptr+1]); ptr++;
		}
	}
	cout << ((n - 1) - (find(p.begin(), p.end(), Nastya) - p.begin())) << endl;
}
/*****************************************/
