...
/************** [GLOBAL VARIABLES] **************/
int n, m; vector<vi> a;
vector<vi> Row, Col; vector<set<int>> RowSet,ColSet;
/************************************************/
...
void Solve() {
	for (int i=0; i<n; i++) {
		for (auto it=RowSet[i].begin(); it!=RowSet[i].end(); it++) Row[i].pub(*it);
	}
	for (int j=0; j<m; j++) {
		for (auto it=ColSet[j].begin(); it!=ColSet[j].end(); it++) Col[j].pub(*it);
	}
	for (int i=0; i<n; i++) {
		for (int j=0; j<m; j++) {
			int LowerRow = lower_bound(Row[i].begin(), Row[i].end(), a[i][j]) - Row[i].begin();
			int LowerCol = lower_bound(Col[j].begin(), Col[j].end(), a[i][j]) - Col[j].begin();
			int UpperRow = Row[i].size() - 1 - LowerRow, UpperCol = Col[j].size() - 1 - LowerCol;
			cout << max(LowerRow, LowerCol) + 1 + max(UpperRow, UpperCol) << " ";
		}
		cout << endl;
	}
}
