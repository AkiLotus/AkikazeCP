...
/***********************************************
** File name: CFR545D1C.cpp                   **
** Time created: Fri Mar 8 2019 08:57am       **
***********************************************/
...
struct chash_pii {
    int operator()(pii x) const { return (x.fi + x.se) ^ RANDOM; }
};

int n, m, d; vector<vi> adj1, adj2; vector<vc> open;
map<int, int> SCC_ID; int SCCcnt = 0;
vector<vector<vector<pii>>> SCCstates_Member;
vector<vi> SCCstates_Count;
map<pii, pii> SCCstates; vi StatesCount;
/************************************************/

/************** [FUNCTIONS] **************/
void firstDFS(int z, vb &visited, stack<int> &s) {
	visited[z] = true;
	for (int i=0; i<adj1[z].size(); i++) {
		if (!visited[adj1[z][i]]) firstDFS(adj1[z][i], visited, s);
	}
	s.push(z);
}
 
void secondDFS(int z, vb &visited) {
	visited[z] = true; SCC_ID[z] = SCCcnt;
	for (int i=0; i<adj2[z].size(); i++) {
		if (!visited[adj2[z][i]]) secondDFS(adj2[z][i], visited);
	}
}

void SCC_Kosaraju() {
	vb vis(n, false); stack<int> Stack;
	for (int i=0; i<n; i++) {
		if (!vis[i]) firstDFS(i, vis, Stack);
	}
...
void DFS(int x, int y, int id1, int id2, vector<vb> &vis, vector<vi> &SCCstates_Count, vector<vector<vector<pii>>> &SCCstates_Member, set<int> &MuseumVisited) {
	vis[x][y] = true; SCCstates[mp(x,y)] = mp(id1, id2);
	SCCstates_Member[id1][id2].pub(mp(x, y));
	if (open[x][y] == '1') {
		if (MuseumVisited.find(x) == MuseumVisited.end()) {
			SCCstates_Count[id1][id2]++;
			MuseumVisited.insert(x);
		}
	}
	int q = (y + 1) % d;
	for (auto t: adj1[x]) {
		int sccID = SCC_ID[t];
		if (sccID != id1) continue;
		if (vis[t][q]) continue;
		DFS(t, q, id1, id2, vis, SCCstates_Count, SCCstates_Member, MuseumVisited);
	}
}

void SCC_Refinement() {
	SCCstates_Count.rsz(SCCcnt, vi(d+4, 0));
	SCCstates_Member.rsz(SCCcnt, vector<vector<pii>>(d+4));
	StatesCount.rsz(SCCcnt, 0);
	vector<vb> vis(n, vb(d, false));
	for (int i=0; i<n; i++) {
		int id = SCC_ID[i];
		for (int j=0; j<d; j++) {
			if (vis[i][j]) continue;
			set<int> MuseumVisited;
			int id1 = id, id2 = StatesCount[id];
			DFS(i, j, id1, id2, vis, SCCstates_Count, SCCstates_Member, MuseumVisited);
			StatesCount[id]++;
		}
	}
}

int Final_DFS(int x, int y, vector<vb> &vis, int ans) {
	vis[x][y] = true;
	int tmp = 0;
	for (auto Primal: SCCstates_Member[x][y]) {
		int x1 = Primal.fi, y1 = Primal.se;
		for (auto x2: adj1[x1]) {
			int y2 = (y1 + 1) % d;
			pii AdjState = SCCstates[mp(x2,y2)];
			if (AdjState == mp(x, y)) continue;
			int z = AdjState.fi, t = AdjState.se;
			if (vis[z][t]) continue;
			tmp = max(tmp, Final_DFS(z, t, vis, SCCstates_Count[z][t]));
		}
	}
	return (ans + tmp);
}
...
void Input() {
	cin >> n >> m >> d; open.rsz(n, vc(d, '0'));
	adj1.rsz(n); adj2.rsz(n);
	while (m--) {
		int u, v; cin >> u >> v; u--; v--;
		adj1[u].pub(v); adj2[v].pub(u);
	}
	for (int i=0; i<n; i++) {
		for (int j=0; j<d; j++) {
			cin >> open[i][j];
...
void Solve() {
	SCC_Kosaraju(); SCC_Refinement();
	vector<vb> vis(SCCcnt, vb(d+4, false));
	pii InitialState = SCCstates[mp(0,0)];
	int id1 = InitialState.fi, id2 = InitialState.se;
	cout << Final_DFS(id1, id2, vis, SCCstates_Count[id1][id2]) << endl;
}
/*****************************************/
