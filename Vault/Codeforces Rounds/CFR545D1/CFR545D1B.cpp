...
void Solve() {
	...
	if (cnt[0] < tcnt[0] || cnt[1] < tcnt[1]) {cout << s << endl; return;}
	cout << t; cnt[0] -= tcnt[0]; cnt[1] -= tcnt[1];
	int id = 0;
	for (int i=1; i<m; i++) {
		if (Z[i] + i == m) {id = i; break;}
	}
	string newToken = "";
	if (id == 0) newToken = t;
	else {
		tcnt.clear(); tcnt.rsz(2);
		for (int i=Z[id]; i<m; i++) {
			tcnt[0] += (t[i] == '0');
			tcnt[1] += (t[i] == '1');
		}
		newToken = t.substr(Z[id]);
	}
