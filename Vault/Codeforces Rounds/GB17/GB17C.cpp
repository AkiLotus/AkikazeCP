...
void VarInput() {
	...
	cin >> n >> r; x.resize(n); ans.resize(n);
...
void ProSolve() {
	...
	for (i64 i=1; i<n; i++) {
		...
		for (i64 j=0; j<i; j++) {
			...
			ld tmp = sqrt((ld)4*r*r - (ld)abs(x[i] - x[j])*abs(x[i] - x[j])) + ans[j];
			ans[i] = max(ans[i], tmp);
		}
	}
	for (i64 i=0; i<n; i++) cout << fixed << setprecision(9) << ans[i] << " "; cout << endl;
	for (i64 i=0; i<n; i++) printf("%lf ", ans[i]); cout << endl;
}
