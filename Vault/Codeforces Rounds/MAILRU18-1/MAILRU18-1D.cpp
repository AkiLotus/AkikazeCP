...
/************** [GLOBAL VARIABLES] **************/
i64 n, k; vi a, px; map<i64, i64> Map;
/************************************************/
...
void Input() {
	cin >> n >> k; a.rsz(n); px.rsz(n); Map[0]++;
	for (i64 i=0; i<n; i++) {
		cin >> a[i];
		if (i == 0) px[i] = a[i];
		else px[i] = px[i-1] ^ a[i];
		Map[min(px[i], comp(px[i]))]++;
	}
}
...
void Solve() {
	...
	for (auto it=Map.begin(); it!=Map.end(); it++) {
		i64 s = it->se, L = s/2, R = s / 2 + s % 2;
		ans -= L * (L - 1) / 2; ans -= R * (R - 1) / 2;
	}
	cout << ans;
}
/*****************************************/
