...
void Input() {
	cin >> n; l.rsz(n); r.rsz(n); A.rsz(n, vi(n));
	for (i64 i=0; i<n; i++) cin >> l[i];
	for (i64 i=0; i<n; i++) cin >> r[i];
}

void Solve() {
	i64 Last = 0; a.rsz(n, -1);
	for (i64 x=n; x>0; x--) {
		i64 add = 0, Left = 0;
		for (i64 i=0; i<n; i++) {
			if (a[i] > x) {Left++; continue;}
			if (l[i] + r[i] != Last) continue;
			if (l[i] != Left) {cout << "NO"; return;}
			a[i] = x; add++;
		}
		Last += add;
	}
	if (*min_element(a.begin(), a.end()) < 1) {cout << "NO"; return;}
	cout << "YES\n";
	for (auto x: a) cout << x << " ";
}
/*****************************************/
