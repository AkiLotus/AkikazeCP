...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi DSUpar; vector<vi> DSUlist;
/************************************************/
...
void DSUmerge(i64 x, i64 y) {
	...
	if (x == y) return;
	bool swapped = false;
	if (DSUlist[x].size() < DSUlist[y].size()) swap(x, y);
	DSUpar[y] = x; for (auto z: DSUlist[y]) DSUlist[x].pub(z);
}

void Input() {
	cin >> n; DSUpar.rsz(n, -1); DSUlist.rsz(n);
	for (i64 i=0; i<n; i++) DSUlist[i].pub(i);
}
...
void Solve() {
	...
	i64 id = 0;
	while (DSUlist[id].size() != n) id++;
	for (auto x: DSUlist[id]) cout << x+1 << " "; cout << endl;
}
/*****************************************/
