...
void Solve() {
	...
	vi Left, Right;
	if (n % 2 == 0) {
		Left.pub(a.back()); a.pop_back();
		Right.pub(a.back()); a.pop_back();
	}
	else {
		Left.pub(a.back()); Right.pub(a.back());
		a.pop_back();
	}
	while (!a.empty()) {
		Left.pub(a.back()); a.pop_back();
	...
	for (i64 i=Left.size()-1; i>=0; i--) cout << Left[i] << " ";
	for (i64 i=n%2; i<Right.size(); i++) cout << Right[i] << " ";
	cout << endl;
}
/*****************************************/
