...
/***********************************************
** File name: CFR541D2E.cpp                   **
** Time created: Sat Feb 23 2019 10:01am      **
***********************************************/
...
void Solve() {
	vi beautyGraph(26, 0);
	for (i64 i=0; i<p[0].size(); i++) {
		i64 j = i;
		while (j+1 < p[0].size() && p[0][j+1] == p[0][i]) j++;
		beautyGraph[p[0][i]-'a'] = max(beautyGraph[p[0][i]-'a'], j-i+1);
		i = j;
	}
	//tracker2(beautyGraph, 0);
	for (i64 id=1; id<n; id++) {
		if (*min_element(p[id].begin(), p[id].end()) == *max_element(p[id].begin(), p[id].end())) {
			i64 pos = p[id][0] - 'a';
			i64 largestBeauty = max((beautyGraph[pos] + 1) * ((i64)p[id].size() + 1) - 1, 1LL);
			for (i64 z=0; z<26; z++) {
				if (z == pos) beautyGraph[z] = largestBeauty;
				else beautyGraph[z] = (beautyGraph[z] > 0);
			}
			//tracker2(beautyGraph, id);
			continue;
		}
		vi newGraph(26, 0);
		for (i64 i=0; i<p[id].size(); i++) {
			i64 j = i;
			while (j+1 < p[id].size() && p[id][j+1] == p[id][i]) j++;
			newGraph[p[id][i]-'a'] = max(newGraph[p[id][i]-'a'], j-i+1);
			i = j;
		}
		i64 posPrefix = p[id][0] - 'a', posSuffix = p[id][p[id].size()-1] - 'a';
		i64 lenPrefix = 1, lenSuffix = 1;
		while (p[id][0 + lenPrefix] == p[id][0]) lenPrefix++;
		while (p[id][(i64)p[id].size() - 1 - lenSuffix] == p[id][p[id].size()-1]) lenSuffix++;
		vi tmpGraph = newGraph;
		if (posPrefix == posSuffix) {
			if (beautyGraph[posPrefix] > 0) tmpGraph[posPrefix] = max(tmpGraph[posPrefix], lenPrefix + 1 + lenSuffix);
		}
		else {
			if (beautyGraph[posPrefix] > 0) tmpGraph[posPrefix] = max(tmpGraph[posPrefix], lenPrefix + 1);
			if (beautyGraph[posSuffix] > 0) tmpGraph[posSuffix] = max(tmpGraph[posSuffix], lenSuffix + 1);
		}
		for (i64 z=0; z<26; z++) {
			if (beautyGraph[z] > 0) tmpGraph[z] = max(tmpGraph[z], 1LL);
		}
		beautyGraph = tmpGraph;
		//tracker2(beautyGraph, id);
	}
	cout << *max_element(beautyGraph.begin(), beautyGraph.end()) << endl;
}
/*****************************************/
