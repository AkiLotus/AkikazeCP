...
void VarInput() {
	...
	cin >> n >> k; r.rsz(n); qrlAdj.rsz(n);
	for (i64 i=0; i<n; i++) cin >> r[i];
	while (k--) {
		i64 x, y; cin >> x >> y; x--; y--;
		qrlAdj[x].pub(r[y]); qrlAdj[y].pub(r[x]);
	}
}

void ProSolve() {
	vi rc = r; sort(rc.begin(), rc.end());
	for (i64 i=0; i<n; i++) sort(qrlAdj[i].begin(), qrlAdj[i].end());
	for (i64 i=0; i<n; i++) {
		i64 shigher = lower_bound(rc.begin(), rc.end(), r[i]) - rc.begin();
		i64 sqhigher = lower_bound(qrlAdj[i].begin(), qrlAdj[i].end(), r[i]) - qrlAdj[i].begin();
		cout << (shigher - sqhigher) << " ";
	}
}
