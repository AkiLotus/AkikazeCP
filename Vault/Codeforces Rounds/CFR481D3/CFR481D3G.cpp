...
void VarInput() {
	...
	cin >> n >> m; given.rsz(m, -1); ans.rsz(n);
	exam.rsz(n, -1); prepleft.rsz(m, 0); examList.rsz(m);
	for (i64 i=0; i<m; i++) {
		cin >> s >> d >> c; s--; d--;
		prepleft[i] = c; given[i] = s; exam[d] = i;
		examList[i] = mp(d, i);
	}
}
...
void ProSolve() {
	...
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<m; j++) {
			if (given[j] == i) Set.insert(examList[j]);
		}
		if (exam[i] != -1 && Set.find(examList[exam[i]]) != Set.end()) Set.erase(examList[exam[i]]);
		if (exam[i] != -1) ans[i] = m+1;
		else if (Set.empty()) ans[i] = 0;
		else {
			i64 z = Set.begin()->second; prepleft[z]--; ans[i] = z+1;
			if (!prepleft[z]) Set.erase(Set.begin());
		}
		//cout << ans << endl;
	}
	for (i64 i=0; i<m; i++) {
		if (prepleft[i]) {cout << "-1"; return;}
	}
