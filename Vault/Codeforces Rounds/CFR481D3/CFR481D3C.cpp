...
void VarInput() {
	...
	cin >> n >> m; a.rsz(n); s.rsz(n); b.rsz(m);
	for (i64 i=0; i<n; i++) {
		cin >> a[i]; if (i == 0) s[i] = a[i];
		else s[i] = s[i-1] + a[i];
	}
...
void ProSolve() {
	for (i64 i=0; i<m; i++) {
		i64 dorm = lower_bound(s.begin(), s.end(), b[i]) - s.begin();
		if (dorm == 0) cout << "1 " << b[i] << endl;
		else cout << dorm+1 << " " << b[i] - s[dorm-1] << endl;
	}
}
