...
void ProSolve() {
	...
	map<i64, i64> Map; vi diff;
	for (i64 i=1; i<n; i++) Map[b[i]-b[i-1]]++;
	if (Map.size() > 48) {cout << "-1"; return;}
	if (Map.size() == 1) {cout << "0"; return;}
	for (auto it=Map.begin(); it!=Map.end(); it++) diff.pub(it->first);
	i64 ans = LINF;
	for (auto y: diff) {
		for (i64 q=-2; q<=2; q++) {
			i64 x = y+q; //tracker1(x);
			for (i64 z=-1; z<=1; z++) {
				vi a = b; a[0] = b[0] + z;
				for (i64 i=1; i<n; i++) a[i] = a[i-1] + x;
				bool flag = true; i64 tmp = 0;
				for (i64 i=0; i<n; i++) {
					if (a[i] == b[i]) continue;
					if (abs(a[i] - b[i]) < 2) tmp++;
