...
/** -----GLOBAL VARIABLES----- **/
string n; vi cnt(10, 0), ftrl(20, 1); i64 res = 0;

/** -----EXTENSIVE FUNCTIONS----- **/
void Try(i64 i, vi Cnt, i64 siz) {
	if (i == 10) {
		i64 ans = ftrl[siz], nonzerocnt = 0, nonzeroperm = 0;
		for (i64 i=0; i<10; i++) {ans /= ftrl[Cnt[i]]; nonzerocnt += ((i > 0) ? Cnt[i] : max(0LL, Cnt[i]-1));}
		if (Cnt[0] == 0) {res += ans; return;}
		nonzeroperm = ftrl[nonzerocnt] / ftrl[max(0LL, Cnt[0]-1)];
		for (i64 i=1; i<10; i++) nonzeroperm /= ftrl[Cnt[i]];
		res += (ans - nonzeroperm); return;
	}
	if (cnt[i] == 0) Try(i+1, Cnt, siz);
	for (i64 q=1; q<=cnt[i]; q++) {
		Cnt[i]++; Try(i+1, Cnt, siz+q);
	}
}
...
void VarInput() {
	...
	for (i64 i=1; i<20; i++) ftrl[i] = (ftrl[i-1] * i);
	cin >> n;
}

void ProSolve() {
	for (auto c: n) cnt[c-'0']++; vi Default(10, 0); Try(0, Default, 0);
	cout << res;
}
