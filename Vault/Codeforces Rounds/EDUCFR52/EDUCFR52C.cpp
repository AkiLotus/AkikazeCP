...
void Solve() {
	if (*min_element(h.begin(), h.end()) == *max_element(h.begin(), h.end())) {cout << "0"; return;}
	for (i64 i=200008; i>0; i--) hcnt[i-1] += hcnt[i];
	i64 ans = 0, tmp = 0;
	for (i64 i=200008; i>=0; i--) {
		tmp += hcnt[i];
		if (tmp > k || hcnt[i] == n) {
			ans++; tmp = hcnt[i];
			if (tmp == n) break;
