...
/************** [GLOBAL VARIABLES] **************/
i64 N; vector<vi> A; vector<pii> coor;
vector<vector<vector<vi>>> Dist;
i64 kx[] = {-2, -2, -1, -1, +1, +1, +2, +2};
i64 ky[] = {-1, +1, -2, +2, -2, +2, -1, +1};
// x == 0: knight
// x == 1: rook
// x == 2: bishop
/************************************************/
...
void ProcessDistance() {
	Dist.rsz(N*N+1, vector<vector<vi>>(3, vector<vi>(N*N+1, vi(3, LINF))));
	for (i64 x=0; x<3; x++) {
		for (i64 i=1; i<=N*N; i++) {
			queue<i64> Q; vb vis(N*N+1, false);
			Q.push(i); vis[i] = true; Dist[i][x][i][x] = 0;
			while (!Q.empty()) {
				i64 z = Q.front(); Q.pop();
				if (x == 0) {
					for (i64 R=0; R<8; R++) {
						if (coor[z].fi+kx[R] < 0 || coor[z].fi+kx[R] >= N) continue;
						if (coor[z].se+ky[R] < 0 || coor[z].se+ky[R] >= N) continue;
						if (vis[A[coor[z].fi+kx[R]][coor[z].se+ky[R]]]) continue;
						vis[A[coor[z].fi+kx[R]][coor[z].se+ky[R]]] = true;
						Dist[i][x][A[coor[z].fi+kx[R]][coor[z].se+ky[R]]][x] = Dist[i][x][z][x] + INF;
						Q.push(A[coor[z].fi+kx[R]][coor[z].se+ky[R]]);
					}
				}
				if (x == 1) {
					for (i64 R=0; R<N; R++) {
						if (vis[A[coor[z].fi][R]]) continue;
						vis[A[coor[z].fi][R]] = true; Dist[i][x][A[coor[z].fi][R]][x] = Dist[i][x][z][x] + INF; Q.push(A[coor[z].fi][R]);
					}
					for (i64 R=0; R<N; R++) {
						if (vis[A[R][coor[z].se]]) continue;
						vis[A[R][coor[z].se]] = true; Dist[i][x][A[R][coor[z].se]][x] = Dist[i][x][z][x] + INF; Q.push(A[R][coor[z].se]);
					}
				}
				if (x == 2) {
					for (i64 R=-N; R<=N; R++) {
						if (coor[z].fi+R < 0 || coor[z].fi+R >= N) continue;
						if (coor[z].se+R < 0 || coor[z].se+R >= N) continue;
						if (vis[A[coor[z].fi+R][coor[z].se+R]]) continue;
						vis[A[coor[z].fi+R][coor[z].se+R]] = true;
						Dist[i][x][A[coor[z].fi+R][coor[z].se+R]][x] = Dist[i][x][z][x] + INF;
						Q.push(A[coor[z].fi+R][coor[z].se+R]);
					}
					for (i64 R=-N; R<=N; R++) {
						if (coor[z].fi+R < 0 || coor[z].fi+R >= N) continue;
						if (coor[z].se-R < 0 || coor[z].se-R >= N) continue;
	...
	for (i64 i=1; i<=N*N; i++) {
		for (i64 x=0; x<3; x++) {
			for (i64 y=0; y<3; y++) {
				if (x == y) Dist[i][x][i][y] = 0;
				else Dist[i][x][i][y] = INF + 1;
			}
		}
	}
	for (i64 k1=1; k1<=N*N; k1++) {
		for (i64 k2=0; k2<3; k2++) {
			for (i64 i1=1; i1<=N*N; i1++) {
				for (i64 i2=0; i2<3; i2++) {
					for (i64 j1=1; j1<=N*N; j1++) {
						for (i64 j2=0; j2<3; j2++) {
...
void Solve() {
	ProcessDistance();
	vector<vi> dp(N*N+1, vi(3, LINF));
	for (i64 i=0; i<3; i++) dp[1][i] = 0;
	for (i64 i=2; i<=N*N; i++) {
		for (i64 j=0; j<3; j++) {
			for (i64 z=0; z<3; z++) {
				dp[i][j] = min(dp[i][j], dp[i-1][z] + Dist[i-1][z][i][j]);
			}
		}
	}
	i64 Res = *min_element(dp[N*N].begin(), dp[N*N].end());
	cout << Res / INF << " " << Res % INF << endl;
}
/*****************************************/
