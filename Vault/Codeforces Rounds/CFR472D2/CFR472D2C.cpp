...
void ProSolve() {
	for (i64 i=0; i<n; i++) {
		i64 pos = upper_bound(a.begin(), a.end(), U+a[i]) - a.begin() - 1;
		if (pos - i == 1 || a[pos] == a[i]) continue;
		ans = max(ans, (ld)(a[pos] - a[i+1])/(a[pos] - a[i]));
	}
	if (abs(ans + 1.0) < EPS) cout << "-1";
