...
/** -----GLOBAL VARIABLES----- **/
i64 a, m, n; vector<pii> seg, umb, opt;
...
void VarInput() {
	...
	cin >> a >> n >> m; seg.rsz(n); umb.rsz(m); opt.rsz(n);
	for (i64 i=0; i<n; i++) cin >> seg[i].fi >> seg[i].se;
	for (i64 i=0; i<m; i++) cin >> umb[i].fi >> umb[i].se;
}

void ProSolve() {
	i64 ans = 0;
	if (umb[0].fi > seg[0].fi) {cout << "-1"; return;}
	sort(seg.begin(), seg.end()); sort(umb.begin(), umb.end());
	for (i64 i=0; i<n; i++) {
		i64 tmp = LINF;
		for (i64 j=0; j<m; j++) {
			if (umb[j].fi > seg[i].fi) break;
			if (i == 0) {
				if (tmp > (seg[i].se - umb[j].fi) * umb[j].se) {
					tmp = (seg[i].se - umb[j].fi) * umb[j].se;
					opt[i] = umb[j];
				}
			}
			else if (umb[j].fi < seg[i-1].fi) {
				if (umb[j] != opt[i-1]) continue;
				if (tmp > (seg[i].se - seg[i-1].se - 1) * umb[j].se) {
					tmp = (seg[i].se - seg[i-1].se - 1) * umb[j].se;
					opt[i] = umb[j];
				}
			}
			else if (umb[j].fi > seg[i-1].se) {
				if (tmp > (seg[i].se - umb[j].fi) * umb[j].se) {
					tmp = (seg[i].se - umb[j].fi) * umb[j].se;
					opt[i] = umb[j];
				}
				...
				if (tmp > (seg[i].se - umb[j].fi) * umb[j].se - (seg[i-1].se - umb[j].fi) * (opt[i-1].se - umb[j].se)) {
					tmp = (seg[i].se - umb[j].fi) * umb[j].se - (seg[i-1].se - umb[j].fi) * (opt[i-1].se - umb[j].se);
					opt[i] = umb[j];
				}
			}
		}
