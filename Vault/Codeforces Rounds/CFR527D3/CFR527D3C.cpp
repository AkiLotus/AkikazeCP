...
/***********************************************
** File name: CFR527D3C.cpp                   **
** Time created: Tue Dec 18 2018 17:05pm      **
***********************************************/
...
i64 n; vector<psi> A; vc Z;
/************************************************/

/************** [FUNCTIONS] **************/
bool stringsort(psi x, psi y) {
	return (x.fi.size() < y.fi.size());
}

bool valid(string s, vector<psi> A) {
	for (i64 i=0; i<n*2-2; i+=2) {
		i64 len = i / 2 + 1;
		if (A[i].fi == s.substr(0, len) && A[i+1].fi == s.substr(n-len, len)) continue;
		if (A[i].fi == s.substr(n-len, len) && A[i+1].fi == s.substr(0, len)) continue;
		return false;
	}
	return true;
}
...
void Input() {
	cin >> n; A.rsz(n*2-2); Z.rsz(n*2-2);
	for (i64 i=0; i<n*2-2; i++) {
		cin >> A[i].fi; A[i].se = i;
	}
}
...
void Solve() {
	if (n == 2) {cout << "PS\n"; return;}
	sort(A.begin(), A.end(), stringsort);
	string s = "";
	if (A[n*2-4].fi.substr(1, n-2) == A[n*2-3].fi.substr(0, n-2)) {
		string s1 = ""; s1 = A[n*2-4].fi; s1 += A[n*2-3].fi.back();
		if (valid(s1, A)) {s = s1; Z[A[n*2-4].se] = 'S'; Z[A[n*2-3].se] = 'P';}
	}
	if (A[n*2-3].fi.substr(1, n-2) == A[n*2-4].fi.substr(0, n-2)) {
		string s1 = ""; s1 = A[n*2-3].fi; s1 += A[n*2-4].fi.back();
		if (valid(s1, A)) {s = s1; Z[A[n*2-4].se] = 'P'; Z[A[n*2-3].se] = 'S';}
	}
	for (i64 i=0; i<n*2-2; i+=2) {
		i64 id0 = A[i].se, id1 = A[i+1].se;
		i64 len = i / 2 + 1;
		if (A[i].fi == s.substr(0, len) && A[i+1].fi == s.substr(n-len, len)) {Z[id0] = 'P'; Z[id1] = 'S';}
		else {Z[id0] = 'S'; Z[id1] = 'P';}
	}
	for (auto z: Z) cout << z; cout << endl;
}
/*****************************************/
