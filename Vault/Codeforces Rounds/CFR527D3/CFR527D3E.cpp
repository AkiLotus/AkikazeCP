...
/***********************************************
** File name: CFR527D3E.cpp                   **
** Time created: Tue Dec 18 2018 15:18pm      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, u, v; vector<vi> adj; vb vis;
i64 centroid = -1, diameter = 0;
vector<pii> AdditionalEdges;
/************************************************/

/************** [FUNCTIONS] **************/
void DFS(i64 z, i64 last, vi &Dist, vi &Parent) {
	Parent[z] = last;
	if (last != -1) Dist[z] = Dist[last] + 1;
	for (auto t: adj[z]) {
		if (t == last) continue;
		DFS(t, z, Dist, Parent);
	}
...
void ProcessCentroid(i64 start, i64 &Centroid, i64 &Diameter) {
	queue<pii> Q; Q.push(mp(start, -1LL));
	while (!Q.empty()) {
		i64 z = Q.front().fi, last = Q.front().se;
		start = z; Q.pop(); vis[z] = true;
		for (auto t: adj[z]) {
			if (t == last) continue;
			Q.push(mp(t, z));
		}
	}
	vi Dist(n, 0), Parent(n, -1); DFS(start, -1, Dist, Parent);
	Diameter = *max_element(Dist.begin(), Dist.end());
	i64 MaximumRadius = Diameter / 2 + Diameter % 2;
	Centroid = max_element(Dist.begin(), Dist.end()) - Dist.begin();
	while (Dist[Centroid] > MaximumRadius) Centroid = Parent[Centroid];
	if (Diameter == 0) Centroid = start;
}

void Input() {
	cin >> n >> m; adj.rsz(n); vis.rsz(n, false);
	while (m--) {
		cin >> u >> v; u--; v--;
...
void Solve() {
	...
	for (i64 i=0; i<n; i++) {
		if (vis[i]) continue;
		i64 newcentroid = -1, newdiameter = 0;
		ProcessCentroid(i, newcentroid, newdiameter);
		Heap.push(mp(newdiameter, i));
	}
	
	vis.clear(); vis.rsz(n, false);
	
	while (!Heap.empty()) {
		i64 i = Heap.top().se; Heap.pop();
		if (vis[i]) continue;
		i64 newcentroid = -1, newdiameter = 0;
		ProcessCentroid(i, newcentroid, newdiameter);
		if (centroid != -1) {
			AdditionalEdges.pub(mp(centroid+1, newcentroid+1));
			adj[centroid].pub(newcentroid); adj[newcentroid].pub(centroid);
		}
		ProcessCentroid(i, centroid, diameter);
	}
	
	cout << diameter << endl;
	for (auto x: AdditionalEdges) {
		cout << x.fi << " ";
		cout << x.se << endl;
	}
}
