...
/***********************************************
** File name: CFR529D3D.cpp                   **
** Time created: Thu Dec 27 2018 15:44pm      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n; vector<pii> A; vi List; vector<set<i64>> adj;
/************************************************/
...
void Input() {
	cin >> n; A.rsz(n); adj.rsz(n+1);
	for (auto &Pair: A) cin >> Pair.fi >> Pair.se;
}

void Solve() {
	for (i64 i=1; i<n; i++) {
		adj[A[i].fi].insert(A[i].se);
		adj[A[i].se].insert(A[i].fi);
	}
	if (adj[1].find(A[0].se) != adj[1].end()) swap(A[0].fi, A[0].se);
	queue<i64> Q; Q.push(A[0].se); List.pub(A[0].se);
	while (!Q.empty()) {
		i64 z = Q.front(); Q.pop();
		if (adj[z].empty()) break;
		i64 t = *adj[z].begin();
		adj[z].erase(t); adj[t].erase(z);
		Q.push(t); List.pub(t);
	}
