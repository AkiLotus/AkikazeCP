...
i64 sumFrom1(i64 z) {
	z %= MOD;
	return (((((z + 1) * z) % MOD) * modPow(2, MOD-2)) % MOD);
}
...
void ProSolve() {
	if (k == 0) {cout << (x * 2) % MOD; return;}
	if (x == 0) {cout << "0"; return;}
	i64 mx = ((x % MOD) * modPow(2LL, k)) % MOD, mn = mx;
	mn -= modPow(2LL, k); while (mn < 0) mn += MOD;
	i64 ans = (sumFrom1(mx) - sumFrom1(mn)) % MOD;
	while (ans < 0) ans += MOD;
	ans *= modPow(modPow(2LL, k-1), MOD-2); ans %= MOD;
	cout << ans;
}
