...
void VarInput() {
	...
	cin >> n; w.rsz(n);
	for (i64 i=0; i<n; i++) cin >> w[i]; cin >> s;
}

void ProSolve() {
	priority_queue<pii> notOcc, halfOcc;
	for (i64 i=0; i<n; i++) notOcc.push(mp(-w[i], i+1));
	for (i64 i=0; i<2*n; i++) {
		if (s[i] == '0') {
			pii z = notOcc.top(); notOcc.pop();
			z.fi = -z.fi; halfOcc.push(z);
			cout << z.se << " ";
		}
		else {
			pii z = halfOcc.top(); halfOcc.pop();
			cout << z.se << " ";
		}
	}
}
