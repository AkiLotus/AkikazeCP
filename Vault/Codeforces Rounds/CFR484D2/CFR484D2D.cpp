...
void ProSolve() {
	i64 ans = *max_element(a.begin(), a.end())+1;
	vi distinctVal;
	for (auto it=Map.begin(); it!=Map.end(); it++) distinctVal.pub(-(it->first));
	sort(distinctVal.begin(), distinctVal.end());
	i64 maxLocation = -LINF;
	set<pii> segment; unordered_map<i64, i64> Size;
	segment.insert(mp(0, n-1)); Size[n]++;
	for (auto it=Location.begin(); it!=Location.end(); it++) {
		i64 z = -(it->first); vi List = it->second;
		//tracker1(z);
		for (auto t: List) {
			auto ite = segment.upper_bound(mp(t+1, 0LL)); ite--;
			pii seg = *ite; Size[seg.se-seg.fi+1]--; segment.erase(seg);
			i64 st = seg.fi, en = seg.se;
			if (Size[en-st+1] == 0) Size.erase(en-st+1);
			//cout << "removed " << st << "," << en << endl;
			if (st < t) {
				segment.insert(mp(st, t-1)); Size[t-1-st+1]++;
				//cout << "inserted " << st << "," << t-1 << endl;
			}
			if (t < en) {
				segment.insert(mp(t+1, en)); Size[en-t-1+1]++;
				//cout << "inserted " << t+1 << "," << en << endl;
			}
		}
		//cout << "Size = " << Size.size() << endl;
		//cout << "Location = " << segment.size() << endl;
		if (Size.size() == 1 && segment.size() >= maxLocation) {
			i64 opos = lower_bound(distinctVal.begin(), distinctVal.end(), z) - distinctVal.begin() - 1;
			if (opos < 0) z = 0; else z = distinctVal[opos]+1;
			if (segment.size() > maxLocation) {ans = z; maxLocation = segment.size();}
			else ans = min(ans, z);
		}
		//cout << "-------------------\n";
	}
