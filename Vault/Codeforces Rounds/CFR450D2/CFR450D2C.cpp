...
void ProSolve() {
	...
	last = -1;
	if (seq1.size() > 1) {
		for (i64 i=seq1[seq1.size()-2]; i<n; i++) {
			if (last < p[i] && p[i] != n) {
				seq3.pub(i);
				last = p[i];
			}
	...
	//cout << seq2.size() << endl;
	//cout << seq3.size() << endl;
	if (seq1.size() == n) cout << 1;
	else if (seq1.size() > seq2.size() && seq3.size() == 1) {
		vector<i64> visited(n, false);
		for (i64 i=0; i<seq1.size(); i++) visited[seq1[i]] = true;
		i64 ans = LINF;
		for (i64 i=0; i<n; i++) {
			if (!visited[i]) ans = min(ans, p[i]);
		}
		cout << ans;
	}
	else if (seq1.size() + seq3.size() - 1 > seq2.size()) cout << n;
	else if (seq1.size() < seq2.size()) cout << p[0];
	else {
		vector<i64> visited(n, false);
		for (i64 i=0; i<seq1.size(); i++) visited[seq1[i]] = true;
		i64 ans = LINF;
