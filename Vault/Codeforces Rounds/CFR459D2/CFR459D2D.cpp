...
i64 n, m; vector<vector<pic>> adj(111, vector<pic>(0));
vector<vvi> dp(111, vvi(111, vi(26, 0)));

/** -----EXTENSIVE FUNCTIONS----- **/
i64 calculate(i64 a, i64 b, i64 c) {
	if (dp[a][b][c] > 0) return dp[a][b][c];
	dp[a][b][c] = 2;
	for (i64 i=0; i<adj[a].size(); i++) {
		if (adj[a][i].second-'a' < c) continue;
		if (calculate(b, adj[a][i].first, adj[a][i].second-'a') == 2) {
			dp[a][b][c] = 1; return 1;
		}
	}
	return 2;
