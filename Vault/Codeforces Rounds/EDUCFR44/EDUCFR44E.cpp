...
void ProSolve() {
	sort(a.begin(), a.end());
	i64 last = -k, safe = -1;
	queue<pii> cachedsafe;
	for (i64 i=0; i<n; i++) {
		tracker1(i);
		cout << "last/safe = " << last << "/" << safe << endl;
		if (last + k - 1 >= i) continue;
		REcheck;
		if (i + k > n) {
			cout << "NO"; return;
		}
		i64 pos = upper_bound(a.begin(), a.end(), a[i]+d) - a.begin();
		tracker2(i, pos);
		if (pos - i < k) {
			if (!cachedsafe.empty() && i <= cachedsafe.front().fi) {
				last = cachedsafe.front().se; safe = cachedsafe.front().fi-1;
				while (!cachedsafe.empty() && cachedsafe.front().se <= last) cachedsafe.pop();
				continue;
			}
			cout << "NO"; return;
		}
		if (i > safe) {last = i; safe = pos-1;} else cachedsafe.push(mp(pos, i));
		if (safe >= n-1 || (!cachedsafe.empty() && cachedsafe.back().fi >= n-1)) break;
	}
	cout << "YES";
}
