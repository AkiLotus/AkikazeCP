...
void VarInput() {
	...
	cin >> n >> k >> l; a.rsz(n*k);
	for (i64 i=0; i<n*k; i++) cin >> a[i];
}

void ProSolve() {
	sort(a.begin(), a.end()); i64 ans = 0;
	if (a[n-1] - a[0] > l) {cout << ans; return;}
	i64 pos = upper_bound(a.begin(), a.end(), a[0]+l) - a.begin();
	for (i64 i=pos-n; i<pos; i++) ans += a[i];
	i64 pt1 = pos - n, required = 0;
	while (pt1 > required) {
		ans -= a[pt1]; ans += a[required];
		pt1++; required += k;
	}
	cout << ans;
}
