...
void VarInput() {
	...
	cin >> n >> m; a.rsz(n, vector<char>(m, '0'));
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<m; j++) {
			cin >> a[i][j];
		}
...
void ProSolve() {
	vector<vi> cnt(n, vi(m, 0)); vi cc(m, 0);
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<m; j++) {
			cnt[i][j] += (a[i][j] == '1');
			cc[j] += (a[i][j] == '1');
	...
	for (i64 i=0; i<n; i++) {
		vi tmp = cc; bool flag = true;
		for (i64 j=0; j<m; j++) tmp[j] -= cnt[i][j];
		for (i64 j=0; j<m; j++) if (tmp[j] == 0) {flag = false; break;}
