...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, x, y, qv, qd, qx; vector<vi> adj;
vi RemoveEffect(444444, 0);
i64 accumulated = 0;
vector<map<i64, i64>> Map; vi val;
/************************************************/

/************** [FUNCTIONS] **************/
void DFS(i64 z, i64 last, i64 depth) {
	for (auto it=Map[z].begin(); it!=Map[z].end(); it++) {
		i64 k = it->fi, v = it->se;
		accumulated += v;
		if (depth+k+1 < 444444) {RemoveEffect[depth+k+1] += v;}
	}
	accumulated -= RemoveEffect[depth];
	val[z] += accumulated;
	for (auto t: adj[z]) {
		if (t == last) continue;
		DFS(t, z, depth+1);
...
void Input() {
	cin >> n; adj.rsz(n); Map.rsz(n); val.rsz(n, 0);
	for (i64 i=1; i<n; i++) {
		cin >> x >> y; x--; y--;
