...
/************** [FUNCTIONS] **************/
bool solvable(i64 Table, i64 Formula, i64 First) {
	if (First == 0) {
		if (Formula > (Table - 1) * k) return false;
		if (Table > (Formula + 1) * k) return false;
		return true;
	}
	if (First == 1) {
		if (Table > (Formula - 1) * k) return false;
...
void Input() {
	cin >> n >> k; x.rsz(n); y.rsz(n);
	for (i64 i=0; i<n; i++) cin >> x[i];
	for (i64 i=0; i<n; i++) cin >> y[i];
}

void Solve() {
	vi MinCrawlT(n, LINF), MinCrawlF(n, LINF);
	MinCrawlT[0] = 0; MinCrawlF[0] = 0;
	for (i64 i=0; i<n; i++) {
		if (!solvable(x[i]-MinCrawlT[i], y[i], 1) && !solvable(x[i], y[i]-MinCrawlF[i], 0)) {
			cout << "NO"; return;
		}
	}
	cout << "YES";
