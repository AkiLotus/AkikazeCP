...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, k, u, v, w; vector<vector<pii>> adj; map<ppi, i64> Map;
/************************************************/
...
void Input() {
	...
	for (i64 i=1; i<=m; i++) {
		cin >> u >> v >> w; u--; v--;
		Map[mp(mp(u,v),w)] = i; Map[mp(mp(v,u),w)] = i;
		adj[u].pub(mp(v,w)); adj[v].pub(mp(u,w));
	}
}

void Solve() {
	vi List; priority_queue<pii> Heap; vi Dist(n, LINF), Last(n, -1);
	vb vis(n, false); Dist[0] = 0; Heap.push(mp(-Dist[0], 0LL));
	while (!Heap.empty() && List.size() < k) {
		pii Z = Heap.top(); Heap.pop();
		i64 z = Z.se, d = -Z.fi;
		if (vis[z]) continue;
		vis[z] = true;
		if (Last[z] != -1) List.pub(Last[z]);
		for (auto T: adj[z]) {
			i64 t = T.fi, p = T.se;
			if (vis[t]) continue;
			if (d + p >= Dist[t]) continue;
			i64 id = Map[mp(mp(z,t),p)];
			Dist[t] = d + p; Last[t] = id;
