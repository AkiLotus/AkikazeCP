...
void Input() {
	cin >> n >> m; x.rsz(n); x1.rsz(m);
	x2.rsz(m); y.rsz(m);
	for (i64 i=0; i<n; i++) cin >> x[i];
	for (i64 i=0; i<m; i++) cin >> x1[i] >> x2[i] >> y[i];
}

void Solve() {
	sort(x.begin(), x.end()); x.pub(INF); vi ps(n+1, 0);
	for (i64 i=0; i<m; i++) {
		if (x1[i] != 1) continue;
		i64 id = upper_bound(x.begin(), x.end(), x2[i]) - x.begin() - 1;
		if (id < 0) continue; ps[0]++; if (id < n) ps[id+1]--;
	}
	for (i64 i=1; i<=n; i++) ps[i] += ps[i-1];
	i64 ans = LINF;
	for (i64 i=0; i<=n; i++) ans = min(ans, ps[i] + i);
	cout << ans;
}
/*****************************************/
