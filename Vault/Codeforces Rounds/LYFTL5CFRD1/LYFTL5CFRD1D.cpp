...
/************** [GLOBAL VARIABLES] **************/
i64 q, t, l, r, x, last = 0; map<i64, i64> DSUpar, DSUsize, DSUparpath;
/************************************************/

/************** [FUNCTIONS] **************/
i64 DSUfind(i64 z) {
	while (DSUpar.find(z) != DSUpar.end()) z = DSUpar[z];
	return z;
}

i64 dist(i64 L, i64 R) {
	i64 ans = 0;
	while (DSUpar.find(L) != DSUpar.end()) {ans ^= DSUparpath[L]; L = DSUpar[L];}
	while (DSUpar.find(R) != DSUpar.end()) {ans ^= DSUparpath[R]; R = DSUpar[R];}
	return ans;
}
...
void DSUmerge(i64 X, i64 Y, i64 d) {
	...
	if (DSUsize.find(a) == DSUsize.end()) DSUsize[a] = 1;
	if (DSUsize.find(b) == DSUsize.end()) DSUsize[b] = 1;
	if (DSUsize[a] < DSUsize[b]) swap(a, b);
	DSUsize[a] += DSUsize[b]; DSUpar[b] = a; DSUparpath[b] = d;
}
...
void Solve() {
	while (q--) {
		...
		if (t == 1) {
			cin >> x; x ^= last;
			if (DSUfind(l) == DSUfind(r+1) && dist(l, r+1) != x) continue;
			DSUmerge(l, r+1, x);
		}
		else if (t == 2) {
			if (DSUfind(l) != DSUfind(r+1)) {last = 1; cout << "-1\n"; continue;}
