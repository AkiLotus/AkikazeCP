...
i64 N, q, maxNode = -1;
vector<i64> A(666666, 0);
vector<i64> leafPos(666666, 0);
vector<i64> tree(6000007, 0);
...
void ST_Build(i64 n, i64 s, i64 e) {
	...
	if (s > e) return;
	if (s == e) {
		tree[n] = A[s]; leafPos[s] = n;
		maxNode = max(maxNode, n); return;
	}
	ST_Build(left(n), s, (s+e)/2);
	ST_Build(right(n), (s+e)/2+1, e);
	tree[n] = __gcd(tree[left(n)], tree[right(n)]);
}

i64 Query(i64 n, i64 L, i64 R, i64 s, i64 e, i64 val, i64 tmp) {
	//tracker2(L,R);
	//cout << "tmp = " << tmp << endl;
	if (L > R || L > e || R < s) return tmp;
	if (tmp > 1) return INF;
	if (L >= s && R <= e) {
		if (L == R) {
			if (tree[n] % val != 0) return (tmp+1);
			else return tmp;
		}
		if (tree[left(n)] % val != 0) {
			tmp = Query(left(n), L, (L+R)/2, s, e, val, tmp);
			if (tmp > 1) return INF;
		}
		if (tree[right(n)] % val != 0) {
			tmp = Query(right(n), (L+R)/2+1, R, s, e, val, tmp);
			if (tmp > 1) return INF;
		}
		return tmp;
	}
...
void IncUpdate(i64 i, i64 x) {
	tree[i] = x;
	if (i == 1) return;
	IncUpdate(i/2, __gcd(tree[i], tree[brother(i)]));
}

/** -----COMPULSORY FUNCTIONS----- **/
void VarInput() {
	//cin >> T; // for multi-testcase problems
	cin >> N; for (i64 i=1; i<=N; i++) cin >> A[i];
	ST_Build(1, 1, N); cin >> q;
	//for (i64 i=1; i<=maxNode; i++) cout << tree[i] << " "; cout << endl;
}
...
void ProSolve() {
	while (q--) {
		...
		if (cmd == 2) {
			i64 i, x; cin >> i >> x;
			IncUpdate(leafPos[i], x);
			//for (i64 i=1; i<=maxNode; i++) cout << tree[i] << " "; cout << endl;
		}
		else {
			i64 l, r, x; cin >> l >> r >> x;
			i64 ans = Query(1, 1, N, l, r, x, 0);
			//cout << "ans = " << ans << endl;
			if (ans <= 1) cout << "YES\n";
			else cout << "NO\n";
		}
