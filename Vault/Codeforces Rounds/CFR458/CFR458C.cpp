...
string n; i64 k, ans = 0;
vi dp(1009, LINF), ftrl(1009, 1);
...
i64 nCr(i64 n, i64 r) {
	i64 tmp1 = ftrl[n];
	i64 tmp2 = (ftrl[n-r] * ftrl[r]) % MOD;
	return (tmp1 * modPow(tmp2, MOD-2));
}

/** -----COMPULSORY FUNCTIONS----- **/
void VarInput() {
	//cin >> T; // for multi-testcase problems
	for (i64 i=1; i<1009; i++) ftrl[i] = (ftrl[i-1] * i) % MOD;
	cin >> n >> k; dp[1] = 0;
}
...
void ProSolve() {
	...
	i64 len = n.size(); vb used(1009, false);
	for (i64 i=2; i<1009; i++) {
		i64 tmp = cntbit(i);
		dp[i] = min(dp[i], dp[tmp] + 1);
	}
//	for (i64 i=0; i<1009; i++) {
//		if (dp[i] == k-1) cout << i << " ";
//	}
//	cout << endl;
//	for (i64 i=0; i<1009; i++) {
...
//	cout << endl;
	for (i64 i=1; i<len; i++) {
		...
		//cout << "Founded: required more " << i << endl;
		ans = (ans + nCr(len-1, i)) % MOD;
		if (i == 1) ans = (ans - 1) % MOD;
		while (ans < 0) ans += MOD;
		//cout << "ans = " << ans << endl;
	}
	i64 bitCount = 0;
	for (i64 i=0; i<len; i++) bitCount += (n[i] - '0');
	if (dp[bitCount] == k-1 && (k > 1 || (k == 1 && len > 1))) {
		ans = (ans + 1) % MOD; used[bitCount] = true;
	}
	//cout << "ans = " << ans << endl;
	for (i64 i=len-1; i>0; i--) {
		...
		for (i64 j=0; j<=len-1-i; j++) {
			//tracker2(bitCount, j);
			if (dp[bitCount+j] != k-1) continue;
			//cout << "Founded: required more " << j << endl;
			ans = (ans + nCr(len-1-i, j)) % MOD;
			if (used[bitCount+j] && n[i] == '0') ans = (ans - 1) % MOD;
			while (ans < 0) ans += MOD;
			//cout << "ans = " << ans << endl;
		}
	}
