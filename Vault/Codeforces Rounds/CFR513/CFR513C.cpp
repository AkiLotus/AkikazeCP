...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, X, ans = 0, tmp = 0, MinL = LINF; vi a, b;
/************************************************/
...
void binary_search(i64 i, i64 top, i64 bot) {
	if (top > bot) return;
	i64 mid = (top + bot) / 2;
	if (calc(mid) * MinL <= X) {tmp = mid; binary_search(i, mid+1, bot);}
	else binary_search(i, top, mid-1);
}
