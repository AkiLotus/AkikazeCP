...
/***********************************************
** File name: CFR528D1B.cpp                   **
** Time created: Sun Dec 23 2018 13:15pm      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n, s, a, b; vector<vi> adj;
i64 branch = 1, split = 0;
/************************************************/
...
void DFS(i64 z, i64 last) {
	...
	if (children > 1) {
		if (!split) branch += children;
		else branch += (children - 1);
		split++;
	}
}
...
void Solve() {
	queue<i64> Q; vi Dist(n, LINF);
	Q.push(0); Dist[0] = 0;
	while (!Q.empty()) {
		i64 z = Q.front(); Q.pop();
		for (auto t: adj[z]) {
			if (Dist[t] != LINF) continue;
			Dist[t] = Dist[z] + 1; Q.push(t);
		}
	}
	i64 root = max_element(Dist.begin(), Dist.end()) - Dist.begin();
	DFS(root, -1);
	cout << fixed << setprecision(9);
	cout << ((ld)s / branch * ((split > 0) + 1)) << endl;
}
/*****************************************/
