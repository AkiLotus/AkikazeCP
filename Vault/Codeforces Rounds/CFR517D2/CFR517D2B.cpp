...
void Try(i64 id, i64 val) {
	t[id] = val;
	if (id == 0) return;
	for (i64 x=0; x<4; x++) {
		if ((x | val) == a[id-1] && (x & val) == b[id-1]) {
			Try(id-1, x);
		}
	}
}
...
void Input() {
	cin >> n; a.rsz(n); b.rsz(n); t.rsz(n, -1);
	for (i64 i=0; i<n-1; i++) cin >> a[i];
	for (i64 i=0; i<n-1; i++) cin >> b[i];
}

void Solve() {
	for (i64 i=0; i<4; i++) {
		Try(n-1, i);
		if (invalid()) {for (i64 x=0; x<n; x++) t[x] = -1; continue;}
		cout << "YES\n"; for (auto x: t) cout << x << " "; return;
	}
	cout << "NO";
}
/*****************************************/
