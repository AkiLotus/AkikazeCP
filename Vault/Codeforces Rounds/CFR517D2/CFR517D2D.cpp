...
void Solve() {
	...
	vector<vi> dist(n, vi(n, LINF));
	deque<pii> Q; Q.push_front(mp(0LL, 0LL));
	dist[0][0] = (a[0][0] != 'a');
	while (!Q.empty()) {
		pii z = Q.front(); Q.pop_front();
		i64 i = z.fi, j = z.se;
		if (i+1 < n && !vis[i+1][j]) {
			vis[i+1][j] = true;
			dist[i+1][j] = dist[i][j] + (a[i+1][j] != 'a');
			if (a[i+1][j] == 'a') Q.push_front(mp(i+1, j));
			else Q.push_back(mp(i+1, j));
		}
		if (j+1 < n && !vis[i][j+1]) {
			vis[i][j+1] = true;
			dist[i][j+1] = dist[i][j] + (a[i][j+1] != 'a');
			if (a[i][j+1] == 'a') Q.push_front(mp(i, j+1));
			else Q.push_back(mp(i, j+1));
		}
	}
	i64 id = n * 2 - 2; vector<vector<pii>> Coor(26);
	vc s(n*2-1, 'a'); vis.clear(); vis.rsz(n, vb(n, false));
	if (k == 0) {
		id = 0; s[0] = a[0][0];
		vis[0][0] = true;
		Q.push_front(mp(0LL, 0LL));
	}
	else {
		while (id >= 0 && Q.empty()) {
			for (i64 i=0; i<n; i++) {
	...
	while (!Q.empty()) {
		Coor.clear(); Coor.rsz(26);
		i64 ID = Q.front().fi + Q.front().se;
		while (!Q.empty()) {
			pii z = Q.front(); Q.pop_front();
			i64 i = z.fi, j = z.se;
			if (i+1 < n && !vis[i+1][j]) {vis[i+1][j] = true; Coor[a[i+1][j]-'a'].pub(mp(i+1, j));}
			if (j+1 < n && !vis[i][j+1]) {vis[i][j+1] = true; Coor[a[i][j+1]-'a'].pub(mp(i, j+1));}
		}
		for (i64 x=0; x<26; x++) {
			if (Coor[x].empty()) continue;
			for (auto coor: Coor[x]) {
				Q.push_front(coor);
			}
			s[ID+1] = char(97+x); break;
		}
	}
	for (auto z: s) cout << z;
}
