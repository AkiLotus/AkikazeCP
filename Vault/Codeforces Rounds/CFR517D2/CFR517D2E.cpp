...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi a; set<i64> Ichi; vector<pii> A;
/************************************************/
...
void Solve() {
	for (i64 i=0; i<n-2; i++) {
		...
		if (Ichi.size() > 1) {
			i64 x = *Ichi.begin();
			i64 y = *Ichi.upper_bound(x);
			if (y + (y - x) <= n) {
				a[x-1] = 0; a[y-1] = 0;
				Ichi.erase(x); Ichi.erase(y);
				i64 z = y + (y - x);
				if (a[z-1]) Ichi.erase(z); else Ichi.insert(z);
				a[z-1] = 1 - a[z-1]; A.pub(mp(x, y));
			}
			else {
				for (i64 q=-1; q<=1; q++) {
					if (a[x+q]) Ichi.erase(x+q+1);
					else Ichi.insert(x+q+1);
					a[x+q] = 1 - a[x+q];
				}
				A.pub(mp(x, x+1));
			}
			...
			i64 x = *Ichi.begin();
			Ichi.erase(x); Ichi.insert(x+1); Ichi.insert(x+2);
			a[x-1] = 0; a[x-0] = 1; a[x+1] = 1; A.pub(mp(x, x+1));
		}
	}
	if (!Ichi.empty()) {cout << "NO"; return;}
	cout << "YES\n" << A.size() << endl;
	for (auto p: A) {
		i64 x = p.fi, y = p.se, z = y + (y - x);
		cout << x << " " << y << " " << z << endl;
	}
}
/*****************************************/
