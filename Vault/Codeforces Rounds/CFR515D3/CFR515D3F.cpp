...
/************** [GLOBAL VARIABLES] **************/
i64 n, N, x, y; vi A; map<i64, pii> Top, Bot;
/************************************************/
...
void Input() {
	cin >> n; Top[0] = mp(0LL, 0LL); Bot[0] = mp(0LL, 0LL);
	for (i64 i=0; i<n; i++) {
		cin >> x >> y; i64 lvl = max(x, y);
		if (Top.find(lvl) == Top.end()) {
			Top[lvl] = mp(x, -y);
			Bot[lvl] = mp(x, -y);
		}
		else {
			Top[lvl] = min(Top[lvl], mp(x, -y));
			Bot[lvl] = max(Bot[lvl], mp(x, -y));
		}
	}
	for (auto it=Top.begin(); it!=Top.end(); it++) A.pub(it->fi);
}

void Solve() {
	vi dp(2, 0), Tmp(2, LINF); dp[0] = 0; dp[1] = 0; N = A.size();
	for (i64 i=1; i<N; i++) {
		i64 x0T = Top[A[i-1]].fi, y0T = Top[A[i-1]].se, x0B = Bot[A[i-1]].fi, y0B = Bot[A[i-1]].se;
		i64 x1T = Top[A[i]].fi, y1T = Top[A[i]].se, x1B = Bot[A[i]].fi, y1B = Bot[A[i]].se;
		Tmp[0] = min(Tmp[0], dp[0]+abs(x0B-x1T)+abs(y0B-y1T)+abs(x1T-x1B)+abs(y1T-y1B));
		Tmp[1] = min(Tmp[1], dp[0]+abs(x0B-x1B)+abs(y0B-y1B)+abs(x1B-x1T)+abs(y1B-y1T));
		Tmp[0] = min(Tmp[0], dp[1]+abs(x0T-x1T)+abs(y0T-y1T)+abs(x1T-x1B)+abs(y1T-y1B));
		Tmp[1] = min(Tmp[1], dp[1]+abs(x0T-x1B)+abs(y0T-y1B)+abs(x1B-x1T)+abs(y1B-y1T));
		dp[0] = Tmp[0]; dp[1] = Tmp[1]; Tmp[0] = LINF; Tmp[1] = LINF;
	}
	cout << min(dp[0], dp[1]);
}
/*****************************************/
