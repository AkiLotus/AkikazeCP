...
/************** [FUNCTIONS] **************/
bool packable(i64 x) {
	i64 removed = n - x, boxes = 1, Rem = k, id = removed;
	for (; id<n; id++) {
		if (Rem >= a[id]) {Rem -= a[id];}
		else {Rem = k - a[id]; boxes++;}
	}
	return (boxes <= m);
}
...
i64 binary_search(i64 top, i64 bot) {
	if (top > bot) return 0;
	i64 mid = (top + bot) / 2;
	if (packable(mid)) return max(mid, binary_search(mid+1, bot));
	return binary_search(top, mid-1);
}
