...
void ProSolve() {
    vector<point> a, b, c;
    for (i64 i=0; i<4; i++) {
        point pts; cin >> pts.x >> pts.y; a.pub(pts); c.pub(pts);
    }
    for (i64 i=0; i<4; i++) {
        point pts; cin >> pts.x >> pts.y; b.pub(pts); c.pub(pts);
    }
    i64 mnXa = 1e9, mxXa = -1e9, mnYa = 1e9, mxYa = -1e9;
    i64 mnXb = 1e9, mxXb = -1e9, mnYb = 1e9, mxYb = -1e9;
    for (i64 i=0; i<4; i++) {
    	if (mnXa > a[i].x) mnXa = a[i].x;
    	if (mxXa < a[i].x) mxXa = a[i].x;
    	if (mnYa > a[i].y) mnYa = a[i].y;
    	if (mxYa < a[i].y) mxYa = a[i].y;
    	if (mnXb > b[i].x) mnXb = b[i].x;
    	if (mxXb < b[i].x) mxXb = b[i].x;
    	if (mnYb > b[i].y) mnYb = b[i].y;
    	if (mxYb < b[i].y) mxYb = b[i].y;
    }
    i64 lenx = mxXb - mnXb, leny = mxYb - mnYb;
    for (i64 i=mnXb; i<=mxXb; i++) {
    	if (i < mnXa || i > mxXa) continue;
    	i64 position = (mnYb + mxYb) / 2;
    	i64 rangeMin = position - min(i - mnXb, mxXb - i);
    	i64 rangeMax = position + min(i - mnXb, mxXb - i);
    	if (rangeMax < mnYa || rangeMin > mxYa) continue;
    	cout << "Yes"; return;
    }
    cout << "No";
}
