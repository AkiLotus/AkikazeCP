...
void VarInput() {
	...
	cin >> n >> k; a.rsz(n); ans.rsz(n, 0);
	for (i64 i=0; i<n; i++) cin >> a[i].fi.fi;
	for (i64 i=0; i<n; i++) cin >> a[i].fi.se;
	for (i64 i=0; i<n; i++) a[i].se = i;
}
...
void ProSolve() {
	...
	for (i64 i=0; i<n; i++) {
		i64 id = a[i].se, cnt = 0;
		ans[id] = a[i].fi.se;
		if (i == 0) {Set.insert(-a[i].fi.se); continue;}
		for (auto it=Set.begin(); it!=Set.end(); it++) {
			if (cnt == k) break; ans[id] -= *it; cnt++;
		}
		Set.insert(-a[i].fi.se);
	}
