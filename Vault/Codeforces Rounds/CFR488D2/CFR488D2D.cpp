...
/** -----GLOBAL VARIABLES----- **/
i64 n, m; vector<pii> a, b;

/** -----EXTENSIVE FUNCTIONS----- **/
i64 validGivenPair(pii a, pii b) {
	if (a.fi == b.fi && a.se == b.se) return -1;
	if (a.fi == b.se && a.se == b.fi) return -1;
	if (a.fi != b.fi && a.fi != b.se && a.se != b.fi && a.se != b.se) return -1;
...
void VarInput() {
	...
	for (i64 i=0; i<m; i++) cin >> b[i].fi >> b[i].se;
}

void ProSolve() {
	i64 ans = -1, cnt = 0; vector<vi> matchA(n), matchB(m);
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<m; j++) {
			i64 token = validGivenPair(a[i], b[j]);
			if (token == -1) continue;
			matchA[i].pub(j); matchB[j].pub(i); cnt++;
			if (ans == LINF) continue;
			if (ans == -1) {ans = token; continue;}
			if (ans != token) ans = LINF;
		}
	}
	if (ans == -1) {cout << "-1"; return;}
	if (ans != LINF) {cout << ans; return;}
	if (cnt == 1) {cout << ans; return;}
	for (i64 i=0; i<n; i++) {
		i64 token = -1;
		for (auto j: matchA[i]) {
			i64 tmp = validGivenPair(a[i], b[j]);
			if (token == -1) token = tmp;
			else if (token != tmp) token = LINF;
		}
		if (token == LINF) {cout << "-1"; return;}
	}
	for (i64 i=0; i<m; i++) {
		i64 token = -1;
		for (auto j: matchB[i]) {
			i64 tmp = validGivenPair(b[i], a[j]);
			if (token == -1) token = tmp;
			else if (token != tmp) token = LINF;
		}
		if (token == LINF) {cout << "-1"; return;}
