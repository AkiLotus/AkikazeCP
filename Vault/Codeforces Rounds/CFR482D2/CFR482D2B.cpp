...
void VarInput() {
	...
	cin >> n; for (i64 i=0; i<3; i++) cin >> a[i];
}

void ProSolve() {
	vector<vi> cnt(3, vi(256, 0));
	for (i64 i=0; i<3; i++) {
		for (i64 j=0; j<a[i].size(); j++) {
			cnt[i][a[i][j]]++;
		}
	}
	vi mx(3); for (i64 i=0; i<3; i++) mx[i] = *max_element(cnt[i].begin(), cnt[i].end());
	for (i64 i=0; i<3; i++) {
		i64 exceeds = max(n - (a[i].size() - mx[i]), 0LL);
		if (exceeds == 0) mx[i] = mx[i] + n;
		else mx[i] = a[i].size() - (exceeds == 1);
	}
	if (mx[0] > mx[1] && mx[0] > mx[2]) cout << "Kuro";
	else if (mx[1] > mx[0] && mx[1] > mx[2]) cout << "Shiro";
	else if (mx[2] > mx[0] && mx[2] > mx[1]) cout << "Katie";
	else cout << "Draw";
}
