...
/** -----GLOBAL VARIABLES----- **/
i64 n, x, y, ans; vector<vi> adj; vi par;
i64 otherbranch = 0, suby = 0; vb vis;

/** -----EXTENSIVE FUNCTIONS----- **/
void DFSTree(i64 z, i64 last) {
	//tracker2(z, last);
	par[z] = last; vis[z] = true;
	for (auto t: adj[z]) {
		if (vis[t]) continue;
		DFSTree(t, z);
	}
...
void VarInput() {
	...
	cin >> n >> x >> y; adj.rsz(n); par.rsz(n); x--; y--;
	for (i64 i=1; i<n; i++) {
		i64 a, b; cin >> a >> b; a--; b--;
...
void ProSolve() {
	vis.rsz(n, false);
	ans = n * (n - 1); DFSTree(x, -1);
	vis.clear(); vis.rsz(n, false); DFSy(y);
	i64 furthestAnc = y; while (par[furthestAnc] != x) furthestAnc = par[furthestAnc];
	//cout << "----------------FurthestAnc = " << furthestAnc << "--------\n";
	for (auto q: adj[x]) {
		if (q == furthestAnc) continue;
		DFScnt(q);
	}
	//cout << par << endl;
	//cout << otherbranch + 1 << " " << suby << endl;
	ans -= (otherbranch + 1) * suby;
	cout << ans;
}
