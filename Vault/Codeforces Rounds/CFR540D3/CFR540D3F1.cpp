...
/***********************************************
** File name: CFR540D3F1.cpp                  **
** Time created: Wed Feb 20 2019 16:37pm      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi a, cnt(3); vector<vi> adj;
/************************************************/

/************** [FUNCTIONS] **************/
i64 DFS(i64 z, i64 last, i64 &red, i64 &blue) {
	i64 res = 0; red += (a[z] == 1); blue += (a[z] == 2);
	for (auto t: adj[z]) {
		if (t == last) continue;
		i64 sred = 0, sblue = 0;
		res += DFS(t, z, sred, sblue);
		red += sred; blue += sblue;
	}
	if ((red == 0 || blue == 0) && (cnt[1] - red == 0 || cnt[2] - blue == 0) && last != -1) res++;
	return res;
}
