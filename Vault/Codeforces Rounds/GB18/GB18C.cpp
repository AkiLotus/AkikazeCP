...
void Solve() {
	vi Divisors; set<i64> List;
	for (i64 i=1; i<=sqrt(n); i++) {
		if (n % i != 0) continue;
		Divisors.pub(i);
		if (i * i != n) Divisors.pub(n/i);
	}
	for (auto Div: Divisors) {
		if (Div == 1) {List.insert(n*(n+1)/2); continue;}
		i64 MaximumQuotient = n / Div - (n % Div == 0);
		i64 res = (MaximumQuotient + 1) + (MaximumQuotient * (MaximumQuotient + 1) / 2) * Div;
		List.insert(res);
	}
