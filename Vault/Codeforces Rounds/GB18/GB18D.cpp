...
void Solve() {
	Perm.rsz(n+1, 1); Interchange.rsz(n+1, 0);
	if (n >= 3) Interchange[3] = 3;
	for (i64 i=1; i<=n; i++) Perm[i] = (Perm[i-1] * i) % Mod;
	for (i64 i=4; i<=n; i++) {
		Interchange[i] = (Interchange[i-1] + Perm[i-1] + Mod - 1) % Mod;
		Interchange[i] *= i; Interchange[i] %= Mod;
	}
	cout << (Perm[n] + Interchange[n]) % Mod;
}
/*****************************************/
