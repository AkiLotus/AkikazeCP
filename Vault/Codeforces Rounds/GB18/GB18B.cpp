...
/***********************************************
** File name: GB18B.cpp                       **
** Time created: Sun Dec 30 2018 14:08pm      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 n; vector<pii> Obelisk, Directions;
map<pii, i64> DirectionID;
/************************************************/
...
void Input() {
	cin >> n; Obelisk.rsz(n); Directions.rsz(n);
	for (auto &Pair: Obelisk) cin >> Pair.fi >> Pair.se;
	for (auto &Pair: Directions) cin >> Pair.fi >> Pair.se;
}

void Solve() {
	for (i64 i=0; i<n; i++) {
		DirectionID[Directions[i]] = i + 1;
	}
	for (i64 i=0; i<n; i++) {
		set<pii> Set; Set.insert(Directions[i]);
		pii Treasure = mp(Obelisk[0].fi+Directions[i].fi, Obelisk[0].se+Directions[i].se);
		for (i64 x=1; x<n; x++) {
			pii DemandingDirections = mp(Treasure.fi-Obelisk[x].fi, Treasure.se-Obelisk[x].se);
			if (DirectionID.find(DemandingDirections) == DirectionID.end()) continue;
			Set.insert(DemandingDirections);
		}
		if (Set.size() < n) continue;
		cout << Treasure.fi << " " << Treasure.se << endl;
	}
}
/*****************************************/
