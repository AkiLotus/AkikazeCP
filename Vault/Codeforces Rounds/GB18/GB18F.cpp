...
void Solve() {
	i64 ans = 0; bool hasWater = false;
	i64 Diff = 0;
	for (i64 i=0; i<n; i++) {
		i64 j = i, G = 0, W = 0, L = 0;
		G += (s[j] == 'G') * l[j]; W += (s[j] == 'W') * l[j];
		while (j < n-1 && (s[j] != 'L' || (s[j] == 'L' && s[j+1] == 'L'))) {
			j = j + 1;
			G += (s[j] == 'G') * l[j];
			W += (s[j] == 'W') * l[j];
			L += (s[j] == 'L') * l[j];
		}
		if (W > 0) hasWater = true;
		i64 AccumulatedStamina = G + W;
		i64 ConsumedStamina = L;
		i64 BaseTime = G * 5 + W * 3 + L * 1;
		ans += BaseTime;
		if (Diff + AccumulatedStamina <= ConsumedStamina) {
			if (hasWater) ans += (ConsumedStamina - AccumulatedStamina - Diff) * 3;
			else ans += (ConsumedStamina - AccumulatedStamina - Diff) * 5;
		}
		else {
			i64 CurrentStamina = Diff;
			Diff += AccumulatedStamina - ConsumedStamina;
			for (i64 x=i; x<=j; x++) {
				if (s[x] == 'L') break;
				CurrentStamina += l[x];
				if (s[x] == 'W') continue;
				i64 convertible = min(min(CurrentStamina, l[x] * 2), Diff);
				ans -= 4LL * convertible / 2;
				CurrentStamina -= convertible; Diff -= convertible;
			}
		}
		i = j;
	}
