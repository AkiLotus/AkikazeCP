...
/************** [GLOBAL VARIABLES] **************/
int n; vector<string> words;
int Base = 257;
vector<int> powBase(1000001, 1);
/************************************************/
...
void Input() {
	for (int i=1; i<1000001; i++) powBase[i] = (1LL * powBase[i-1] * Base) % Mod;
...
void Solve() {
	cout << words[0];
	for (int i=1; i<n; i++) {
		int curHash = 0; set<int> suffix;
		for (int x=words[i-1].size()-1; x>=0; x--) {
			curHash += (1LL * words[i-1][x] * powBase[words[i-1].size()-1-x]) % Mod;
			curHash %= Mod; suffix.insert(curHash);
		}
		curHash = 0;
		int maxLength = 0;
		for (int x=0; x<words[i].size(); x++) {
			curHash = (1LL * curHash * Base + words[i][x]) % Mod;
			if (suffix.find(curHash) != suffix.end()) maxLength = x + 1;
		}
		for (int x=maxLength; x<words[i].size(); x++) cout << words[i][x];
	}
	cout << endl;
}
/*****************************************/
