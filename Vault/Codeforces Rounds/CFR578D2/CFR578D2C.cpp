...
void Solve() {
	i64 GCD = __gcd(n, m);
	while (q--) {
		i64 sx, sy, ex, ey; cin >> sx >> sy >> ex >> ey;
		i64 sector_s = 0, sector_e = 0;
		if (sx == 1) sector_s = (sy - 1) / (n / GCD);
		else sector_s = (sy - 1) / (m / GCD);
		if (ex == 1) sector_e = (ey - 1) / (n / GCD);
		else sector_e = (ey - 1) / (m / GCD);
		if (sector_s == sector_e) cout << "YES\n";
		else cout << "NO\n";
	}
}
/*****************************************/
