...
/***********************************************
** File name: EDUCFR62C.cpp
** Time created: Sun Mar 31 2019 07:59
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
int n, k; vector<ppi> songs;
/************************************************/

/************** [FUNCTIONS] **************/
bool beautysort(ppi a, ppi b) {
...
void Input() {
	cin >> n >> k; songs.rsz(n);
	for (auto &P: songs) cin >> P.fi.fi >> P.fi.se;
}

void Solve() {
	sort(songs.begin(), songs.end()); int id = 0;
	for (auto &P: songs) {P.se = id++;}
	vector<ppi> songs_copy = songs;
	sort(songs_copy.begin(), songs_copy.end(), beautysort);
	int ptr1 = n - k, Required = 0; long long sum = 0, ans = LLONG_MIN;
	for (int i=ptr1; i<n; i++) sum += songs[i].fi.fi;
	for (int i=0; i<n; i++) {
		if (ptr1 < 0) break;
		int j = i, beauty = songs_copy[i].fi.se;
		while (j+1 < n && songs_copy[j+1].fi.se == beauty) j++;
		ans = max(ans, 1LL * sum * beauty);
		for (int x=i; x<=j; x++) {
			id = songs_copy[x].se;
			if (ptr1 <= id) {
				sum -= songs[id].fi.fi; Required++;
				while (ptr1 > 0 && Required > 0) {
					ptr1--;
					if (songs[ptr1].fi.se >= beauty) {
						sum += songs[ptr1].fi.fi;
						Required--; continue;
					}
				}
			}
