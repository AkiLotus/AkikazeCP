...
/** -----GLOBAL VARIABLES----- **/
i64 n, m, k, s; vi a; vector<vi> adj, List, dist;
...
void ProSolve() {
	...
	for (i64 i=0; i<k; i++) {
		...
		while (!Q.empty()) {
			...
			for (auto t: adj[z]) {
				if (vis[t]) continue;
				if (dist[i][z] + 1 >= dist[i][t]) continue;
				dist[i][t] = dist[i][z] + 1; Q.push(t);
			}
		}
	}
	for (i64 i=0; i<n; i++) {
		i64 ans = 0; vi dlist;
		for (i64 j=0; j<k; j++) dlist.pub(dist[j][i]);
		sort(dlist.begin(), dlist.end());
		for (i64 j=0; j<s; j++) ans += dlist[j];
		cout << ans << " ";
	}
}
