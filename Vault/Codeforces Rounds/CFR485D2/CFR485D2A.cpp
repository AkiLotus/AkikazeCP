...
void ProSolve() {
	cout << (6 - n) << endl;
	if (!Map["purple"]) cout << "Power\n";
	if (!Map["green"]) cout << "Time\n";
	if (!Map["blue"]) cout << "Space\n";
	if (!Map["orange"]) cout << "Soul\n";
	if (!Map["red"]) cout << "Reality\n";
	if (!Map["yellow"]) cout << "Mind\n";
}
