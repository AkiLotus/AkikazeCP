...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, k; vi x; vector<ppi> Edge;
vi DSUpar, DSUsize, DSUspecial;
i64 answer = -1;
/************************************************/
...
void DSUmerge(i64 u, i64 v, i64 w) {
...
bool edgesort (ppi A, ppi B) {
	return (A.se < B.se);
}

void Input() {
	cin >> n >> m >> k; x.rsz(k); Edge.rsz(m);
	DSUpar.rsz(n, -1); DSUsize.rsz(n, +1); DSUspecial.rsz(n, 0);
	for (auto &z: x) {cin >> z; z--; DSUspecial[z]++;}
	for (auto &E: Edge) {
		cin >> E.fi.fi >> E.fi.se >> E.se;
		E.fi.fi--; E.fi.se--;
	}
}
...
void Solve() {
	...
	for (auto E: Edge) {
		i64 u = E.fi.fi, v = E.fi.se;
		i64 w = E.se;
		DSUmerge(u, v, w);
	}
	for (i64 i=0; i<k; i++) cout << answer << " ";
	cout << endl;
}
/*****************************************/
