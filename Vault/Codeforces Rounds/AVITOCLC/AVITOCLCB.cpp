...
void Solve() {
	set<pii> S; i64 Token = 0; vi cnt(n, 0);
	for (i64 i=0; i<n; i++) S.insert(mp(a[i], i));
	while (!S.empty()) {
		pii p = *S.rbegin(); i64 z = n - p.fi;
		while (!S.empty() && S.rbegin()->fi == p.fi && cnt[Token] < z) {
			cnt[Token]++; i64 id = S.rbegin()->se;
			b[id] = Token; pii P = *S.rbegin(); S.erase(P);
		}
		if (cnt[Token] < z) {cout << "Impossible\n"; return;}
		Token++;
	}
	cout << "Possible\n";
	for (auto x: b) cout << x+1 << " "; cout << endl;
}
/*****************************************/
