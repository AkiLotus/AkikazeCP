...
/************** [GLOBAL VARIABLES] **************/
i64 n, L, S, ans = 1; vi w, p, dp, cnt; vector<vi> adj;
/************************************************/

/************** [FUNCTIONS] **************/
void DFS(i64 z) {
	i64 children = 0, Min = LINF;
	for (auto t: adj[z]) {
		if (p[z] == t) continue;
		DFS(t); children++;
		Min = min(Min, dp[z]);
	...
	ans += (children - 1);
	if (Min + w[z] > S) {ans++; dp[z] = S; cnt[z] = 1;}
	else {dp[z] = Min + w[z]; cnt[z];}
}

void Input() {
	cin >> n >> L >> S; w.rsz(n); p.rsz(n, -1);
	adj.rsz(n); dp.rsz(n, 0); cnt.rsz(n, 0);
...
void Solve() {
	if (*max_element(w.begin(), w.end()) > S) {cout << "-1"; return;}
	DFS(0); cout << ans;
}
/*****************************************/
