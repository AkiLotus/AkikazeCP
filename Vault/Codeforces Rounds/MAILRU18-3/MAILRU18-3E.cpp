...
/************** [GLOBAL VARIABLES] **************/
string s, t; i64 n, m, Base = 31;
vi Hash, HashInv, HashPow;
/************************************************/
...
bool validate(i64 x, i64 y) {
	i64 L = 0, ZeroHash = -1, IchiHash = -1;
	for (i64 i=0; i<n; i++) {
		i64 R = 0;
		if (s[i] == '0') R = L + x - 1;
		else R = L + y - 1;
		i64 Decoded = GetHash(L, R);
		if (s[i] == '0') {
			if (ZeroHash == -1) ZeroHash = Decoded;
			else if (ZeroHash != Decoded) return false;
		}
		if (s[i] == '1') {
			if (IchiHash == -1) IchiHash = Decoded;
			else if (IchiHash != Decoded) return false;
		}
		L = R + 1;
	}
	if (x == y) return (ZeroHash != IchiHash);
	else return true;
}
...
void Solve() {
	GenerateHash(); i64 ans = 0;
	i64 Zero = 0, Ichi = 0;
	for (i64 i=0; i<n; i++) {
		Zero += (s[i] == '0');
		Ichi += (s[i] == '1');
	}
	for (i64 i=1; Zero*i<m; i++) {
		if ((m - Zero*i) % Ichi != 0) continue;
		i64 j = (m - Zero*i) / Ichi;
		ans += validate(i, j);
	}
	cout << ans;
