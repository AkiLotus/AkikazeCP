...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, a, b, t; vi p, Paired; vb Chosen;
/************************************************/
...
void Input() {
	cin >> n >> m; p.rsz(n*2+1);
	Paired.rsz(n*2+1, -1); Chosen.rsz(n*2+1, false);
	for (i64 i=1; i<=n*2; i++) cin >> p[i];
	while (m--) {
		cin >> a >> b;
...
void Solve() {
	set<pii> Heroes;
	for (i64 i=1; i<=n*2; i++) {
		Heroes.insert(mp(p[i], i));
	}
	i64 obliged = -1;
	while (!allChosen()) {
		if (t == 1) {
			if (obliged != -1) {
				Heroes.erase(mp(p[obliged], obliged));
				Chosen[obliged] = true;
				cout << obliged << endl; fflush(stdout);
				obliged = -1;
			}
			else {
				pii Data = *Heroes.rbegin();
				i64 id = Data.se; Heroes.erase(Data);
				Chosen[id] = true;
			...
			i64 x; cin >> x;
			if (x == -1) {cout << (4 / 0); return;}
			Heroes.erase(mp(p[x], x)); Chosen[x] = true;
			if (Paired[x] != -1 && !Chosen[Paired[x]]) {
				obliged = Paired[x];
			}
		}
		t = 3 - t;
