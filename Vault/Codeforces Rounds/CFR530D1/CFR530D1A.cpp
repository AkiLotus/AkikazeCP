...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi p, s, h, a; vector<vi> adj;
/************************************************/

/************** [FUNCTIONS] **************/
void DFSDepth(i64 z) {
	if (z != 0) h[z] = h[p[z]] + 1;
	if (h[z] % 2 == 0 && s[z] != -1) {cout << "-1\n"; Exit();}
	if (h[z] % 2 != 0 && s[z] == -1) {cout << "-1\n"; Exit();}
	for (auto t: adj[z]) {
		if (p[z] == t) continue;
		DFSDepth(t);
...
void DFSSum(i64 z) {
	...
	if (leaf && s[z] == -1) {s[z] = s[p[z]]; return;}
	if (s[z] != -1 && Min < s[z]) {cout << "-1\n"; Exit();}
	if (s[z] == -1) s[z] = Min;
	for (auto t: adj[z]) {
		if (p[z] == t) continue;
		a[t] = s[t] - s[z];
...
void Input() {
	cin >> n; p.rsz(n, -1); s.rsz(n);
	h.rsz(n, 1); a.rsz(n); adj.rsz(n);
	for (i64 i=1; i<n; i++) {
		i64 j; cin >> j; j--;
		p[i] = j; adj[i].pub(j); adj[j].pub(i);
	}
	for (auto &x: s) cin >> x;
}
