...
/************** [GLOBAL VARIABLES] **************/
i64 n, id = -1; vs A, B; string s, t;
/************************************************/

/************** [FUNCTIONS] **************/
i64 seek(string src, string smp) {
	if (src.size() < smp.size()) return -1;
	i64 N = src.size(), M = smp.size();
	i64 pos = src.find(smp);
	if (pos >= 0 && pos <= N-M) return pos;
	return -1;
}
...
void Solve() {
	i64 FirstDiff = -1, LastDiff = -1;
	for (i64 x=0; x<n; x++) {
		i64 first = -1, last = -1;
		for (i64 i=0; i<A[x].size(); i++) {
			if (A[x][i] == B[x][i]) continue;
			if (first == -1) first = i;
			last = i;
		}
		if (first == -1) continue;
		FirstDiff = first; LastDiff = last;
		s = A[x].substr(first, last-first+1);
		t = B[x].substr(first, last-first+1);
		tracker2(s, t);
		while (true) {
			if (seek(A[x], s) == FirstDiff) break;
			FirstDiff = seek(A[x], s);
			if (LastDiff != A[x].size() - 1) {
				LastDiff++; s += A[x][LastDiff]; t += B[x][LastDiff];
			}
			else {
				s = A[x].substr(FirstDiff, LastDiff-FirstDiff+1);
				t = B[x].substr(FirstDiff, LastDiff-FirstDiff+1);
			}
		}
		id = x; break;
	}
	...
	for (i64 x=0; x<n; x++) {
		...
		if (pos == -1) {
			if (A[x] != B[x]) {cout << "NO"; return;}
			continue;
		}
		if (last != -1) {
			string s1 = A[x].substr(first, last-first+1);
			string t1 = B[x].substr(first, last-first+1);
			if (s1 != s || t1 != t) {cout << "NO"; return;}
			if (first == pos) continue;
			while (LastDiff+1 < A[id].size() && (last >= A[x].size() || A[id][LastDiff] == A[x][last])) {
				LastDiff++; last++; s += A[id][LastDiff]; t += B[id][LastDiff];
			}
			if (seek(A[x], s) == first) continue;
			while (FirstDiff-1 >= 0 && (first < 0 || A[id][FirstDiff] == A[x][first])) {
				FirstDiff--; first--; s.insert(0, 1, A[id][FirstDiff]); t.insert(0, 1, B[id][FirstDiff]);
			}
			if (seek(A[x], s) == first) continue;
			cout << "NO"; return;
		}
		else {
			first = pos; last = pos + s.size() - 1;
			while (LastDiff+1 < A[id].size() && last+1 < A[x].size() && A[id][LastDiff] == A[x][last]) {
				LastDiff++; last++; s += A[id][LastDiff]; t += B[id][LastDiff];
			}
			if (last >= A[x].size()) continue;
			if (seek(A[x], s) == first) continue;
			while (FirstDiff-1 >= 0 && first-1 >= 0 && A[id][FirstDiff] == A[x][first]) {
				FirstDiff--; first--; s.insert(0, 1, A[id][FirstDiff]); t.insert(0, 1, B[id][FirstDiff]);
			}
			if (first < 0) continue;
			if (seek(A[x], s) == first) continue;
