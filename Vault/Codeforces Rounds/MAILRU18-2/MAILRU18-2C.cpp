...
/************** [FUNCTIONS] **************/
i64 process(i64 g) {
	i64 top = -(LINF*4)/g, bot = (LINF*4)/g, ans = -LINF*8;
	while (top <= bot) {
		i64 mid = (top + bot) / 2;
		if (la + mid * g < lb) {ans = la + mid * g; top = mid + 1;}
		else bot = mid - 1;
	}
	return ans;
}
...
void Solve() {
	i64 LenA = ra - la + 1;
	i64 g = __gcd(ta, tb);
	i64 closestL = process(g);
	i64 shiftedL = closestL + g;
	cout << max(max(min(closestL+LenA-1, rb) - max(closestL, lb) + 1, 0LL), max(min(shiftedL+LenA-1, rb) - max(shiftedL, lb) + 1, 0LL));
}
/*****************************************/
