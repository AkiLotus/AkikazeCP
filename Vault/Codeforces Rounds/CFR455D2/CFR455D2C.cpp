...
i64 N, ans = 0;
vector<char> cmd; vector<vector<i64>> dp(5100, vector<i64>(5100, 0));
vector<i64> level(5100, 0);
...
void ProSolve() {
	...
	for (i64 i=1; i<N; i++) {
		if (cmd[i] == 's') {
			...
			i64 tmp = 1;
			while (tmp <= level[i]) {
				dp[i][tmp] = (dp[i][tmp-1] + dp[i-1][tmp]) % MOD;
				tmp++;
			}
		}
		else if (cmd[i] == 'f') {
			...
			i64 tmp = 1;
			while (tmp <= level[i]) {
				dp[i][tmp] = dp[i-1][tmp-1]; tmp++;
			}
		}
	}
	for (i64 i=0; i<5100; i++) ans = (ans + dp[N-1][i]) % MOD;
	cout << ans;
}
