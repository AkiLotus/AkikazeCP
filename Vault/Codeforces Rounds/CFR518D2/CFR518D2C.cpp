...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, x, y; vector<vector<pii>> Rook;
/************************************************/
...
void Input() {
	cin >> n >> m; Rook.rsz(n);
	for (i64 i=0; i<n; i++) Rook[i].pub(mp(i+1, i+1));
	i64 ycoor = 1000;
	while (m--) {
		cin >> x >> y; x--; y--;
		Rook[x].pub(mp(x+1, ycoor));
		Rook[y].pub(mp(y+1, ycoor));
		ycoor++;
	}
}
...
void Solve() {
	for (i64 x=0; x<n; x++) {
		cout << Rook[x].size() << endl;
		for (auto z: Rook[x]) cout << z.fi << " " << z.se << endl;
	}
}
/*****************************************/
