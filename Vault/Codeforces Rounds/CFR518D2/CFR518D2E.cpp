...
/************** [GLOBAL VARIABLES] **************/
i64 n, k, u, v; vector<set<i64>> adj; vi orgDeg;
/************************************************/
...
void Input() {
	cin >> n >> k; adj.rsz(n); orgDeg.rsz(n, 0);
	for (i64 i=1; i<n; i++) {
		cin >> u >> v; u--; v--;
...
void Solve() {
	if (k > 10) {cout << "No"; return;}
	queue<pii> Q; vb vis(n, false);
	for (i64 i=0; i<n; i++) {
		if (adj[i].size() == 1) {
			vis[i] = true;Q.push(mp(0LL, i));
		}
	}
	i64 cntCentroid = 0;
	while (!Q.empty()) {
		pii Z = Q.front(); Q.pop();
		i64 lvl = Z.fi, z = Z.se;
		if (lvl == 0 && orgDeg[z] > 1) {cout << "No"; return;}
		if (lvl > 0 && lvl < k && orgDeg[z] < 4) {cout << "No"; return;}
		if (lvl == k && orgDeg[z] < 3) {cout << "No"; return;}
		if (adj[z].size() > 1) {cout << "No"; return;}
		if (lvl == k) {cntCentroid++; continue;}
		i64 t = *adj[z].begin();
		if (!vis[t]) {Q.push(mp(lvl+1, t)); vis[t] = true;}
		adj[t].erase(z); adj[z].erase(t);
	}
	if (cntCentroid == 1) cout << "Yes";
	else cout << "No";
}
/*****************************************/
