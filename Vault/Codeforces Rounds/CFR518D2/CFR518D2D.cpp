...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi a; i64 dp[100003][203][2];
/************************************************/
...
void Solve() {
	...
	else for (i64 i=1; i<=200; i++) dp[0][i][0] = 1;
	for (i64 i=0; i<n-1; i++) {
		for (i64 x=1; x<=200; x++) {
			for (i64 v=0; v<2; v++) {
				if (dp[i][x][v] == 0) continue;
				if (a[i+1] == -1) {
					if (v == 0) {
						dp[i+1][x+1][0] += dp[i][x][v];
						dp[i+1][x+1][0] %= Mod;
						dp[i+1][x][1] += dp[i][x][v];
						dp[i+1][x][1] %= Mod;
						dp[i+1][x+1][1] -= dp[i][x][v];
						dp[i+1][x+1][1] %= Mod;
					}
					else {
						dp[i+1][1][1] += dp[i][x][v];
					...
					if (v == 1) {
						dp[i+1][a[i+1]][1] += dp[i][x][v]; dp[i+1][a[i+1]][1] %= Mod;
						dp[i+1][a[i+1]+1][1] -= dp[i][x][v]; dp[i+1][a[i+1]+1][1] %= Mod;
					}
				}
				else if (x == a[i+1]) {
					dp[i+1][a[i+1]][1] += dp[i][x][v]; dp[i+1][a[i+1]][1] %= Mod;
					...
					dp[i+1][a[i+1]][0] += dp[i][x][v]; dp[i+1][a[i+1]][0] %= Mod;
					dp[i+1][a[i+1]+1][0] -= dp[i][x][v]; dp[i+1][a[i+1]+1][0] %= Mod;
				}
			}
		}
		for (i64 v=0; v<2; v++) {
			for (i64 x=1; x<=200; x++) {
				dp[i+1][x][v] += dp[i+1][x-1][v];
				dp[i+1][x][v] %= Mod;
				while (dp[i+1][x][v] < 0) dp[i+1][x][v] += Mod;
			}
		}
	}
	i64 ans = 0;
