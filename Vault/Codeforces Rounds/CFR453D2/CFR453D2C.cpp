...
i64 h, nodeCount = 0; vi a, firstNode;
...
void VarInput() {
	...
	cin >> h; a.resize(h+1); firstNode.resize(h+1);
	for (i64 i=0; i<=h; i++) {
		cin >> a[i]; nodeCount += a[i];
	}
	firstNode[0] = 1;
}
...
void ProSolve() {
	...
	for (i64 x=1; x<=h; x++) {
		if (a[x] > 1 && a[x-1] > 1) {
			...
			//cout << "x = " << x << endl;
			for (i64 i=1; i<=h; i++) {
				firstNode[i] = firstNode[i-1] + a[i-1];
				//cout << firstNode[i] << " ";
			}
			//cout << endl;
			i64 level = 0;
			for (i64 i=1; i<=nodeCount; i++) {
				if (i == 1) cout << "0";
				else cout << " " << firstNode[level-1];
				if (i == firstNode[level+1] - 1) level++;
			}
			cout << endl;
			level = 0;
			for (i64 i=1; i<=nodeCount; i++) {
				if (i == 1) cout << "0";
				else if (i == firstNode[x] + 1) cout << " " << firstNode[x-1] + 1;
				else cout << " " << firstNode[level-1];
				if (i == firstNode[level+1] - 1) level++;
			}
			return;
