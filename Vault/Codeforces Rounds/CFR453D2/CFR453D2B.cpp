...
i64 n, ans = 0; vi par, c, curr;
vvi child;

/** -----EXTENSIVE FUNCTIONS----- **/
void color(i64 z, i64 col) {
	curr[z] = col;
	for (i64 i=0; i<child[z].size(); i++) {
		color(child[z][i], col);
	}
}
...
void VarInput() {
	...
	cin >> n; par.resize(n+1, -1); c.resize(n+1);
	curr.resize(n+1, 0); child.resize(n+1, vi(0));
	for (i64 i=2; i<=n; i++) {
		cin >> par[i]; child[par[i]].pub(i);
	}
