...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi r, TreeLeft, TreeRight;
/************************************************/

/************** [FUNCTIONS] **************/
void UpdateLeft(i64 node, i64 st, i64 en, i64 id, i64 val) {
	if (st > en || en < id || id < st) return;
	if (st == en && en == id) {TreeLeft[node] = val; return;}
	UpdateLeft(node*2, st, (st+en)/2, id, val);
	UpdateLeft(node*2+1, (st+en)/2+1, en, id, val);
	TreeLeft[node] = min(TreeLeft[node*2], TreeLeft[node*2+1]);
}

void UpdateRight(i64 node, i64 st, i64 en, i64 id, i64 val) {
	if (st > en || en < id || id < st) return;
	if (st == en && en == id) {TreeRight[node] = val; return;}
	UpdateRight(node*2, st, (st+en)/2, id, val);
	UpdateRight(node*2+1, (st+en)/2+1, en, id, val);
	TreeRight[node] = min(TreeRight[node*2], TreeRight[node*2+1]);
}

i64 GetLeft(i64 node, i64 st, i64 en, i64 L, i64 R) {
	if (st > en || en < L || R < st) return +LINF;
	if (L <= st && en <= R) return TreeLeft[node];
	i64 p1 = GetLeft(node*2, st, (st+en)/2, L, R);
	i64 p2 = GetLeft(node*2+1, (st+en)/2+1, en, L, R);
	return min(p1, p2);
}

i64 GetRight(i64 node, i64 st, i64 en, i64 L, i64 R) {
	if (st > en || en < L || R < st) return -LINF;
	if (L <= st && en <= R) return TreeRight[node];
	i64 p1 = GetRight(node*2, st, (st+en)/2, L, R);
	i64 p2 = GetRight(node*2+1, (st+en)/2+1, en, L, R);
	return max(p1, p2);
}

i64 binsearch(i64 center) {
	i64 top = 1, bot = n, ans = 1;
	while (top <= bot) {
		i64 mid = (top + bot) / 2;
		i64 L = GetLeft(1, 0, n*3-1, center-mid+1, center);
		i64 R = GetRight(1, 0, n*3-1, center, center+mid-1);
		if (R - L + 1 >= n) {ans = mid; bot = mid - 1;}
		else top = mid + 1;
	}
	return ans;
}
...
void Input() {
	cin >> n; r.rsz(n*3);
	TreeLeft.rsz(n*12); TreeRight.rsz(n*12);
	for (i64 i=0; i<n*3; i++) {
		if (i < n) cin >> r[i];
		else r[i] = r[i-n];
		UpdateLeft(1, 0, n*3-1, i, i-r[i]);
		UpdateRight(1, 0, n*3-1, i, i+r[i]);
	}
}

void Solve() {
	cout << r << endl;
	for (i64 i=0; i<n*3; i++) cout << GetLeft(1, 0, n*3-1, i, i) << " "; cout << endl;
	for (i64 i=0; i<n*3; i++) cout << GetRight(1, 0, n*3-1, i, i) << " "; cout << endl;
	for (i64 i=n; i<n*2; i++) {
		cout << binsearch(i) << " ";
	}
}
