...
ld dist(pdd M, pdd N) {
	ld dx = M.fi - N.fi, dy = M.se - N.se;
	return sqrt(sqr(dx) + sqr(dy));
}

void Input() {
	cin >> a >> b >> c >> x1 >> y1 >> x2 >> y2;
	A = mp(x1, y1); B = mp(x2, y2);
}

void Solve() {
	cout << fixed << setprecision(18);
	ld origin = fabs(x1 - x2) + fabs(y1 - y2);
	if (a == 0 || b == 0) {cout << origin; return;}
	pdd x1Intercept = mp(x1, -(c+a*x1)/b);
	pdd y1Intercept = mp(-(c+b*y1)/a, y1);
	pdd x2Intercept = mp(x2, -(c+a*x2)/b);
	pdd y2Intercept = mp(-(c+b*y2)/a, y2);
	ld dist1 = dist(A, x1Intercept) + dist(x1Intercept, x2Intercept) + dist(x2Intercept, B);
	ld dist2 = dist(A, x1Intercept) + dist(x1Intercept, y2Intercept) + dist(y2Intercept, B);
	ld dist3 = dist(A, y1Intercept) + dist(y1Intercept, x2Intercept) + dist(x2Intercept, B);
	ld dist4 = dist(A, y1Intercept) + dist(y1Intercept, y2Intercept) + dist(y2Intercept, B);
	cout << min(origin, min(min(dist1, dist2), min(dist3, dist4)));
}
/*****************************************/
