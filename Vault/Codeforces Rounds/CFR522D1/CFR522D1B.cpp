...
/************** [GLOBAL VARIABLES] **************/
i64 n, Map = 0; vi a, cnt(101, 0);
vb vis(101, false); vector<vi> nCr(101);
/************************************************/
...
void Input() {
	cin >> n; a.rsz(n); nCr[0].pub(1);
	nCr[1].pub(1); nCr[1].pub(1);
	for (auto &x: a) {
		cin >> x; cnt[x]++;
		Map += (cnt[x] == 1);
	}
...
void Solve() {
	...
	for (i64 i=2; i<101; i++) {
		nCr[i].pub(1);
		for (i64 j=0; j<i-1; j++) {
			nCr[i].pub((nCr[i-1][j] + nCr[i-1][j+1]) % Mod);
		}
		nCr[i].pub(1);
	}
	i64 ans = 1; vector<vi> dp(n+1, vi(n*100+1, 0)); dp[0][0] = 1;
	for (i64 x=0; x<n; x++) {
		for (i64 i=n-2; i>=0; i--) {
			for (i64 j=0; j<=n*100; j++) {
				if (!dp[i][j]) continue;
				if (j + a[x] > n * 100) continue;
				dp[i+1][j+a[x]] += dp[i][j];
				dp[i+1][j+a[x]] %= Mod;
			}
		}
	...
	for (auto x: a) {
		...
		for (i64 i=0; i<=cnt[x]; i++) {
			if (dp[i][x*i] == nCr[cnt[x]][i]) {
				if (Map != 2 || i != cnt[x]) ans = max(ans, i);
				else ans = max(ans, n);
			}
		}
	}
