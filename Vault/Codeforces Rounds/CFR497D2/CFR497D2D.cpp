...
void Input() {
	for (i64 i=1; i<100002; i++) {
		for (i64 j=1; j<=sqrt(i); j++) {
			if (i % j == 0) {
				divi[i].pub(j);
				if (j * j != i) divi[i].pub(i/j);
			}
		}
...
void Solve() {
	while (T--) {
		cin >> A >> B >> C;
		unordered_map<i64, i64> Map123, Map1, Map2, Map3, Map12, Map23, Map31;
		unordered_map<i64, i64> MapBin; i64 ans = 0;
		vector<pii> diviBin; vi diviABC;
		for (auto x: divi[A]) {Map1[x]++; Map12[x]++; Map31[x]++; Map123[x]++;}
		for (auto x: divi[B]) {Map2[x]++; Map12[x]++; Map23[x]++; Map123[x]++;}
		for (auto x: divi[C]) {Map3[x]++; Map23[x]++; Map31[x]++; Map123[x]++;}
		for (auto x: divi[A]) {
			for (auto y: divi[B]) {
				MapBin[min(x,y)*1000000LL+max(x,y)]++;
			}
		}
		for (auto x: divi[A]) {
			for (auto y: divi[C]) {
		...
		for (auto it=Map123.begin(); it!=Map123.end(); it++) diviABC.pub(it->first);
		for (auto it=MapBin.begin(); it!=MapBin.end(); it++) diviBin.pub(mp(it->first/1000000LL, it->first%1000000LL));
		sort(diviABC.begin(), diviABC.end());
		for (auto x: diviBin) {
			i64 it = lower_bound(diviABC.begin(), diviABC.end(), x.se) - diviABC.begin();
			for (i64 i=it; i<diviABC.size(); i++) {
				i64 z = diviABC[i];
				if (Map1[x.fi] && Map2[x.se] && Map3[z]) {ans++; continue;}
				if (Map2[x.fi] && Map1[x.se] && Map3[z]) {ans++; continue;}
				if (Map3[x.fi] && Map2[x.se] && Map1[z]) {ans++; continue;}
				if (Map2[x.fi] && Map3[x.se] && Map1[z]) {ans++; continue;}
				if (Map1[x.fi] && Map3[x.se] && Map2[z]) {ans++; continue;}
				if (Map3[x.fi] && Map1[x.se] && Map2[z]) {ans++; continue;}
			}
		}
		cout << ans << endl;
	}
