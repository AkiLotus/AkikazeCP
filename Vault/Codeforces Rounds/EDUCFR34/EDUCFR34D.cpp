...
i64 n; vector<i64> a; map<i64, i64> M;
bigint sum = bigint(0), ans = bigint(0);
...
void ProSolve() {
	...
	for (i64 i=0; i<n; i++) {
		ans += bigint((sum - (n-i) * a[i]));
		ans -= bigint(M[a[i]+1]); ans += bigint(M[a[i]-1]);
		sum -= bigint(a[i]); M[a[i]]--;
		//cout << ans << endl;
	}
	cout << ans;
}
