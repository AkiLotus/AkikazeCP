...
i64 n, L, ans = 0, minRequired = LINF; vi v, c;
vp mark;
...
void VarInput() {
	...
	cin >> n >> L; c.resize(n);
	v.resize(n, 1); mark.resize(n);
	for (i64 i=0; i<n; i++) {
		cin >> c[i]; if (i > 0) v[i] = v[i-1] * 2;
	}
	for (i64 i=0; i<n; i++) {
		mark[i] = mp(c[i] * (v[n-1] / v[i]), i);
		//cout << mark[i].fi << " ";
	}
	//cout << endl;
...
void ProSolve() {
	...
	for (i64 i=0; i<n; i++) {
		if (L <= 0) break;
		i64 tmp = L / v[mark[i].se];
		if (L % v[mark[i].se] > 0) minRequired = min(minRequired, ans+c[mark[i].se]*(tmp+1));
		L -= tmp * v[mark[i].se];
		ans += c[mark[i].se] * tmp;
	}
	cout << min(minRequired, ans);
}
