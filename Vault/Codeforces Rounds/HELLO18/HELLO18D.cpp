...
i64 n, T; vi a, t; vp timeSort;
vector<ppi> priorSort, ans;

/** -----EXTENSIVE FUNCTIONS----- **/
bool prior1(ppi a, ppi b) {
	if (a.fi.fi > b.fi.fi) return true;
	if (a.fi.fi == b.fi.fi && a.fi.se < b.fi.se) return true;
	return false;
}

bool prior2(ppi a, ppi b) {
	if (a.fi.fi > b.fi.fi) return true;
	if (a.fi.fi == b.fi.fi && a.fi.se > b.fi.se) return true;
	return false;
}
...
void VarInput() {
	...
	cin >> n >> T; a.resize(n); t = a;
	for (i64 i=0; i<n; i++) cin >> a[i] >> t[i];
	timeSort.resize(n); priorSort.resize(n);
	for (i64 i=0; i<n; i++) {
		timeSort[i] = mp(t[i], i);
		priorSort[i] = mp(mp(a[i], t[i]), i);
	}
	sort(timeSort.begin(), timeSort.end());
	sort(priorSort.begin(), priorSort.end(), prior1);
	//for (i64 i=0; i<n; i++) cout << timeSort[i].fi << "-" << timeSort[i].se << " "; cout << endl;
}

void ProSolve() {
	//cout << "Case " << ++cas << ": " << ans << endl; // for multi-testcase problems
	i64 tmpTime = 0, ins = 0, tmp = 0;
	priority_queue<ppi, vector<ppi>, greater<ppi>> ans;
	while (ins < n) {
		//cout << "TLEcheck\n";
		//cout << "tmpTime = " << tmpTime << endl;
		i64 insBefore = ins;
		while (ins < n && tmpTime + timeSort[ins].fi <= T) {
			tmpTime += timeSort[ins].fi; i64 qq = timeSort[ins].se;
			//cout << "pushing " << timeSort[ins].se << endl;
			ans.push(mp(mp(a[qq], t[qq]), qq)); ins++; tmp++;
		}
		while (!ans.empty() && ans.top().fi.fi < tmp) {
			tmpTime -= ans.top().fi.se;
			//cout << "popping " << ans.top().se << endl;
			ans.pop(); tmp--;
		}
		if (insBefore == ins) break;
	}
	i64 firstAns = 0; vi answer;
	while (!ans.empty()) {
		if (ans.top().fi.fi >= tmp) firstAns++;
		answer.pub(ans.top().se); ans.pop();
	}
	cout << firstAns << endl << tmp << endl;
	for (i64 i=0; i<tmp; i++) cout << answer[i]+1 << " ";
}
