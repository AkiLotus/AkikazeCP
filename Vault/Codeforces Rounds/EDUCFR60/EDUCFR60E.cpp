...
/************** [GLOBAL VARIABLES] **************/
vs charmask(26*26*26, "444"), newmask(26*26*26, "444");
map<string, i64> Map; string s; i64 n;
/************************************************/
...
void Input() {
	for (i64 i=0; i<26*26*26; i++) {
		i64 x = i / (26 * 26);
		i64 y = (i - x * 26 * 26) / 26;
		i64 z = i % 26;
		charmask[i][0] = char(97+x);
		charmask[i][1] = char(97+y);
		charmask[i][2] = char(97+z);
		Map[charmask[i]] = i;
	}
	cin >> s; n = s.size();
}
...
void Solve() {
	for (i64 id=0; id<3; id++) {
		cout << "? ";
		for (i64 i=0; i<n; i++) cout << charmask[i][id];
		cout << endl; fflush(stdout);
		string t; cin >> t; if (t == "0") Exit();
		for (i64 i=0; i<n; i++) newmask[i][id] = t[i];
	}
	string t = s; cout << "! ";
	for (i64 i=0; i<n; i++) {
		i64 newID = Map[newmask[i]];
		t[newID] = s[i];
	}
