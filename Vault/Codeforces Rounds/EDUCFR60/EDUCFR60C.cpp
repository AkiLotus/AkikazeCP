...
/***********************************************
** File name: EDUCFR60C.cpp                   **
** Time created: Mon Feb 18 2019 15:30pm      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
i64 x1, y1, x2, y2, n; string s; vi psx, psy;
/************************************************/

/************** [FUNCTIONS] **************/
bool valid(i64 step) {
	i64 cycle = step / n, rem = step % n;
	i64 x = x1, y = y1;
	x += psx[n-1] * cycle; y += psy[n-1] * cycle;
	if (rem > 0) {x += psx[rem-1]; y += psy[rem-1];}
	i64 ManhattanDistance = abs(x - x2) + abs(y - y2);
	return (ManhattanDistance <= step);
}

i64 binsearch(i64 top, i64 bot) {
	i64 res = -1;
	while (top <= bot) {
		i64 mid = (top + bot) / 2;
		if (valid(mid)) {res = mid; bot = mid - 1;}
		else top = mid + 1;
	}
	return res;
}
...
void Input() {
	cin >> x1 >> y1 >> x2 >> y2;
	cin >> n >> s;
	psx.rsz(n, 0); psy.rsz(n, 0);
}

void Solve() {
	if (s[0] == 'L') psx[0] = -1; else if (s[0] == 'R') psx[0] = +1;
	if (s[0] == 'D') psy[0] = -1; else if (s[0] == 'U') psy[0] = +1;
	for (i64 i=1; i<n; i++) {
		psx[i] = psx[i-1]; psy[i] = psy[i-1];
		if (s[i] == 'L') psx[i]--; else if (s[i] == 'R') psx[i]++;
		if (s[i] == 'D') psy[i]--; else if (s[i] == 'U') psy[i]++;
	}
	cout << binsearch(0, LINF) << endl;
}
/*****************************************/
