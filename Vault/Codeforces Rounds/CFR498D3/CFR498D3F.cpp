...
void Solve() {
	vector<vector<map<i64, i64>>> dp1(n, vector<map<i64, i64>>(m));
	vector<vector<map<i64, i64>>> dp2(n, vector<map<i64, i64>>(m));
	dp1[0][0][a[0][0]] = 1; dp2[n-1][m-1][a[n-1][m-1]] = 1;
	for (i64 x=0; x<n-1; x++) {
		for (i64 i=0; i<=x; i++) {
			i64 j = x - i;
			if (i < 0 || i >= n || j < 0 || j >= m) continue;
			for (auto it=dp1[i][j].begin(); it!=dp1[i][j].end(); it++) {
				i64 val = it->first, cnt = it->second;
				if (i+1 < n) dp1[i+1][j][val ^ a[i+1][j]] += cnt;
				if (j+1 < m) dp1[i][j+1][val ^ a[i][j+1]] += cnt;
			}
		}
	}
	for (i64 x=n+m-2; x>n-1; x--) {
		for (i64 i=0; i<=x; i++) {
			i64 j = x - i;
			if (i < 0 || i >= n || j < 0 || j >= m) continue;
			for (auto it=dp2[i][j].begin(); it!=dp2[i][j].end(); it++) {
				i64 val = it->first, cnt = it->second;
				if (i-1 >= 0) dp2[i-1][j][val ^ a[i-1][j]] += cnt;
				if (j-1 >= 0) dp2[i][j-1][val ^ a[i][j-1]] += cnt;
			}
		}
	}
	i64 ans = 0;
