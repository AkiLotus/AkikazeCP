...
/***********************************************
** File name: CFR544D3F1.cpp                  **
** Time created: Sat Mar 16 2019 10:14am      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
int n, m; set<pii> Edge; vector<vi> adj;
vi DSUpar, DSUsize;
/************************************************/
...
void Input() {
	...
	for (int i=0; i<m; i++) {
		int x, y; cin >> x >> y; x--; y--;
		if (x > y) swap(x, y); Edge.insert({x, y});
		adj[x].pub(y); adj[y].pub(x);
	}
}

void Solve() {
	int MaxDeg = 0, rootID = -1;
	for (int i=0; i<n; i++) {
		if (MaxDeg < adj[i].size()) {
			MaxDeg = adj[i].size();
			rootID = i;
		}
	}
	int usedVertices = MaxDeg;
	for (auto t: adj[rootID]) {
		cout << rootID+1 << " " << t+1 << endl;
		DSUmerge(rootID, t);
		Edge.erase({min(rootID, t), max(rootID, t)});
	}
	while (!Edge.empty() && usedVertices < n-1) {
		pii E = *Edge.begin(); Edge.erase(E);
		int x = E.fi, y = E.se;
		if (DSUfind(x) == DSUfind(y)) continue;
		cout << x+1 << " " << y+1 << endl; DSUmerge(x, y);
	}
}
/*****************************************/
