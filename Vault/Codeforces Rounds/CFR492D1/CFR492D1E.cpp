...
/** -----GLOBAL VARIABLES----- **/
i64 u, v, k;
map<i64, i64> vis; bool flag = false;
...
void DFS(i64 z, vi a) {
	...
	vi a1 = a, a2 = a;
	if ((u+k*INF-v)%k <= 199-n) {
		for (i64 i=0; i<(u+k*INF-v)%k; i++) a.pub(2);
		cout << a.size() << endl;
		for (auto k: a) cout << k << " ";
		flag = true; return;
	}
	if ((v+k*INF-u)%k <= 199-n) {
		for (i64 i=0; i<(v+k*INF-u)%k; i++) a.pub(1);
		cout << a.size() << endl;
		for (auto k: a) cout << k << " ";
		flag = true; return;
	}
	for (i64 i=0; i+n+1<=200; i++) {
		if (!vis[modPow((z+k*INF+i)%k, k-2, k)]) {vi b = a1; b.pub(3); DFS(modPow((z+k*INF+i)%k, k-2, k), b);}
		if (flag) return;
		if (!vis[modPow((z+k*INF-i)%k, k-2, k)]) {vi b = a2; b.pub(3); DFS(modPow((z+k*INF-i)%k, k-2, k), b);}
		if (flag) return;
		a1.pub(1); a2.pub(2);
	}
}
