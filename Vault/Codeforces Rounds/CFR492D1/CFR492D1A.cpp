...
/** -----GLOBAL VARIABLES----- **/
i64 n, k; vector<vi> a; map<vector<vi>, bool> Map;
pip defaultPIP = mp(-LINF, mp(-LINF, -LINF));
i64 flag = -1; vector<pip> Moves(24444, defaultPIP);

/** -----EXTENSIVE FUNCTIONS----- **/
void DFS(vector<vi> A, i64 moves) {
	Map[A] = true;
	//tracker1(""); for (i64 i=0; i<4; i++) cout << A[i] << endl;
	...
	for (i64 i=1; i<3; i++) {
		...
		if (containsCars) break;
	}
	if (!containsCars) {
		flag = moves; return;
	}
	if (moves == 20000) return;
	for (i64 i=1; i<3; i++) {
		for (i64 j=0; j<n; j++) {
			if (A[i][j] == 0) continue;
			if (i == 1 && A[i-1][j] == A[i][j]) {
				vector<vi> B = A; B[i][j] = 0;
				if (Map.find(B) == Map.end()) {
					Moves[moves+1] = mp(A[i][j], mp(i+0, j+1));
					DFS(B, moves+1);
				}
			}
			if (flag != -1) break;
			if (i == 2 && A[i+1][j] == A[i][j]) {
				vector<vi> B = A; B[i][j] = 0;
				if (Map.find(B) == Map.end()) {
					Moves[moves+1] = mp(A[i][j], mp(i+2, j+1));
					DFS(B, moves+1);
				}
			}
			if (flag != -1) break;
			if (j > 0 && A[i][j-1] == 0) {
				vector<vi> B = A; swap(B[i][j], B[i][j-1]);
				if (Map.find(B) == Map.end()) {
					Moves[moves+1] = mp(A[i][j], mp(i+1, j+0));
					DFS(B, moves+1);
				}
			}
			if (flag != -1) break;
			if (j < n-1 && A[i][j+1] == 0) {
				vector<vi> B = A; swap(B[i][j], B[i][j+1]);
				if (Map.find(B) == Map.end()) {
					Moves[moves+1] = mp(A[i][j], mp(i+1, j+2));
					DFS(B, moves+1);
				}
			}
			if (flag != -1) break;
			if (i == 2 && A[i-1][j] == 0) {
				vector<vi> B = A; swap(B[i][j], B[i-1][j]);
				if (Map.find(B) == Map.end()) {
					Moves[moves+1] = mp(A[i][j], mp(i+0, j+1));
					DFS(B, moves+1);
				}
...
void ProSolve() {
	...
	if (flag == -1) cout << "-1";
		...
		cout << flag << endl;
		for (i64 i=1; i<=flag; i++) cout << Moves[i].fi << " " << Moves[i].se.fi << " " << Moves[i].se.se << endl;
	}
}
