...
void VarInput() {
	...
	cin >> hh >> mm >> H >> D >> C >> N; Time = hh * 60 + mm;
}

void ProSolve() {
	ld ans1 = 1e18, ans2 = 1e18;
	if (Time < 1200) ans1 = ((H / N) + (H % N != 0)) * C;
	else ans1 = (ld)((H / N) + (H % N != 0)) * C * 4 / 5;
	if (Time < 1200) ans2 = (ld)(((H + D * (1200 - Time)) / N) + (ld)((H + D * (1200 - Time)) % N != 0)) * C * 4 / 5;
