...
/************** [GLOBAL VARIABLES] **************/
i64 n, T, s = 0, ans = 0; vi a; set<pii> Set, FlipSet;
/************************************************/
...
void Input() {
	...
	for (i64 i=0; i<n; i++) {
		cin >> a[i]; s += a[i];
		Set.insert(mp(i, a[i]));
		FlipSet.insert(mp(a[i], i));
	}
}

void Solve() {
	while (!FlipSet.empty() && FlipSet.rbegin()->fi > T) {
		pii q1 = *FlipSet.rbegin(), q2 = mp(q1.se, q1.fi);
		FlipSet.erase(q1); Set.erase(q2); s -= q1.fi;
	}
	while (!Set.empty()) {
		while (T >= s && !Set.empty()) {
			i64 cycle = T / s; ans += cycle * FlipSet.size(); T %= s;
			while (!FlipSet.empty() && FlipSet.rbegin()->fi > T) {
				pii q1 = *FlipSet.rbegin(), q2 = mp(q1.se, q1.fi);
				FlipSet.erase(q1); Set.erase(q2); s -= q1.fi;
			}
