...
i64 calc(i64 x) {
	if (x == 0) return 0;
	i64 Lim = 9, ans = 0;
	for (i64 cc=1; cc<=18; cc++) {
		i64 Max = min(Lim, x);
		Digit = Proceed(Max); i64 N = Digit.size();
		vector<vector<vi>> dp(N, vector<vi>(1024, vi(2, 0)));
		vector<vector<vi>> cnt(N, vector<vi>(1024, vi(2, 0)));
		for (i64 i=(N > 1); i<10; i++) {
			if (i > Digit[N-1]) break;
			if (i < Digit[N-1]) {
				dp[N-1][1LL << i][0] = modPow(10, N-1) * i;
				cnt[N-1][1LL << i][0] = 1;
			}
			else {
				dp[N-1][1LL << i][1] = modPow(10, N-1) * i;
				cnt[N-1][1LL << i][1] = 1;
			}
		}
		for (i64 i=N-1; i>0; i--) {
			for (i64 mask=0; mask<1024; mask++) {
				for (i64 equal=0; equal<2; equal++) {
					if (equal == 0) {
						for (i64 x=0; x<10; x++) {
							dp[i-1][mask | (1LL << x)][0] += dp[i][mask][equal] + ((modPow(10, i-1) * x) % Mod) * cnt[i][mask][equal];
							dp[i-1][mask | (1LL << x)][0] %= Mod;
							cnt[i-1][mask | (1LL << x)][0] += cnt[i][mask][equal]; cnt[i-1][mask | (1LL << x)][0] %= Mod;
						}
					}
					else {
						for (i64 x=0; x<10; x++) {
							...
							if (x < Digit[i-1]) {
								...
								dp[i-1][mask | (1LL << x)][1] += dp[i][mask][equal] + ((modPow(10, i-1) * x) % Mod) * cnt[i][mask][equal];
								dp[i-1][mask | (1LL << x)][1] %= Mod;
								cnt[i-1][mask | (1LL << x)][1] += cnt[i][mask][equal]; cnt[i-1][mask | (1LL << x)][1] %= Mod;
							}
						}
					}
				}
