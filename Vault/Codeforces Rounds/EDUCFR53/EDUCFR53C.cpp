...
bool valid(i64 Len) {
	...
	for (i64 i=0; i<n; i++) {
		...
		if (i >= Len-1) {
			i64 X = x + dx, Y = y + dy;
			if (abs(X) + abs(Y) <= Len && (abs(X) + abs(Y) - Len) % 2 == 0) return true;
			z = s[i-Len+1];
			if (z == 'U') dy--;
			if (z == 'R') dx--;
			if (z == 'L') dx++;
...
void binsearch(i64 top, i64 bot) {
	while (top <= bot) {
		i64 mid = (top + bot) / 2;
		if (valid(mid)) {len = mid; bot = mid - 1;}
		else top = mid + 1;
	}
}
...
void Solve() {
	...
	if (x == 0 && y == 0) {cout << "0"; return;}
	binsearch(1, n); cout << len;
}
/*****************************************/
