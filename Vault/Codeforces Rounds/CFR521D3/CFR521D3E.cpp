...
/************** [GLOBAL VARIABLES] **************/
i64 n; vi a; map<i64, i64> Map; multiset<i64> cc;
/************************************************/
...
void Solve() {
	for (auto it=Map.begin(); it!=Map.end(); it++) cc.insert(it->se);
	i64 ans = 0;
	for (i64 i=1; i<=n; i++) {
		i64 x = i, tmp = 0; vi Add, Delete;
		while (cc.lower_bound(x) != cc.end()) {
			i64 r = *cc.lower_bound(x);
			cc.erase(cc.find(r)); Delete.pub(r);
			tmp += x; x *= 2;
		}
		for (auto z: Delete) cc.insert(z);
		for (auto z: Add) cc.erase(cc.find(z));
		ans = max(ans, tmp);
	}
	cout << ans;
}
