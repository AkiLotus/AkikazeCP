...
/************** [GLOBAL VARIABLES] **************/
i64 n, k, maxcut = 0; vi a; map<i64, i64> Map;
/************************************************/

/************** [FUNCTIONS] **************/
bool valid(i64 cut) {
	priority_queue<pii> Q; i64 k2 = k;
	for (auto it=Map.begin(); it!=Map.end(); it++) {
		if (it->se >= cut) Q.push(mp(it->se, it->fi));
	}
	while (k2 > 0 && !Q.empty()) {
		pii Data = Q.top(); Q.pop(); Data.fi -= cut;
		if (Data.fi >= cut) Q.push(Data); k2--;
	}
	return (k2 == 0);
}
...
void binsearch() {
	...
	while (top <= bot) {
		i64 mid = (top + bot) / 2;
		if (valid(mid)) {maxcut = mid; top = mid + 1;}
		else bot = mid - 1;
	}
}
...
void Input() {
	cin >> n >> k; a.rsz(n);
	for (auto &x: a) {cin >> x; Map[x]++;}
}

void Solve() {
	binsearch(); priority_queue<pii> Q;
	for (auto it=Map.begin(); it!=Map.end(); it++) Q.push(mp(it->se, it->fi));
	while (k--) {
		pii Data = Q.top(); Q.pop();
		cout << Data.se << " "; Data.fi -= maxcut;
		Q.push(Data);
	}
}
/*****************************************/
