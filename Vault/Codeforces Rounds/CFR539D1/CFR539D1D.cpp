...
/***********************************************
** File name: Untitled                        **
** Time created: Sat Feb 16 2019 16:32pm      **
***********************************************/
...
i64 A(i64 n, i64 r) {
	return ((nCr(n, r) * ftrl[r]) % Mod);
}

i64 cayley(i64 x, i64 y) {
	if (x == y) return 1;
	return ((y * modPow(x, x-y-1)) % Mod);
}

void Input() {
	cin >> n >> m >> a >> b; ftrl.rsz(max(n, m)*4, 1);
	for (i64 i=1; i<max(n, m)*4; i++) ftrl[i] = (ftrl[i-1] * i) % Mod;
}

void Solve() {
	i64 res = 0;
	for (i64 x=2; x<=n; x++) {
		i64 comp1 = nCr(m-1, x-2);
		i64 comp2 = A(n-2, x-2);
		i64 comp3 = cayley(n, x);
		i64 comp4 = modPow(m, n-x);
		res += ((((comp1 * comp2) % Mod) * ((comp3 * comp4) % Mod)) % Mod);
		res %= Mod;
	}
	cout << res << endl;
}
...
/*********************************************************************************************************************************************************************************************************
 *tti;.                               .:,:i:         :,;;itt;.        fDDEDDEEEEEEKEEEEEKEEEEEEEEEEEEEEEEE###WKKKKKKKKKKKKKKKKKKKKWWWWWWWWWWWWWWWWWWW#WWWWWKKKKKEE      :,:.    f::::. .,ijLGDDDDDDDEEEEEEE*
