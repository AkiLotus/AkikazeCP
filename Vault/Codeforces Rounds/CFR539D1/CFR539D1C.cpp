...
/************** [GLOBAL VARIABLES] **************/
i64 q, N = 0; vector<vi> Queries; set<i64> possibleTime;
map<i64, i64> TimeID; vi TimeList;
/************************************************/
...
void Input() {
	...
	for (i64 i=0; i<q; i++) {
		vi Tmp;
		i64 f; cin >> f; Tmp.pub(f);
		if (f == 1) {
			i64 t, s; cin >> t >> s;
			Tmp.pub(t); Tmp.pub(s);
			possibleTime.insert(t);
		}
		else if (f == 2) {
			i64 t; cin >> t; Tmp.pub(t);
		}
		else if (f == 3) {
			i64 l, r, v; cin >> l >> r >> v;
			Tmp.pub(l); Tmp.pub(r); Tmp.pub(v);
			possibleTime.insert(l); possibleTime.insert(v);
		}
	}
}

void Solve() {
	for (auto it=possibleTime.begin(); it!=possibleTime.end(); it++) {
		TimeID[*it] = N++; TimeList.pub(*it);
	}
}
/*****************************************/
