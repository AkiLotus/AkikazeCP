...
/************** [FUNCTIONS] **************/
i64 modMul(i64 x, i64 y, i64 M) {
	if (y == 0) return 0;
	i64 tmp = modMul(x, y/2, M);
...
i64 modPow(i64 x, i64 y, i64 M) {
	if (y == 0) return (1 % M);
	i64 tmp = modPow(x, y/2, M);
	if (y % 2 == 0) return modMul(tmp, tmp, M);
	return modMul(modMul(tmp, tmp, M), x, M);
}

pii MulDiv(pii x, i64 y, i64 M) {
	if (y == 0) return mp(0LL, 0LL);
	pii tmp = MulDiv(x, y/2, M);
	pii res = mp(tmp.fi + tmp.fi, tmp.se + tmp.se);
	res.fi += (res.se / M); res.se %= M;
	if (y % 2 != 0) {
		res.fi += x.fi; res.se += x.se;
		res.fi += (res.se / M); res.se %= M;
	}
	return res;
}
...
void Solve() {
	//tracker4(n, a, b, B);
	i64 initQuotient = modMul(a, modPow(B, n-1, b), b);
	//cout << "initQuotient = " << initQuotient << endl;
	pii Quo_Rem = MulDiv(mp(initQuotient/b, initQuotient%b), B, b);
	//cout << "Quo_Rem = " << Quo_Rem << endl;
	cout << Quo_Rem.fi << endl;
}
/*****************************************/
