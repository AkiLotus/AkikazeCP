...
void Solve() {
	sort(a.begin(), a.end());
	if (a[0] >= 0) {cout << a[k-1]; return;}
	if (a[n-1] <= 0) {reverse(a.begin(), a.end()); cout << -a[k-1]; return;}
	i64 ans = LINF, id = lower_bound(a.begin(), a.end(), 0) - a.begin();
	if (id + k - 1 < n) ans = min(ans, a[id+k-1]);
	if (a[id] == 0) {if (id-k+1 >= 0) ans = min(ans, -a[id-k+1]);}
	else {if (id-k >= 0) ans = min(ans, -a[id-k]);}
	for (i64 i=id; i<min(n, id+k-1); i++) {
		if (i < k-1) continue;
		ans = min(ans, a[i]*2-a[i-k+1]);
	}
	for (i64 i=id-1; i>=max(0LL, id-k); i--) {
		if (i + k-1 >= n) continue;
		ans = min(ans, -a[i]*2+a[i+k-1]);
	}
	cout << ans;
