...
/************** [GLOBAL VARIABLES] **************/
i64 n, x, w, N; vector<pii> A; vi Ele, dp;
/************************************************/

/************** [FUNCTIONS] **************/
bool segmentSort(pii X, pii Y) {
...
void Solve() {
	sort(A.begin(), A.end(), segmentSort);
	dp.rsz(n, 1LL); vi Right(n);
	for (i64 i=0; i<n; i++) Right[i] = A[i].se;
	for (i64 i=1; i<n; i++) {
		dp[i] = max(dp[i], dp[i-1]);
		i64 id = upper_bound(Right.begin(), Right.end(), A[i].fi) - Right.begin();
		if (id == 0) dp[i] = max(dp[i], 1LL);
		else dp[i] = max(dp[i], dp[id-1] + 1);
	}
	cout << dp[n-1];
}
/*****************************************/
