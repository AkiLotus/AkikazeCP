...
/************** [GLOBAL VARIABLES] **************/
i64 t, h, w, fractalSize; vector<vc> A, B;
vi ShiroFractal, ShiroPR, ShiroPC;
/************************************************/

/************** [FUNCTIONS] **************/
void Recurse(i64 L, i64 R) {
	if (L == R) return;
	i64 size = R - L + 1;
	ShiroFractal[L+size/3]++; ShiroFractal[L+size*2/3]--;
	Recurse(L, L+size/3-1); Recurse(L+size*2/3, R);
}

void GenerateFractalMatrix() {
	Recurse(0, fractalSize-1);
	for (i64 i=1; i<fractalSize; i++) ShiroFractal[i] += ShiroFractal[i-1];
}
...
void GeneratePatternMatrix() {
	...
	for (i64 i=0; i<h; i++) {
		if (!ShiroPR[i]) continue;
		for (i64 j=0; j<w; j++) B[i][j] = '.';
	}
	for (i64 j=0; j<w; j++) {
		if (!ShiroPC[j]) continue;
		for (i64 i=0; i<h; i++) B[i][j] = '.';
	}
	if (A != B) {cout << "0"; Exit();}
}

bool FullShiro(vector<vc> a) {
...
i64 SubarrayCount(vi Source, vi Pattern) {
	i64 PatternSize = Pattern.size();
	Pattern.pub(2); i64 res = 0;
	for (auto z: Source) Pattern.pub(z);
	vi Z = Z_Algorithm(Pattern);
	for (auto z: Z) res += (z == PatternSize);
	return res;
}
...
void Input() {
	cin >> t >> h >> w; fractalSize = 1;
	for (i64 i=0; i<t; i++) fractalSize *= 3;
	ShiroFractal.rsz(fractalSize, 0LL);
	ShiroPR.rsz(h, 0LL); ShiroPC.rsz(w, 0LL);
	A.rsz(h, vc(w, 'x'));
	for (i64 i=0; i<h; i++) {
		for (i64 j=0; j<w; j++) {
			cin >> A[i][j];
...
void Solve() {
	GenerateFractalMatrix();
	GeneratePatternMatrix();
	i64 rcnt = SubarrayCount(ShiroFractal, ShiroPR);
	i64 ccnt = SubarrayCount(ShiroFractal, ShiroPC);
	tracker5(rcnt, ccnt, h, w, rcnt * (fractalSize - w + 1) + ccnt * (fractalSize - h + 1));
	if (FullShiro(A)) cout << rcnt * (fractalSize - w + 1) + ccnt * (fractalSize - h + 1) - rcnt * ccnt;
	else cout << rcnt * ccnt;
}
/*****************************************/
