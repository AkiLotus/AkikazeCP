...
ld solve(i64 z) {
	...
	ld res = 0.0L;
	for (i64 i=1; i<=36; i++) {
		i64 LowerBound = (1LL << (i - 1));
		if (i == 1) LowerBound = 0;
		if (z < LowerBound) break;
		if (z == LowerBound) {res += (ld)1.0L / i; break;}
		if ((i > 1 && z >= LowerBound * 2 - 1) || (i == 1 && z >= 1)) {
			ld tmp = i * (1LL << (i - 1)) - (i - 1) * (1LL << (i - 2));
			tmp /= i; res += tmp; continue;
		}
		ld tmp = sumbit(z) - sumbit(LowerBound-1);
		tmp /= i; res += tmp; break;
	}
	return res;
}
