...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, k, s, t, a, b, l, u, v, q;
vector<vector<pii>> adjIn, adjOut;
/************************************************/
...
void Solve() {
	vi DistOut(n, LINF), DistIn(n, LINF); i64 ans = LINF;
	DistOut[s] = 0; DistIn[t] = 0; vb vis(n, false);
	priority_queue<pii> Q; Q.push(mp(-DistOut[s], s));
	while (!Q.empty()) {
		pii Z = Q.top(); Q.pop();
		i64 z = Z.se, d = -Z.fi;
		...
		for (auto T: adjOut[z]) {
			...
			if (d+p < DistOut[w]) {
				DistOut[w] = d + p;
				Q.push(mp(-DistOut[w], w));
			}
		}
	}
	vis.clear(); vis.rsz(n, false); Q.push(mp(-DistIn[t], t));
	while (!Q.empty()) {
		pii Z = Q.top(); Q.pop();
		i64 z = Z.se, d = -Z.fi;
		if (vis[z]) continue; vis[z] = true;
		for (auto T: adjIn[z]) {
			...
			if (d+p < DistIn[w]) {
				DistIn[w] = d + p;
				Q.push(mp(-DistIn[w], w));
			}
		}
	}
	ans = min(ans, DistOut[t]);
	while (k--) {
		cin >> u >> v >> q; u--; v--;
		ans = min(ans, DistOut[u] + q + DistIn[v]);
		ans = min(ans, DistOut[v] + q + DistIn[u]);
	}
	if (ans == LINF) cout << "1";
	else cout << ans;
}
