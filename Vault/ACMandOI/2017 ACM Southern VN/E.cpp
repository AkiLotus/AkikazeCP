...
void Solve() {
	...
	for (i64 i=0; i<n/2; i++) {
		for (i64 j=0; j<m/2; j++) {
			i64 tmp = 0; map<i64, i64> Map;
			Map[A[i][j]]++; Map[A[n-1-i][m-1-j]]++;
			Map[A[n-1-i][j]]++; Map[A[i][m-1-j]]++;
			for (auto it=Map.begin(); it!=Map.end(); it++) tmp = max(tmp, it->se);
			ans += (4 - tmp);
		}
	}
	if (n % 2 != 0) {
