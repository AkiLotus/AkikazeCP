...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, A, B, u, v; vector<vi> adj; vi Vis;
map<pii, i64> Polygon; map<i64, i64> cnt; i64 Pol = 0;
stack<pii> Edge; stack<pii> PolyID;
/************************************************/
...
void DFS(i64 z, i64 last) {
	...
	if (!PolyID.empty()) {
		Polygon[mp(z, last)] = PolyID.top().fi;
		Polygon[mp(last, z)] = PolyID.top().fi;
		cnt[PolyID.top().fi]++;
		if (last == PolyID.top().se) PolyID.pop();
	}
	Vis[z] = 1;
}
...
void Solve() {
	DFS(A, -1); vb vis(n, false); vi Last(n, -1);
	queue<i64> Q; Q.push(A); vis[A] = true;
	while (!Q.empty()) {
		i64 z = Q.front(); Q.pop();
		if (z == B) break;
		for (auto t: adj[z]) {
	...
	i64 ans = 0, x = B, lastpoly = -1;
	vi cc(Pol+1, 0);
	while (Last[x] != -1) {
		i64 y = Last[x];
		if (Polygon[mp(x,y)] != lastpoly) {
			if (lastpoly != -1) ans += max(cc[lastpoly], cnt[lastpoly]-cc[lastpoly]);
		}
		cc[Polygon[mp(x,y)]]++;
		lastpoly = Polygon[mp(x,y)]; x = y;
	}
	ans += max(cc[lastpoly], cnt[lastpoly]-cc[lastpoly]);
	cout << ans;
}
/*****************************************/
