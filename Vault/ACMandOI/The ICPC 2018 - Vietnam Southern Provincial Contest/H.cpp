...
/************** [GLOBAL VARIABLES] **************/
i64 m, n, xs, ys, xf, yf; vector<vc> A, B;
vector<vb> vis; vector<vi> Dist;
i64 dx[] = {-1, +0, +0, +1};
i64 dy[] = {+0, -1, +1, +0};
/************************************************/
...
void Input() {
	cin >> m >> n; A.rsz(m, vc(n));
	B.rsz(m, vc(n)); vis.rsz(m, vb(n, false));
	Dist.rsz(m, vi(n, INF));
	for (i64 i=0; i<m; i++) {
		for (i64 j=0; j<n; j++) {
			cin >> A[i][j];
...
void Solve() {
	...
	while (!Q.empty()) {
		...
		for (i64 i=0; i<4; i++) {
			...
			if (A[x+dx[i]][y+dy[i]] == '#' || B[x+dx[i]][y+dy[i]] == '#') continue;
			Dist[x+dx[i]][y+dy[i]] = Dist[x][y] + 1;
			Q.push(mp(x+dx[i], y+dy[i])); vis[x+dx[i]][y+dy[i]] = true;
		}
	}
	if (!vis[xf][yf]) cout << "-1"; else cout << Dist[xf][yf] - 1;
}
/*****************************************/
