...
/************** [GLOBAL VARIABLES] **************/
i64 l, m, p, c, t, r; string n; string z;
/************************************************/

/************** [FUNCTIONS] **************/
i64 intParse(string str) {
	i64 res = 0;
...
void preprocess() {
	i64 comma = 0; string s = ""; z += ',';
	for (auto x: z) {
		if (x == ',') {
			if (comma == 0) n = s;
			else if (comma == 1) p = intParse(s);
			else if (comma == 2) c = intParse(s);
			else if (comma == 3) t = intParse(s);
			else if (comma == 4) r = intParse(s);
			comma++; s = "";
		}
		else s += x;
	}
...
bool valid() {
	for (i64 i=1; i<=20000; i++) {
		i64 remainingTime = 10080 - r * i;
		if (remainingTime <= 0) break;
		if (remainingTime * c >= l && (i - 1) * t <= remainingTime && remainingTime <= t * (i + 1)) {
...
void Solve() {
	i64 optimal = LINF; vs A;
	for (i64 i=0; i<m; i++) {
		getline(cin, z);
		preprocess();
		tracker5(n, p, c, t, r);
		if (valid()) {
			if (optimal < p) continue;
			if (optimal > p) A.clear();
			optimal = p; A.pub(n);
		}
	}
	if (A.empty()) cout << "no such mower";
	else for (auto x: A) cout << x << endl;
}
/*****************************************/
