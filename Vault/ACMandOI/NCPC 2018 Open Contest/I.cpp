...
i64 n; string s; vector<pss> A; vs W;
/************************************************/

/************** [FUNCTIONS] **************/
bool nonZero(string st) {
	for (auto x: st) {
		if (x != '0') return true;
	}
	return false;
...
string subtract(string a, string b) {
	i64 carry = 0; string c = "";
	for (i64 i=0; i<1000; i++) c += '0';
	for (i64 i=999; i>=0; i--) {
		i64 x = a[i] - '0', y = b[i] - '0';
		y += carry; carry = 0;
		while (x < y) {x += 10; carry++;}
		c[i] = char(48+x-y);
	}
	return c;
}
...
void Input() {
	cin >> n >> s; A.rsz(n);
	s.insert(0, 1000-s.size(), '0');
	for (i64 i=0; i<n; i++) {
		cin >> A[i].se >> A[i].fi;
		A[i].fi.insert(0, 1000-A[i].fi.size(), '0');
	}
}
...
void Solve() {
	...
	for (i64 i=0; i<n; i++) {
		...
		s = subtract(s, A[i].fi); W.pub(A[i].se);
	}
	if (nonZero(s)) {cout << "0"; return;}
	cout << W.size() << endl;
