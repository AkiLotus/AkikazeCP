...
void DFS(i64 z) {
	vi cnt(k, 0LL);
	if (adj[z].size() == 1 && z != 0) {
		for (i64 x=0; x<k; x++) dp[z][x] = 1;
		return;
	}
	for (auto t: adj[z]) {
		if (p[z] == t) continue; DFS(t);
		for (i64 x=0; x<k; x++) cnt[x] += dp[t][x];
	}
}

void Input() {
	cin >> n >> k; adj.rsz(n);
	dp.rsz(n, vi(n, 0LL)); p.rsz(n, -1);
	for (i64 i=1; i<n; i++) {
		cin >> p[i]; adj[i].pub(p[i]); adj[p[i]].pub(i);
	}
}

void Solve() {
	DFS(0); i64 ans = 0;
	for (i64 i=0; i<k; i++) ans = (ans + dp[0][i]) % Mod;
	cout << ans;
}
/*****************************************/
