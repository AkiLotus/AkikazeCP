...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, xs, ys, xf, yf, xi, yi;
i32 Dist[5002][5002]; pii Queue[37500040];
i32 Back = 0, Front = 0;
bool vis[5002][5002], obs[5002][5002];
i64 DistX[] = {0, -1, -1};
i64 DistY[] = {0, -1, +1};
/************************************************/
...
void Solve() {
	Queue[0] = mp(xs, ys);
	Dist[xs][ys] = 0; vis[xs][ys] = true;
	while (Front <= Back) {
		pii z = Queue[Front]; Front++;
		i64 x = z.fi, y = z.se;
		if (x == xf && y == yf) {cout << Dist[x][y]; return;}
		for (i64 i=1; i<n; i++) {
			if (vis[x-i][y-i]) break;
			Dist[x-i][y-i] = Dist[x][y] + 1;
			vis[x-i][y-i] = true;
			Queue[++Back] = mp(x-i, y-i);
		}
		for (i64 i=1; i<n; i++) {
			if (vis[x+i][y+i]) break;
			Dist[x+i][y+i] = Dist[x][y] + 1;
	...
	if (Dist[xf][yf] == 1000000000) cout << "-1";
