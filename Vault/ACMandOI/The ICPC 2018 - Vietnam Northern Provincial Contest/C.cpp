...
/************** [GLOBAL VARIABLES] **************/
typedef pair<char, i64> pci;
string Task, X, Y, S; i64 p, c;
/************************************************/

/************** [FUNCTIONS] **************/
vector<pci> toVector(string z) {
	if (z.empty()) return vector<pci>(0);
	z += "/"; char Token = '\0'; i64 tmp = 0; vector<pci> A;
	for (i64 i=0; i<z.size(); i++) {
		if (z[i] >= '0' && z[i] <= '9') {
			tmp = tmp * 10 + (z[i] - '0');
		}
		else {
			if (tmp > 0) A.pub(mp(Token, tmp));
			tmp = 0; Token = z[i];
		}
	}
	return A;
...
i64 stringSize(string z) {
	z += "/"; char Token = '\0'; i64 tmp = 0, Size = 0;
	for (i64 i=0; i<z.size(); i++) {
		if (z[i] >= '0' && z[i] <= '9') {
			tmp = tmp * 10 + (z[i] - '0');
		}
...
string Substr(string x, i64 L, i64 R) {
	vector<pci> A = toVector(x), Z; vi cnt; i64 Last = 0;
	for (auto z: A) {cnt.pub(Last); Last += z.se;}
	i64 st = upper_bound(cnt.begin(), cnt.end(), L) - cnt.begin() - 1;
	i64 en = upper_bound(cnt.begin(), cnt.end(), R) - cnt.begin() - 1;
	if (st == en) Z.pub(mp(A[st].fi, R - L + 1));
	else {
		for (i64 z=st; z<=en; z++) {
			if (z == st) Z.pub(mp(A[z].fi, cnt[z+1] - L));
			else if (z == en) Z.pub(mp(A[z].fi, R - cnt[z] + 1));
			else Z.pub(A[z]);
		}
	}
	return toString(Z);
...
string Merge(string x, string y) {
	vector<pci> A = toVector(x), B = toVector(y), C = A;
	if (!A.empty() && !B.empty() && A[A.size()-1].fi == B[0].fi) C[C.size()-1].se += B[0].se;
	else if (!B.empty()) C.pub(B[0]);
	for (i64 i=1; i<B.size(); i++) C.pub(B[i]);
	return toString(C);
}
...
void Solve() {
	while (cin >> Task) {
		if (Task == "@1") {
			cin >> X >> Y;
			cout << Task << ": " << Merge(X, Y) << endl;
		}
		else if (Task == "@2") {
			cin >> S >> p >> c;
			cout << Task << ": " << Merge(Substr(S, 0, p-2), Substr(S, p+c-1, stringSize(S)-1)) << endl;
		}
		else if (Task == "@3") {
			cin >> S >> p >> c;
			cout << Task << ": " << Substr(S, p-1, p+c-2) << endl;
		}
		else if (Task == "@4") {
			cin >> X >> Y >> p;
			string Z = Substr(X, 0, p-2);
			string T = Substr(X, p-1, stringSize(X)-1);
			cout << Task << ": " << Merge(Merge(Z, Y), T) << endl;
		}
	}
}
/*****************************************/
