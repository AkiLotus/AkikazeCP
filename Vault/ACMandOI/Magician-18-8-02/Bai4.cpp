...
i64 N, M; vector<ppp> segments; vector<pii> cows, limits;
vi cowPack; map<i64, i64> MapX, MapY;
vector<vi> cowID; i64 A, B; vector<vb> vis, denied;
/************************************************/

/************** [FUNCTIONS] **************/
void DFS(i64 x, i64 y) {
	if (cowID[x][y]) cowPack.pub(4LL); vis[x][y] = true;
	if (x-1 >= 0 && !vis[x-1][y] && !denied[x-1][y]) DFS(x-1, y);
	if (y-1 >= 0 && !vis[x][y-1] && !denied[x][y-1]) DFS(x, y-1);
	if (x+1 < A && !vis[x+1][y] && !denied[x+1][y]) DFS(x+1, y);
	if (y+1 < B && !vis[x][y+1] && !denied[x][y+1]) DFS(x, y+1);
}

void Input() {
	cin >> N >> M; segments.rsz(N); cows.rsz(M);
	for (i64 i=0; i<N; i++) {
		cin >> segments[i].fi.fi >> segments[i].fi.se >> segments[i].se.fi >> segments[i].se.se;
		MapX[segments[i].fi.fi]++; MapY[segments[i].fi.se]++;
		MapX[segments[i].se.fi]++; MapY[segments[i].se.se]++;
	}
	for (i64 i=0; i<M; i++) {
		cin >> cows[i].fi >> cows[i].se;
		MapX[cows[i].fi]++; MapY[cows[i].se]++;
	}
}

void Solve() {
	// Compress - Discretization
	vi xcoor, ycoor; map<i64, i64> xComp, yComp;
	for (auto it=MapX.begin(); it!=MapX.end(); it++) {
		xComp[it->fi] = xcoor.size();
		xcoor.pub(it->fi);
	}
	for (auto it=MapY.begin(); it!=MapY.end(); it++) {
		yComp[it->fi] = ycoor.size();
		ycoor.pub(it->fi);
	}
	for (i64 i=0; i<N; i++) {
		segments[i].fi.fi = xComp[segments[i].fi.fi] * 2;	
		segments[i].fi.se = yComp[segments[i].fi.se] * 2;
		segments[i].se.fi = xComp[segments[i].se.fi] * 2;
		segments[i].se.se = yComp[segments[i].se.se] * 2;
		segments[i].fi.fi++; segments[i].fi.se++;
		segments[i].se.fi++; segments[i].se.se++;
	}
	for (i64 i=0; i<M; i++) {
		cows[i].fi = xComp[cows[i].fi] * 2;
		cows[i].se = yComp[cows[i].se] * 2;
		cows[i].fi++; cows[i].se++;
	}
	
	// Initiate Minecraft world
	A = xcoor.size() * 2 + 4; B = ycoor.size() * 2 + 4;
	cowID.rsz(A, vi(B, 0)); map<pii, i64> Map;
	denied.rsz(A, vb(B, false));
	for (i64 i=0; i<M; i++) cowID[cows[i].fi][cows[i].se] = i + 1;
	for (auto x: segments) {
		i64 x1 = x.fi.fi, y1 = x.fi.se, x2 = x.se.fi, y2 = x.se.se;
		limits.pub(mp(x1, y1)); limits.pub(mp(x2, y2));
		Map[mp(x1, y1)]++; Map[mp(x2, y2)]++;
		if (x1 == x2) {
			for (i64 j=y1; j<=y2; j++) denied[x1][j] = true;
			for (i64 j=y1; j>=y2; j--) denied[x1][j] = true;
		}
		else {
			for (i64 i=x1; i<=x2; i++) denied[i][y1] = true;
			for (i64 i=x1; i>=x2; i--) denied[i][y1] = true;
		}
	}
	
	// DFS and similar
	vis.rsz(A, vb(B, false)); i64 ans = 0;
	for (auto Cow: cows) {
		if (vis[Cow.fi][Cow.se]) continue;
		cowPack.clear(); DFS(Cow.fi, Cow.se);
		ans = max(ans, (i64)cowPack.size());
	}
