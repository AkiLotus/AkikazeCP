...
string add(string a, string b) {
	// standardized strings, length 40
	string res = ""; res.insert(0, 40, '0');
	i64 carry = 0;
	for (i64 i=39; i>=0; i--) {
		i64 val = (a[i] + b[i] - 96) + carry;
		res[i] = val % 10 + 48; carry = val / 10;
	}
	return res;
}
...
string subtract(string a, string b) {
	...
	i64 carry = 0;
	for (i64 i=39; i>=0; i--) {
		i64 val = (a[i] - '0') - (b[i] - '0') - carry;
		carry = 0; while (val < 0) {val += 10; carry++;}
		res[i] = val + 48;
	}
	return res;
}
...
string multiply(string a, string b) {
	...
	string Default = res;
	for (i64 i=39; i>=0; i--) {
		i64 multiplier = b[i] - '0';
		if (multiplier == 0) continue;
		string tmp = Default; i64 carry = 0;
		for (i64 j=39; j>=0; j--) {
			i64 val = (a[j] - '0') * multiplier + carry;
			tmp[j] = val % 10 + 48; carry = val / 10;
		}
		for (i64 j=i; j<39; j++) {tmp += '0'; tmp.erase(0, 1);}
		res = add(res, tmp);
	}
	return res;
}

string toStr40(i64 z) {
	string res = "";
	if (z == 0) res += "0";
	else while (z > 0) {res.insert(0, 1, char(48+z%10)); z /= 10;}
	res.insert(0, 40-res.size(), '0');
	return res;
}

i64 sqrtI128(string z) {
	i64 top = 1, bot = LINF, ans = LINF;
	z.insert(0, 40-z.size(), '0');
	while (top <= bot) {
		i64 mid = (top + bot) / 2;
		string M = toStr40(mid);
		string sqrM = multiply(M, M);
...
void Solve() {
	while (m--) {
		cin >> n; n.insert(0, 40-n.size(), '0');
		i64 segment = sqrtI128(n);
		if (multiply(toStr40(segment), toStr40(segment)) < n) segment++;
		i64 id = I64Parse(subtract(n, multiply(toStr40(segment - 1), toStr40(segment - 1))));
