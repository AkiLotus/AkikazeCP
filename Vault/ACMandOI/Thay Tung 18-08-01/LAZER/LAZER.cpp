...
int n, t, x, dj; vector<ppp> a;
unordered_map<int, vector<int>> totalPower;
/************************************************/

/************** [FUNCTIONS] **************/
void Input() {
	cin >> n; a.rsz(n);
	for (int i=0; i<n; i++) cin >> a[i].fi.fi >> a[i].fi.se >> a[i].se.fi >> a[i].se.se;
}

void Solve() {
	// Initializing prefix sum
	for (int i=-240000; i<=240000; i++) {
		totalPower[i].rsz(4, 0);
	}

	// Phase 1: Direction d = 1
	for (auto x: a) {
		int accumulated = x.se.se - x.fi.se;
		totalPower[x.fi.fi][1] += accumulated;
		totalPower[x.se.fi+1][1] -= accumulated;
	}
	for (int i=-239999; i<=240000; i++) totalPower[i][1] += totalPower[i-1][1];

	// Phase 2: Direction d = 2
	for (auto x: a) {
		int minimumRaiseL = x.fi.fi + x.fi.se;
		int stableL = x.fi.fi + x.se.se;
		int stableR = x.se.fi + x.fi.se;
		int minimumLowerR = x.se.fi + x.se.se;
		totalPower[minimumRaiseL+1][2]++; totalPower[stableL+1][2]--;
		totalPower[stableR+1][2]--; totalPower[minimumLowerR+1][2]++;
	}
	for (int i=-239999; i<=240000; i++) totalPower[i][2] += totalPower[i-1][2];
	for (int i=-239999; i<=240000; i++) totalPower[i][2] += totalPower[i-1][2];

	// Phase 3: Direction d = 3
	for (auto x: a) {
		int minimumRaiseL = x.fi.fi - x.se.se;
		int stableL = x.fi.fi - x.fi.se;
		int stableR = x.se.fi - x.se.se;
		int minimumLowerR = x.se.fi - x.fi.se;
		totalPower[minimumRaiseL+1][3]++; totalPower[stableL+1][3]--;
		totalPower[stableR+1][3]--; totalPower[minimumLowerR+1][3]++;
	}
	for (int i=-239999; i<=240000; i++) totalPower[i][3] += totalPower[i-1][3];
	for (int i=-239999; i<=240000; i++) totalPower[i][3] += totalPower[i-1][3];

	cin >> t;
	while (t--) {
		cin >> x >> dj;
		if (dj == 1) cout << 1LL * totalPower[x][dj] * totalPower[x][dj] << endl;
		else cout << 2LL * totalPower[x][dj] * totalPower[x][dj] << endl;
	}
}
/*****************************************/
