...
i64 N, K; vector<pppi> Data; vb vis;
/************************************************/

/************** [FUNCTIONS] **************/
bool cmp1(pppi x, pppi y) {
	return (x.fi.fi.fi > y.fi.fi.fi);
}

bool cmp2(pppi x, pppi y) {
	return (x.fi.fi.se > y.fi.fi.se);
}

bool cmp3(pppi x, pppi y) {
	return (x.fi.se > y.fi.se);
}

void Input() {
	cin >> N >> K; Data.rsz(N); vis.rsz(N, false);
	for (i64 i=0; i<N; i++) {cin >> Data[i].fi.fi.fi >> Data[i].fi.fi.se >> Data[i].fi.se; Data[i].se = i;}
	
}

void Solve() {
	sort(Data.begin(), Data.end(), cmp1);
	for (i64 i=0; i<K; i++) vis[Data[i].se] = true;
	sort(Data.begin(), Data.end(), cmp2);
	for (i64 i=0; i<K; i++) vis[Data[i].se] = true;
	sort(Data.begin(), Data.end(), cmp3);
	for (i64 i=0; i<K; i++) vis[Data[i].se] = true;
	i64 ans = 0;
	for (i64 i=0; i<N; i++) ans += (vis[i]);
	cout << ans;
}
/*****************************************/
