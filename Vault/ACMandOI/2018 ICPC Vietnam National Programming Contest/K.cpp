...
/************** [GLOBAL VARIABLES] **************/
i64 Q, t, x1, y1, x2, y2, j;
int qx1[100007], qy1[100007], qx2[100007], qy2[100007];
short int Tree[20008][20008], Lazy[20008][20008];
/************************************************/

/************** [FUNCTIONS] **************/
void Update(i64 nodex, i64 nodey, i64 stx, i64 enx, i64 sty, i64 eny, i64 x1, i64 y1, i64 x2, i64 y2, i64 val) {
	if (sty > eny || eny < y1 || y2 < sty) return;
	if (stx > enx || enx < x1 || x2 < stx) return;
	if (Lazy[nodex][nodey] != 0) {
		Tree[nodex][nodey] += (enx - stx + 1) * (eny - sty + 1) * Lazy[nodex][nodey];
		if (stx != enx && sty != eny) {
			Lazy[nodex*2+0][nodey*2+0] += Lazy[nodex][nodey];
			Lazy[nodex*2+1][nodey*2+0] += Lazy[nodex][nodey];
			Lazy[nodex*2+0][nodey*2+1] += Lazy[nodex][nodey];
			Lazy[nodex*2+1][nodey*2+1] += Lazy[nodex][nodey];
		}
		else if (stx != enx) {
			Lazy[nodex*2+0][nodey] += Lazy[nodex][nodey];
			Lazy[nodex*2+1][nodey] += Lazy[nodex][nodey];
		}
		else if (sty != eny) {
			Lazy[nodex][nodey*2+0] += Lazy[nodex][nodey];
			Lazy[nodex][nodey*2+1] += Lazy[nodex][nodey];
		}
		Lazy[nodex][nodey] = 0;
	}
	if (x1 <= stx && enx <= x2 && y1 <= sty && eny <= y2) {
		Tree[nodex][nodey] += (enx - stx + 1) * (eny - sty + 1) * val;
		if (stx != enx && sty != eny) {
			Lazy[nodex*2+0][nodey*2+0] += val;
			Lazy[nodex*2+1][nodey*2+0] += val;
			Lazy[nodex*2+0][nodey*2+1] += val;
			Lazy[nodex*2+1][nodey*2+1] += val;
		}
		else if (stx != enx) {
			Lazy[nodex*2+0][nodey] += val;
			Lazy[nodex*2+1][nodey] += val;
		}
		else if (sty != eny) {
			Lazy[nodex][nodey*2+0] += val;
			Lazy[nodex][nodey*2+1] += val;
		}
		return;
	}
	if (!(x1 <= stx && enx <= x2) && !(y1 <= sty && eny <= y2)) {
		Update(nodex*2+0, nodey*2+0, stx, (stx+enx)/2, sty, (sty+eny)/2, x1, y1, x2, y2, val);
		Update(nodex*2+0, nodey*2+1, stx, (stx+enx)/2, (sty+eny)/2+1, eny, x1, y1, x2, y2, val);
		Update(nodex*2+1, nodey*2+0, (stx+enx)/2+1, enx, sty, (sty+eny)/2, x1, y1, x2, y2, val);
		Update(nodex*2+1, nodey*2+1, (stx+enx)/2+1, enx, (sty+eny)/2+1, eny, x1, y1, x2, y2, val);
	}
	else if (!(x1 <= stx && enx <= x2)) {
		Update(nodex*2+0, nodey, stx, (stx+enx)/2, sty, eny, x1, y1, x2, y2, val);
		Update(nodex*2+1, nodey, (stx+enx)/2+1, enx, sty, eny, x1, y1, x2, y2, val);
	}
	else if (!(y1 <= sty && eny <= y2)) {
		Update(nodex, nodey*2+0, stx, enx, sty, (sty+eny)/2, x1, y1, x2, y2, val);
		Update(nodex, nodey*2+1, stx, enx, (sty+eny)/2+1, eny, x1, y1, x2, y2, val);
	}
}

i64 Get(i64 nodex, i64 nodey, i64 stx, i64 enx, i64 sty, i64 eny, i64 x1, i64 y1, i64 x2, i64 y2) {
	if (Lazy[nodex][nodey] != 0) {
		Tree[nodex][nodey] += (enx - stx + 1) * (eny - sty + 1) * Lazy[nodex][nodey];
		if (stx != enx && sty != eny) {
			Lazy[nodex*2+0][nodey*2+0] += Lazy[nodex][nodey];
	...
	if (sty > eny || eny < y1 || y2 < sty) return 0;
	if (stx > enx || enx < x1 || x2 < stx) return 0;
	if (x1 <= stx && enx <= x2 && y1 <= sty && eny <= y2) return Tree[nodex][nodey];
	i64 p1 = 0, p2 = 0, p3 = 0, p4 = 0;
	if (!(x1 <= stx && enx <= x2) && !(y1 <= sty && eny <= y2)) {
		p1 = Get(nodex*2+0, nodey*2+0, stx, (stx+enx)/2, sty, (sty+eny)/2, x1, y1, x2, y2);
		p2 = Get(nodex*2+0, nodey*2+1, stx, (stx+enx)/2, (sty+eny)/2+1, eny, x1, y1, x2, y2);
		p3 = Get(nodex*2+1, nodey*2+0, (stx+enx)/2+1, enx, sty, (sty+eny)/2, x1, y1, x2, y2);
		p4 = Get(nodex*2+1, nodey*2+1, (stx+enx)/2+1, enx, (sty+eny)/2+1, eny, x1, y1, x2, y2);
	}
	else if (!(x1 <= stx && enx <= x2)) {
		p1 = Get(nodex*2+0, nodey, stx, (stx+enx)/2, sty, eny, x1, y1, x2, y2);
		p2 = Get(nodex*2+1, nodey, (stx+enx)/2+1, enx, sty, eny, x1, y1, x2, y2);
	}
	else if (!(y1 <= sty && eny <= y2)) {
		p1 = Get(nodex, nodey*2+0, stx, enx, sty, (sty+eny)/2, x1, y1, x2, y2);
		p2 = Get(nodex, nodey*2+1, stx, enx, (sty+eny)/2+1, eny, x1, y1, x2, y2);
	}
	return (p1 + p2 + p3 + p4);
}
...
void Solve() {
	for (i64 q=1; q<=Q; q++) {
		...
		if (t == 1) {
			cin >> x1 >> y1 >> x2 >> y2;
			qx1[q] = x1; qy1[q] = y1; qx2[q] = x2; qy2[q] = y2;
			Update(1, 1, 0, 5000, 0, 5000, x1, y1, x2, y2, 1);
		}
		else if (t == 2) {
			cin >> j;
			Update(1, 1, 0, 5000, 0, 5000, qx1[j], qy1[j], qx2[j], qy2[j], -1);
		}
		else if (t == 3) {
			cin >> x1 >> y1 >> x2 >> y2;
			i64 Region1 = Get(1, 1, 0, 5000, 0, 5000, x1, y1, x1, y1);
			i64 Region2 = Get(1, 1, 0, 5000, 0, 5000, x2, y2, x2, y2);
			tracker2(Region1, Region2);
			if (Region1 == Region2) cout << "Y\n"; else cout << "N\n";
		}
		for (i64 x=1; x<=8; x++) {
			for (i64 y=1; y<=8; y++) {
				cout << Get(1, 1, 0, 5000, 0, 5000, x, y, x, y) << " ";
			}
			cout << endl;
		}
		cout << endl;
