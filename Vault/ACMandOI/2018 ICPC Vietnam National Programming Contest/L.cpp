...
/************** [GLOBAL VARIABLES] **************/
i64 N; vi V, C; vb isPrime;
vector<vector<vi>> HappinessTrigger;
/************************************************/
...
void Preprocess() {
	...
	for (i64 i=2; i<=N; i++) {
		...
		i64 Mul = i;
		for (i64 j=1; j<20; j++) {
			for (i64 k=1; k<=N/Mul; k++) {
				if (k % i == 0) continue;
				if (j != 1 || k != 1) isPrime[Mul*k] = false;
				HappinessTrigger[i][j].pub(Mul*k);
			}
			if (LINF / i <= Mul) break;
			Mul *= i;
		}
...
void Input() {
	cin >> N; HappinessTrigger.rsz(N+1, vector<vi>(20, vi(0)));
	V.rsz(N+1); C.rsz(N+1);
...
void Solve() {
	Preprocess(); vector<vi> dp(N+1, vi(20, -LINF)); dp[1][0] = V[1];
	cout << isPrime << endl;
	for (i64 i=1; i<=10; i++) {
		for (i64 j=0; j<=4; j++) {
			tracker3(i, j, HappinessTrigger[i][j]);
		}
	}
	for (i64 i=2; i<=N; i++) {
		for (i64 j=0; j<20; j++) {
			if (j == 0) {dp[i][j] = max(dp[i][j], *max_element(dp[i-1].begin(), dp[i-1].end())); continue;}
			i64 Increase = 0, Decrease = (j * 2 - 1) * C[i];
			for (auto x: HappinessTrigger[i][j]) Increase += V[x];
			dp[i][j] = max(dp[i][j], dp[i][j-1] + Increase - Decrease);
		}
		cout << "dp[" << i << "] = " << dp[i] << endl;
	}
