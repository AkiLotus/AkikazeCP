...
void ProSolve() {
	while (T--) {
		cin >> D >> P; i64 N = P.size();
		i64 Dmg = 0, Curr = 1, Cnt = 0, ans = 0;
		for (i64 i=0; i<N; i++) {
			if (P[i] == 'C') Curr *= 2;
			else {Dmg += Curr; Cnt++;}
		}
		if (Dmg <= D) {cout << "Case #" << ++cas << ": 0\n"; continue;}
		if (Cnt > D) {cout << "Case #" << ++cas << ": IMPOSSIBLE\n"; continue;}
		for (i64 i=N-1; i>0;) {
