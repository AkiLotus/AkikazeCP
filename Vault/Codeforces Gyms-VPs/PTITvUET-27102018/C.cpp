...
void TimerStop();
void Exit();
string cppstr_infile = "codecoder.in";
string cppstr_outfile = "codecoder.out";
/*********************************************************/
...
int comp_pos(node a, node b) {
	return (a.pos < b.pos);
}

int comp_pos2(node a, node b) {
...
int comp_y(node a, node b) {
	return (a.y < b.y);
}

int next(int u) {
	if (a[d[u]].r == u) return u;
	else return(a[u].r = next(a[d[u]].r));
}
...
void Solve() {
	int i;
	sort(a,a+n,comp); //sort x
	d[0] = 0;
	for (i = 1; i < n; i++) 
		d[i] = (a[i].y > a[d[i-1]].y) ? i : d[i-1];
	//for (i = 0; i < n; i++) cout << d[i] << '-' << endl;
	for (i = 0; i < n; i++) a[i].pos2 = i;
	sort(a, a+n, comp_y); //sort y
	a[0].r = a[0].pos2;
	for (i = 1; i < n; i++) a[i].r = max(a[i].pos2, a[i-1].r);
	
	sort(a,a+n,comp);
	a[0].r = next(0);
	for (i = 1; i < n; i++) {
		a[i].r = max(max(a[i].r, a[i-1].r), next(i));
	}
	sort(a, a+n, comp_pos);
	for (i = 0; i < n; i++) cout << a[i].r << endl;
}
/*****************************************/
