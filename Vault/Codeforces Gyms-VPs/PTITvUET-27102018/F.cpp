...
void TimerStop();
void Exit();
string cppstr_infile = "folding.in";
string cppstr_outfile = "folding.out";
/*********************************************************/
...
/************** [FUNCTIONS] **************/
i64 Try(i64 x1, i64 y1, i64 x2, i64 y2) {
	if (x1 < x2 || y1 < y2) return LINF;
	i64 res = 0;
	while (x1 > x2) {x1 = max(x1/2+x1%2, x2); res++;}
	while (y1 > y2) {y1 = max(y1/2+y1%2, y2); res++;}
	return res;
}
...
void Solve() {
	if (min(Try(W, H, w, h), Try(W, H, h, w)) == LINF) cout << "-1";
	else cout << min(Try(W, H, w, h), Try(W, H, h, w));
}
/*****************************************/
