...
void ProSolve() {
	while (T--) {
		cin >> N >> X >> Y;
		if (Y % X != 0) {cout << "-1\n"; continue;}
		vi a(N); for (i64 i=0; i<N; i++) cin >> a[i];
		if (Y == 1) {
			i64 ans = 0;
			for (i64 i=0; i<N; i++) ans += (a[i] != 1);
			cout << ans << endl; continue;
		}
		vi prime, mini, maxi;
		for (i64 i=2; i<=sqrt(Y); i++) {
			if (Y % i != 0) continue;
			i64 cnt = 0; while (Y % i == 0) {Y /= i; cnt++;}
			prime.pub(i); mini.pub(0); maxi.pub(cnt);
		}
		if (Y != 0) {prime.pub(Y); mini.pub(0); maxi.pub(1);}
		for (i64 i=0; i<prime.size(); i++) {
			i64 cnt = 0; while (X % prime[i] == 0) {X /= prime[i]; mini[i]++;}
		}
		i64 ans = 0;
		for (auto x: a) {
			bool flag = false;
			for (i64 i=0; i<prime.size(); i++) {
				...
				if (cnt < mini[i] || cnt > maxi[i]) {flag = true; break;}
			}
			if (x != 1) flag = true; ans += (flag == true);
		}
		cout << ans << endl;
	}
}
