...
void ProSolve() {
	while (T--) {
		cin >> s1 >> s2; N = s1.size();
		bool flag = true, Switch = false; i64 ans = 0;
		if (s1[N/2] != s2[N/2]) {cout << "-1\n"; continue;}
		for (i64 i=0; i<N/2; i++) {
			if (!Switch && s1[i] == s2[i] && s1[N-1-i] == s2[N-1-i]) continue;
			if (Switch && s1[i] == s2[N-1-i] && s1[N-1-i] == s2[i]) continue;
			if (!Switch && s1[i] == s2[N-1-i] && s1[N-1-i] == s2[i]) {
				ans++; Switch = true;
				while (i+1 < N/2 && s1[i+1] == s2[N-1-i-1] && s1[N-1-i-1] == s2[i+1]) i++;
				continue;
			}
			if (Switch && s1[i] == s2[i] && s1[N-1-i] == s2[N-1-i]) {
				ans++; Switch = false;
				while (i+1 < N/2 && s1[i+1] == s2[i+1] && s1[N-1-i-1] == s2[N-1-i-1]) i++;
				continue;
			}
			flag = false; break;
		}
