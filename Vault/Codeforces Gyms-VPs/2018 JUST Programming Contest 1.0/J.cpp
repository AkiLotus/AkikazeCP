...
string toString(i64 n) {
	string res = "";
	if (n == 0) return ("0");
	while (n > 0) {
		char c = char(48 + n % 10);
		res.insert(0, 1, c); n /= 10;
	}
	return res;
}
...
i64 valid(i64 z) {
	...
	string s = toString(z); i64 n = s.size();
	i64 tmp1 = 0, tmp2 = 0;
	if (n % 2 == 0) {
		tmp1 = toInt(s.substr(0, n/2));
		tmp2 = toInt(s.substr(n/2, n/2));
		if (tmp2 == 0) {
			for (i64 i=0; i<n/2; i++) tmp1 *= 10;
			return tmp1;
		}
	}
	else {
		tmp1 = toInt(s.substr(0, n/2+1));
		tmp2 = toInt(s.substr(n/2+1, n/2));
		if (tmp2 == 0) {
			for (i64 i=0; i<n/2; i++) tmp1 *= 10;
			return tmp1;
		}
	}
	if (__gcd(tmp1, tmp2) == 1) return 1;
	return 0;
}
