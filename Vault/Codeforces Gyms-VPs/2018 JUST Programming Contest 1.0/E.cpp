...
void ProSolve() {
	while (T--) {
		cin >> n >> x >> y; vi d(n), m(n);
		for (i64 i=0; i<n; i++) cin >> d[i] >> m[i];
		i64 index = -1, optimalD = LINF, optimalM = -LINF;
		for (i64 i=0; i<n; i++) {
			if (d[i] < optimalD && d[i] <= x && m[i] >= y) {index = i+1; optimalD = d[i]; optimalM = m[i];}
			else if (d[i] == optimalD && m[i] > optimalM && d[i] <= x && m[i] >= y) {index = i+1; optimalD = d[i]; optimalM = m[i];}
		}
		cout << index << endl;
	}
}
