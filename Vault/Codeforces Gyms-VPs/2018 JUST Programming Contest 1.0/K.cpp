...
/** -----GLOBAL VARIABLES----- **/
i64 T, n, m, k, x, y, z; string t;

/** -----EXTENSIVE FUNCTIONS----- **/
bool chronological(sub a, sub b) {
	return (a.t < b.t);
}
...
void ProSolve() {
	while (T--) {
		cin >> n >> m >> k;
		vi firstSolve(n, -1), solved(n);
		vector<vb> Solved(n, vb(m));
		vi firstAttemptCnt(m);
		vector<sub> submissions;
		vector<vi> incorrect(n, vi(m));
		i64 a = -1, b = -1, c = -1, d = -1;
		while (k--) {
			cin >> x >> y >> z >> t;
			i64 Minute = toInt(t.substr(0, 3));
			i64 Second = toInt(t.substr(4, 2));
			sub q; q.x = x-1; q.y = y-1; q.z = z; q.t = Minute * 60 + Second;
			submissions.pub(q);
		}
		sort(submissions.begin(), submissions.end(), chronological);
		for (i64 i=0; i<submissions.size(); i++) {
			//cout << submissions[i].x << " " << submissions[i].y << " " << submissions[i].z << " " << submissions[i].t << endl;
			if (submissions[i].z) {
				if (!Solved[submissions[i].x][submissions[i].y]) {
					solved[submissions[i].x]++; if (solved[submissions[i].x] == 1) firstSolve[submissions[i].x] = submissions[i].y + 1;
					if (incorrect[submissions[i].x][submissions[i].y] == 0) firstAttemptCnt[submissions[i].y]++; Solved[submissions[i].x][submissions[i].y] = true;
				}
				if (a == -1) a = submissions[i].y+1; b = submissions[i].y+1;
			}
			else {
				if (!Solved[submissions[i].x][submissions[i].y]) incorrect[submissions[i].x][submissions[i].y]++;
			}
		}
		c = max_element(firstAttemptCnt.begin(), firstAttemptCnt.end()) - firstAttemptCnt.begin() + 1;
		i64 tmp = -LINF;
		for (i64 i=0; i<m; i++) {
			for (i64 j=0; j<n; j++) {
				if (!Solved[j][i]) continue;
				if (incorrect[j][i] > tmp) {
					tmp = incorrect[j][i]; d = i+1;
				}
			}
		}
		for (i64 i=0; i<n; i++) cout << firstSolve[i] << " "; cout << endl;
		cout << a << " " << b << " " << c << " " << d << endl;
	}
}
