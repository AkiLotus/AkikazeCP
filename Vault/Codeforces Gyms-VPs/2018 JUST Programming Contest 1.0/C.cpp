...
/** -----GLOBAL VARIABLES----- **/
i64 T, m;
vi EulerTotient(1000009, 1);
vb isPrime(1000009, true);

/** -----EXTENSIVE FUNCTIONS----- **/
void GenerateTotient() {
	...
	for (i64 i=2; i<1000009; i++) {
		if (isPrime[i]) {
			...
			i64 mul = i, power = 1;
			while (mul < 1000009) {
				for (i64 j=1; j<=1000008/mul; j++) {
					if (j % i == 0) continue;
					EulerTotient[mul*j] *= ((mul / i) * (i - 1));
				}
				mul *= i; power++;
			}
		}
