...
void ProSolve() {
	while (T--) {
		...
		vector<vector<vi>> cnt(n+1, vector<vi>(m+1, vi(501, 0)));
		vector<vector<vi>> dp(n+1, vector<vi>(m+1, vi(501, 0)));
		for (i64 i=1; i<=n; i++) {
			for (i64 j=1; j<=m; j++) {
				cin >> A[i][j]; cnt[i][j][A[i][j]]++;
			}
		}
		for (i64 i=1; i<=n; i++) {
			for (i64 j=1; j<=m; j++) {
...
//		}
		while (q--) {
			cin >> a >> b >> c >> d;
			i64 total = (c - a + 1) * (d - b + 1);
			i64 medianIndex = total / 2 + total % 2;
			i64 top = 1, bot = 500;
			while (top < bot) {
				//tracker2(top, bot);
				i64 mid = (top + bot) / 2;
				//cout << "mid = " << mid << endl;
				i64 res = dp[c][d][mid] + dp[a-1][b-1][mid] - dp[a-1][d][mid] - dp[c][b-1][mid];
				//cout << "res = " << res << endl;
				if (res >= medianIndex) bot = mid; else top = mid+1;
			}
			cout << top << endl;
		}
	}
