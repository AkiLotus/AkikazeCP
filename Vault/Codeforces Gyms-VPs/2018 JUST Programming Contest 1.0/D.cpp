...
/** -----GLOBAL VARIABLES----- **/
i64 T, n, m, k, u, v, c, x, ans = LINF;
vector<vector<pii>> Adj(16, vector<pii>(0));
vb important(16, false);
vb chosen(16, false);

/** -----EXTENSIVE FUNCTIONS----- **/
i64 MST() {
	//cout << chosen << endl;
	i64 res = 0; vector<vector<pii>> adj(16, vector<pii>(0));
	vi Chosen(16, false); priority_queue<pii> Q;
	for (i64 i=1; i<=n; i++) {
		//tracker1(i);
		if (!chosen[i]) continue;
		//cout << "found a random chosen one!\n";
		for (i64 j=0; j<Adj[i].size(); j++) {
			if (!chosen[Adj[i][j].fi]) continue;
			adj[i].pub(Adj[i][j]);
		}
	}
	for (i64 i=1; i<=n; i++) {
		...
		if (chosen[i]) {
			//cout << "found a random chosen one!\n";
			Q.push(mp(0LL, i)); break;
		}
	}
	if (Q.empty()) return LINF;
	//REcheck;
	while (!Q.empty()) {
		i64 z = Q.top().se;
		i64 tmp = Q.top().fi; Q.pop();
		if (Chosen[z]) continue; Chosen[z] = true;
		res -= tmp;
		for (i64 i=0; i<adj[z].size(); i++) {
			pii zz = adj[z][i];
			if (Chosen[zz.fi]) continue;
			Q.push(mp(-zz.se, zz.fi));
		}
	}
	return res;
}
...
void Try(i64 cnt) {
	...
	if (!important[cnt+1]) Try(cnt+1);
	chosen[cnt+1] = true; Try(cnt+1);
}
...
void ProSolve() {
	while (T--) {
		...
		while (k--) {
			cin >> x; important[x] = true;
		}
		if (!important[x]) Try(1);
		chosen[1] = true; Try(1);
		cout << ans << endl;
	}
}
