...
void ProSolve() {
	while (T--) {
		cin >> n >> m >> s;
		vb cnt(n, false); i64 ans = 0, tmp = 0;
		for (i64 i=0; i<n/2; i++) {
			if (s[i] != s[n-1-i]) {
				cnt[i] = true; cnt[n-1-i] = true; tmp += 2;
			}
		}
		while (m--) {
			cin >> p >> c; p--; s[p] = c;
			if (s[p] != s[n-1-p]) {
				tmp += (1 - cnt[p]);
				if (p != n-1-p) tmp += (1 - cnt[n-1-p]);
				cnt[p] = true; cnt[n-1-p] = true;
			}
			else {
				tmp -= cnt[p];
				if (p != n-1-p) tmp -= cnt[n-1-p];
				cnt[p] = false; cnt[n-1-p] = false;
			}
			if (tmp == 0) ans++;
			//cout << s << ' ' << ans << endl;
		}
