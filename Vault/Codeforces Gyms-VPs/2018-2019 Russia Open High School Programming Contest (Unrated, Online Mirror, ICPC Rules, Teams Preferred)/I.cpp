...
/************** [GLOBAL VARIABLES] **************/
i64 n, l, r, x, y, z; vi a; vector<u64> b;
/************************************************/
...
void Input() {
	cin >> n >> l >> r >> x >> y >> z;
	b.clear(); a.clear(); b.rsz(max(n, 2LL)); a.rsz(n);
	cin >> b[0] >> b[1];
	for (i64 i=2; i<n; i++) b[i] = ((b[i-2] % (1LL << 32) * x) % (1LL << 32) + (b[i-1] % (1LL << 32) * y) % (1LL << 32) + z) % (1LL << 32);
	for (i64 i=0; i<n; i++) a[i] = (b[i] % (r - l + 1)) + l;
}

void Solve() {
	if (n == 1) {cout << "IMPOSSIBLE\n"; return;}
	u64 ans = (u64)LINF*18; i64 sign = +1;
	if (find(a.begin(), a.end(), 0LL) != a.end()) ans = 0;
	if (*max_element(a.begin(), a.end()) < 0) {for (auto &z: a) z *= -1; reverse(a.begin(), a.end());}
	vi MinPrefix(n, a[0]), MaxSuffix(n, a[n-1]), MinSuffix(n, a[n-1]);
	for (i64 i=1; i<n; i++) MinPrefix[i] = min(MinPrefix[i-1], a[i]);
	for (i64 i=n-2; i>=0; i--) MaxSuffix[i] = max(MaxSuffix[i+1], a[i]);
	for (i64 i=n-2; i>=0; i--) MinSuffix[i] = min(MinSuffix[i+1], a[i]);
	for (i64 i=0; i<n-1; i++) {
		if (MinPrefix[i] >= 0 || MaxSuffix[i+1] <= 0) continue;
		sign = -1; if (ans == (u64)LINF*18) ans = 0;
		ans = max(ans, (u64)1LL * abs(MinPrefix[i]) * abs(MaxSuffix[i+1]));
	}
	i64 j = n;
	for (i64 i=n-1; i>0; i--) {
		while (j-1 > i-1 && MinPrefix[i-1] < MinSuffix[j-1]) j--;
		if (j < n && MinPrefix[i-1] > 0 && MinSuffix[j] > 0 && sign == +1) {
			ans = min(ans, (u64)1LL * MinPrefix[i-1] * MinSuffix[j]);
		}
	}
	if (ans == (u64)LINF*18) {cout << "IMPOSSIBLE\n"; return;}
	if (sign == -1) cout << "-";
	cout << ans << endl;
}
/*****************************************/
