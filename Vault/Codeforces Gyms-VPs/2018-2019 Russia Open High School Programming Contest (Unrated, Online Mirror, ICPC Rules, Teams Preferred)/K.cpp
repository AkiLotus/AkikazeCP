...
void Input() {
	cin >> n; s.rsz(n); t.rsz(n);
	for (i64 i=0; i<n; i++) cin >> s[i] >> t[i];
}

void Solve() {
	for (i64 i=0; i<n; i++) {
		vb head(26, false), tail(26, false);
		string H = "", T = "";
		for (auto ch: s[i]) {
			if (head[ch-'a']) continue;
			head[ch-'a'] = true; H += ch;
		}
		for (auto ch: t[i]) {
			if (tail[ch-'a']) continue;
			tail[ch-'a'] = true; T += ch;
		}
		for (auto ch: T) {
			if (head[ch-'a']) continue;
	...
	for (auto it=Map.begin(); it!=Map.end(); it++) {
		vi Vector = it->se; cout << Vector.size();
		for (auto x: Vector) cout << " " << x; cout << endl;
	}
}
/*****************************************/
