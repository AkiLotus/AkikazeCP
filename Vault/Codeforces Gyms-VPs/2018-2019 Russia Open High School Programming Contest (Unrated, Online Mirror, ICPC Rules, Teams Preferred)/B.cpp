...
/***********************************************
** File name: B.cpp                           **
** Time created: Sat Dec 15 2018 01:45am      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
string Token; map<string, string> Contents;
string begincmd, endcmd;
vs citeList, bibitemList;
/************************************************/

/************** [FUNCTIONS] **************/
string extractcite(string Tok) {
	string res = "";
	for (i64 i=6; i<Tok.size(); i++) {
		if (Tok[i] == '}') break;
		res += Tok[i];
	}
	return res;
...
string extractbibitem(string Tok) {
	string res = "";
	for (i64 i=9; i<Tok.size(); i++) {
		if (Tok[i] == '}') break;
		res += Tok[i];
	}
	return res;
...
i64 exist(string str, char ch, i64 i=0) {
    for (; i<str.size(); i++) {
		char c = str[i];
        if (c == ch) return i;
    }
...
void Input() {
	while (getline(cin, Token)) {
		if (Token == "\\begin{thebibliography}{99}") {begincmd = Token; continue;}
		if (Token == "\\end{thebibliography}") {endcmd = Token; continue;}
		if (Token.size() > 10 && Token.substr(0, 9) == "\\bibitem{" && exist(Token, '}')) {
			string bibitem = extractbibitem(Token);
			bibitemList.pub(bibitem);
			Contents[bibitem] = Token; continue;
		}
		for (i64 i=0; i<Token.size(); i++) {
			if (Token[i] != '\\') continue;
			i64 j = exist(Token, '}', i);
			string sub = Token.substr(i, j-i+1);
			citeList.pub(extractcite(sub));
		}
	}
}

void Solve() {
	if (citeList == bibitemList) {cout << "Correct\n"; return;}
	cout << "Incorrect\n";
	cout << begincmd << endl;
	for (auto cite: citeList) {
		cout << Contents[cite] << endl;
	}
	cout << endcmd << endl;
}
/*****************************************/
