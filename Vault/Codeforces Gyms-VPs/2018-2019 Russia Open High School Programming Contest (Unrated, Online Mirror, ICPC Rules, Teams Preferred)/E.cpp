...
/************** [GLOBAL VARIABLES] **************/
i64 k; vs coor, movements;
vector<vb> occupied(8, vb(8, false));
vector<vb> blanked(8, vb(8, true));
vector<vb> locked(8, vb(8, false));
i64 dx[] = {-2, -2, -1, -1, +1, +1, +2, +2};
i64 dy[] = {-1, +1, -2, +2, -2, +2, -1, +1};
/************************************************/

/************** [FUNCTIONS] **************/
pii compile_t2n(string z) {
	i64 x = (z[0] - 'a');
	i64 y = (z[1] - '1');
	return mp(x, y);
}

string compile_n2t(pii xy) {
	i64 x = xy.fi;
	i64 y = xy.se;
	string str = "";
	str += char(97+x);
...
void BFS1(i64 x0, i64 y0) {
	pii Default = mp(-1LL, -1LL);
	vector<vb> vis(8, vb(8, false));
	vector<vector<pii>> Last(8, vector<pii>(8, Default));
	queue<pii> Q; Q.push(mp(x0, y0));
	vis[x0][y0] = true;
	i64 x = -1, y = -1;
	while (!Q.empty()) {
		pii xy = Q.front(); Q.pop();
		x = xy.fi; y = xy.se;
		if (occupied[x][y]) break;
		for (i64 i=0; i<8; i++) {
			if (x+dx[i] < 0 || x+dx[i] > 7) continue;
			if (y+dy[i] < 0 || y+dy[i] > 7) continue;
			if (vis[x+dx[i]][y+dy[i]]) continue;
			if (locked[x+dx[i]][y+dy[i]]) continue;
...
void Solve() {
	...
	for (auto cmd: movements) cout << cmd << endl;
}
/*****************************************/
