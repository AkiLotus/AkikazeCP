...
/***********************************************
** File name: F.cpp                           **
** Time created: Sat Dec 15 2018 01:44am      **
***********************************************/
...
/************** [GLOBAL VARIABLES] **************/
typedef pair<i64, ppi> pi_iii;
i64 n; vi A;
/************************************************/

/************** [FUNCTIONS] **************/
i64 min(i64 a, i64 b, i64 c) {
...
bool invalid(vector<pi_iii> Result, vi val, vi p) {
	vi Tmp(p.size()); for (i64 i=0; i<Tmp.size(); i++) Tmp[i] = val[p[i]];
	for (auto Token: Result) {
		i64 sum = Token.fi, x = Token.se.fi.fi, y = Token.se.fi.se, z = Token.se.se;
		if (min(Tmp[x], Tmp[y], Tmp[z]) + max(Tmp[x], Tmp[y], Tmp[z]) != sum) return true;
	}
	return false;
}

void FirstDetermination() {
	vector<pi_iii> Result(10); vi val(5), p(5);
	for (i64 i=0; i<5; i++) p[i] = i;
	cout << "? 1 2 3\n"; fflush(stdout);
	cin >> Result[0].fi; Result[0].se = mp(mp(0LL, 1LL), 2LL);
	cout << "? 1 2 4\n"; fflush(stdout);
	cin >> Result[1].fi; Result[1].se = mp(mp(0LL, 1LL), 3LL);
	cout << "? 1 2 5\n"; fflush(stdout);
	cin >> Result[2].fi; Result[2].se = mp(mp(0LL, 1LL), 4LL);
	cout << "? 1 3 4\n"; fflush(stdout);
	cin >> Result[3].fi; Result[3].se = mp(mp(0LL, 2LL), 3LL);
	cout << "? 1 3 5\n"; fflush(stdout);
	cin >> Result[4].fi; Result[4].se = mp(mp(0LL, 2LL), 4LL);
	cout << "? 1 4 5\n"; fflush(stdout);
	cin >> Result[5].fi; Result[5].se = mp(mp(0LL, 3LL), 4LL);
	cout << "? 2 3 4\n"; fflush(stdout);
	cin >> Result[6].fi; Result[6].se = mp(mp(1LL, 2LL), 3LL);
	cout << "? 2 3 5\n"; fflush(stdout);
	cin >> Result[7].fi; Result[7].se = mp(mp(1LL, 2LL), 4LL);
	cout << "? 2 4 5\n"; fflush(stdout);
	cin >> Result[8].fi; Result[8].se = mp(mp(1LL, 3LL), 4LL);
	cout << "? 3 4 5\n"; fflush(stdout);
	cin >> Result[9].fi; Result[9].se = mp(mp(2LL, 3LL), 4LL);
	sort(Result.begin(), Result.end());
	i64 val13 = Result[0].fi, val14 = Result[1].fi;
	i64 val25 = Result[8].fi, val35 = Result[9].fi;
	i64 val15 = Result[3].fi, val24 = Result[6].fi;
	if (Result[4].fi == Result[6].fi) swap(val15, val24);
	i64 val12345 = (val13 + val35 + val24 * 2 + val15) / 2;
	val[0] = val12345 - val24 - val35;
	val[1] = val12345 - val14 - val35;
	val[2] = val12345 - val15 - val24;
	val[3] = val12345 - val13 - val25;
	val[4] = val12345 - val13 - val24;
	do {
		if (!invalid(Result, val, p)) break;
	}
	while (next_permutation(p.begin(), p.end()));
	for (i64 x=0; x<5; x++) A[x] = val[p[x]];
}

void SequentialDetermination(i64 i) {
	vector<pi_iii> Result(4); vi val(4), p(4);
	for (i64 x=0; x<4; x++) p[x] = x;
	cout << "? " << i+1 << " " << i+2 << " " << i+3 << endl; fflush(stdout);
	cin >> Result[0].fi; Result[0].se = mp(mp(0LL, 1LL), 2LL);
	cout << "? " << i+1 << " " << i+2 << " " << i+4 << endl; fflush(stdout);
	cin >> Result[1].fi; Result[1].se = mp(mp(0LL, 1LL), 3LL);
	cout << "? " << i+1 << " " << i+3 << " " << i+4 << endl; fflush(stdout);
	cin >> Result[2].fi; Result[2].se = mp(mp(0LL, 2LL), 3LL);
	cout << "? " << i+2 << " " << i+3 << " " << i+4 << endl; fflush(stdout);
	cin >> Result[3].fi; Result[3].se = mp(mp(1LL, 2LL), 3LL);
	sort(Result.begin(), Result.end());
	i64 val13 = Result[0].fi, val14 = Result[1].fi, val24 = Result[3].fi;
	i64 val1234 = val13 + val24;
	val[0] = A[i]; val[1] = A[i+1]; val[2] = A[i+2];
	val[3] = val1234 - val[0] - val[1] - val[2];
	sort(val.begin(), val.end());
	do {
		vi Tmp(4); for (i64 x=0; x<4; x++) Tmp[x] = val[p[x]];
		if (Tmp[0] != A[i+0] || Tmp[1] != A[i+1] || Tmp[2] != A[i+2]) continue;
		A[i+3] = Tmp[3]; break;
	}
	while (next_permutation(p.begin(), p.end()));
}
...
void Solve() {
	FirstDetermination();
	for (i64 i=5; i<n; i++) SequentialDetermination(i-3);
	cout << "!"; for (auto z: A) cout << " " << z; cout << endl; fflush(stdout);
}
/*****************************************/
