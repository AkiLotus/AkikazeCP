...
void Solve() {
	i64 floorfoot = 0, floorchain = 0;
	for (i64 i=0; i<n; i++) floorfoot += (a[i] - b[i]) * (a[i] - b[i]);
	for (i64 i=0; i<n; i++) floorchain += (a[i] - c[i]) * (a[i] - c[i]);
	if (floorfoot <= floorchain) cout << "Yan"; else cout << "MaratonIME";
}
/*****************************************/
