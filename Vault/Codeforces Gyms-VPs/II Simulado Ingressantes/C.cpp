...
void Solve() {
	while (x != 0 || y != 0) {
		if (x == 0) {cout << "2 " << y << endl; y = 0;}
		else if (y == 0) {cout << "1 " << x << endl; x = 0;}
		else if (x == 1) {cout << "2 " << (y - 1) << endl; y = 1;}
		else if (y == 1) {cout << "1 " << (x - 1) << endl; x = 1;}
		else if (x > y) {cout << "1 " << (x - y) << endl; x = y;}
		else {cout << "2 " << (y - x) << endl; y = x;}
		cout.flush(); if (x == 0 && y == 0) break;
		cin >> z >> a; if (z == 1) x -= a; else y -= a;
	}
}
/*****************************************/
