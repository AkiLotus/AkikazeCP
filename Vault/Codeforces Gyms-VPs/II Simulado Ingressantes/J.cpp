...
bool intercept(i64 i, i64 j) {
	i64 sqrCent = (x[i] - x[j]) * (x[i] - x[j]) + (y[i] - y[j]) * (y[i] - y[j]);
	return ((r[i] + r[j]) * (r[i] + r[j]) >= sqrCent);
}

void Input() {
	cin >> n; x.rsz(n); y = x; r = x;
	for (i64 i=0; i<n; i++) cin >> x[i] >> y[i] >> r[i];
}
