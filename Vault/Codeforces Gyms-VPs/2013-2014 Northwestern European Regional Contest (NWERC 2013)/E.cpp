...
/************** [GLOBAL VARIABLES] **************/
string s; i64 a, b, c; vi Seek(124605, -1);
/************************************************/

/************** [FUNCTIONS] **************/
i64 root(i64 lvl, i64 x) {
	i64 top = 0, bot = INF, ans = -1;
	while (top <= bot) {
		i64 mid = (top + bot) / 2;
		i64 res = 1;
		for (i64 i=0; i<lvl; i++) {
...
i64 seek(i64 x) {
	if (Seek[x] != -1) return Seek[x];
	i64 res = 1;
	for (i64 i=2; i<log2(x)+4; i++) {
		if (root(i, x) == -1) continue;
		res += seek(i);
	}
	Seek[x] = res; return res;
}
...
void Solve() {
	i64 Multiplier = 1; map<i64, i64> Map;
	for (i64 i=15; i>1; i--) {
		if (root(i, a) == -1) continue;
		Multiplier = i; break;
	}
	...
	if (b > 1) Map[b] += c;
	for (i64 i=2; i<=sqrt(Multiplier); i++) {
		while (Multiplier % i == 0) {
			Map[i]++; Multiplier /= i;
		}
	}
	if (Multiplier > 1) Map[Multiplier]++;
	i64 MaxExp = -LINF, ans = 0;
	for (auto it=Map.begin(); it!=Map.end(); it++) MaxExp = max(MaxExp, (i64)it->se);
	for (i64 x=MaxExp; x>1; x--) {
		i64 cnt = 1;
		for (auto it=Map.begin(); it!=Map.end(); it++) cnt *= (it->se / x + 1);
		ans += (cnt - 1) * seek(x);
	}
	cout << ans << endl;
}
