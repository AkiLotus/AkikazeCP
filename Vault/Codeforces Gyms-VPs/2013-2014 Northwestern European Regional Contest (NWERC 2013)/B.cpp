...
i64 v, e, st, en; int ans = 0; vector<int> s;
vector<pii> chain; vector<vb> conn;
/************************************************/
...
void Input() {
	s.clear(); conn.clear(); chain.clear(); ans = 0;
	s.rsz(v); conn.rsz(v, vb(v, false)); chain.rsz(e);
	for (i64 i=0; i<v; i++) cin >> s[i];
	for (i64 i=0; i<e; i++) {
		cin >> chain[i].fi >> chain[i].se;
		chain[i].fi--; chain[i].se--;
		conn[chain[i].fi][chain[i].se] = true;
		conn[chain[i].se][chain[i].fi] = true;
		ans = max(ans, s[chain[i].fi] + s[chain[i].se]);
	}
}
...
void Solve() {
	for (i64 i=0; i<v; i++) {
		for (i64 j=i+1; j<v; j++) {
			...
			for (i64 k=j+1; k<v; k++) {
				if (!conn[i][k] || !conn[j][k]) continue;
				ans = max(ans, s[i] + s[j] + s[k]);
				for (i64 l=k+1; l<v; l++) {
					if (!conn[i][l] || !conn[j][l] || !conn[k][l]) continue;
