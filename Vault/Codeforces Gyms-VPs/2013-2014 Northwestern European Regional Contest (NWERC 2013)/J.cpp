...
/************** [GLOBAL VARIABLES] **************/
string s; i64 N = 1; vi L(1, -1), R(1, -1);
vi decor; vi subtree;
bool impossible = false; i64 ans = 0;
/************************************************/

/************** [FUNCTIONS] **************/
bool RemoveFrom(i64 z) {
	if (decor[z] == 0) return false;
	if (L[z] == -1) {decor[z]--; return true;}
	if (decor[L[z]] > decor[R[z]]) {
		if (!RemoveFrom(L[z])) {
			impossible = true; return false;
		}
		else decor[z] = decor[L[z]] + decor[R[z]];
	}
	else if (decor[L[z]] < decor[R[z]]) {
		if (!RemoveFrom(R[z])) {
			impossible = true; return false;
		}
		else decor[z] = decor[L[z]] + decor[R[z]];
	}
	else if (!RemoveFrom(L[z]) && !RemoveFrom(R[z])) {
		impossible = true; return false;
	}
	else decor[z] = decor[L[z]] + decor[R[z]];
	return true;
...
bool InsertTo(i64 z) {
	if (decor[z] == subtree[z]) return false;
	if (L[z] == -1) {decor[z]++; return true;}
	if (decor[L[z]] < decor[R[z]]) {
		if (!InsertTo(L[z])) {
			impossible = true; return false;
	...
	else if (decor[R[z]] < decor[L[z]]) {
		if (!InsertTo(R[z])) {
			impossible = true; return false;
		}
		else decor[z] = decor[L[z]] + decor[R[z]];
	}
	else if (!InsertTo(L[z]) && !InsertTo(R[z])) {
		impossible = true; return false;
	}
	else decor[z] = decor[L[z]] + decor[R[z]];
	return true;
...
void DFS(i64 z) {
	if (L[z] != -1) {
		...
		subtree[z] += subtree[L[z]];
		decor[z] += decor[L[z]];
	}
	if (R[z] != -1) {
		DFS(R[z]);
		if (impossible) return;
		subtree[z] += subtree[R[z]];
		decor[z] += decor[R[z]];
	}
	if (L[z] == -1) {subtree[z] = 1; return;}
	while (!impossible && decor[L[z]] - decor[R[z]] > 1) {
		ans++;
		if (!RemoveFrom(L[z])) impossible = true;
		if (!InsertTo(R[z])) impossible = true;
	}
	while (!impossible && decor[R[z]] - decor[L[z]] > 1) {
		ans++;
		if (!RemoveFrom(R[z])) impossible = true;
		if (!InsertTo(L[z])) impossible = true;
	}
}
...
void Input() {
	N = 1; L.clear(); R.clear();
	L.rsz(1, -1); R.rsz(1, -1);
	decor.clear(); subtree.clear();
	decor.pub(0); subtree.pub(0);
	impossible = false; ans = 0;
}
...
void Solve() {
	...
	for (i64 i=1; i<s.size()-1; i++) {
		if (s[i] == '(') {
			if (L[S.top()] == -1) L[S.top()] = N;
			else R[S.top()] = N;
			S.push(N); N++; L.pub(-1); R.pub(-1);
			decor.pub(0); subtree.pub(0);
		}
		else if (s[i] == ')') S.pop();
		else decor[S.top()]++;
