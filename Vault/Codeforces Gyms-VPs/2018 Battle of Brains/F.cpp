...
vector<bool> valid(1000001, false);
...
void Preprocess() {
	...
	for (int i=1; i<1000001; i++) {
		if (valid[i]) validList.push_back(i);
	}
}
...
void Solve() {
	int M = upper_bound(validList.begin(), validList.end(), N) - validList.begin();
	int gcd = __gcd(M, N); M /= gcd; N /= gcd;
	cout << "Case " << (++test) << ": " << M << "/" << N << endl;
}
/*****************************************/
