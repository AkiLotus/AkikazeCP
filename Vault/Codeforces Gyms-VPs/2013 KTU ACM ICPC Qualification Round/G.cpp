...
/************** [GLOBAL VARIABLES] **************/
i64 Test = 0; i64 n; vi a; map<i64, i64> Map;
/************************************************/
...
void Input() {
	cin >> n; a.clear(); a.rsz(n); Map.clear();
	for (i64 i=0; i<n; i++) {
		cin >> a[i]; Map[a[i]]++;
	}
}
...
void Solve() {
	...
	for (auto it=Map.begin(); it!=Map.end(); it++) {
		i64 k1 = it->fi, v1 = it->se;
		i64 k2 = modPow(k1, Mod-2), v2 = 0;
		if (Map.find(k2) != Map.end()) v2 = Map[k2];
		if (k1 == k2) ans += v1 - v1 % 2;
		else ans += min(v1, v2);
	}
	ans /= 2;
