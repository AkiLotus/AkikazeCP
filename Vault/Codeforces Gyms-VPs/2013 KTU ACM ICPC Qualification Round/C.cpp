...
/************** [GLOBAL VARIABLES] **************/
i64 Test = 0; i64 n, q, l, r; string s; char c;
/************************************************/
...
void Solve() {
	cout << "Case #" << (++Test) << ":\n";
	vector<vi> cnt(n, vi(26, 0)); cnt[0][s[0]-'A']++;
	for (i64 i=1; i<n; i++) {
		for (i64 j=0; j<26; j++) {
			cnt[i][j] = cnt[i-1][j] + (s[i] == char(65+j));
		}
	}
	while (q--) {
		cin >> c >> l >> r;
		if (c == 'g') {
			...
			for (i64 i=l; i<=r; i++) {
				for (i64 j=0; j<26; j++) {
					if (i == 0) {cnt[i][j] = (s[i] == char(65+j));}
					else cnt[i][j] = cnt[i-1][j] + (s[i] == char(65+j));
				}
			}
		}
	}
