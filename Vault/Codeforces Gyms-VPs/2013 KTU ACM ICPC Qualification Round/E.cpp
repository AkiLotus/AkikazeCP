...
/************** [GLOBAL VARIABLES] **************/
i64 Test = 0; i64 c, n, x; string s; i64 id[15];
i64 dp[15][1013]; char Appended[15][1013]; i64 Hash[10004], Base[10004];
/************************************************/
...
void Input() {
	cin >> c >> s; s.insert(0, "#"); n = s.size(); x = 0;
	for (i64 i=0; i<15; i++) {
		for (i64 j=0; j<1013; j++) {
			dp[i][j] = 0;
			Appended[i][j] == '\0';
		}
	}
	for (i64 i=0; i<15; i++) id[i] = -1;
	for (i64 i=0; i<n; i++) {
		if (s[i] != '?' && s[i] != '#') continue;
		id[x] = i; x++;
	}
	for (i64 i=0; i<10004; i++) {Hash[i] = 0; Base[i] = 0;}
}

void Solve() {
	produceHash(); dp[x][0] = 1; i64 Last = LINF;
	//tracker4(c, s, n, x);
	//for (i64 i=0; i<x; i++) cout << id[i] << " "; cout << endl;
	//for (i64 i=0; i<n; i++) cout << Base[i] << " "; cout << endl;
	for (i64 i=x-1; i>=0; i--) {
		for (i64 j=0; j<1013; j++) {
			if (dp[i+1][j] == 0) continue;
			//tracker3(i+1, j, dp[i+1][j]);
			i64 val = Hash[id[i]]; if (Last != LINF) val -= Hash[id[i+1]]; Last = id[i];
			val %= 1013; while (val < 0) val += 1013; //tracker5(i, j, val, Base[id[i]], s[id[i]]);
			if (s[id[i]] == '#') {dp[i][(j+val)%1013] += dp[i+1][j]; Appended[i][(j+val)%1013] = '#';}
			else for (char C='a'; C<='z'; C++) {dp[i][(j+val+C*Base[id[i]])%1013] += dp[i+1][j]; Appended[i][(j+val+C*Base[id[i]])%1013] = C;}
		}
	}
	cout << "Case #" << (++Test) << ": ";
	//cout << "dp[0][c] = " << dp[0][c] << endl;
	if (dp[0][c] != 1) {cout << dp[0][c] << endl; return;}
	i64 Code = c, petr = 0;
	while (petr < x) {
		char Token = Appended[petr][Code]; s[id[petr]] = Token;
		i64 val = Hash[id[petr]]; if (petr < x-1) val -= Hash[id[petr+1]];
		val %= 1013; while (val < 0) val += 1013;
		//tracker4(Code, petr, Token, val);
		Code -= val; Code %= 1013; while (Code < 0) Code += 1013; petr++;
	}
