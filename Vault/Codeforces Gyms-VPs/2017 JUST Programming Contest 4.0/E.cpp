...
i64 T, n, x; vector<vi> f; map<i64, i64> Map1, Map2;
/************************************************/
...
void Try1(i64 x, i64 m) {
	if (x == n/2) {
		Map1[m % MOD]++;
		return;
	}
	for (i64 i=0; i<6; i++) Try1(x+1, m*f[x][i]);
}

void Try2(i64 x, i64 m) {
	if (x == n) {
		Map2[m % MOD]++;
		return;
	}
	for (i64 i=0; i<6; i++) Try2(x+1, m*f[x][i]);
}
...
void Solve() {
	while (T--) {
		...
		Try1(0, 1); Try2(n/2, 1); i64 ans = 0;
		for (auto it=Map1.begin(); it!=Map1.end(); it++) {
			i64 val = it->first, cnt = it->second;
			i64 demand = (x * modPow(val, MOD-2)) % MOD;
			ans += (cnt * Map2[demand]);
		}
		cout << ans << endl;
	}
