...
i64 T, n, m; vector<vector<char>> a;
/************************************************/
...
void Solve() {
	while (T--) {
		cin >> n >> m; a.clear();
		a.rsz(n, vector<char>(m, '0'));
		i64 border = 0, interior = 0;
		for (i64 i=0; i<n; i++) {
			for (i64 j=0; j<m; j++) {
				cin >> a[i][j];
				if (a[i][j] == '0') continue;
				if (i == 0 || j == 0 || i == n-1 || j == m-1) border++;
				else interior++;
			}
		}
		if (border + interior < 2 * n + 2 * m - 4) cout << "-1\n";
		else cout << (2 * n + 2 * m - 4 - border) << endl;
	}
}
/*****************************************/
