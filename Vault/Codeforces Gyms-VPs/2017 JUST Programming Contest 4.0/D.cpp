...
i64 T, n, q, l, r, id; string s; char c;
/************************************************/
...
void Solve() {
	while (T--) {
		...
		while (q--) {
			...
			if ((i64)l / n == (i64)r / n) {
				l %= n; r %= n;
				if (l == 0) cout << cnt[r][id] << endl;
				else cout << (cnt[r][id] - cnt[l-1][id]) << endl;
			}
			else {
				i64 ans = cnt[n-1][id] * ((i64)r / n - (i64)l / n - 1);
				ans += cnt[r % n][id]; ans += cnt[n-1][id];
				if (l % n > 0) ans -= cnt[l % n - 1][id]; cout << ans << endl;
			}
		}
	}
}
