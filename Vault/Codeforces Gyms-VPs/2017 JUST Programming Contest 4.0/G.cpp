...
i64 T, n; vi a(1000007), Min(1000007), Max(1000007);
/************************************************/
...
void Solve() {
	while (T--) {
		...
		for (i64 i=1; i<n; i++) Max[i] = max(Max[i-1], a[i]);
		for (i64 i=n-2; i>=0; i--) Min[i] = min(Min[i+1], a[i]);
		for (i64 i=1; i<n-1; i++) ans += (Max[i] == a[i] && Min[i] == a[i]);
		cout << ans << endl;
	}
}
/*****************************************/
