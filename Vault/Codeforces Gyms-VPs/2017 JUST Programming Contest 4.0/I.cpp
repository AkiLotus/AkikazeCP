...
i64 T, n; vi a; vector<vi> c(200009, vi(0));
/************************************************/
...
void Solve() {
	while (T--) {
		...
		for (i64 i=0; i<200009; i++) c[i].clear();
		vi dp(n, LINF); dp[0] = 0;
		for (i64 i=0; i<n; i++) {
			cin >> a[i];
			c[a[i]].pub(i);
		...
		for (i64 i=0; i<n-1; i++) {
			...
			if (upper_bound(c[a[i]].begin(), c[a[i]].end(), i) != c[a[i]].end()) {
				dp[*upper_bound(c[a[i]].begin(), c[a[i]].end(), i)] = min(dp[*upper_bound(c[a[i]].begin(), c[a[i]].end(), i)], dp[i] + 1);
			}
		}
		cout << dp[n-1] << endl;
	}
