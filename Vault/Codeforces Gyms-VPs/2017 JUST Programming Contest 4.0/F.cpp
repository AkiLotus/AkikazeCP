...
i64 T, n, q; string s1, s2; vs a; vi cc;
unordered_map<string, i64> Map; unordered_map<i64, vi> ID; vi tree;
/************************************************/
...
void STBuild(i64 node, i64 l, i64 r) {
	if (l > r) return;
	if (l == r) {
		tree[node] = cc[l];
		return;
	}
	STBuild(node*2, l, (l+r)/2);
	STBuild(node*2+1, (l+r)/2+1, r);
...
void Solve() {
	while (T--) {
		cin >> n >> q; a.clear(); a.rsz(n);
		Map.clear(); ID.clear(); cc.clear(); cc.rsz(n, 0);
		tree.clear(); tree.rsz(4*n, -LINF);
		for (i64 i=0; i<n; i++) {
			cin >> a[i]; Map[a[i]] = i;
		}
		for (i64 i=0; i<n; i++) {
			for (i64 j=0; j<a[i].size(); j++) {
				i64 l = j-1, r = j+1; cc[i]++;
				while (l >= 0 && r < a[i].size() && a[i][l] == a[i][r]) {
					l--; r++; cc[i]++;
				}
			}
			for (i64 j=0; j<a[i].size()-1; j++) {
				if (a[i][j] != a[i][j+1]) continue;
				i64 l = j-1, r = j+2; cc[i]++;
				while (l >= 0 && r < a[i].size() && a[i][l] == a[i][r]) {
					l--; r++; cc[i]++;
				}
			}
		...
		STBuild(1, 0, n-1);
		while (q--) {
			cin >> s1 >> s2; i64 demanded = getMax(1, 0, n-1, min(Map[s1], Map[s2]), max(Map[s1], Map[s2]));
			cout << *lower_bound(ID[demanded].begin(), ID[demanded].end(), min(Map[s1], Map[s2]) + 1) << endl;
		}
	}
}
/*****************************************/
