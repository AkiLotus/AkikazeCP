...
void Solve() {
	while (T--) {
		cin >> n >> s; string t = ""; vi cnt(26, 0);
		for (i64 i=0; i<n; i++) cnt[s[i]-'a']++;
		i64 odd = 0;
		for (i64 i=0; i<26; i++) {
			for (i64 j=0; j<cnt[i]/2; j++) t += char(97+i);
			odd += cnt[i] % 2;
		}
		if (odd > 1) cout << "0\n";
		else {
			i64 ans = 1; sort(t.begin(), t.end());
			while (next_permutation(t.begin(), t.end())) ans++;
			cout << ans << endl;
		}
	}
}
