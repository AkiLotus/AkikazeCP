...
void Solve() {
	while (T--) {
		...
		a.rsz(N); b.rsz(N); map<i64, i64> Map;
		for (i64 i=0; i<N; i++) {
			cin >> a[i]; b[i] = a[i];
			Map[a[i]]++;
		}
		sort(b.begin(), b.end());
		for (i64 i=0; i<N; i++) {
			i64 id = lower_bound(b.begin(), b.end(), MOD-1-a[i]) - b.begin();
			if (id == N) {
				if (b[id-1] != a[i] || Map[b[id-1]] > 1) cout << ((b[id-1] + a[i]) % MOD) << " ";
				else cout << ((b[id-2] + a[i]) % MOD) << " ";
			}
			else if (b[id] == MOD-1-a[i]) {
				if (b[id] != a[i] || Map[MOD-1-a[i]] > 1) cout << MOD-1 << " ";
				else if (id > 0) cout << ((b[id-1] + a[i]) % MOD) << " ";
				else cout << ((b[N-1] + a[i]) % MOD) << " ";
			}
			else {
				if (id > 0) {
					if (b[id-1] != a[i] || Map[b[id-1]] > 1) cout << ((b[id-1] + a[i]) % MOD) << " ";
					else if (id > 1) cout << ((b[id-2] + a[i]) % MOD) << " ";
					else cout << ((b[N-1] + a[i]) % MOD) << " ";
				}
				else if (b[N-1] != a[i] || Map[a[i]] > 1) cout << ((b[N-1] + a[i]) % MOD) << " ";
				else cout << ((b[N-2] + a[i]) % MOD) << " ";
			}
		}
		cout << endl;
	}
