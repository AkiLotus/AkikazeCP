...
/** -----GLOBAL VARIABLES----- **/
i64 n, m, d; vector<vector<char>> grid;
vector<vb> vis; vector<vb> killable;
queue<ppi> MQ, BFSQ; pii endPoint;
...
void VarInput() {
	...
	cin >> n >> m >> d; grid.resize(n, vector<char>(m, '.'));
	vis.resize(n, vb(m, false)); killable = vis;
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<m; j++) {
			cin >> grid[i][j];
			if (grid[i][j] == 'S') BFSQ.push(mp(mp(i,j), 0LL));
			else if (grid[i][j] == 'F') endPoint = mp(i, j);
			else if (grid[i][j] == 'M') {
				killable[i][j] = true; vis[i][j] = true;
				MQ.push(mp(mp(i,j), 0LL));
			}
		}
	}
}
...
void ProSolve() {
	while (!MQ.empty()) {
		pii z = MQ.front().fi;
		i64 st = MQ.front().se; MQ.pop();
		if (st == d) continue;
		if (z.fi-1 >= 0 && !vis[z.fi-1][z.se]) {killable[z.fi-1][z.se] = true; vis[z.fi-1][z.se] = true; MQ.push(mp(mp(z.fi-1,z.se), st+1));}
		if (z.fi+1 < n && !vis[z.fi+1][z.se]) {killable[z.fi+1][z.se] = true; vis[z.fi+1][z.se] = true; MQ.push(mp(mp(z.fi+1,z.se), st+1));}
		if (z.se-1 >= 0 && !vis[z.fi][z.se-1]) {killable[z.fi][z.se-1] = true; vis[z.fi][z.se-1] = true; MQ.push(mp(mp(z.fi,z.se-1), st+1));}
		if (z.se+1 < m && !vis[z.fi][z.se+1]) {killable[z.fi][z.se+1] = true; vis[z.fi][z.se+1] = true; MQ.push(mp(mp(z.fi,z.se+1), st+1));}
	}
	for (i64 i=0; i<n; i++) {
		for (i64 j=0; j<m; j++) vis[i][j] = false;
	}
	vis[BFSQ.front().fi.fi][BFSQ.front().fi.se] = true;
	while (!BFSQ.empty()) {
		pii z = BFSQ.front().fi;
		i64 st = BFSQ.front().se; BFSQ.pop();
		if (killable[z.fi][z.se]) {cout << "-1"; return;}
		if (z == endPoint) {cout << st; return;}
		if (z.fi-1 >= 0 && !vis[z.fi-1][z.se] && !killable[z.fi-1][z.se]) {vis[z.fi-1][z.se] = true; BFSQ.push(mp(mp(z.fi-1,z.se), st+1));}
		if (z.fi+1 < n && !vis[z.fi+1][z.se] && !killable[z.fi+1][z.se]) {vis[z.fi+1][z.se] = true; BFSQ.push(mp(mp(z.fi+1,z.se), st+1));}
		if (z.se-1 >= 0 && !vis[z.fi][z.se-1] && !killable[z.fi][z.se-1]) {vis[z.fi][z.se-1] = true; BFSQ.push(mp(mp(z.fi,z.se-1), st+1));}
		if (z.se+1 < m && !vis[z.fi][z.se+1] && !killable[z.fi][z.se+1]) {vis[z.fi][z.se+1] = true; BFSQ.push(mp(mp(z.fi,z.se+1), st+1));}
	}
	cout << "-1";
}
