...
typedef pair<vi, i64> pvi;

/** -----GLOBAL VARIABLES----- **/
i64 n; vector<pvi> adj; vb vis;
vector<vi> anc_suc; vi Parent;
vector<pii> edgeList; bool flag = true;
vi checkTree_vis; vector<vi> Adj;
vector<pvi> succList; vi tmp;

/** -----EXTENSIVE FUNCTIONS----- **/
bool adjlistSort(pvi a, pvi b) {
...
void DFS(i64 z, i64 last) {
	...
	//tracker2(z, last);
	for (auto t: Adj[z]) {
		if (checkTree_vis[t] == 1) continue;
		if (t == last) continue;
		if (checkTree_vis[t] == 2) {
			flag = false; return;
		}
		DFS(t, z);
	}
...
void DFS_SuccList(i64 z, i64 last) {
	checkTree_vis[z] = 1;
	//tracker2(z, last);
	for (auto t: Adj[z]) {
		if (checkTree_vis[t]) continue;
		if (t == last) continue;
		tmp.pub(t); DFS_SuccList(t, z);
	}
}

bool isTree() {
	for (i64 i=0; i<n; i++) checkTree_vis[i] = 0;
	if (edgeList.size() != n-1) return false;
	DFS(0, -1);
	if (!flag) return false;
	for (i64 i=0; i<n; i++) {
		if (!checkTree_vis[i]) return false;
	}
	return true;
}
...
bool validSucc() {
	for (i64 i=0; i<n; i++) {
		for (i64 x=0; x<n; x++) checkTree_vis[x] = 0;
		tmp.clear();
		DFS_SuccList(i, Parent[i]);
		sort(tmp.begin(), tmp.end());
		//cout << "tmp = " << tmp << endl;
		succList.pub(mp(tmp, i));
	}
	sort(succList.begin(), succList.end(), adjlistSort);
	return (succList == adj);
}
...
void VarInput() {
	...
	cin >> n; adj.rsz(n); vis.rsz(n, false);
	anc_suc.rsz(n, vi(n, 0)); Parent.rsz(n, -1);
	for (i64 i=0; i<n; i++) {
		adj[i].se = i;
		i64 x; cin >> x;
		while (x--) {
...
void ProSolve() {
	//for (i64 i=0; i<n; i++) cout << anc_suc[i] << endl;
	sort(adj.begin(), adj.end(), adjlistSort);
	set<i64> direct;
	for (i64 i=0; i<n; i++) {
		i64 u = adj[i].se;
		//cout << "u = " << u << endl;
		for (auto v: adj[i].fi) {
			//cout << "v = " << v << endl;
			if (!vis[v]) {cout << "NO"; return;}
			i64 z = v;
			while (Parent[z] != -1) z = Parent[z];
			//cout << "anc_suc[" << u << "][" << z << "] = " << anc_suc[u][z] << endl;
			if (!anc_suc[u][z] && z != u) {cout << "NO"; return;}
			if (direct.find(v) == direct.end()) continue;
			edgeList.pub(mp(u, v)); direct.erase(v); Parent[v] = u;
		}
		vis[u] = true; direct.insert(u);
	}
	checkTree_vis.rsz(n, 0); Adj.rsz(n);
	for (i64 i=0; i<edgeList.size(); i++) {
		i64 u = edgeList[i].fi, v = edgeList[i].se;
		Adj[u].pub(v); Adj[v].pub(u);
	}
	//cout << edgeList << endl;
	if (!isTree() || !validSucc()) cout << "NO";
	else {
		cout << "YES\n";
		for (i64 i=0; i<edgeList.size(); i++) {
			cout << edgeList[i].fi+1 << " " << edgeList[i].se+1 << endl;
		}
	}
}
