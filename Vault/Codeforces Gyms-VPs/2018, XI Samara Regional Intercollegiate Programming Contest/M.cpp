...
/** -----GLOBAL VARIABLES----- **/
vector<string> s(3); i64 n;
vector<map<vi, i64>> Map(3);
vector<vi> pow29(200009, vi(4, 1));
...
void VarInput() {
	...
	cin >> s[0] >> s[1] >> s[2]; n = s[0].size();
}

void ProSolve() {
	vi diff;
	for (i64 i=0; i<n; i++) {
		if (s[0][i] != s[1][i] || s[1][i] != s[2][i] || s[2][i] != s[0][i]) diff.pub(i);
	}
	if (diff.size() > 3) {cout << "Impossible"; return;}
	if (diff.size() < 2) {cout << "Ambiguous"; return;}
	for (i64 i=1; i<200009; i++) {
		for (i64 j=0; j<4; j++) {
			pow29[i][j] = (pow29[i-1][j] * 29) % HashMod[j];
		}
	}
	vector<vi> HashKey(3, vi(4, 0));
	for (i64 i=0; i<3; i++) {
		for (i64 j=0; j<n; j++) {
			for (i64 k=0; k<4; k++) {
				HashKey[i][k] += pow29[j][k] * (s[i][j] - 'a' + 1);
				HashKey[i][k] %= HashMod[k];
			}
		}
		Map[i][HashKey[i]]++;
	}
	for (i64 i=0; i<3; i++) {
		for (i64 j=0; j<n; j++) {
			for (i64 k=0; k<26; k++) {
				...
				vi tmp = HashKey[i];
				for (i64 x=0; x<4; x++) {
					tmp[x] += pow29[j][x] * (k - (s[i][j] - 'a'));
					tmp[x] %= HashMod[x]; while (tmp[x] < 0) tmp[x] += HashMod[x];
				}
				Map[i][tmp]++;
			}
		}
	...
	for (auto it=Map[0].begin(); it!=Map[0].end(); it++) {
		if (Map[1][it->first] && Map[2][it->first]) ans.pub(it->first);
		if (ans.size() > 1) {cout << "Ambiguous"; return;}
	}
	if (ans.empty()) {cout << "Impossible"; return;}
	if (ans.size() > 1) {cout << "Ambiguous"; return;}
	for (i64 j=0; j<n; j++) {
		for (i64 k=0; k<26; k++) {
			if (s[0][j] - 'a' == k) continue;
			vi tmp = HashKey[0];
			for (i64 x=0; x<4; x++) {
				tmp[x] += pow29[j][x] * (k - (s[0][j] - 'a'));
				tmp[x] %= HashMod[x]; while (tmp[x] < 0) tmp[x] += HashMod[x];
			}
			if (tmp == ans[0]) {
				s[0][j] = char(97 + k); cout << s[0]; return;
			}
		}
	}
	cout << s[0];
