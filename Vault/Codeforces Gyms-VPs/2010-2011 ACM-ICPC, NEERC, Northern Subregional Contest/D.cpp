...
void Input() {
	...
	for (i64 i=0; i<n; i++) {
		cin >> A[i].fi >> A[i].se;
		r.pub(A[i].fi); c.pub(A[i].se);
	}
	r.pub(0); r.pub(w+1); c.pub(0); c.pub(h+1);
}

void Solve() {
	sort(r.begin(), r.end()); sort(c.begin(), c.end());
	i64 mdR = -LINF, mdC = -LINF;
	for (i64 i=1; i<r.size(); i++) mdR = max(mdR, r[i] - r[i-1] - 1);
	for (i64 i=1; i<c.size(); i++) mdC = max(mdC, c[i] - c[i-1] - 1);
	cout << mdR * mdC;
}
/*****************************************/
