...
/************** [GLOBAL VARIABLES] **************/
i64 n, st, en, a, b, l; vi m(2);
vector<vector<vector<pii>>> Adj;
vector<vector<vector<vi>>> Matrix;
vector<vi> dp; bool cycle = false; vector<vi> Vis;
/************************************************/

/************** [FUNCTIONS] **************/
void DFS(i64 z, i64 x) {
	//tracker3(z, x, dp[z][x]);
	Vis[z][x] = 2;
	for (i64 t=0; t<n; t++) {
		if (Matrix[z][x][t][1-x] == LINF) continue;
		if (Vis[t][1-x] == 2) {cycle = true; return;}
		dp[t][1-x] = max(dp[t][1-x], dp[z][x] + Matrix[z][x][t][1-x]);
		DFS(t, 1-x);
		if (cycle) return;
	}
	if (cycle) return; Vis[z][x] = 1;
	//cout << "ended - "; tracker2(z, x);
}

void Input() {
	cin >> n >> st >> en; st--; en--;
	Adj.rsz(2, vector<vector<pii>>(n));
	Matrix.rsz(n, vector<vector<vi>>(2, vector<vi>(n, vi(2, LINF))));
	for (i64 i=0; i<2; i++) {
		cin >> m[i];
		for (i64 x=0; x<m[i]; x++) {
			cin >> a >> b >> l; a--; b--;
...
void Solve() {
	vector<vi> Dist(2, vi(n, LINF));
	Dist[0][en] = 0; Dist[1][en] = 0;
	vector<vb> vis(2, vb(n, false));
	for (i64 x=0; x<2; x++) {
		priority_queue<pii> Q; Q.push(mp(-Dist[x][en], en));
		while (!Q.empty()) {
			pii Z = Q.top(); Q.pop();
			i64 dist = -Z.fi, z = Z.se;
			if (vis[x][dist]) continue; vis[x][dist] = true;
			for (auto T: Adj[x][z]) {
				i64 t = T.fi, d = T.se;
				if (Dist[x][t] <= dist + d) continue;
				Dist[x][t] = dist + d; Q.push(mp(-Dist[x][t], t));
			}
		}
	}
	for (i64 i=0; i<n; i++) {
		for (i64 x=0; x<2; x++) {
			for (auto T: Adj[x][i]) {
				i64 j = T.fi, d = T.se;
				if (Dist[x][j] >= Dist[x][i]) continue;
				Matrix[i][x][j][1-x] = min(Matrix[i][x][j][1-x], d);
			}
		}
	}
	dp.rsz(n, vi(2, 0)); Vis.rsz(n, vi(2, 0)); DFS(st, 0);
	//for (auto zx: Adj[0]) cout << zx << endl;
	//cout << "------------\n";
	//for (auto zx: Adj[1]) cout << zx << endl;
	//cout << Dist[0] << endl << Dist[1] << endl;
	//for (i64 i=0; i<n; i++) {
	//	for (i64 j=0; j<2; j++) {
	//		tracker2(i, j);
	//		for (auto xz: Matrix[i][j]) cout << xz << endl;
	//	}
	//}
	if (cycle) cout << "-1"; else cout << max(dp[en][0], dp[en][1]);
}
/*****************************************/
