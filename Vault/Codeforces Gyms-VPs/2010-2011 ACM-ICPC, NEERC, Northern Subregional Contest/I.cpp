...
string Line; i64 P = 0, T = 0; vector<Data> A;
ld V = 0.0L, O = 0.0L, E = 0.0L, U = 0.0L, N = 0.0L;
vd I; vi solvedCnt, lowestSolved;
/************************************************/

/************** [FUNCTIONS] **************/
vs Tokenizer(string s) {
	vs Z; string tmp = "";
	for (auto x: s) {
		if (x == ' ') {
			if (tmp.size() > 0) Z.pub(tmp);
			tmp = "";
		}
		else tmp += x;
	}
...
i64 IntParse(string s) {
	i64 res = 0;
	for (auto x: s) res = res * 10 + (x - '0');
	return res;
}

void generateData(vs X) {
	Data New = Data(P); New.Rank = IntParse(X[X.size()-1]);
	for (i64 i=X.size()-3-P; i<X.size()-3; i++) {
		if (X[i] == ".") continue;
		if (X[i][0] == '+') {New.slv++; New.Solved[i-(X.size()-3-P)] = true; solvedCnt[i-(X.size()-3-P)]++; lowestSolved[i-(X.size()-3-P)] = New.Rank;}
		New.Penalty[i-(X.size()-2-P)] = IntParse(X[i].substr(1));
	}
	A.pub(New);
}

bool singularDash(string s) {
	for (auto x: s) {
		if (x != '-') return false;
	}
	return true;
...
void Input() {
	...
	while (getline(cin, Line)) {
		if (!singularDash(Line)) {LastLine = Line; continue;}
		vs X = Tokenizer(LastLine); P = X.size() - 4; break;
	}
	I.rsz(P, 0.0L); solvedCnt.rsz(P, 0); lowestSolved.rsz(P, -1);
	while (getline(cin, Line)) {
		vs X = Tokenizer(Line);
		//tracker2(X, Line);
		generateData(X); T++;
		//cout << "Participant " << T << ": (solved: " << A[T-1].slv << ")\n";
		//for (i64 i=0; i<P; i++) cout << A[T-1].Solved[i] << " "; cout << endl;
		//for (i64 i=0; i<P; i++) cout << A[T-1].Penalty[i] << " "; cout << endl;
	}
}

void Solve() {
	//cout << "lowestSolved: " << lowestSolved << endl;
	//cout << "solvedCnt: " << solvedCnt << endl;
	if (T > 0 && P > 0) {
		for (auto New: A) {if (New.slv == 0) V += 1.0L; else if (New.slv == P) O += 1.0L;}
		for (i64 i=0; i<P; i++) U += 1.0L * (solvedCnt[i] == 0);
		for (i64 i=0; i<P; i++) {
			for (auto DataTmp: A) {
				if (!DataTmp.Solved[i] && DataTmp.Rank < lowestSolved[i]) {
					I[i] += 1.0L;
				}
			}
			I[i] /= T;
		...
		V /= T; O /= T; U /= P;
		for (i64 i=1; i<T; i++) {
			if (A[i-1].slv == A[i].slv) continue;
			E += 1.0L / P * (A[i-1].slv - A[i].slv - 1);
		}
		N = 1.03L * V + 3.141L * O + 2.171L * E + 1.414L * U;
		for (i64 i=0; i<P; i++) N += I[i] / P;
	}
	...
	for (i64 i=0; i<P; i++) cout << "Instability " << i+1 << " = " << I[i] << endl;
