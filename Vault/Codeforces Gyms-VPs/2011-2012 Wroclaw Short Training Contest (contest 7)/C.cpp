...
void ProSolve() {
	while (cin >> k) {
		set<pii> diff; vector<vi> a(k, vi(k));
		vector<queue<i64>> Q(k); i64 sum = 0;
		for (i64 i=0; i<k; i++) {
			for (i64 j=0; j<k; j++) {
				cin >> a[i][j];
			}
			...
			sum += a[i][0];
			for (i64 j=1; j<k; j++) {
				Q[i].push(a[i][j] - a[i][j-1]);
				if (j == 1) diff.insert(mp(Q[i].front(), i));
			}
		}
		for (i64 i=0; i<k; i++) {
			cout << sum << " ";
			pii z = *diff.begin(); diff.erase(diff.begin()); sum += z.fi;
			cout << "removed " << z << endl;
			Q[z.se].pop(); diff.insert(mp(Q[z.se].front(), z.se));
			cout << "added " << mp(Q[z.se].front(), z.se) << endl;
		}
		cout << endl;
	}
}
