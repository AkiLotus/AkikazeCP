...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, l; vector<pii> Fish;
vector<pii> Fisherwoman; vi cc;
map<i64, i64> Map;
/************************************************/
...
void Input() {
	cin >> n >> m >> l; Fish.rsz(n); Fisherwoman.rsz(m); cc.rsz(m, 0);
	for (i64 i=0; i<n; i++) cin >> Fish[i].fi >> Fish[i].se;
	for (i64 i=0; i<m; i++) {
		cin >> Fisherwoman[i].fi;
		Fisherwoman[i].se = i;
		Map[Fisherwoman[i].fi] = 0;
	}
}

void Solve() {
	vi ps(m, 0); sort(Fisherwoman.begin(), Fisherwoman.end());
	for (auto fish: Fish) {
		i64 x = fish.fi, y = fish.se;
		if (y > l) continue;
		i64 L = x + (y - l), R = x - (y - l);
		i64 st = lower_bound(Fisherwoman.begin(), Fisherwoman.end(), mp(L, -1000000000)) - Fisherwoman.begin();
		i64 en = lower_bound(Fisherwoman.begin(), Fisherwoman.end(), mp(R, +1000000000)) - Fisherwoman.begin();
		if (st < m) ps[st]++; if (en < m) ps[en]--;
	}
	for (i64 i=1; i<m; i++) ps[i] += ps[i-1];
	for (i64 i=0; i<m; i++) cc[Fisherwoman[i].se] = ps[i];
	for (i64 i=0; i<m; i++) cout << cc[i] << endl;
}
/*****************************************/
