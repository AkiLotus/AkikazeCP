...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, u, v; vi Kuro; vector<vi> adj;
/************************************************/
...
void Input() {
	cin >> n >> m; adj.rsz(n); Kuro.rsz(n);
	for (i64 i=0; i<n; i++) cin >> Kuro[i];
	for (i64 i=1; i<n; i++) {
		cin >> u >> v; u--; v--;
...
void Solve() {
	...
	vector<vi> Dist(n, vi(n, LINF)), Last(n, vi(n, -1));
	for (i64 i=0; i<n; i++) {
		queue<i64> Q; vb vis(n, false);
		Dist[i][i] = 0; vis[i] = true; Q.push(i);
		while (!Q.empty()) {
			i64 z = Q.front(); Q.pop();
			for (auto t: adj[z]) {
				if (vis[t]) continue;
				Dist[i][t] = Dist[i][z] + 1;
				Q.push(t); Last[i][t] = z; vis[t] = true;
			}
		}
	}
	i64 ans = LINF;
	for (i64 i=0; i<n; i++) {
		...
		for (i64 j=i+1; j<n; j++) {
			if (!Kuro[j]) continue;
			i64 Len = Dist[i][j], kuro = 2;
			vi dist(n, LINF); vb vis(n, false); 
			dist[i] = Len; dist[j] = Len; vis[i] = true; vis[j] = true;
			queue<i64> Q; Q.push(i); Q.push(j);
			while (!Q.empty()) {
				i64 z = Q.front(); Q.pop();
			}
		}
