...
i64 n, m; map<string, i64> CatPrice, PickUp, CostPerKm;
map<string, i64> Bill; map<string, bool> Inconsistent;
/************************************************/
...
void Input() {
	cin >> n >> m; CatPrice.clear(); PickUp.clear();
	CostPerKm.clear(); Bill.clear(); Inconsistent.clear();
	while (n--) {
		string Name; i64 catalog, pickup, costperkm;
		cin >> Name >> catalog >> pickup >> costperkm;
		CatPrice[Name] = catalog; PickUp[Name] = pickup;
		CostPerKm[Name] = costperkm;
	}
}

void Solve() {
	map<string, psi> CurrentCar;
	while (m--) {
		i64 Time; string spy, cmd;
		cin >> Time >> spy >> cmd;
		if (Bill.find(spy) == Bill.end()) Bill[spy] = 0;
		if (cmd == "p") {
			string car; cin >> car;
			if (Inconsistent[spy]) continue;
			if (CurrentCar.find(spy) != CurrentCar.end()) {
				Inconsistent[spy] = true; continue;
			}
			CurrentCar[spy] = mp(car, Time);
		}
		else if (cmd == "r") {
			i64 dist; cin >> dist;
			if (Inconsistent[spy]) continue;
			...
			string car = CurrentCar[spy].fi;
			Bill[spy] += PickUp[car];
			Bill[spy] += CostPerKm[car] * dist;
			CurrentCar.erase(spy);
		}
		else if (cmd == "a") {
			i64 percentage; cin >> percentage;
			if (Inconsistent[spy]) continue;
			if (CurrentCar.find(spy) == CurrentCar.end()) {
				Inconsistent[spy] = true; continue;
			}
			string car = CurrentCar[spy].fi;
			i64 megaprice = CatPrice[car] * percentage;
			i64 cost = megaprice / 100 + (megaprice % 100 != 0);
			Bill[spy] += cost;
		}
	}
	for (auto it=Bill.begin(); it!=Bill.end(); it++) {
		string spy = it->fi;
		if (CurrentCar.find(spy) != CurrentCar.end()) {
			cout << spy << " INCONSISTENT\n"; continue;
		}
		if (Inconsistent[spy]) {cout << spy << " INCONSISTENT\n"; continue;}
		cout << spy << " " << Bill[spy] << endl;
	}
}
/*****************************************/
