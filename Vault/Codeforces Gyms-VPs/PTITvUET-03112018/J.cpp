...
/************** [GLOBAL VARIABLES] **************/
i64 h, w, x1, y1, x2, y2; vector<vc> A; vector<pii> InitList;
vector<vi> Dist1, Dist2; vector<vector<vi>> DistExodus;
i64 dx[] =  {-1, +0, +0, +1}, dy[] = {+0, -1, +1, +0};
pii Default = mp(-1LL, -1LL);
/************************************************/
...
void CalculateDist1() {
	...
	deque<pii> Deque; vis[x1][y1] = true;
	Deque.push_front(mp(x1, y1));
	while (!Deque.empty()) {
		pii Z = Deque.front(); Deque.pop_front();
		i64 x = Z.fi, y = Z.se;
		for (i64 i=0; i<4; i++) {
			...
			Dist1[x+dx[i]][y+dy[i]] = Dist1[x][y] + (A[x+dx[i]][y+dy[i]] == '#');
			if (A[x+dx[i]][y+dy[i]] == '#') {
				Deque.push_back(mp(x+dx[i], y+dy[i]));
			}
			else Deque.push_front(mp(x+dx[i], y+dy[i]));
		}
	}
}
...
void CalculateDist2() {
	Dist2[x2][y2] = 0;
	vector<vb> vis(h, vb(w, false));
	deque<pii> Deque; vis[x2][y2] = true;
	Deque.push_front(mp(x2, y2));
	while (!Deque.empty()) {
		pii Z = Deque.front(); Deque.pop_front();
		i64 x = Z.fi, y = Z.se;
		for (i64 i=0; i<4; i++) {
			...
			if (A[x+dx[i]][y+dy[i]] == '*') continue;
			vis[x+dx[i]][y+dy[i]] = true;
			Dist2[x+dx[i]][y+dy[i]] = Dist2[x][y] + (A[x+dx[i]][y+dy[i]] == '#');
			if (A[x+dx[i]][y+dy[i]] == '#') {
				Deque.push_back(mp(x+dx[i], y+dy[i]));
			}
			else Deque.push_front(mp(x+dx[i], y+dy[i]));
...
void Input() {
	cin >> h >> w; x1 = -1; y1 = -1; x2 = -1; y2 = -1;
	A.clear(); A.rsz(h, vc(w, '.')); InitList.clear();
	Dist1.clear(); Dist2.clear(); DistExodus.clear();
	Dist1.rsz(h, vi(w, LINF)); Dist2.rsz(h, vi(w, LINF));
	for (i64 i=0; i<h; i++) {
		for (i64 j=0; j<w; j++) {
			cin >> A[i][j];
			if (A[i][j] != '*' && ((i == 0 || i == h-1) || (j == 0 || j == w-1))) {
				InitList.pub(mp(i, j)); DistExodus.pub(Dist1);
			}
			if (A[i][j] == '$') {
				if (x1 == -1) {x1 = i; y1 = j;}
				else {x2 = i; y2 = j;}
...
void Solve() {
	CalculateDist1(); CalculateDist2();
	i64 ans = LINF;
	for (i64 qqqq=0; qqqq<InitList.size(); qqqq++) {
		pii qwerty = InitList[qqqq];
		i64 X = qwerty.fi, Y = qwerty.se;
		vector<vb> vis(h, vb(w, false));
		deque<pii> Deque; vis[X][Y] = true;
		DistExodus[qqqq][X][Y] = (A[X][Y] == '#');
		Deque.push_front(mp(X, Y));
		while (!Deque.empty()) {
			pii Z = Deque.front(); Deque.pop_front();
			i64 x = Z.fi, y = Z.se;
			for (i64 i=0; i<4; i++) {
				...
				if (A[x+dx[i]][y+dy[i]] == '*') continue;
				vis[x+dx[i]][y+dy[i]] = true;
				DistExodus[qqqq][x+dx[i]][y+dy[i]] = DistExodus[qqqq][x][y] + (A[x+dx[i]][y+dy[i]] == '#');
				if (A[x+dx[i]][y+dy[i]] == '#') {
					Deque.push_back(mp(x+dx[i], y+dy[i]));
				}
				else Deque.push_front(mp(x+dx[i], y+dy[i]));
		...
		for (i64 x=0; x<h; x++) {
			for (i64 y=0; y<w; y++) {
				ans = min(ans, Dist1[x][y]+Dist2[x][y]+DistExodus[qqqq][x][y]-(A[x][y] == '#')*2);
			}
		}
	}
	cout << ans << endl;
