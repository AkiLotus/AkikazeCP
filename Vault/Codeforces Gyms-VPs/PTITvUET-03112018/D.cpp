...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, tt, s, g, h, a, b, D, Zap; vi T, List;
vector<vector<pii>> adj; vi DistS, DistG, DistH;
/************************************************/

/************** [FUNCTIONS] **************/
void PreprocessSPath() {
	priority_queue<pii> Q; vb vis(n, false);
	DistS[s] = 0; Q.push(mp(-DistS[s], s));
	while (!Q.empty()) {
		pii Z = Q.top(); Q.pop();
		i64 z = Z.se, d = -Z.fi;
		if (vis[z]) continue; vis[z] = true;
		for (auto Token: adj[z]) {
			i64 t = Token.fi, p = Token.se;
			if (vis[t]) continue;
			if (d + p >= DistS[t]) continue;
			DistS[t] = d + p; Q.push(mp(-DistS[t], t));
		}
	}
	vis.clear(); vis.rsz(n, false);
	DistG[g] = 0; Q.push(mp(-DistG[g], g));
	while (!Q.empty()) {
		pii Z = Q.top(); Q.pop();
		i64 z = Z.se, d = -Z.fi;
		if (vis[z]) continue; vis[z] = true;
		for (auto Token: adj[z]) {
			i64 t = Token.fi, p = Token.se;
			if (vis[t]) continue;
			if (d + p >= DistG[t]) continue;
			DistG[t] = d + p; Q.push(mp(-DistG[t], t));
		}
	}
	vis.clear(); vis.rsz(n, false);
	DistH[h] = 0; Q.push(mp(-DistH[h], h));
	while (!Q.empty()) {
		pii Z = Q.top(); Q.pop();
		i64 z = Z.se, d = -Z.fi;
		if (vis[z]) continue; vis[z] = true;
		for (auto Token: adj[z]) {
			i64 t = Token.fi, p = Token.se;
			if (vis[t]) continue;
			if (d + p >= DistH[t]) continue;
			DistH[t] = d + p; Q.push(mp(-DistH[t], t));
		}
	}
}

void Input() {
	cin >> n >> m >> tt >> s >> g >> h; s--; g--; h--;
	T.clear(); List.clear(); adj.clear(); T.rsz(tt); adj.rsz(n);
	DistS.clear(); DistG.clear(); DistH.clear();
	DistS.rsz(n, LINF); DistG.rsz(n, LINF); DistH.rsz(n, LINF);
	while (m--) {
		cin >> a >> b >> D; a--; b--;
		if ((a == g && b == h) || (a == h && b == g)) Zap = D;
		adj[a].pub(mp(b, D)); adj[b].pub(mp(a, D));
	}
	for (i64 i=0; i<tt; i++) {cin >> T[i]; T[i]--;}
}

void Solve() {
	PreprocessSPath();
	for (auto x: T) {
		if (DistG[s] + Zap + DistH[x] == DistS[x]) {List.pub(x); continue;}
		if (DistH[s] + Zap + DistG[x] == DistS[x]) {List.pub(x); continue;}
	}
	sort(List.begin(), List.end());
	for (auto x: List) cout << x+1 << " "; cout << endl;
}
/*****************************************/
