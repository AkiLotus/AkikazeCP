...
/************** [GLOBAL VARIABLES] **************/
i64 n, c, m; vi b, p; ld ans = 0.0L;
vector<vd> Prob; vi Sum; queue<pii> Q;
/************************************************/
...
void Input() {
	cin >> n >> c >> m; b.clear(); p.clear();
	while (!Q.empty()) Q.pop();
	b.rsz(n); p.rsz(n); Prob.clear(); Sum.clear();
	Prob.rsz(1LL << n, vd(n+1, 0.0L)); Sum.rsz(1LL << n, 0);
	for (i64 i=0; i<n; i++) {
		cin >> b[i] >> p[i];
		Prob[1LL << i][1] = (ld)p[i] / 100;
		Prob[1LL << i][0] = (ld)(100 - p[i]) / 100;
		Q.push(mp(1LL << i, 0)); Q.push(mp(1LL << i, 1));
	}
	for (i64 i=0; i<(1LL << n); i++) {
		for (i64 x=0; x<n; x++) {
			if (((1LL << x) & i) == 0) continue;
			Sum[i] += b[x];
		}
	}
}
...
void Solve() {
	vector<vb> vis(1LL << n, vb(n+1, false));
	while (!Q.empty()) {
		pii Z = Q.front(); Q.pop();
		i64 mask = Z.fi, cnt = Z.se;
		if (cnt >= c) ans = max(ans, Prob[mask][cnt]);
		for (i64 x=0; x<n; x++) {
			if (((1LL << x) & mask) > 0) continue;
			i64 mask1 = mask + (1LL << x);
			Prob[mask1][cnt+1] = max(Prob[mask1][cnt+1], Prob[mask][cnt] * p[x] / 100);
			Prob[mask1][cnt] = max(Prob[mask1][cnt], Prob[mask][cnt] * (100 - p[x]) / 100);
			Q.push(mp(mask1, cnt+1)); Q.push(mp(mask1, cnt));
		}
	}
	cout << ans << endl;
}
