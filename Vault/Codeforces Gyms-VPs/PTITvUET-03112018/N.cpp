...
/************** [GLOBAL VARIABLES] **************/
i64 n, m, setcnt = 1; map<i64, i64> Map;
vector<set<i64>> SetList(1); vi ftrl(1000003, 1);
/************************************************/
...
void Input() {
	for (i64 i=1; i<1000003; i++) ftrl[i] = (ftrl[i-1] * i) % Mod;
	cin >> n >> m;
	for (i64 i=1; i<=m; i++) {
		SetList[0].insert(i);
		Map[i] = 0;
...
void Solve() {
	while (n--) {
		i64 g; cin >> g; vi A(g);
		map<i64, i64> cc; map<i64, vi> Set;
		for (i64 i=0; i<g; i++) {cin >> A[i]; cc[A[i]]++;}
		for (auto it=cc.begin(); it!=cc.end(); it++) {
			i64 k = it->fi, cnt = it->se;
			Set[cnt].pub(k);
		}
		for (auto it=Set.begin(); it!=Set.end(); it++) {
			i64 cnt = it->fi; map<i64, i64> ID;
			for (auto z: Set[cnt]) {
				i64 id = Map[z];
				if (ID.find(id) == ID.end()) {ID[id] = setcnt; set<i64> Tmp; SetList.pub(Tmp); setcnt++;}
				i64 mapped = ID[id]; Map[z] = mapped;
				SetList[id].erase(z); SetList[mapped].insert(z);
			}
		}
	}
	i64 ans = 1;
	for (i64 i=0; i<setcnt; i++) {
		i64 siz = SetList[i].size();
		ans *= ftrl[siz]; ans %= Mod;
	}
	cout << ans;
}
/*****************************************/
