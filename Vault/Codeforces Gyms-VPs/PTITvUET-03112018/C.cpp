...
/************** [GLOBAL VARIABLES] **************/
i64 n, matched = 0; vs M; string D, X; vector<vi> cc(26, vi(26, 0));
/************************************************/

/************** [FUNCTIONS] **************/
bool correctMatch(string s, string t) {
	if (s.size() != t.size()) {return false;}
	vi Map(26, -1), used(26, 0);
	for (i64 i=0; i<t.size(); i++) {
		if (Map[t[i]-'a'] == -1) {
			Map[t[i]-'a'] = s[i] - 'a'; used[s[i]-'a']++;
			if (used[s[i]-'a'] > 1) {return false;}
			continue;
		}
		if (Map[t[i]-'a'] != s[i]-'a') {return false;}
	}
	return true;
}

void Deduction() {
	i64 missingcnt1 = 0, missingcnt2 = 0, missed1 = -1, missed2 = -1;
	for (i64 i=0; i<26; i++) {
		...
		if (!flag1) {missingcnt1++; missed1 = i;}
		for (i64 j=0; j<26; j++) {
			if (cc[j][i] == matched) {
				flag2 = true; break;
			}
		}
		if (!flag2) {missingcnt2++; missed2 = i;}
	}
	if (missingcnt1 != 1 || missingcnt2 != 1) return;
	cc[missed1][missed2] = matched;
}

void Input() {
	cin >> n; M.clear(); M.rsz(n); matched = 0;
	cc.clear(); cc.rsz(26, vi(26, 0));
	for (i64 i=0; i<n; i++) cin >> M[i];
	cin >> D >> X;
}

void Solve() {
	for (auto s: M) {
		if (!correctMatch(s, D)) continue;
		matched++; vector<vb> vis(26, vb(26, false));
		for (i64 x=0; x<D.size(); x++) {
			if (vis[D[x]-'a'][s[x]-'a']) continue;
			vis[D[x]-'a'][s[x]-'a'] = true;
			cc[D[x]-'a'][s[x]-'a']++;
		}
	}
	if (matched == 0) {cout << "IMPOSSIBLE\n"; return;}
	Deduction();
	for (i64 i=0; i<X.size(); i++) {
		i64 id = -1;
		for (i64 x=0; x<26; x++) {
			if (cc[x][X[i]-'a'] == matched) {
				id = x; break;
			}
		}
		if (id == -1) cout << "?";
		else cout << char(97+id);
	}
	cout << endl;
}
/*****************************************/
