...
/************** [GLOBAL VARIABLES] **************/
i64 n, a; string s; multiset<i64> Singular;
map<string, priority_queue<i64>> Map;
/************************************************/
...
void Input() {
	...
	while (n--) {
		cin >> s >> a;
		Map[s].push(a);
		if (s == "10" || s == "01") Singular.insert(a);
	}
}

void Solve() {
	i64 ans = 0;
	while (!Map["10"].empty() && !Map["01"].empty()) {
		ans += Map["10"].top(); ans += Map["01"].top();
		Singular.erase(Singular.find(Map["10"].top()));
		Singular.erase(Singular.find(Map["01"].top()));
		Map["10"].pop(); Map["01"].pop();
	}
	i64 L = Map["11"].size(), i = 0;
	while (!Map["11"].empty()) {ans += Map["11"].top(); Map["11"].pop();}
	while (!Map["00"].empty()) {Singular.insert(Map["00"].top()); Map["00"].pop();}
	while (!Singular.empty() && i < L) {
		i64 rr = *Singular.rbegin();
		Singular.erase(Singular.find(rr));
		i++; ans += rr;
	}
	cout << ans;
}
