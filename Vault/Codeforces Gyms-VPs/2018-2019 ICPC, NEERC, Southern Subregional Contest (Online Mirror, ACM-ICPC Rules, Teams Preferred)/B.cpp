...
/************** [GLOBAL VARIABLES] **************/
i64 n; vs A; vb Kuro; set<pii> KuroSeg, ShiraSeg, Seg;
/************************************************/

/************** [FUNCTIONS] **************/
string StringParse(i64 z) {
	if (z == 0) return "0";
	string res = "";
	while (z > 0) {
...
bool Collide(i64 x, bool kuro) {
	if (kuro) {
		auto it = ShiraSeg.upper_bound(mp(x, LINF));
		if (it == ShiraSeg.begin()) return false; it--;
		return (it->fi <= x && x <= it->se);
	}
	else {
		auto it = KuroSeg.upper_bound(mp(x, LINF));
		if (it == KuroSeg.begin()) return false; it--;
		return (it->fi <= x && x <= it->se);
	}
}

void SplitShiro(i64 L, i64 R) {
	auto it1 = ShiraSeg.upper_bound(mp(L, LINF));
	auto it2 = ShiraSeg.upper_bound(mp(R, LINF));
	i64 x1 = -1, y1 = -1, x2 = -1, y2 = -1;
	if (it1 != ShiraSeg.begin()) {
		it1--; x1 = it1->fi; y1 = it1->se;
		if (x1 <= L && L <= y1) L = x1;
	}
	if (it2 != ShiraSeg.begin()) {
		it2--; x2 = it2->fi; y2 = it2->se;
		if (x2 <= L && L <= y2) R = y2;
	}
	if (L == x1) ShiraSeg.erase(mp(x1, y1));
	if (y2 == R) ShiraSeg.erase(mp(x2, y2));
	tracker3("ShiraInsert", L, R);
	ShiraSeg.insert(mp(L, R));
	
	it1 = Seg.upper_bound(mp(L, LINF));
	it2 = Seg.upper_bound(mp(R, LINF));
	x1 = -1; y1 = -1; x2 = -1; y2 = -1;
	i64 L1 = L, R1 = R;
	if (it1 != Seg.begin()) {
		it1--; x1 = it1->fi; y1 = it1->se;
	...
	if (L == y1 && x2 == R) {
		if (x1 == x2 && y1 == y2) {
			cout << "caught into the net!\n";
			Seg.erase(mp(x1, y1));
			if (x1 < L1) Seg.insert(mp(x1, L1-1));
			if (R1 < y2) Seg.insert(mp(R1+1, y2));
		}
		else {
			Seg.erase(mp(x1, y1));
			if (x1 < L1) Seg.insert(mp(x1, L1-1));
	...
	tracker8(x1, y1, x2, y2, L, R, L1, R1);
	tracker1(Seg);
}

pii Process(string s) {
	i64 L = 0, id = s.find("/"), neglected = 0, tmp = 0;
	for (i64 i=0; i<=id; i++) {
		if (s[i] >= '0' && s[i] <= '9') {tmp = tmp * 10 + (s[i] - '0');}
		else {L = L * 256 + tmp; tmp = 0;}
	}
	for (i64 i=id+1; i<s.size(); i++) neglected = neglected * 10 + (s[i] - '0');
	L /= (1LL << (32 - neglected)); L *= (1LL << (32 - neglected));
	return (mp(L, L + (1LL << (32 - neglected)) - 1));
}

void Input() {
	cin >> n; A.rsz(n); Kuro.rsz(n, false); Seg.insert(mp(0LL, (1LL << 32) - 1));
	for (i64 i=0; i<n; i++) {
		cin >> A[i]; Kuro[i] = (A[i][0] == '-'); A[i].erase(0, 1);
		if (A[i].find("/") < 0 || A[i].find("/") >= A[i].size()) A[i] += "/32";
	}
}

void Solve() {
	for (i64 i=0; i<n; i++) {
		pii zazz = Process(A[i]);
		i64 L = zazz.fi, R = zazz.se;
		tracker3(L, R, Kuro[i]);
		if (Collide(L, Kuro[i]) || Collide(R, Kuro[i])) {
			cout << "-1"; return;
		}
		if (!Kuro[i]) SplitShiro(L, R);
	}
	...
	for (auto it=Seg.begin(); it!=Seg.end(); it++) {
		...
		while (L <= R) {
			for (i64 i=32; i>=0; i--) {
				if (R-L+1 < (1LL << i)) continue;
				i64 Mid = L + (1LL << i) - 1;
				string IP = "";
				IP += StringParse(L / (1LL << 24)); IP += ".";
				IP += StringParse(L / (1LL << 16) - (L / (1LL << 24)) * (1LL << 8)); IP += ".";
				IP += StringParse(L / (1LL << 8) - (L / (1LL << 16)) * (1LL << 8)); IP += ".";
				IP += StringParse(L / (1LL << 0) - (L / (1LL << 8)) * (1LL << 8)); IP += "/";
				IP += StringParse(32 - i); L = Mid + 1; IPList.pub(IP); break;
			}
		}
	}
	cout << IPList.size() << endl;
	for (auto x: IPList) cout << x << endl;
}
/*****************************************/
