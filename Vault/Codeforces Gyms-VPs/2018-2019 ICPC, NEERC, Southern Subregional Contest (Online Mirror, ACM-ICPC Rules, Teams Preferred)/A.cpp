...
/************** [GLOBAL VARIABLES] **************/
i64 d, s; vector<vb> vis; vector<vector<pii>> Last;
/************************************************/

/************** [FUNCTIONS] **************/
void Traceback(i64 rem, i64 sum) {
	string ans = "";
	while (Last[rem][sum] != mp(-1LL, -1LL)) {
		i64 R = Last[rem][sum].fi, S = Last[rem][sum].se;
		ans += char(48 + sum - S); rem = R; sum = S;
	}
...
void Solve() {
	vis.rsz(d, vb(s+1, false));
	Last.rsz(d, vector<pii>(s+1, mp(-1LL, -1LL)));
	queue<pii> Q; vis[0][0] = true; Q.push(mp(0LL, 0LL));
	while (!Q.empty()) {
		pii z = Q.front(); Q.pop();
		i64 rem = z.fi, sum = z.se;
		if (rem == 0 && sum == s) {Traceback(rem, sum); return;}
		for (i64 i=(z == mp(0LL, 0LL) ? 1 : 0); i<10; i++) {
			i64 newr = (rem * 10 + i) % d, news = sum + i;
			if (news <= s && !vis[newr][news]) {
				vis[newr][news] = true;
				Last[newr][news] = z;
				Q.push(mp(newr, news));
			}
