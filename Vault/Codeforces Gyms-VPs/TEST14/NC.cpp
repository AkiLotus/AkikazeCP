...
i64 n; vs a;
vp cc(10, mp(0LL, 0LL)); vi assigned(10, -1);
vb leadingZ(10, false);
...
void VarInput() {
	...
	for (i64 i=0; i<n; i++) {
		cin >> a[i];
		leadingZ[a[i][0]-'a'] = true;
		for (i64 i=0; i<10; i++) cc[i].se = i;
		i64 mul = 1;
		for (i64 j=a[i].size(); j>=0; j--) {
			cc[a[i][j]-'a'].fi += mul;
			mul *= 10;
		}
	}
	...
	for (i64 i=0; i<10; i++) {
		if (leadingZ[cc[i].se]) continue;
		assigned[cc[i].se] = 0; break;
	}
	i64 tmp = 1, ans = 0;
	for (i64 i=0; i<10; i++) {
		if (assigned[cc[i].se] >= 0) continue;
		assigned[cc[i].se] = tmp;
		ans += tmp * cc[i].fi; tmp++;
	}
	cout << ans / 10;
}
