...
/** -----GLOBAL VARIABLES----- **/
i64 Q, cnt = 0, siz = 0;
vi rangeSingle(100000, 0), rangeDouble(100000, -1);
string query; vector<char> s(100000, '\0');
...
void ProSolve() {
	for (auto c: query) {
		if (c == '-') {
			for (i64 i=siz-3; i>=0; i-=2) {
				if (s[i] != s[siz-1]) continue;
				if (rangeSingle[(siz-1+i)/2] + (siz-1+i)/2 - 1 != siz-1) continue;
				rangeSingle[(siz-1+i)/2]--; cnt--;
			}
			for (i64 i=siz-2; i>=0; i-=2) {
				if (s[i] != s[siz-1]) continue;
				if (rangeDouble[(siz-1+i)/2] == -1 && i != siz-2) continue;
				if (i != siz-2 && rangeDouble[(siz-1+i)/2] + (siz-1+i)/2 + 1 != siz-1) continue;
				rangeDouble[(siz-1+i)/2]--; cnt--;
			}
			rangeSingle[siz-1] = 0; cnt--;
			s[siz-1] = '\0'; siz--;
		}
		else {
			for (i64 i=siz-2; i>=0; i-=2) {
				if (s[i] != c) continue;
				if (rangeSingle[(siz+i)/2] + (siz+i)/2 != siz) continue;
				rangeSingle[(siz+i)/2]++; cnt++;
			}
			for (i64 i=siz-1; i>=0; i-=2) {
				if (s[i] != c) continue;
				if (rangeDouble[(siz+i)/2] == -1 && i != siz-1) continue;
				if (i != siz-1 && rangeDouble[(siz+i)/2] + (siz+i)/2 + 2 != siz) continue;
				rangeDouble[(siz+i)/2]++; cnt++;
			}
			rangeSingle[siz] = 1; cnt++;
			s[siz] = c; siz++;
		}
		cout << cnt << " ";
	}
