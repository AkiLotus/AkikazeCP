...
void ProSolve() {
	...
	if (zero.empty()) {cout << "0 0"; return;}
	if (zero.size() == 1) {cout << zero[0] << " 1"; return;}
	if (zero.size() == N) {cout << "0 " << N; return;}
	i64 st = zero[0], len = 1, optimal = 0;
	for (i64 i=1; i<zero.size(); i++) {
		if (((i == zero.size()-1) ? (N-1 - zero[i]) : (zero[i+1] - zero[i] - 1)) > ((optimal == zero.size()-1) ? (N-1 - zero[optimal]) : (zero[optimal+1] - zero[optimal] - 1))) {len = (zero[i] - zero[0] + 1); optimal = i;}
	}
	cout << st << " " << len;
}
