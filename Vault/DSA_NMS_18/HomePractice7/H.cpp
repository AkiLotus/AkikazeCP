...
void ProSolve() {
	while (T--) {
		cin >> N >> M; vector<vi> A(N, vi(M));
		vector<vi> dist(N, vi(M, LINF));
		vector<vb> vis(N, vb(M, false));
		for (i64 i=0; i<N; i++) {
			for (i64 j=0; j<M; j++) cin >> A[i][j];
		}
		dist[0][0] = A[0][0];
		priority_queue<pip> heap; heap.push(mp(-A[0][0], mp(0LL, 0LL)));
		while (!heap.empty()) {
			pip z = heap.top(); heap.pop();
			i64 d = -z.fi, x = z.se.fi, y = z.se.se;
			if (vis[x][y]) continue; vis[x][y] = true;
			if (x-1 >= 0 && !vis[x-1][y] && d+A[x-1][y] < dist[x-1][y]) {
				dist[x-1][y] = d+A[x-1][y]; heap.push(mp(-(d+A[x-1][y]), mp(x-1, y)));
			}
			if (y-1 >= 0 && !vis[x][y-1] && d+A[x][y-1] < dist[x][y-1]) {
				dist[x][y-1] = d+A[x][y-1]; heap.push(mp(-(d+A[x][y-1]), mp(x, y-1)));
			}
			if (x+1 < N && !vis[x+1][y] && d+A[x+1][y] < dist[x+1][y]) {
				dist[x+1][y] = d+A[x+1][y]; heap.push(mp(-(d+A[x+1][y]), mp(x+1, y)));
			}
			if (y+1 < M && !vis[x][y+1] && d+A[x][y+1] < dist[x][y+1]) {
				dist[x][y+1] = d+A[x][y+1]; heap.push(mp(-(d+A[x][y+1]), mp(x, y+1)));
			}
		}
