...
/** -----GLOBAL VARIABLES----- **/
i64 N, M; vector<vector<pii>> adj;
vi dist, dp; vb vis; i64 cnt = 0;
...
void VarInput() {
	...
	cin >> N >> M; adj.rsz(N); dp.rsz(N, 0); dp[0] = 1;
	dist.rsz(N, LINF); vis.rsz(N, false); dist[0] = 0;
	while (M--) {
		i64 u, v, c; cin >> u >> v >> c; u--; v--;
		adj[u].pub(mp(v,c)); adj[v].pub(mp(u,c));
	}
}

void ProSolve() {
	priority_queue<pii> heap; heap.push(mp(-dist[0], 0));
	while (!heap.empty()) {
		i64 D = -heap.top().fi, z = heap.top().se; heap.pop();
		if (vis[z]) continue; vis[z] = true;
		for (i64 i=0; i<adj[z].size(); i++) {
			i64 t = adj[z][i].fi, d = adj[z][i].se;
			if (vis[t]) continue;
			if (D+d == dist[t]) dp[t] += dp[z];
			else if (D+d < dist[t]) {
				dist[t] = D+d; dp[t] = dp[z];
				heap.push(mp(-(D+d), t));
			}
		}
	}
