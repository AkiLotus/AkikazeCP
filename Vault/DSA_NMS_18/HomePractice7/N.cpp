...
/** -----GLOBAL VARIABLES----- **/
i64 N, X, Y, ans = 0, cnt = 0;
vector<pii> C, D; vector<ppi> List;
...
void VarInput() {
	...
	cin >> N >> X >> Y;
	while (N--) {
		i64 b, p; string c; cin >> b >> p >> c;
		List.pub(mp(mp(b, p), (i64)(c == "C")));
		if (c == "C") C.pub(mp(p, b));
		if (c == "D") D.pub(mp(p, b));
	}
}
...
void ProSolve() {
	...
	sort(C.begin(), C.end());
	sort(D.begin(), D.end());
	vi maxC(1000007, 0), maxD(1000007, 0);
	for (i64 i=1; i<1000007; i++) {
		i64 pos = upper_bound(C.begin(), C.end(), mp(i, -LINF)) - C.begin() - 1;
		if (pos == -1) maxC[i] = 0; else maxC[i] = max(maxC[i-1], C[pos].se);
		pos = upper_bound(D.begin(), D.end(), mp(i, -LINF)) - D.begin() - 1;
		if (pos == -1) maxD[i] = 0; else maxD[i] = max(maxD[i-1], D[pos].se);
	}
	for (i64 i=0; i<List.size(); i++) {
        i64 x = X, y = Y, tmp = List[i].fi.fi;
        if (List[i].se) x -= List[i].fi.se; else y -= List[i].fi.se;
        if (x != 0) {
