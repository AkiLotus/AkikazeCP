...
static const long long hashmod[] = {1000000007LL, 1000000009LL, 1000000021LL, 1000000033LL};
vi HashMod(hashmod, hashmod + sizeof(hashmod) / sizeof(i64));
...
void ProSolve() {
	while (T--) {
		cin >> N >> M; vector<vi> adj(N);
		vb contained(N, false); i64 monitored = 0;
		vi selfLoop(N, 0); i64 totalSL = 0;
		vector<pii> edge(M); i64 ans = 0;
		for (i64 i=0; i<M; i++) {
			i64 x, y; cin >> x >> y; x--; y--;
			edge[i] = mp(x, y);
			if (x != y) {adj[x].pub(y); adj[y].pub(x);}
			else {adj[x].pub(y); selfLoop[x]++; totalSL++;}
			contained[x] = true; contained[y] = true;
		}
		for (i64 i=0; i<N; i++) monitored += contained[i];
		//cout << "monitored = " << monitored << endl;
		vb vis(N, false); queue<i64> Q;
		bool flag = true;
		for (i64 i=0; i<N; i++) {
			if (vis[i]) continue;
			i64 cnt = 1, ecnt = 0; vis[i] = true; Q.push(i);
			while (!Q.empty()) {
				i64 z = Q.front(); Q.pop();
				ecnt += adj[z].size();
				for (i64 i=0; i<adj[z].size(); i++) {
					i64 t = adj[z][i]; if (vis[t]) continue;
					vis[t] = true; cnt++; Q.push(t);
				}
			}
			//cout << "cnt = " << cnt << endl;
			if (ecnt > 0 && cnt != monitored) {flag = false; break;}
		}
		if (!flag) {cout << "0\n"; continue;}
		//cout << "Came down here?\n";
		for (i64 i=0; i<M; i++) {
			i64 x = edge[i].fi, y = edge[i].se;
			if (x != y) ans += ((i64)adj[x].size() + (i64)adj[y].size() - 2 + (totalSL - selfLoop[x] - selfLoop[y]));
			else ans += (M - 1);
			//tracker2(x, y);
			//cout << "ans = " << ans << endl;
		}
