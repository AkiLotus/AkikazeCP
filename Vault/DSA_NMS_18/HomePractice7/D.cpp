...
void ProSolve() {
	while (T--) {
		cin >> N >> M; vector<vector<pii>> adj(N);
		while (M--) {
			i64 u, v, c; cin >> u >> v >> c; u--; v--;
			adj[u].pub(mp(v, c)); adj[v].pub(mp(u, c));
		}
		vb vis(N, false); priority_queue<pii> heap; heap.push(mp(0LL, 0LL));
		i64 ans = 0;
		while (!heap.empty()) {
			i64 z = heap.top().se;
			i64 tmp = heap.top().fi; heap.pop();
			if (vis[z]) continue; vis[z] = true;
			ans -= tmp;
			for (i64 i=0; i<adj[z].size(); i++) {
				pii zz = adj[z][i];
				if (vis[zz.fi]) continue;
				heap.push(mp(-zz.se, zz.fi));
			}
		}
		cout << ans << endl;
	}
