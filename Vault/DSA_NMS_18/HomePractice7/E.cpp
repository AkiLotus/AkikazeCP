...
ld DistCalc(ld xA, ld yA, ld xB, ld yB){
	ld dx = xB - xA, dy = yB - yA;
...
void ProSolve() {
	while (T--) {
		cin >> N; vector<pdd> a(N);
		vector<vd> dist(N, vd(N)); vb vis(N, false);
		for (i64 i=0; i<N; i++) cin >> a[i].fi >> a[i].se;
		for (i64 i=0; i<N; i++) {
			for (i64 j=0; j<N; j++) {
				dist[i][j] = DistCalc(a[i].fi, a[i].se, a[j].fi, a[j].se);
			}
		}
		priority_queue<pdi> heap; heap.push(mp(0.0L, 0));
		ld ans = 0.0L;
		while (!heap.empty()) {
			i64 z = heap.top().se;
			ld tmp = heap.top().fi; heap.pop();
			if (vis[z]) continue; vis[z] = true;
			ans -= tmp;
			for (i64 i=0; i<N; i++) {
				if (i == z) continue;
				if (vis[i]) continue;
				heap.push(mp(-dist[z][i], i));
			}
		}
