...
void ProSolve() {
	while (T--) {
		pii endPoint; queue<ppip> Q; pii Default = mp(0LL, 0LL);
		cin >> N >> M; vector<vector<char>> Map(N, vector<char>(M, '.'));
		vector<vector<vector<vb>>> vis(N, vector<vector<vb>>(M, vector<vb>(3, vb(4, false))));
		for (i64 i=0; i<N; i++) {
			for (i64 j=0; j<M; j++) {
				cin >> Map[i][j];
				if (Map[i][j] == 'S') Q.push(mp(mp(mp(i, j), 0LL), Default));
				if (Map[i][j] == 'T') endPoint = mp(i, j);
			}
		}
		bool flag = false;
		while (!Q.empty()) {
			ppip z = Q.front(); Q.pop();
			//tracker1(z);
			if (z.fi.fi == endPoint) {flag = true; break;}
			i64 a = z.fi.fi.fi, b = z.fi.fi.se, c = z.fi.se; pii Dir = z.se;
			if (a-1 >= 0 && Map[a-1][b] != '*' && (Dir == Default || Dir == mp(-1LL, 0LL)) && !vis[a-1][b][c][0]) {
				vis[a-1][b][c][0] = true; Q.push(mp(mp(mp(a-1, b), c), mp(-1LL, 0LL)));
			}
			if (a-1 >= 0 && Map[a-1][b] != '*' && (Dir != Default && Dir != mp(-1LL, 0LL)) && c < 2 && !vis[a-1][b][c+1][0]) {
				vis[a-1][b][c+1][0] = true; Q.push(mp(mp(mp(a-1, b), c+1), mp(-1LL, 0LL)));
			}
			if (b-1 >= 0 && Map[a][b-1] != '*' && (Dir == Default || Dir == mp(0LL, -1LL)) && !vis[a][b-1][c][1]) {
				vis[a][b-1][c][0] = true; Q.push(mp(mp(mp(a, b-1), c), mp(0LL, -1LL)));
			}
			if (b-1 >= 0 && Map[a][b-1] != '*' && (Dir != Default && Dir != mp(0LL, -1LL)) && c < 2 && !vis[a][b-1][c+1][1]) {
				vis[a][b-1][c+1][0] = true; Q.push(mp(mp(mp(a, b-1), c+1), mp(0LL, -1LL)));
			}
			if (a+1 < N && Map[a+1][b] != '*' && (Dir == Default || Dir == mp(+1LL, 0LL)) && !vis[a+1][b][c][2]) {
				vis[a+1][b][c][0] = true; Q.push(mp(mp(mp(a+1, b), c), mp(+1LL, 0LL)));
			}
			if (a+1 < N && Map[a+1][b] != '*' && (Dir != Default && Dir != mp(+1LL, 0LL)) && c < 2 && !vis[a+1][b][c+1][2]) {
				vis[a+1][b][c+1][0] = true; Q.push(mp(mp(mp(a+1, b), c+1), mp(+1LL, 0LL)));
			}
			if (b+1 < M && Map[a][b+1] != '*' && (Dir == Default || Dir == mp(0LL, +1LL)) && !vis[a][b+1][c][3]) {
				vis[a][b+1][c][0] = true; Q.push(mp(mp(mp(a, b+1), c), mp(0LL, +1LL)));
			}
			if (b+1 < M && Map[a][b+1] != '*' && (Dir != Default && Dir != mp(0LL, +1LL)) && c < 2 && !vis[a][b+1][c+1][3]) {
				vis[a][b+1][c+1][0] = true; Q.push(mp(mp(mp(a, b+1), c+1), mp(0LL, +1LL)));
			}
		}
		if (!flag) cout << "NO\n"; else cout << "YES\n";
