...
void ProSolve() {
	while (N--) {
		cin >> infix; i64 siz = infix.size();
		stack<i64> S; queue<char> Q; stack<char> ope;
		for (i64 i=0; i<siz; i++) {
			if (infix[i] >= 'a' && infix[i] <= 'z') Q.push(infix[i]);
			else if (infix[i] == '(') S.push(i);
			else if (infix[i] == ')') {
				while (!Q.empty()) {
					cout << Q.front(); Q.pop();
				}
				cout << ope.top(); S.pop(); ope.pop();
			}
			else ope.push(infix[i]);
		}
		cout << endl;
	}
}
