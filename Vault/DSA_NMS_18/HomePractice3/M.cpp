...
void ProSolve() {
	while (T--) {
		cin >> S; i64 N = S.size();
		stack<i64> Stack; bool flag = true;
		for (i64 i=0; i<N; i++) {
			if (!flag) break;
			if (S[i] == '(') Stack.push(1);
			else if (S[i] == '[') Stack.push(2);
			else if (S[i] == '{') Stack.push(3);
			else if (S[i] == ')') {
				if (Stack.empty() || (!Stack.empty() && Stack.top() != 1)) {flag = false; break;}
				else Stack.pop();
			}
			else if (S[i] == ']') {
				if (Stack.empty() || (!Stack.empty() && Stack.top() != 2)) {flag = false; break;}
				else Stack.pop();
			}
			else if (S[i] == '}') {
				if (Stack.empty() || (!Stack.empty() && Stack.top() != 3)) {flag = false; break;}
				else Stack.pop();
			}
		}
