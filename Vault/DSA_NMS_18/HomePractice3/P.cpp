...
void ProSolve() {
	while (T--) {
		cin >> n; vi Q(10000); i64 ptr1 = 0, ptr2 = -1;
		while (n--) {
			cin >> cmd;
			if (cmd == 1) cout << max(ptr2-ptr1+1, 0LL) << endl;
			else if (cmd == 2) {
				if (ptr1 > ptr2) cout << "YES\n"; else cout << "NO\n";
			}
			else if (cmd == 3) {
				i64 x; cin >> x; Q[++ptr2] = x;
			}
			else if (cmd == 4) {
				if (ptr1 <= ptr2) ptr1++;
			}
				...
				if (ptr1 > ptr2) cout << "-1\n";
				else cout << Q[ptr1] << endl;
			}
			else if (cmd == 6) {
				if (ptr1 > ptr2) cout << "-1\n";
				else cout << Q[ptr2] << endl;
			}
		}
	}
}
