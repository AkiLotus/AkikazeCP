...
/** -----GLOBAL VARIABLES----- **/
i64 M, N; vs words, ans;
string token; bool flag = false;
vector<vector<char> > mat;
vector<vb> vis(10, vb(10, false));

/** -----EXTENSIVE FUNCTIONS----- **/
void DFS(i64 i, i64 j, i64 cnt) {
	//tracker3(i, j, cnt);
	vis[i][j] = true;
	if (cnt + 1 == token.size()) {vis[i][j] = false; flag = true; return;}
	for (i64 x=-1; x<=1; x++) {
		for (i64 y=-1; y<=1; y++) {
			if (x == 0 && y == 0) continue;
			...
			if (mat[i+x][j+y] != token[cnt+1]) continue;
			DFS(i+x,j+y,cnt+1);
			if (flag) {vis[i][j] = false; return;}
		}
	}
	vis[i][j] = false;
}
...
void ProSolve() {
	...
	do {
		map<string, i64> Map;
		mat.clear(); mat.resize(N, vector<char>(N));
		for (i64 i=0; i<N; i++) {
			for (i64 j=0; j<N; j++) {
				cin >> mat[i][j];
			}
		...
		ans.clear();
		for (i64 a=0; a<words.size(); a++) {
			token = "";
			bool valid = true;
			for (i64 x=0; x<words[a].size(); x++) {
				token += words[a][x];
				if (words[a][x] == 'q') {
					x++; if (x == words[a].size() || (x < words[a].size() && words[a][x] != 'u')) {
						valid = false; break;
					}
				}
			}
			...
			for (i64 i=0; i<N; i++) {
				for (i64 j=0; j<N; j++) {
					if (mat[i][j] != token[0]) continue;
					DFS(i, j, 0);
					if (flag) break;
				}
				if (flag) break;
			}
			if (flag && !Map[words[a]]) {ans.pub(words[a]); Map[words[a]]++;}
		}
		sort(ans.begin(), ans.end());
		for (i64 i=0; i<ans.size(); i++) cout << ans[i] << endl; cout << "-\n";
		cin >> N;
	} while (N != 0);
}
