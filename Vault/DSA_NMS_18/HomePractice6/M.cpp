	...
	while (T--) {
		...
		for (i64 i=0; i<N; i++) {
			for (i64 j=0; j<M; j++) {
				...
				ans++; queue<pii> Q; vis[i][j] = true; Q.push(mp(i,j));
				while (!Q.empty()) {
					pii z = Q.front(); Q.pop();
					for (i64 x=-1; x<=1; x++) {
						for (i64 y=-1; y<=1; y++) {
							if (x == 0 && y == 0) continue;
							if (z.fi+x >= 0 && z.fi+x < N && z.se+y >= 0 && z.se+y < M && !vis[z.fi+x][z.se+y] && A[z.fi+x][z.se+y]) {
								vis[z.fi+x][z.se+y] = true; Q.push(mp(z.fi+x, z.se+y));
							}
						}
					}
				}
