...
/** -----GLOBAL VARIABLES----- **/
i64 N, K, M; map<i64, i64> blocked;
vi conncomp; i64 tmp = 0; vector<vb> vis, isSheep;

/** -----EXTENSIVE FUNCTIONS----- **/
void DFS(i64 x, i64 y) {
	//tracker2(x, y);
	vis[x][y] = true; if (isSheep[x][y]) tmp++;
	i64 leftX = x, leftY = y-1;
	i64 rightX = x, rightY = y+1;
	i64 upX = x-1, upY = y;
	i64 downX = x+1, downY = y;
	if (leftY >= 0 && !blocked[(x*N+y)*1000000000LL+(leftX*N+leftY)] && !vis[leftX][leftY]) DFS(leftX, leftY);
	if (rightY < N && !blocked[(x*N+y)*1000000000LL+(rightX*N+rightY)] && !vis[rightX][rightY]) DFS(rightX, rightY);
	if (upX >= 0 && !blocked[(x*N+y)*1000000000LL+(upX*N+upY)] && !vis[upX][upY]) DFS(upX, upY);
	if (downX < N && !blocked[(x*N+y)*1000000000LL+(downX*N+downY)] && !vis[downX][downY]) DFS(downX, downY);
}

/** -----COMPULSORY FUNCTIONS----- **/
void VarInput() {
	//ios_base::sync_with_stdio(0); cin.tie(NULL);
	cin >> N >> K >> M; vis.resize(N, vb(N, false)); isSheep = vis;
	while (M--) {
		i64 u, v, x, y; cin >> u >> v >> x >> y; u--; v--; x--; y--;
		blocked[(u*N+v)*1000000000LL+(x*N+y)]++;
		blocked[(x*N+y)*1000000000LL+(u*N+v)]++;
	}
	for (i64 i=0; i<K; i++) {
		i64 u, v; cin >> u >> v; u--; v--;
		isSheep[u][v] = true;
...
void ProSolve() {
	...
	i64 ans = 0;
	for (i64 i=0; i<conncomp.size(); i++) ans += conncomp[i] * (K - conncomp[i]);
	ans /= 2; cout << ans;
}
