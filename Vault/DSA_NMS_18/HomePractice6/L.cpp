...
/** -----GLOBAL VARIABLES----- **/
i64 T, N, M; bool flag = true;
vi section; vector<vi> Adj; vb vis;

/** -----EXTENSIVE FUNCTIONS----- **/
void DFS(i64 z, i64 sec) {
	section[z] = sec; vis[z] = true;
	for (i64 i=0; i<Adj[z].size(); i++) {
		if (section[Adj[z][i]] == sec) {
			flag = false; return;
		}
...
void ProSolve() {
	while (T--) {
		cin >> N >> M; flag = true;
		section.clear(); Adj.clear(); vis.clear();
		section.resize(N+1); Adj.resize(N+1); vis.resize(N+1);
		while (M--) {
			i64 u, v; cin >> u >> v;
