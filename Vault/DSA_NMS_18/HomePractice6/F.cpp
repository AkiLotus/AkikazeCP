...
/** -----GLOBAL VARIABLES----- **/
i64 N, M, A, B, K, G, ans = LINF; vi presidentPath; vector<vi> Adj;
map<pii, i64> Duration; vb vis;
map<pii, pii> Map; pii Default = mp(-LINF, LINF);
...
void VarInput() {
	...
	cin >> N >> M >> A >> B >> K >> G; presidentPath.resize(G);
	Adj.resize(N+1); vis.resize(N+1, false);
	for (i64 i=0; i<G; i++) cin >> presidentPath[i];
	while (M--) {
		i64 X, Y, L; cin >> X >> Y >> L;
	...
	i64 Time = 0;
	for (i64 i=1; i<G; i++) {
		i64 X = presidentPath[i-1];
		i64 Y = presidentPath[i];
		Map[mp(X,Y)] = mp(Time, Time+Duration[mp(X,Y)]-1);
		Map[mp(Y,X)] = mp(Time, Time+Duration[mp(Y,X)]-1);
		Time += Duration[mp(X,Y)];
	}
}

void ProSolve() {
	vi dist(N+1, LINF); dist[A] = K;
	priority_queue<pii> Q; Q.push(mp(-K, A));
	while (!Q.empty()) {
		pii z = Q.top(); Q.pop();
		//tracker2(z.fi, z.se);
		i64 Time = -z.fi; if (vis[z.se]) continue;
		vis[z.se] = true;
		for (i64 i=0; i<Adj[z.se].size(); i++) {
			i64 t = Adj[z.se][i];
			if (vis[t]) continue;
			i64 NextTime = Time + Duration[mp(z.se,t)];
			if (Map[mp(z.se,t)].fi <= Time && Time <= Map[mp(z.se,t)].se) NextTime = Map[mp(z.se,t)].se + 1 + Duration[mp(z.se,t)];
			dist[t] = min(dist[t], NextTime); Q.push(mp(-NextTime, t));
		}
	}
	//cout << dist << endl;
	cout << dist[B] - K;
