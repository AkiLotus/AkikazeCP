...
int findAge (int id) {
	for (int i=0; i<children[id].size(); i++) {
		if (id == x) a[children[id][i]].age = 100 - a[children[id][i]].dAge;
		else a[children[id][i]].age = a[id].age - a[children[id][i]].dAge;
		if (children[children[id][i]].size() != 0) findAge(children[id][i]);
	}
}
 
int bcCmp (bc y, bc z) {
	if (y.age > z.age) return 1;
	if (y.age == z.age && y.uName < z.uName) return 1;
	return 0;
}
...
void ProSolve() {
	while (test-- > 0) {
		...
		for (int i=0; i<101; i++) {
			children[i].clear();
			if (i != 100) {
				a[i].dName = ""; a[i].uName = ""; a[i].age = 0; a[i].dID = 0; a[i].uID = 0; a[i].dAge = 0;
			}
		}
		cin >> x;
		for (int i=0; i<x; i++) {
			cin >> a[i].dName >> a[i].uName >> a[i].dAge;
			a[i].uID = i;
			user.push_back(a[i].uName);
		}
		for (int i=0; i<x; i++) {
			a[i].dID = find(user.begin(), user.end(), a[i].dName) - user.begin();
			children[a[i].dID].push_back(a[i].uID);
		}
		findAge(x);
		sort(a, a+x, bcCmp);
		for (int i=0; i<x; i++) {
			cout << a[i].uName << " " << a[i].age << endl;
		}
	}
}
