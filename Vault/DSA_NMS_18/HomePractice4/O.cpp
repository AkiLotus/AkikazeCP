...
void ProSolve() {
	vector<vector<vb> > dp(N, vector<vb>(N+1, vb(K, false)));
	dp[0][0][0] = true; dp[0][1][a[0] % K] = true;
	for (i64 i=1; i<N; i++) {
		for (i64 j=0; j<=i+1; j++) {
			for (i64 k=0; k<K; k++) {
				dp[i][j][k] = dp[i-1][j][k];
				if (j > 0) {
					i64 previousModulo = k - (a[i] % K);
					while (previousModulo < 0) previousModulo += K;
					dp[i][j][k] = (dp[i][j][k] | dp[i-1][j-1][previousModulo]);
				}
			}
		}
	}
