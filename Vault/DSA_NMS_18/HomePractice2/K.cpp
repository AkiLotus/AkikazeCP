...
/** -----GLOBAL VARIABLES----- **/
i64 N, sum = 0; vi sumrow; vector<vi> mat;
...
void VarInput() {
	...
	cin >> N; sumrow.resize(N, 0); mat.resize(N, vi(N, 0));
	for (i64 i=0; i<N; i++) {
		for (i64 j=0; j<N; j++) {
			cin >> mat[i][j]; sumrow[i] += mat[i][j];
			sum += mat[i][j];
		}
	}
}
...
void ProSolve() {
	sum /= (2 * (N - 1));
	if (N == 2) {
		cout << mat[0][1] / 2 << " " << mat[0][1] / 2; return;
	}
	for (i64 i=0; i<N; i++) cout << (sumrow[i] - sum) / (N - 2) << " ";
}
