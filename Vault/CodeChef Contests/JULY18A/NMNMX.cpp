...
i64 T, N, K; vi A, cnt; vi ftrl(5555, 1);
/************************************************/
...
void Input() {
	cin >> N >> K; A.clear(); A.rsz(N);
	cnt.clear(); cnt.rsz(N, 0LL);
...
void Solve() {
	sort(A.begin(), A.end());
	for (i64 i=1; i<N-1; i++) {
		for (i64 j=max(1, K-(N-i)); j<=min(K-2,i); j++) {
			cnt[i] += (nCr(i, j) * nCr(N-i-1, K-1-j)) % MOD;
		}
	}
	for (i64 i=0; i<n; i++) {
		cnt[i] *= modPow(ftrl[K-1], MOD-2);
		cnt[i] %= (MOD-1);
	}
	i64 ans = 1;
	for (i64 i=0; i<n; i++) {
...
int main(int argc, char* argv[]) {
	...
	ControlIO(argc, argv);
	for (i64 i=1; i<5555; i++) ftrl[i] = (ftrl[i-1] * i) % MOD;
	cin >> T;
	while (T--) {
	Input(); TimerStart();
	Solve(); TimerStop();
