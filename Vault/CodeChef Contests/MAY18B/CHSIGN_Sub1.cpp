...
void ProSolve() {
	while (T--) {
		...
		i64 sum = LINF; vi ans = A;
		for (i64 i=0; i<(1LL << N); i++) {
			vi B = A;
			for (i64 j=0; j<N; j++) {
				if ((i & (1LL << j)) != 0) B[j] = -B[j];
			}
			//cout << B << endl;
			bool flag = true;
			for (i64 j=0; j<N; j++) {
				for (i64 k=j+1; k<N; k++) {
					i64 tmp = 0;
					for (i64 x=j; x<=k; x++) tmp += B[x];
					if (tmp <= 0) {flag = false; break;}
				}
				if (!flag) break;
			}
			if (flag) {
				i64 s = 0; for (i64 x=0; x<N; x++) s += B[x];
				//cout << sum << " " << B << endl;
				if (s < sum) {sum = s; ans = B;}
			}
		}
		for (i64 i=0; i<ans.size(); i++) cout << ans[i] << " "; cout << endl;
	}
}
