...
void ProSolve() {
	Fib[1] = 1; for (i64 i=2; i<100007; i++) {Fib[i] = Fib[i-1] + Fib[i-2]; Fib[i] %= MOD;}
	while (T--) {
		cin >> M >> N; vi A(M), B(M);
		i64 result = 0;
		for (i64 i=0; i<M; i++) {cin >> A[i]; result += (((A[i] * M) % MOD) * Fib[max(N-2, 0LL)]) % MOD; result %= MOD;}
		for (i64 i=0; i<M; i++) {cin >> B[i]; result += (((B[i] * M) % MOD) * Fib[max(N-1, 0LL)]) % MOD; result %= MOD;}
		if (N == 1) {
			result = 0; for (i64 i=0; i<M; i++) {result += A[i] * M;  result %= MOD;}
		}
		cout << result << endl;
	}
}
