...
void ProSolve() {
	while (T--) {
		cin >> N >> Q; vi A(N); map<i64, i64> Map;
		for (i64 i=0; i<N; i++) {cin >> A[i]; Map[A[i]] = i+1;}
		vi B = A; sort(B.begin(), B.end());
		while (Q--) {
			cin >> X; i64 pos = Map[X];
			// tracking pair: i64 pointerPosition, i64(bool) needLower
			vector<pii> tracking; i64 lo = 1, hi = N, ptr = 0;
			i64 low = 0, high = 0;
			while (ptr != pos) {
				ptr = (lo + hi) / 2; if (ptr == pos) continue;
				if (ptr > pos) {
					// Pointer is higher, require higher val
					tracking.pub(mp(ptr, 0LL)); high++; hi = ptr - 1;
				}
				else {
					// Pointer is lower, require lower val
					tracking.pub(mp(ptr, 1LL)); low++; lo = ptr + 1;
				}
			}
			i64 sortedIndex = lower_bound(B.begin(), B.end(), X) - B.begin();
//			cout << tracking << endl; cout << "low/high = " << low << "/" << high << endl;
//			cout << "sortedIndex = " << sortedIndex << endl;
			if (low > sortedIndex || high > (N - 1 - sortedIndex)) {cout << "-1\n"; continue;}
			i64 lowAns = 0, higAns = 0;
			for (i64 i=0; i<tracking.size(); i++) {
				i64 index = tracking[i].fi-1, isLower = tracking[i].se;
				if (isLower && A[index] > X) lowAns++;
				if (!isLower && A[index] < X) higAns++;
			}
			cout << max(lowAns, higAns) << endl;
		}
	}
}
