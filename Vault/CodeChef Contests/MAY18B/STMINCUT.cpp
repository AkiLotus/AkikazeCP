...
/** -----GLOBAL VARIABLES----- **/
i64 T, N, ans; vector<vi> A, resMat;
vector<vi> MaxSpTree_Adj; vb vis;

/** -----EXTENSIVE FUNCTIONS----- **/
void DFS(i64 u, i64 v, i64 last, i64 minimum) {
	resMat[u][v] = minimum; resMat[v][u] = minimum;
	vis[v] = true;
	for (auto w: MaxSpTree_Adj[v]) {
		if (vis[w]) continue;
		DFS(u, w, v, min(minimum, A[v][w]));
	}
}
...
void ProSolve() {
	while (T--) {
		cin >> N; A.clear(); A.rsz(N, vi(N)); ans = 0;
		resMat.clear(); resMat.rsz(N, vi(N));
		MaxSpTree_Adj.clear();
		MaxSpTree_Adj.rsz(N);
		...
		priority_queue<pip> heap; heap.push(mp(0LL, mp(-4LL, 0LL)));
		vb chosen(N, false);
		while (!heap.empty()) {
			i64 d = heap.top().fi, u = heap.top().se.fi, v = heap.top().se.se; heap.pop();
			if (chosen[v]) continue; chosen[v] = true;
			if (u != -4) {MaxSpTree_Adj[u].pub(v); MaxSpTree_Adj[v].pub(u);}
			for (i64 i=0; i<N; i++) {
				if (i == v || chosen[i]) continue;
				heap.push(mp(A[v][i], mp(v, i)));
			}
		}
		for (i64 i=0; i<N; i++) {
			for (i64 j=0; j<N; j++) vis[j] = false;
			vis[i] = true;
			for (auto j: MaxSpTree_Adj[i]) {
				if (vis[j]) continue;
				DFS(i, j, i, A[i][j]);
			}
		}
