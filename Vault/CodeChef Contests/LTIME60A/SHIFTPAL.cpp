...
/** -----GLOBAL VARIABLES----- **/
i64 T, len, N, Base = 257; string str, S, R;
vector<vi> hashS, hashR, powBase;

/** -----EXTENSIVE FUNCTIONS----- **/
void initPower() {
	powBase.resize(444444, vi(4, 1));
	for (i64 i=1; i<444444; i++) {
		for (i64 j=0; j<4; j++) {
...
void GenerateHashVal() {
	hashS.clear(); hashR.clear();
	hashS.resize(N, vi(4, 0)); hashR.resize(N, vi(4, 0));
	for (i64 i=0; i<N; i++) {
		for (i64 j=0; j<4; j++) {
			if (i == 0) {
...
void ProSolve() {
	while (T--) {
		...
		S = str + str.substr(0, len-1);
		R = S; reverse(R.begin(), R.end());
		N = 2 * len - 1; i64 ans = 0;
		GenerateHashVal();
		for (i64 i=0; i<len; i++) ans += (match(i, i+len-1));
		cout << ans << endl;
	}
}
