...
/** -----GLOBAL VARIABLES----- **/
i64 T; ld K, xA, yA, xB, yB, xC, yC, xD, yD;
...
ld length(ld xA, ld yA, ld xB, ld yB) {
	return ((ld)sqrt((xA-xB)*(xA-xB)+(yA-yB)*(yA-yB)));
}

pdd getVector(ld xA, ld yA, ld xB, ld yB) {
	return (mp(xB-xA, yB-yA));
}
...
void ProSolve() {
	while (T--) {
		cin >> K >> xA >> yA >> xB >> yB >> xC >> yC >> xD >> yD;
		ld k0 = length(xA, yA, xD, yD) / length(xA, yA, xB, yB);
		if (cmp(1.0L/K, k0) > 0) {cout << "No solution\n"; continue;}
		pdd vec = getVector(xA, yA, xC, yC);
		//cout << "vec = " << vec << endl;
		printf("%.2lf %.2lf\n", (xA + vec.fi / (K * k0)), (yA + vec.se / (K * k0)));
	}
}
