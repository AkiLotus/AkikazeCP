...
void ProSolve() {
	ld maximum = -LINF, minimum = LINF;
	for (i64 i=0; i<k; i++) {
		for (i64 j=i+1; j<k; j++) {
			if (a[i].fi == a[j].fi) continue;
			maximum = max(maximum, -(ld)(a[i].se - a[j].se) / (a[i].fi - a[j].fi));
			minimum = min(minimum, -(ld)(a[i].se - a[j].se) / (a[i].fi - a[j].fi));
		}
	}
