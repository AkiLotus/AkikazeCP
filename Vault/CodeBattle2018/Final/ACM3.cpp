...
/** -----GLOBAL VARIABLES----- **/
i64 k, x1, x2, mx = -LINF; vector<pii> a, aSet;
map<i64, i64> MapMx, MapMn, MapCnt;
...
void VarInput() {
	...
	cin >> k >> x1 >> x2; a.resize(k);
	for (i64 i=0; i<k; i++) {
		cin >> a[i].fi >> a[i].se;
		aSet.pub(mp(a[i].fi*x1+a[i].se, a[i].fi*x2+a[i].se));
		if (MapCnt[a[i].fi*x1+a[i].se] == 0) {MapMx[a[i].fi*x1+a[i].se] = -LINF; MapMn[a[i].fi*x1+a[i].se] = LINF;}
		MapCnt[a[i].fi*x1+a[i].se]++;
		MapMx[a[i].fi*x1+a[i].se] = max(MapMx[a[i].fi*x1+a[i].se], a[i].fi*x2+a[i].se);
		MapMn[a[i].fi*x1+a[i].se] = min(MapMn[a[i].fi*x1+a[i].se], a[i].fi*x2+a[i].se);
	}
}
...
void ProSolve() {
	for (auto it=MapCnt.begin(); it!=MapCnt.end(); it++) {
		i64 index = it->first;
		if (MapMn[index] < mx) {cout << "YES"; return;}
		mx = max(mx, MapMx[index]);
	}
	cout << "NO";
}
