...
#define fi first
#define se second
const long long MOD = 100000000LL, INF = 1e9, LINF = 1e18;
...
/** -----GLOBAL VARIABLES----- **/
i64 n, m, n1, m1, ans = 0; vector<vector<vector<vi>>> dp;
map<pii, i64> Map;
...
void ProSolve() {
	dp.resize(n+1, vector<vector<vi>>(m+1, vector<vi>(n1+1, vi(m1+1, 0))));
	dp[n][m][0][0] = 1; queue<pii> Q; Q.push(mp(n, m)); Map[mp(n,m)]++;
	while (!Q.empty()) {
		pii z = Q.front(); Q.pop();
		//tracker2(z.fi, z.se);
		for (i64 i=0; i<=n1; i++) {
			for (i64 j=0; j<=m1; j++) {
				if (z.fi > 0 && i+1 <= n1) {dp[z.fi-1][z.se][i+1][0] += dp[z.fi][z.se][i][j]; dp[z.fi-1][z.se][i+1][0] %= MOD;}
				if (z.se > 0 && j+1 <= m1) {dp[z.fi][z.se-1][0][j+1] += dp[z.fi][z.se][i][j]; dp[z.fi][z.se-1][0][j+1] %= MOD;}
			}
		}
		if ((z.fi-1 > 0 || z.se > 0) && z.fi-1 >= 0 && Map[mp(z.fi-1, z.se)] == 0) {Q.push(mp(z.fi-1, z.se)); Map[mp(z.fi-1, z.se)]++;}
		if ((z.fi > 0 || z.se-1 > 0) && z.se-1 >= 0 && Map[mp(z.fi, z.se-1)] == 0) {Q.push(mp(z.fi, z.se-1)); Map[mp(z.fi, z.se-1)]++;}
	}
	for (i64 i=0; i<=n1; i++) {
		for (i64 j=0; j<=m1; j++) {
			ans += dp[0][0][i][j];
