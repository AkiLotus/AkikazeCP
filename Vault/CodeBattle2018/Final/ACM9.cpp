...
map<i64, i64> HyperGate; vb hasGate;
vb opened, vis;
...
void ProSolve() {
	...
	vis[1] = true; i64 opened = 0;
	if (hasGate[1]) {
		opened++;
		for (i64 i=0; i<Adj[1].size(); i++) {
			HyperGate[Adj[1][i].fi] = Adj[1][i].se;
		}
	}
	priority_queue<pip> Q; Q.push(mp(0LL,mp(1LL,opened)));
	while (!Q.empty()) {
		pip z = Q.front(); Q.pop();
