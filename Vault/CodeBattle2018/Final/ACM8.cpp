...
void VarInput() {
	...
	cin >> n >> m; a.resize(n); b.resize(m); ans.resize(m);
	for (i64 i=0; i<n; i++) {cin >> a[i].fi; a[i].se = i;}
	for (i64 i=0; i<m; i++) {cin >> b[i].fi; b[i].se = i; Map[b[i].fi] = i;}
}

void ProSolve() {
	sort(b.begin(), b.end()); set<pii> S;
	for (i64 i=0; i<n; i++) S.insert(a[i]);
	i64 maximum = -LINF, curr = 0, st = -1, en = -1, currSt = 0;
	for (i64 i=0; i<n; i++) {
		curr += a[i].fi;
		if (curr < maximum) {
			st = currSt; en = i-1;
