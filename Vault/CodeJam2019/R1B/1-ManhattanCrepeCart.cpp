...
/***********************************************
** File name: 1-ManhattanCrepeCart.cpp
** Time created: Sun Apr 28 2019 16:01
***********************************************/
...
void Solve() {
	...
	while (P--) {
		...
		if (d == 'N') cnty[y+1]++;
		if (d == 'S') {cnty[0]++; cnty[y]--;}
		if (d == 'E') cntx[x+1]++;
		if (d == 'W') {cntx[0]++; cntx[x]--;}
	}
	for (int i=1; i<=Q; i++) {
		cntx[i] += cntx[i-1];
		cnty[i] += cnty[i-1];
	}
	int x = max_element(cntx.begin(), cntx.end()) - cntx.begin();
	int y = max_element(cnty.begin(), cnty.end()) - cnty.begin();
